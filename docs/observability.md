...
Completed spans are written as NDJSON records to `serverTracePath` (by default, `~/.bahew/userdata/logs/server.trace.ndjson`).
...
```bash
tail -f "$BAHEW_HOME/userdata/logs/server.trace.ndjson"
```
...
jq -c 'select(.exit._tag != "Success") | {
  ...
  exit,
  attributes
}' "$BAHEW_HOME/userdata/logs/server.trace.ndjson"
```
...
jq -r 'select(.traceId == "TRACE_ID_HERE") | [
  ...
  (.parentSpanId // "-"),
  .durationMs
] | @tsv' "$BAHEW_HOME/userdata/logs/server.trace.ndjson"
```
