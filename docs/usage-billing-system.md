...
当前版本以 `unit` 作为产品层计量单位，避免直接把不同模型、不同功能的 token 等价处理。网关返回窗口字段时以网关为准；旧网关只返回 token 时，客户端会临时用 token 数作为 used units 的兜底展示。

建议服务端采用以下计量方式：

| 行为 | 建议计量 |
| --- | --- |
| 普通文本对话 | `ceil((input_tokens * input_multiplier + output_tokens * output_multiplier) / 1000)` |
...
最终扣量必须由 sub2api 在服务端完成，客户端展示值不能作为计费依据。
