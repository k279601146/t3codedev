...
it.layer(NodeServices.layer)("dev-runner", (it) => {
  ...
  describe("createDevRunnerEnv", () => {
    it.effect("defaults BAHEW_HOME to ~/.bahew when not provided", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.BAHEW_HOME, path.resolve(NodeOS.homedir(), ".bahew"));
      }),
    );
    ...
    it.effect("supports explicit typed overrides", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.BAHEW_HOME, path.resolve("/tmp/custom-t3"));
    ...
    it.effect("uses custom t3Home when provided", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.BAHEW_HOME, path.resolve("/tmp/my-t3"));
      }),
    );
