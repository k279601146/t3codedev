...
it.layer(Layer.mergeAll(NodeServices.layer, ServerSettingsService.layerTest(), TestHttpClientLive))(
  ...
  (it) => {
    describe("checkCodexProviderStatus", () => {
      ...
      it.effect("surfaces commercial model catalog failures instead of returning ready with no models", () =>
        Effect.gen(function* () {
            ...
            () =>
              Effect.succeed(
                makeCodexProbeSnapshot({
                  models: [],
                  modelCatalogError:
                    'Model catalog returned HTTP 401: {"code":"API_KEY_REQUIRED"}',
                }),
              ),
            {
    ...
    describe("ProviderRegistryLive", () => {
      ...
      it("uses the latest refreshed model list when a refresh returns none", () => {
          ...
          const serverSettings = yield* makeMutableServerSettingsService(
            decodeServerSettings(
              deepMerge(encodedDefaultServerSettings, {
                ...
                providerInstances: {
                  // Matches the shape the user had in `.bahew/dev/settings.json`
          ...
          Effect.gen(function* () {
            ...
            yield* Effect.gen(function* () {
              ...
              assert.strictEqual(
                ...
                "Cursor is disabled in Bahew settings.",
              );
      ...
      it.effect("skips codex probes entirely when the provider is disabled", () =>
        Effect.gen(function* () {
          ...
          assert.strictEqual(status.installed, false);
          assert.strictEqual(status.message, "Codex is disabled in Bahew settings.");
        }),
      );
    });
