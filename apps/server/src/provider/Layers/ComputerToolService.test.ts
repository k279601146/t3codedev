...
import { T3_COMPUTER_TOOL_NAMESPACE } from "../computerTools.ts";
import * as ComputerToolServiceShape from "../Services/ComputerToolService.ts";
import * as ComputerToolService from "./ComputerToolService.ts";
...
describe("ComputerToolService", () => {
  it("returns a clear error when the desktop host is unavailable", async () => {
    await withEnv(
      {
        T3CODE_COMPUTER_USE_ENDPOINT: undefined,
        T3CODE_COMPUTER_USE_TOKEN: undefined,
      },
      async () => {
        const scope = Effect.runSync(Scope.make());
        const context = await Effect.runPromise(
          Layer.buildWithScope(ComputerToolService.layer, scope),
        );
        const service = Effect.runSync(
          Effect.service(ComputerToolServiceShape.ComputerToolService).pipe(
  ...
  it("rejects unknown namespace and tool names", async () => {
    const scope = Effect.runSync(Scope.make());
    const context = await Effect.runPromise(Layer.buildWithScope(ComputerToolService.layer, scope));
    const service = Effect.runSync(
      Effect.service(ComputerToolServiceShape.ComputerToolService).pipe(Effect.provide(context)),
    );
    assert.equal((await service.call({ ...basePayload, namespace: "other" })).success, false);
    assert.equal((await service.call({ ...basePayload, tool: "computer_cookie" })).success, false);
    await Effect.runPromise(Scope.close(scope, Exit.void));
  });
  ...
  it("proxies successful host responses", async () => {
    await withHost(
      ...
      async (endpoint, token) =>
        withEnv(
          {
            T3CODE_COMPUTER_USE_ENDPOINT: endpoint,
            T3CODE_COMPUTER_USE_TOKEN: token,
          },
          async () => {
            const scope = Effect.runSync(Scope.make());
            const context = await Effect.runPromise(
