...
import { T3_BROWSER_TOOL_NAMESPACE } from "../browserTools.ts";
import * as BrowserToolServiceShape from "../Services/BrowserToolService.ts";
import * as BrowserToolService from "./BrowserToolService.ts";
...
async function withHost(
  ...
  const server = NodeHttp.createServer((request, response) => {
    void (async () => {
      ...
      response.end(
        JSON.stringify(
          authorized ? handler(payload, token) : { success: false, contentItems: [] },
        ),
      );
    })();
  });
...
describe("BrowserToolService", () => {
  it("returns a clear error when the desktop host is unavailable", async () => {
    await withEnv(
      {
        T3CODE_BROWSER_USE_ENDPOINT: undefined,
        T3CODE_BROWSER_USE_TOKEN: undefined,
      },
      async () => {
        const scope = Effect.runSync(Scope.make());
        const context = await Effect.runPromise(Layer.buildWithScope(BrowserToolService.layer, scope));
        const service = Effect.runSync(
          Effect.service(BrowserToolServiceShape.BrowserToolService).pipe(Effect.provide(context)),
        );
        const response = await service.call(basePayload);
        assert.equal(response.success, false);
        assert.match(response.contentItems[0]?.type === "inputText" ? response.contentItems[0].text : "", /unavailable/i);
  ...
  it("proxies successful host responses", async () => {
    await withHost(
      ...
      async (endpoint, token) =>
        withEnv(
          {
            T3CODE_BROWSER_USE_ENDPOINT: endpoint,
            T3CODE_BROWSER_USE_TOKEN: token,
          },
          async () => {
            const scope = Effect.runSync(Scope.make());
            const context = await Effect.runPromise(Layer.buildWithScope(BrowserToolService.layer, scope));
