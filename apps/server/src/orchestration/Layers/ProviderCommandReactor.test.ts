...
describe("ProviderCommandReactor", () => {
  ...
  it("advances an active goal when the previous session was stopped by cleanup", async () => {
    const harness = await createHarness();
    const now = "2026-01-01T00:00:00.000Z";
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        ...
        commandId: CommandId.make("cmd-session-stopped-before-goal"),
        threadId: ThreadId.make("thread-1"),
        session: {
          threadId: ThreadId.make("thread-1"),
          status: "stopped",
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.goal.set",
        commandId: CommandId.make("cmd-goal-set-stopped-session"),
        threadId: ThreadId.make("thread-1"),
        objective: "继续推进目标",
        status: "active",
        createdAt: now,
  ...
  it("retries active goal advancement while the thread is temporarily busy", async () => {
    const harness = await createHarness();
    const now = "2026-01-01T00:00:00.000Z";
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.session.set",
        commandId: CommandId.make("cmd-session-running-before-goal"),
        threadId: ThreadId.make("thread-1"),
        session: {
          threadId: ThreadId.make("thread-1"),
          status: "running",
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.goal.set",
        commandId: CommandId.make("cmd-goal-set-busy-session"),
        threadId: ThreadId.make("thread-1"),
        objective: "等待忙碌线程继续",
        status: "active",
        createdAt: now,
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.session.set",
        commandId: CommandId.make("cmd-session-ready-after-goal-busy"),
        threadId: ThreadId.make("thread-1"),
        session: {
          threadId: ThreadId.make("thread-1"),
          status: "ready",
  ...
  it("does not auto-advance an active goal after the user stops the session", async () => {
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.goal.set",
        commandId: CommandId.make("cmd-goal-set-before-user-stop"),
        threadId: ThreadId.make("thread-1"),
        objective: "do not auto advance after user stop",
        status: "active",
        createdAt: now,
      }),
    );
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        ...
        commandId: CommandId.make("cmd-session-stop-active-goal"),
        threadId: ThreadId.make("thread-1"),
        createdAt: now,
      }),
    );
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.session.set",
        commandId: CommandId.make("cmd-session-ready-after-user-stop"),
        threadId: ThreadId.make("thread-1"),
        session: {
          threadId: ThreadId.make("thread-1"),
          status: "ready",
  ...
  it("does not overwrite a title edited while first-turn title generation is running", async () => {
    const harness = await createHarness();
    const now = "2026-01-01T00:00:00.000Z";
    const seededTitle = "Please investigate reconnect failures after restar...";
    const generatedTitle = await Effect.runPromise(Deferred.make<{ title: string }>());
    harness.generateThreadTitle.mockReturnValue(Deferred.await(generatedTitle));

    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.meta.update",
        commandId: CommandId.make("cmd-thread-title-race-seed"),
        threadId: ThreadId.make("thread-1"),
        title: seededTitle,
      }),
    );
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.turn.start",
        commandId: CommandId.make("cmd-turn-start-title-race"),
        threadId: ThreadId.make("thread-1"),
        message: {
          messageId: asMessageId("user-message-title-race"),
          role: "user",
          text: "Please investigate reconnect failures after restarting the session.",
          attachments: [],
        },
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.meta.update",
        commandId: CommandId.make("cmd-thread-title-race-manual"),
        threadId: ThreadId.make("thread-1"),
        title: "Keep the manual title",
      }),
    );
    await Effect.runPromise(Deferred.succeed(generatedTitle, { title: "Generated title" }));

    await harness.drain();
    const readModel = await harness.readModel();
    const thread = readModel.threads.find((entry) => entry.id === ThreadId.make("thread-1"));
    expect(thread?.title).toBe("Keep the manual title");
  });
