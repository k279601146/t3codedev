...
export default function ChatView(props: ChatViewProps) {
  ...
  const rightPanelResizeDragRef = useRef<{
    startX: number;
    startWidth: number;
  } | null>(null);
  ...
  }, [isConversationDraft, setNewThreadScope]);
  ...
  const selectRightPanelSurface = useCallback(
    ...
    [activeThreadKey, setRightPanelSurface],
  );
  const closeThreadRightPanel = useCallback(() => {
    if (rightPanelSurface === "summary") {
      closeSummaryPanel();
      return;
    }
    if (rightPanelSurface === "review" && diffOpen && isServerThread) {
      void navigate({
        to: "/$environmentId/$threadId",
        params: {
          environmentId,
  ...
  const onRightPanelResizePointerDown = useCallback(
    (event: React.PointerEvent<HTMLDivElement>) => {
      event.preventDefault();
      rightPanelResizeDragRef.current = {
        startX: event.clientX,
        startWidth: rightPanelWidthPx,
      };
      event.currentTarget.setPointerCapture(event.pointerId);
    },
    [rightPanelWidthPx],
  );
  const onRightPanelResizePointerMove = useCallback(
    (event: React.PointerEvent<HTMLDivElement>) => {
      const drag = rightPanelResizeDragRef.current;
      if (!drag) return;
      const nextWidth = drag.startWidth + (drag.startX - event.clientX);
      setRightPanelWidthPx(nextWidth);
    },
    [setRightPanelWidthPx],
  );
  const onRightPanelResizePointerUp = useCallback((event: React.PointerEvent<HTMLDivElement>) => {
      ...
      <header
        className={cn(
          ...
          isElectron
            ...
            : "pb-1.5 pl-[calc(env(safe-area-inset-left)+0.75rem)] pr-[calc(env(safe-area-inset-right)+0.75rem)] pt-1.5 sm:pb-2 sm:pl-[calc(env(safe-area-inset-left)+1rem)] sm:pr-[calc(env(safe-area-inset-right)+1rem)] sm:pt-2",
        )}
      >
        <ChatHeader
          activeThreadEnvironmentId={activeThread.environmentId}
        ...
        <>
          ...
          <div className="flex min-h-0 min-w-0 flex-1 overflow-hidden">
            ...
            <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden">
              ...
              <div
                className={cn(
                  "shrink-0 bg-background pl-[calc(env(safe-area-inset-left)+0.75rem)] pr-[calc(env(safe-area-inset-right)+0.75rem)] pt-1.5 sm:pl-[calc(env(safe-area-inset-left)+1.25rem)] sm:pr-[calc(env(safe-area-inset-right)+1.25rem)] sm:pt-2",
                  isGitRepo
