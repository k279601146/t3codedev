...
describe("friendlyErrors", () => {
  ...
  it("normalizes gateway provider errors for thread surfaces", () => {
    const cases = [
      {
        raw: "unexpected status 404 Not Found: model is not found, url: https://www.bahew.com/v1/responses, cf-ray: a13956896e4a115c-ORD, request id: 10c59330-90a7-44e4-ae39-6c351ac8a5a5",
        expected:
          "模型不存在或暂不可用。请求 ID：10c59330-90a7-44e4-ae39-6c351ac8a5a5",
      },
      {
        raw: "unexpected status 503 Service Unavailable: Service temporarily unavailable, url: https://www.bahew.com/v1/responses, cf-ray: a13955484ca3ee11-ORD, request id: 43a06c06-65fb-41b3-842f-bbfca9053c38",
        expected:
          "服务暂时不可用，请稍后重试。请求 ID：43a06c06-65fb-41b3-842f-bbfca9053c38",
      },
      {
        raw: "unexpected status 403 Forbidden: 中国地区内容安全网关配置不可用，请联系管理员, url: https://www.bahew.com/v1/responses, cf-ray: a13a2186dcf472e5-ORD, request id: 913e8326-abdd-4b8a-b64c-52b3a95da266",
        expected:
          "中国地区内容安全网关配置不可用，请联系管理员。请求 ID：913e8326-abdd-4b8a-b64c-52b3a95da266",
      },
      {
        raw: "unexpected status 403 Forbidden: insufficient balance, url: https://www.bahew.com/v1/responses, cf-ray: a139e8ad79d41060-ORD, request id: 0720b576-b83d-4fa7-8c5b-ba882537a4ba",
        expected:
          "账户余额不足，请充值或等待额度刷新后继续使用。请求 ID：0720b576-b83d-4fa7-8c5b-ba882537a4ba",
      },
      {
        raw: 'unexpected status 401 Unauthorized: {"code":"USER_INACTIVE","message":"User account is not active"}, url: https://www.bahew.com/v1/responses, cf-ray: a13a22d1eec6381e-ORD, request id: 329fc1b9-cd22-49a4-b474-f0d33aae577f',
        expected:
