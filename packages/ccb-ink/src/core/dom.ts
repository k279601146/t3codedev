...
import { LayoutDisplay, LayoutMeasureMode } from "./layout/node.ts"
import measureText from "./measure-text.ts"
import { addPendingClear, nodeCache } from "./node-cache.ts"
import squashTextNodes from "./squash-text-nodes.ts"
import type { Styles, TextStyles } from "./styles.ts"
import { expandTabs } from "./tabstops.ts"
...
export type DOMElement = {
  ...
  // scrollTo's direct write races past React's async re-render 鈥?instead
  ...
  // el.yogaNode.getComputedTop() (FRESH 鈥?same Yoga pass as scrollHeight)
  ...
  // Only set on ink-root. The document owns focus 鈥?any node can
...
function collectRemovedRects(
  ...
  // its painted pixels may overlap non-siblings 鈥?flag for global blit
...
// No stringWidth, no wrapping, no tab expansion 鈥?the producer (e.g. ColorDiff)
