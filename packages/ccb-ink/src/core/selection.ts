 ...
 * the cursor is now). The rendered highlight normalizes to start 鈮?end.
 */
...
export type SelectionState = {
   ...
   *  even when dragging backward past it. Null 鈬?char mode. The kind
  ...
  /** Soft-wrap bits parallel to scrolledOffAbove 鈥?true means the row
   ...
   *  Without this, PgDn (clamps anchor) 鈫?PgUp leaves anchor at the wrong
   *  row AND scrolledOffAbove stale 鈥?highlight 鈮?copy. Undefined when
   ...
   *  signal that VS Code's macOptionClickForcesSelection is OFF 鈥?if it
...
export function startSelection(
  ...
  // drag leaves focus null 鈫?hasSelection/selectionBounds return false/null
...
export function findPlainTextUrlAt(
  ...
  // cell is a boundary 鈥?no need to step over spacers like wordBoundsAt.
  let lo = c
  while (lo > 0) {
    const prev = lo - 1
    if (noSelect[rowOff + prev] === 1) break
  ...
  // column offset. Find the last scheme anchor at or before the click 鈥?
  ...
  // if unbalanced 鈥?`/wiki/Foo_(bar)` keeps `)`, `/arr[0]` keeps `]`.
 ...
 * moves relative to anchor. Drops to char mode (clears anchorSpan) 鈥?
...
export function moveFocus(s: SelectionState, col: number, row: number): void {
  ...
  s.anchorSpan = null
  s.focus = { col, row }
  // Explicit user repositioning 鈥?any stale virtual focus (from a prior
 ...
 * point that hits a clamp bound gets its col reset to the full-width edge 鈥?
 ...
 *
 * If both ends overshoot the SAME viewport edge (select text 鈫?Home/End/g/G
 * jumps far enough that both are out of view), clear 鈥?otherwise both clamp
...
export function shiftSelection(
  ...
  // correctly. Without this, clamp(5鈫?) + shift(+10) = 10, not the true 5,
  // and scrolledOffAbove stays stale (highlight 鈮?copy).
  ...
  // shrinks (reverse scroll), those rows are back on-screen 鈥?pop from
  ...
  // Invariant: accumulator length 鈮?debt. If the accumulator exceeds debt,
  // the excess is stale 鈥?e.g., moveFocus cleared virtualFocusRow without
  ...
  // so at entry the accumulator is populated but oldDebt is still 0 鈥?
  // that's the normal establish-debt path, not stale.
  if (s.scrolledOffAbove.length > newAboveDebt) {
    // Above pushes newest at END 鈫?keep END.
    s.scrolledOffAbove =
  ...
  if (s.scrolledOffBelow.length > newBelowDebt) {
    // Below unshifts newest at FRONT 鈫?keep FRONT.
  ...
  // shift 鈥?dRow-based clampCol would give it the bottom col.
...
export function shiftAnchor(
  ...
  // drag鈫抐ollow transition hands off to shiftSelectionForFollow, which reads
  // (virtualAnchorRow ?? anchor.row). Without this, drag-phase clamping
  // leaves virtual undefined 鈫?follow initializes from the already-clamped
  // row, under-counting total drift 鈫?shiftSelection's invariant-restore
  ...
  // keyboard-scroll round-trip) 鈥?plain clamp from current row.
  if (s.anchorSpan) {
 ...
 * while a selection is active 鈥?native terminal behavior is for the
 ...
 * the selection is text-anchored at both ends 鈥?both must move. The
 ...
 * text has scrolled entirely off the top. Clear it 鈥?otherwise a single
 ...
 * (useHasSelection) 鈥?the caller is inside onRender so it can't use
...
export function shiftSelectionForFollow(
  ...
  // though the clamp below occurred 鈫?a later PgUp computes debt from the
  // clamped row instead of the true pre-clamp row and never pops the
  // accumulator 鈥?getSelectedText double-counts the off-screen rows.
  ...
  // Clamp from raw, not p.row+dRow 鈥?so a virtual position coming back
...
function extractRowText(
  ...
  for (let col = colStart; col <= lastCol; col++) {
    ...
    if (!cell) continue
    // Skip spacer tails (second half of wide chars) 鈥?the head already
 ...
 * marks a row as a word-wrap continuation 鈥?those rows are concatenated
 ...
 * OPPOSITE span side 鈥?which would otherwise still hold the original word
...
export function captureScrolledRows(
  ...
  // the selection aren't captured 鈥?they weren't selected.
  ...
  if (side === 'above') {
    ...
    // Newest rows go at the TOP of the below-accumulator 鈥?they're
 ...
 * renderer produces the Frame but before the diff 鈥?the normal diffEach
 ...
 * setSelectionBg) that REPLACES each cell's bg while PRESERVING its fg 鈥?
...
export function applySelectionOverlay(
  ...
  for (let row = start.row; row <= end.row && row < screen.height; row++) {
    ...
    for (let col = colStart; col <= colEnd; col++) {
      ...
      // Skip noSelect cells 鈥?gutters stay visually unchanged so it's
