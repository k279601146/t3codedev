 ...
 * as applySelectionOverlay 鈥?the diff picks up highlighted cells as ordinary
 * changes, LogUpdate stays a pure diff engine.
 *
 * Case-insensitive. Handles wide characters (CJK, emoji) by building a
 * col-of-char map per row 鈥?the Nth character isn't at col N when wide chars
 * are present (each occupies 2 cells: head + SpacerTail).
 *
 * This ONLY inverts 鈥?there is no "current match" logic here. The yellow
 ...
 *
 * Returns true if any match was highlighted (damage gate 鈥?caller forces
...
export function applySearchHighlight(
  ...
  for (let row = 0; row < height; row++) {
    const rowOff = row * w
    // Build row text (already lowercased) + code-unit鈫抍ell-index map.
    ...
    //   - noSelect: gutters (鈳? line numbers) 鈥?same exclusion as
    ...
    // codeUnitToCell maps positions in the LOWERCASED text 鈥?U+0130
    // (Turkish 陌) lowercases to 2 code units, so lowering the joined
    ...
    while (pos >= 0) {
      ...
      // 'aa' at 0 AND 1 in 'aaa' 鈫?double-invert cell 1.
