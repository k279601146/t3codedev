...
export class LogUpdate {
  ...
  render(
    ...
    // CURSOR_HOME after RESET_SCROLL_REGION is defensive 鈥?DECSTBM reset
    // homes cursor per spec but terminal implementations vary.
    //
    // decstbmSafe: caller passes false when the DECSTBM鈫抎iff sequence
    // can't be made atomic (no DEC 2026 / BSU/ESU). Without atomicity the
    // outer terminal renders the intermediate state 鈥?region scrolled,
    // edge rows not yet painted 鈥?a visible vertical jump on every frame
    ...
    // this, the diff loop treats that row as reachable 鈥?but the cursor clamps
...
function renderFrameSlice(
  ...
  for (let y = startY; y < endY; y += 1) {
    ...
    // Reset at start of each line 鈥?no cell rendered yet
    ...
    for (let x = 0; x < screenWidth; x += 1, index += 1) {
      ...
      // Style transition 鈥?cached string, zero allocations after warmup
    ...
    // CR+LF at end of row 鈥?\r resets to column 0, \n moves to next line.
 ...
 * viewport edge). Callers MUST gate currentStyleId updates on this 鈥?when
...
function writeCellWithStyleStr(
  ...
  // it. Write a styled space there first 鈥?on correct terminals the emoji
  ...
  // Update cursor 鈥?mutate in place to avoid Point allocation
  if (px >= vw) {
 ...
 *    Examples: 鈿旓笍 (U+2694), 鈽狅笍 (U+2620), 鉂わ笍 (U+2764).
 */
...
class VirtualScreen {
  ...
  // File-private class 鈥?not exposed outside log-update.ts.
  cursor: Point
  diff: Diff = []
