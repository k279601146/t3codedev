...
export type Styles = {
  ...
  /**
   * It specifies the 鈥渇lex shrink factor鈥? which determines how much the flex item will shrink relative to the rest of the flex items in the flex container when there isn鈥檛 enough space on the row.
   ...
   * `backgroundColor` but without emitting any SGR 鈥?the terminal's
   ...
   * and the copied text 鈥?useful for fencing off gutters (line numbers,
   ...
   * right edge for every row it occupies 鈥?this covers any upstream
...
const applyBorderStyles = (
  ...
  if ('borderStyle' in style) {
    ...
    // Skip undefined values 鈥?they mean the prop was removed or never set,
