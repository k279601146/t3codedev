...
export default function sliceAnsi(
  ...
  // Don't pass `end` to tokenize 鈥?it counts code units, not display cells,
  ...
  for (const token of tokens) {
    ...
    // matras, virama, diacritics) are width 0 鈥?counting them via .length
    ...
    // Break AFTER trailing zero-width marks 鈥?a combining mark attaches to
    // the preceding base char, so "啶ぞ" (啶?+ 啶? 1 display cell) sliced at
    // end=1 must include the 啶? Breaking on position >= end BEFORE the
    // zero-width check would drop it and render 啶?bare. ANSI codes are
    ...
    if (token.type === 'ansi') {
      ...
      if (!include && position >= start) {
        // Skip leading zero-width marks at the start boundary 鈥?they belong
        // to the preceding base char in the left half. Without this, the
        // mark appears in BOTH halves: left+right 鈮?original. Only applies
