...
import { getYogaCounters } from "./yoga-layout/index.ts";
...
import { TerminalWriteProvider } from "../hooks/useTerminalNotification.ts";
...
export default class Ink {
  ...
  // "current" (yellow). null clears. Positions are known upfront 鈥?
  ...
  constructor(private readonly options: Options) {
    ...
    // @ts-expect-error MACRO-replaced comparison 鈥?always false in production builds
  ...
  exitAlternateScreen(): void {
    this.options.stdout.write(
      (this.altScreenActive ? ENTER_ALT_SCREEN : '') + // re-enter alt 鈥?vim's rmcup dropped us to main
    ...
    for (const patch of diff) {
      if (patch.type === 'clearTerminal') {
        ...
        if (isDebugRepaintsEnabled() && patch.debug) {
          ...
          this.logger.debug(
            `[REPAINT] full reset 路 ${patch.reason} 路 row ${patch.debug.triggerY}\n` +
  ...
  scanElementSubtree(el: dom.DOMElement): MatchPosition[] {
    ...
    // renderNodeToOutput wrote our offset positions to nodeCache 鈥?
  ...
  setSelectionBgColor(color: string): void {
    ...
    // colorize returns the input unchanged if the color string is bad 鈥?
  ...
  shiftSelectionForScroll(dRow: number, minRow: number, maxRow: number): void {
    ...
    // useHasSelection updates. Safe to call notifySelectionChange here 鈥?
  ...
  private patchStderr(): () => void {
    ...
    const intercept = (
      ...
      // Reentrancy guard: logger.debug 鈫?writeToStderr 鈫?here. Pass
