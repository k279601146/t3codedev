...
type TokenizerOptions = {
   ...
   * Only enable for stdin input 鈥?`\x1b[M` is also CSI DL (Delete Lines) in
  ...
  while (i < data.length) {
    ...
    switch (result.state) {
      ...
      case 'csi':
        ...
        // M immediately after [ (offset 2) means no params 鈥?SGR mouse
        // (CSI < 鈥?M) has a `<` param byte first and reaches M at offset > 2.
        ...
        //
        // Gated on x10Mouse 鈥?`\x1b[M` is also CSI DL (Delete Lines) and
        ...
        // The 鈮?x20 check on each payload slot is belt-and-suspenders: X10
        // guarantees Cb鈮?2, Cx鈮?3, Cy鈮?3, so a control byte (ESC=0x1B) in
        ...
        // oriented and stdin uses utf8 encoding (App.tsx). At col 162-191 脳
        // row 96-159 the two coord bytes (0xC2-0xDF, 0x80-0xBF) form a valid
        // UTF-8 2-byte sequence and collapse to one char 鈥?the length check
        ...
        if (
          ...
          if (i + 4 <= data.length) {
            ...
            // Incomplete 鈥?exit loop; end-of-input buffers from seqStart.
