...
// DECRPM: CSI ? Ps ; Pm $ y  鈥?response to DECRQM (request mode)
// biome-ignore lint/suspicious/noControlCharactersInRegex: terminal escape sequence parsing
const DECRPM_RE = /^\x1b\[\?(\d+);(\d+)\$y$/
// DA1: CSI ? Ps ; ... c  鈥?primary device attributes response
// biome-ignore lint/suspicious/noControlCharactersInRegex: terminal escape sequence parsing
const DA1_RE = /^\x1b\[\?([\d;]*)c$/
// DA2: CSI > Ps ; ... c  鈥?secondary device attributes response
// biome-ignore lint/suspicious/noControlCharactersInRegex: terminal escape sequence parsing
const DA2_RE = /^\x1b\[>([\d;]*)c$/
// Kitty keyboard flags: CSI ? flags u  鈥?response to CSI ? u query
...
// Ctrl+F3 = CSI 1;5 R, etc.) 鈥?plain CSI row;col R is genuinely ambiguous.
// biome-ignore lint/suspicious/noControlCharactersInRegex: terminal escape sequence parsing
const CURSOR_POSITION_RE = /^\x1b\[\?(\d+);(\d+)R$/
// OSC response: OSC code ; data (BEL|ST)
// biome-ignore lint/suspicious/noControlCharactersInRegex: terminal escape sequence parsing
const OSC_RESPONSE_RE = /^\x1b\](\d+);(.*?)(?:\x07|\x1b\\)$/s
// XTVERSION: DCS > | name ST  鈥?terminal name/version string (answer to CSI > 0 q).
 ...
 *
 * These patterns are syntactically distinguishable from keyboard input 鈥?
  ...
  for (const token of tokens) {
    if (token.type === 'sequence') {
      ...
      if (inPaste) {
        ...
        // Orphaned SGR/X10 mouse tail (fullscreen only 鈥?mouse tracking is off
        ...
        // the wheel range [\x60-\x7f] (0x40|modifiers + 32) 鈥?a full [\x20-]
 ...
 * sequences can't express super 鈥?it only arrives via kitty keyboard
 ...
 *  out of the input stream. Not user input 鈥?consumers should dispatch
...
function parseKeypress(s: string = ''): ParsedKey {
  ...
  // Must run before FN_KEY_RE 鈥?FN_KEY_RE only allows 2 params before ~ and
  ...
  // Ctrl=0x10) 鈥?modified wheel events (e.g. Ctrl+scroll, button=80)
  ...
  // X10 events (clicks/drags) are swallowed here 鈥?we only enable mouse
