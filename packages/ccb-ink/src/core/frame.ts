...
export type Frame = {
  ...
  /** A ScrollBox has remaining pendingScrollDelta 鈥?schedule another frame. */
...
export type FrameEvent = {
  ...
  phases?: {
    /** createRenderer output: DOM 鈫?yoga layout 鈫?screen buffer */
    renderer: number
    /** LogUpdate.render(): screen diff 鈫?Patch[] (the hot path this PR optimizes) */
    diff: number
    /** optimize(): patch merge/dedupe */
    optimize: number
    /** writeDiffToTerminal(): serialize patches 鈫?ANSI 鈫?stdout */
    write: number
    ...
    yoga: number
    /** React reconcile time: scrollMutated 鈫?resetAfterCommit. 0 if no commit. */
    commit: number
    /** layoutNode() calls this frame (recursive, includes cache-hit returns) */
    yogaVisited: number
    /** measureFunc (text wrap/width) calls 鈥?the expensive part */
    yogaMeasured: number
...
export type Patch =
  ...
  // Pre-serialized style transition string from StylePool.transition() 鈥?
 ...
 * 1. Terminal has been resized (viewport dimensions changed) 鈫?'resize'
 * 2. Current frame screen height exceeds available terminal rows 鈫?'offscreen'
 * 3. Previous frame screen height exceeded available terminal rows 鈫?'offscreen'
 */
