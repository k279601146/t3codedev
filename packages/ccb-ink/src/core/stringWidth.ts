 ...
 * characters like 鈿?(U+26A0) which string-width incorrectly reports as width 2.
 *
...
// Note: complex-script graphemes like Devanagari 啶曕啶?(ka+virama+ZWJ+ssa) render
...
// we need for cursor positioning 鈥?the JS fallback's grapheme-cluster width of 1
// would desync Ink's layout from the terminal.
//
// Bun.stringWidth is resolved once at module scope rather than checked on every
// call 鈥?typeof guards deopt property access and this is a hot path (~100k calls/frame).
