 ...
 * through to the -256color regex 鈫?level 2. At level 2, chalk.rgb()
 * downgrades to the nearest 6脳6脳6 cube color: rgb(215,119,87) (Claude
 * orange) 鈫?idx 174 rgb(215,135,135) 鈥?washed-out salmon.
 *
 * Gated on level === 2 (not < 3) to respect NO_COLOR / FORCE_COLOR=0 鈥?
 ...
 *
 * Must run BEFORE the tmux clamp 鈥?if tmux is running inside a VS Code
 ...
 * WITHOUT the bg sequence 鈥?outer terminal's buffer has bg=default 鈫?black on
...
function clampChalkLevelForTmux(): boolean {
  ...
  // through 鈥?skip the clamp. General escape hatch for anyone who's
...
// Computed once at module load 鈥?terminal/tmux environment doesn't change mid-session.
// Order matters: boost first so the tmux clamp can re-clamp if tmux is running
// inside a VS Code terminal. Exported for debugging 鈥?tree-shaken if unused.
