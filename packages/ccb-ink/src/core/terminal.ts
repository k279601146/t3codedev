...
// XTVERSION (CSI > 0 q 鈫?DCS > | name ST) goes through the pty 鈥?the query
 ...
 *  SSH 鈥?query/reply goes through the pty). Early calls may miss the probe
 *  reply 鈥?call lazily (e.g. in an event handler) if SSH detection matters. */
...
// terminals silently ignore unknown CSI 鈥?but some terminals honor the enable
...
// Computed once at module load 鈥?terminal capabilities don't change mid-session.
