...
export type RenderOptions = {
  ...
  // or reset to 0脳0 (forceRedraw). Blitting from such a prevScreen would
...
export default function createRenderer(
  ...
  // persists 鈥?most lines don't change between renders.
  ...
  return options => {
    ...
    // Read pools from the back buffer's screen 鈥?pools may be replaced
    ...
    // Alt-screen: the screen buffer IS the alt buffer 鈥?always exactly
    ...
    if (options.altScreen && yogaHeight > terminalRows) {
      logForDebugging(
        `alt-screen: yoga height ${yogaHeight} > terminalRows ${terminalRows} 鈥?` +
    ...
    // with blanks, or forceRedraw() reset it to 0脳0. Blit on the NEXT frame
    ...
    return {
      ...
      viewport: {
        ...
        // scrolls 鈥?the cursor.y clamp below keeps the cursor-restore
