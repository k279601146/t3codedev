 ...
 * (CSI c) 鈥?every terminal since VT100 responds to DA1, and terminals
...
import { csi } from "./termio/csi.ts"
import { osc } from "./termio/osc.ts"
...
/** Primary Device Attributes query (CSI c). Every terminal answers this 鈥?
 ...
 *  Terminal replies with CSI ? row ; col R. The `?` marker is critical 鈥?
 *  the plain DSR form (CSI 6 n 鈫?CSI row;col R) is ambiguous with
 ...
 *  This survives SSH 鈥?the query goes through the pty, not the environment,
...
export class TerminalQuerier {
   ...
   * before it 鈥?concurrent batches from independent callers stay isolated.
   */
   ...
   * resolved with `undefined` (terminal didn't respond 鈫?doesn't support
   * the query). This is the barrier that makes send() timeout-free.
   *
   * Safe to call with no pending queries 鈥?still waits for a round-trip.
   */
  flush(): Promise<void> {
    return new Promise(resolve => {
   ...
   *   params 鈥?a separate DA1 write means the terminal sends TWO DA1
   ...
   *   (the terminal answered DA1 without answering them 鈫?unsupported)
