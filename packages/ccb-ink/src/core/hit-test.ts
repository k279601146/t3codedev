...
import type { EventHandlerProps } from "./events/event-handlers.ts"
import { MouseActionEvent } from "./events/mouse-action-event.ts"
import { nodeCache } from "./node-cache.ts"

/**
 * Find the deepest DOM element whose rendered rect contains (col, row).
 *
 * Uses the nodeCache populated by renderNodeToOutput 鈥?rects are in screen
 ...
 *
 * Returns the hit node even if it has no onClick 鈥?dispatchClick walks up
 ...
 * mouseenter/mouseleave: does NOT bubble 鈥?moving between children does
