...
// Matches detectXtermJsWheel() in ScrollKeybindingHandler.tsx 鈥?the curve
...
// shift layout 鈫?narrow damage bounds 鈫?O(changed cells) diff instead of
// O(rows脳cols).
...
// three paths 鈥?full-render nodeCache.set, node-level blit early-return,
// blitEscapingAbsoluteDescendants 鈥?so clean-overlay consecutive scrolls
// still have the rect.
...
// the next frame blits root and never reaches the scrollbox 鈥?drain stalls.
...
// (native terminal behavior 鈥?the selection walks up the screen as content
// scrolls, eventually clipping at the top). The frontFrame screen buffer
// still holds the old content at that point 鈥?captureScrolledRows reads
...
// 鈹€鈹€ Native terminal drain (iTerm2/Ghostty/etc. 鈥?proportional events) 鈹€鈹€
// Minimum rows applied per frame. Above this, drain is proportional (~3/4
// of remaining) so big bursts catch up in log鈧?frames while the tail
// decelerates smoothly. Hard cap is innerHeight-1 so DECSTBM hint fires.
const SCROLL_MIN_PER_FRAME = 4

// 鈹€鈹€ xterm.js (VS Code) smooth drain 鈹€鈹€
// Low pending (鈮?) drains ALL in one frame 鈥?slow wheel clicks should be
// instant (click 鈫?visible jump 鈫?done), not micro-stutter 1-row frames.
// Higher pending drains at a small fixed step so fast-scroll animation
// stays smooth (no big jumps). Pending >MAX snaps excess.
const SCROLL_INSTANT_THRESHOLD = 5 // 鈮?this: drain all at once
const SCROLL_HIGH_PENDING = 12 // threshold for HIGH step
const SCROLL_STEP_MED = 2 // pending (INSTANT, HIGH): catch-up
const SCROLL_STEP_HIGH = 3 // pending 鈮?HIGH: fast flick
...
function drainAdaptive(
  ...
  // 鈮?: drain all (slow click = instant). Above: small fixed step.
  const step =
...
// OSC 8 hyperlink escape sequences. Empty params (;;) 鈥?ansi-tokenize only
 ...
 * they fall through with softWrap undefined 鈥?no tracking, no behavior
...
function applyPaddingToText(
  ...
  if (yogaNode) {
    ...
    if (softWrap && offsetY > 0) {
      ...
      // with text.split('\n'). Mutate in place 鈥?caller owns the array.
...
function renderNodeToOutput(
  ...
  if (yogaNode) {
    if (yogaNode.getDisplay() === LayoutDisplay.None) {
      ...
      if (node.dirty) {
        ...
        if (cached) {
          ...
          // Drop descendants' cache too 鈥?hideInstance's markDirty walks UP
          ...
          // prevScreen (cleared here) 鈫?content vanishes.
    ...
    // down so the top rows are visible and the bottom overflows below 鈥?the
    ...
    if (
      ...
      // cells, the blit above only restored this node's own rect 鈥?the
    ...
    // Read before deleting 鈥?hasRemovedChild disables prevScreen blitting
    ...
    // parent) AND a sibling lands at the same y. Skip rendering 鈥?both would
    ...
    // already handled the visible鈫抯queezed transition.
    //
    ...
    // y+1, not y). HelpV2's third shortcuts column hits this 鈥?skipping
    ...
    if (node.nodeName === 'ink-raw-ansi') {
      ...
      // style re-application 鈥?output.write() parses ANSI directly into cells.
      ...
      // the region 鈥?including blits from prevScreen when the box is
      ...
      // 鈥?a multi-row drag over a diff gutter shouldn't pick up the
      // `  鈳? ` prefix on row 0 or the blank cells under it on row 1+.
      ...
      if (isScrollY) {
        ...
        // Do NOT add getComputedTop() 鈥?that's the wrapper's offset
        ...
        // Capture previous scroll bounds BEFORE overwriting 鈥?the at-bottom
        ...
        // viewport top (plus offset). Yoga is FRESH 鈥?same calculateLayout
        ...
        // moved scrollTop without firing React's notify 鈫?parent's quantized
        // store snapshot never updated 鈫?StickyTracker got stale range props
        // 鈫?firstVisible wrong. Also: SCROLL_MIN_PER_FRAME=4 with snap-at-1
        ...
        // previous max, pin to the new max. Scroll away 鈫?stop following;
        // scroll back (or scrollToBottom/sticky attr) 鈫?resume. The sticky
        ...
        // 鈥?the imperative field takes precedence over the attribute so
        ...
        // Positional check only valid when content grew 鈥?virtualization can
        ...
        if (atBottom && (node.pendingScrollDelta ?? 0) >= 0) {
          ...
          // state 鈥?sticky-broken-but-at-bottom (wheel tremor, click-select
          ...
          // undefined (never set by user action) leave it alone 鈥?setting it
        ...
        // startup) the probe has resolved 鈥?same timing guarantee the
        ...
        if (pending !== undefined && pending !== 0) {
          // Drain continues even past the clamp 鈥?the render-clamp below
          // holds the VISUAL at the mounted edge regardless. Hard-stopping
          // here caused stop-start jutter: drain hits edge 鈫?pause 鈫?React
          // commits 鈫?clamp widens 鈫?drain resumes 鈫?edge again. Letting
          ...
          // Opposite scrollBy calls cancelled to zero 鈥?clear so we don't
        ...
        // node.scrollTop 鈥?the clamped value is for this paint only; the real
        ...
        // clamp passive 鈥?React's commit 鈫?resetAfterCommit 鈫?onRender will
        ...
        if (content && contentYoga) {
          ...
          // cache 鈥?so the children's positionChanged check can't fire.
          ...
          if (contentCached && contentCached.y !== contentY) {
            ...
            // and the shift fits within the viewport 鈥?otherwise the full
          ...
          // nested clip keeps child writes out of stable rows 鈥?a tall child
          ...
          // scroll), we still use the fast path 鈥?the dirty child is almost
          ...
          // doesn't match the scroll delta 鈥?fall back to the full path so
          ...
          // the DECSTBM shift 鈥?emitting DECSTBM leaves stale rows (seen as
          ...
          if (hint && prevScreen && safeForFastPath) {
            ...
            // Snapshot dirty children before the first pass 鈥?the first
            ...
            // Covers TWO cases:
            //   1. Dirty children 鈥?their content changed, blitted pixels are
            //      stale regardless of position.
            //   2. Clean children BELOW a middle-growth point 鈥?when a dirty
            ...
            //   the shift put them at the right place 鈥?skipped here, fast
            //   path preserved.
            if (dirtyChildren) {
              ...
              // check cached.y鈭抎elta === screenY reduces to delta === delta
              // (tautology) 鈫?skip without yoga reads. Restores O(dirty)
              ...
              // leaves shift non-zero 鈫?clean children after the growth
              ...
              for (const childNode of content.childNodes) {
                ...
                if (!isDirty && cumHeightShift === 0) {
                  ...
                  // never painted it 鈫?fall through to yoga + render.
                ...
                // Clean children reaching here have cumHeightShift 鈮?0 OR
                // no cache. Re-check precisely: cached.y 鈭?delta is where
                // the shift left old pixels; if it equals new screenY the
                // blit is correct (shift re-balanced at this child, or
                // yogaTop happens to net out). No cache 鈫?blit never
                // painted it 鈫?render.
                if (!isDirty) {
                ...
                // blitted content 鈥?output.clear() only expands damage and
            ...
            // pixels sit at (rect.y - delta) 鈥?neither edge render nor the
            ...
            // frame 鈥?on wide terminals over tmux (no BSU/ESU) the
            ...
            // positionChanged (ScrollBox height shrunk 鈥?pill mount) means a
        ...
        renderChildren(
          ...
          // on re-render 鈫?/permissions body blanked on Down arrow, #25436).
...
// Disable blit for siblings after a dirty child 鈥?but still pass prevScreen
...
// onto later siblings 鈥?their content is confined to their layout bounds.
...
// tick and the bottom prompt section never blits 鈫?100% writes every frame.
//
...
// from prevScreen 鈥?the clipped child just rewrote those cells this frame.
...
// descent (the full-width rect has transparent gaps 鈫?stale blit) while
...
// absolute siblings fill their entire rect 鈥?direct blit is safe.
function renderChildren(
  ...
  for (const childNode of node.childNodes) {
    const childElem = childNode as DOMElement
    // Capture dirty before rendering 鈥?renderNodeToOutput clears the flag
...
// lands at the same computed top 鈥?then both write to that row and the
...
function siblingSharesY(node: DOMElement, yogaNode: LayoutNode): boolean {
  ...
  // No next sibling with a yoga node 鈥?check previous. A run of h=0 boxes
  ...
  for (const child of node.childNodes) {
    ...
    if (elem.style.position === 'absolute') {
      ...
      if (cached) {
        ...
        // Only blit rects that extend outside the parent's layout bounds 鈥?
    ...
    // Recurse 鈥?absolute descendants can be nested arbitrarily deep
...
function renderScrolledChildren(
  ...
  inheritedBackgroundColor: Color | undefined,
  // When true (DECSTBM fast path), culled children keep their cache 鈥?
  ...
  // has the dirty child last 鈫?all prior clean children hit cache 鈫?
  // O(dirty) not O(mounted). Middle-growth leaves shift non-zero after
  // the dirty child 鈫?subsequent children yoga-read (needed for correct
  ...
  for (const childNode of node.childNodes) {
    ...
    if (cy) {
      ...
      if (
        ...
        // is the ONLY refresh point 鈥?without it, a middle-growth frame
        // leaves stale tops that misfire next frame.
        if (cached) cached.top = top
      }
      const bottom = top + height
      if (bottom <= scrollTopY || top >= scrollBottomY) {
        // Culled 鈥?outside visible window. Drop stale cache entries from
