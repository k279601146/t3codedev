...
import { getGraphemeSegmenter } from "./utils/grapheme.ts"
import sliceAnsi from "./utils/sliceAnsi.ts"

/** Debug logger 鈥?no-op placeholder until proper logger injection is added */
const logForDebugging = (_message: string) => {}
import { reorderBidi } from "./bidi.ts"
import { type Rectangle, unionRect } from "./layout/geometry.ts"
import {
  blitRegion,
  CellWidth,
 ...
 * is just property reads + setCellAt 鈥?no stringWidth, no style interning,
...
type WriteOperation = {
   ...
   * raw-ansi) 鈥?the screen's per-row bitmap is left untouched.
   */
  softWrap?: boolean[]
}
...
type ClearOperation = {
   ...
   * prevScreen. Normal-flow siblings' clears don't have this problem 鈥?
...
export default class Output {
   ...
   * growth. Preserving charCache across frames is the main win 鈥?most
  ...
  get(): Screen {
    ...
    // flow clears don't need this 鈥?a normal-flow node's old position
    ...
    for (const operation of this.operations) {
      switch (operation.type) {
        ...
        case 'clip':
          ...
          // extend below the scrollbox viewport 鈥?writes escape into
        ...
        case 'blit': {
          ...
          // Intersect with active clip 鈥?a child's clean-blit passes its full
          ...
          // that region holds the absolute node's stale paint 鈥?blitting
 ...
 *
 * Also precomputes styleId + hyperlink per style run (not per char) 鈥?an
 ...
 * monomorphic loop independently 鈥?better register allocation,
...
function writeLineToScreen(
  ...
  for (let charIdx = 0; charIdx < characters.length; charIdx++) {
    ...
    // don't occupy terminal cells 鈥?storing them as Narrow cells
    ...
    // Wide char at last column can't fit 鈥?terminal would wrap it to
    ...
    // reads 鈥?no intern, no extract, no filter per frame.
    setCellAt(screen, offsetX, y, {
