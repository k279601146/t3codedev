...
function parseKey(keypress: ParsedKey): [Key, string] {
  const key: Key = {
    ...
    // Super (Cmd on macOS / Win key) 鈥?only arrives via kitty keyboard
  ...
  // (which maps 'space' 鈫?' '). Without this, ctrl+space leaks the literal
  ...
  // continuation arrives as a text token with name='' 鈥?which falls through
  ...
  // after [ 鈥?real CSI u is always [<digits>鈥, and a bare startsWith('[')
  ...
  if (/^\[\d/.test(input) && input.endsWith('u')) {
    if (!keypress.name) {
      // Unmapped Kitty functional key (Caps Lock 57358, F13鈥揊35, KP nav,
      // bare modifiers, etc.) 鈥?keycodeToName() returned undefined. Swallow
      // so the raw "[57358u" doesn't leak into the prompt. See #38781.
      input = ''
    } else {
      // 'space' 鈫?' '; 'escape' 鈫?'' (key.escape carries it;
  ...
  // extraction as CSI u 鈥?without this, printable-char keycodes (single-letter
  // names) skip the nonAlphanumericKeys clear and leak "[27;..." as input.
  if (input.startsWith('[27;') && input.endsWith('~')) {
    if (!keypress.name) {
      // Unmapped modifyOtherKeys keycode 鈥?swallow for consistency with
