...
import { BEL, ESC, SEP } from "./termio/ansi.ts"
...
export class CharPool {
  ...
  private ascii: Int32Array = initCharAscii() // charCode 鈫?index, -1 = not interned
...
// so bit 0 of the resulting styleId is set 鈫?renderer won't skip inverted
// spaces as invisible.
...
// Bold (SGR 1) 鈥?stacks cleanly, no reflow in monospace. endCode 22
...
// Underline (SGR 4). Kept alongside yellow+bold 鈥?the underline is the
...
// the existing cell styling 鈥?the overlay IS finding the match.
const UNDERLINE_CODE: AnsiCode = {
  type: 'ansi',
  code: '\x1b[4m',
  endCode: '\x1b[24m',
}
// fg鈫抷ellow (SGR 33). With inverse already in the stack, the terminal
// swaps fg鈫攂g at render 鈥?so yellow-fg becomes yellow-BG. Original bg
// becomes fg (readable on most themes: dark-bg 鈫?dark-text on yellow).
// endCode 39 is 'default fg' 鈥?cancels any prior fg color cleanly.
...
export class StylePool {
   ...
   * another. Cached by (fromId, toId) 鈥?zero allocations after first call
   * for a given pair.
   */
   ...
   *  OTHER matches are plain inverse 鈥?bg inherits from the theme. Current
   ...
   *  overrides any existing fg (syntax highlighting) on those cells 鈥?fine,
  ...
  withCurrentMatch(baseId: number): number {
    ...
    if (id === undefined) {
      ...
      // still set, inverse swaps yellow-fg鈫攇rey-bg 鈫?grey-on-yellow on
      ...
      // fine 鈥?SGR 1 is fg-attribute-only, order-independent vs 7.
      ...
      // Underline as the unambiguous marker 鈥?yellow-bg can clash with
   ...
   * Matches native terminal selection 鈥?a dedicated bg color, not SGR-7
   ...
   *
   * Strips any existing bg (endCode 49m 鈥?REPLACES, so diff-added green
   * etc. don't bleed through) and any existing inverse (endCode 27m 鈥?
   * inverse on top of a solid bg would re-swap and look wrong).
   *
   * bg is set via setSelectionBg(); null 鈫?fallback to withInverse() so the
   * overlay still works before theme wiring sets a color (tests, first frame).
   * Cache is keyed by baseId only 鈥?setSelectionBg() clears it on change.
   */
...
// Unwritten cell as BigInt64 鈥?both words are 0, so the 64-bit value is 0n.
// Used by BigInt64Array.fill() for bulk clears (resetScreen, clearRegion).
// Not used for comparison 鈥?BigInt element reads cause heap allocation.
 ...
 *   word0: charId (full 32 bits 鈥?index into CharPool)
...
export type Screen = Size & {
  // Packed cell data 鈥?2 Int32s per cell: [charId, packed(styleId|hyperlinkId|width)]
  // cells and cells64 are views over the same ArrayBuffer.
  cells: Int32Array
  cells64: BigInt64Array // 1 BigInt64 per cell 鈥?used for bulk fill in resetScreen/clearRegion

  // Shared pools 鈥?IDs are valid across all screens using the same pools
  charPool: CharPool
  ...
  /**
   * Per-cell noSelect bitmap 鈥?1 byte per cell, 1 = exclude from text
   ...
   * content ends at absolute column N (exclusive 鈥?cells [0..N) are the
   ...
   * the packed typed array (both all-zero) 鈥?without it we'd either drop
   ...
   * old sw[r+1] 鈥?which correctly says the new row r is a continuation
...
export function resetScreen(
  ...
  // Reset all cells 鈥?single fill call, no loop
  ...
  // Shared pools accumulate 鈥?no clearing needed. Unique char/hyperlink sets are bounded.
 ...
 * Used for generational pool reset 鈥?after migrating, the screen's
 ...
 * index computation 鈥?caller must ensure index is valid.
 */
...
export function visibleCellAtIndex(
  ...
  // Then word1 >>> STYLE_SHIFT is the foreground style 鈥?skip if it's zero
...
export function setCellAt(
  ...
  if (cell.width === CellWidth.Wide) {
    ...
    if (spacerX < screen.width) {
      ...
      // Scenario: [a, 馃捇, spacer] 鈫?[鏈? spacer, ORPHAN spacer] when
      // yoga squishes a馃捇 to height 0 and 鏈?renders at the same y.
...
export function setCellStyleId(
  ...
  const word1 = cells[ci + 1]!
  const width = word1 & WIDTH_MASK
  // Skip spacer cells 鈥?inverse on the head cell visually covers both columns
 ...
 *
 * Clamps negative regionX/regionY to 0 (matching clearRegion) 鈥?absolute-
...
export function blitRegion(
  ...
  // softWrap is per-row 鈥?copy the row range regardless of stride/width.
  ...
  if (regionX === 0 && maxX === src.width && src.width === dst.width) {
    ...
    // noSelect is 1 byte/cell vs cells' 8 鈥?same region, different scale
...
export function shiftRows(
  ...
  if (n > 0) {
    // SU: row top+n..bottom 鈫?top..bottom-n; clear bottom-n+1..bottom
    ...
    // SD: row top..bottom+n 鈫?top-n..bottom; clear top..top-n-1
...
// OSC8 prefix: ESC ] 8 ; 鈥?cheap check to skip regex for the vast majority of styles (SGR = ESC [)
 ...
 * retain references to the Cell objects 鈥?their contents are overwritten each call.
 *
 ...
 * Cannot skip empty cells 鈥?the terminal still has content from the
 ...
 * renders. No damage tracking 鈥?noSelect doesn't affect terminal output,
