...
import * as warn from "../core/warn.ts";
...
export type Props = Except<Styles, 'textWrap'> & {
   ...
   * attribute 鈥?the FocusManager calls `focus(node)` during the
   * reconciler's `commitMount` phase.
   */
  autoFocus?: boolean;
  /**
   * Fired on left-button click (press + release without drag). Only works
   * inside `<AlternateScreen>` where mouse tracking is enabled 鈥?no-op
   ...
   * `mouseenter`, does NOT bubble 鈥?moving between children does not
