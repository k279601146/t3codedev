...
import { markDirty, scheduleRenderFrom } from "../core/dom.ts";
import { markCommitStart } from "../core/reconciler.ts";
import type { Styles } from "../core/styles.ts";
...
export type ScrollBoxHandle = {
   ...
   * render fires, this defers the position read to render time 鈥?
   ...
   * Does NOT fire for stickyScroll updates done by the Ink renderer 鈥?those
...
function ScrollBox({ children, ref, stickyScroll, ...style }: PropsWithChildren<ScrollBoxProps>): React.ReactNode {
  ...
  // The Ink renderer reads scrollTop from the node 鈥?no React state needed,
  ...
  // render 鈥?otherwise scheduleRender's leading edge fires on the FIRST
  ...
  function scrollMutated(el: DOMElement): void {
    ...
    // check) to skip their next tick 鈥?they compete for the event loop and
    // contributed to 1402ms max frame gaps during scroll drain.
    // noop 鈥?injected by business layer via onScrollActivity callback
    markDirty(el);
    markCommitStart();
    notify();
  ...
  useImperativeHandle(
    ...
    (): ScrollBoxHandle => ({
      ...
      scrollBy(dy: number) {
        ...
        if (!el) return;
        el.stickyScroll = false;
        // Wheel input cancels any in-flight anchor seek 鈥?user override.
        el.scrollAnchor = undefined;
      ...
      getPendingDelta() {
        ...
        // this to mount the union [committed, committed+pending] range 鈥?
    ...
    // refs + imports 鈥?stable. Empty deps avoids rebuilding the handle on
  ...
  // inner content (flexGrow:1, flexShrink:0 鈥?fills at least the viewport
  ...
  // available on the first render 鈥?ref callbacks fire after the initial
