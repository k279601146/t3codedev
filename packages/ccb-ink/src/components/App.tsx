...
// Business-layer callbacks 鈥?replaced with inline defaults so this package
...
import { EventEmitter } from "../core/events/emitter.ts";
import { InputEvent } from "../core/events/input-event.ts";
import { TerminalFocusEvent } from "../core/events/terminal-focus-event.ts";
import {
  INITIAL_STATE,
  type ParsedInput,
  type ParsedKey,
...
} from "../core/parse-keypress.ts";
import reconciler from "../core/reconciler.ts";
import { finishSelection, hasSelection, type SelectionState, startSelection } from "../core/selection.ts";
import { isXtermJs, setXtversionName, supportsExtendedKeys } from "../core/terminal.ts";
import { getTerminalFocused, setTerminalFocused } from "../core/terminal-focus-state.ts";
import { TerminalQuerier, xtversion } from "../core/terminal-querier.ts";
import {
...
import { DBP, DFE, DISABLE_MOUSE_TRACKING, EBP, EFE, HIDE_CURSOR, SHOW_CURSOR } from "../core/termio/dec.ts";
...
// a terminal mode re-assert (mouse tracking). Catches tmux detach鈫抋ttach,
// ssh reconnect, and laptop wake 鈥?the terminal resets DEC private modes
...
type Props = {
  ...
  // Dispatch a click at (col, row) 鈥?hit-tests the DOM tree and bubbles
...
export default class App extends PureComponent<Props, State> {
  ...
  lastClickRow = -1;
  clickCount = 0;
  // Deferred hyperlink-open timer 鈥?cancelled if a second click arrives
  ...
  // NOT deferred 鈥?it returns true from onClickAt and skips this timer.
  ...
  handleSetRawMode = (isEnabled: boolean): void => {
    ...
    if (isEnabled) {
      ...
      if (this.rawModeEnabledCount === 0) {
        ...
        // push (CSI >1u) and xterm modifyOtherKeys level 2 (CSI >4;2m) 鈥?
        ...
        // init sequence completes 鈥?avoids interleaving with alt-screen/mouse
    ...
    if (--this.rawModeEnabledCount === 0) {
      ...
      // replacing the tree (e.g., showSetupDialog 鈫?launchResumeChooser).
  ...
  handleTerminalFocus = (isFocused: boolean): void => {
    ...
    // and Clock (interval speed) 鈥?no App setState needed.
  ...
  handleSuspend = (): void => {
    ...
    // wasn't enabled, so it's safe to emit unconditionally 鈥?without
...
function processKeysInBatch(app: App, items: ParsedInput[], _unused1: undefined, _unused2: undefined): void {
  ...
  // Mode-1003 no-button motion is also excluded 鈥?passive cursor drift is
  ...
  for (const item of items) {
    ...
    // input 鈥?route them to the querier to resolve pending promises.
    ...
    if (sequence === FOCUS_OUT) {
      ...
      // terminal window 鈥?some emulators drop it rather than capturing the
...
export function handleMouseEvent(app: App, m: ParsedMouse): void {
  ...
  if (m.action === 'press') {
    if ((m.button & 0x20) !== 0 && baseButton === 3) {
      ...
      // past the edge, came back" 鈥?and tmux drops focus events unless
    ...
    if ((m.button & 0x20) !== 0) {
      ...
      // calls notifySelectionChange internally 鈥?no extra onSelectionChange.
    ...
    if (app.clickCount >= 2) {
      // Cancel any pending hyperlink-open from the first click 鈥?this is
    ...
    // SGR bit 0x08 = alt (xterm.js wires altKey here, not metaKey 鈥?see
  ...
  // over from pre-SGR X10 encoding) 鈥?filtering those would orphan
  ...
  // trackpad jitter during an intended double-click (press鈫抴obble鈫抮elease
  // 鈫抪ress) now correctly resolves to word-select instead of breaking to a
  // fresh single click. The nearLast window (500ms, 1 cell) bounds the
  // effect 鈥?a deliberate drag past that just starts a fresh chain.
  // A press+release with no drag in char mode is a click: anchor set,
  // focus null 鈫?hasSelection false. In word/line mode the press already
  ...
  // prior press 鈥?e.g. button was held when mouse tracking was enabled).
  if (!hasSelection(sel) && sel.anchor) {
    ...
    if (!app.props.onClickAt(col, row)) {
      ...
      // still reflects what the user clicked 鈥?deferring only the
      ...
      // here would open the URL 鈥?twice. We can't filter on Cmd: xterm.js
      ...
      if (url && process.env.TERM_PROGRAM !== 'vscode' && !isXtermJs()) {
        // Clear any prior pending timer 鈥?clicking a second link
