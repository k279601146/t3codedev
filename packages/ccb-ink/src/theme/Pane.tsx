...
import { useIsInsideModal } from "./modalContext.ts";
...
/**
 * A pane 鈥?a region of the terminal that appears below the REPL prompt,
 ...
 * `<Dialog>` instead 鈥?it registers its own keybindings. For a full
...
export function Pane({ children, color }: PaneProps): React.ReactNode {
  // When rendered inside FullscreenLayout's modal slot, its 鈻?divider IS
  ...
  // /model 鈫?ModelPicker) route through the modal slot unchanged.
  if (useIsInsideModal()) {
    ...
    // parent 鈥?/permissions body blanks on Down arrow. See #23592.
    return (
