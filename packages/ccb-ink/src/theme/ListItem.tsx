...
import { useDeclaredCursor } from "../hooks/use-declared-cursor.ts";
import { Box, Text } from "../index.ts";

type ListItemProps = {
  /**
   * Whether this item is currently focused (keyboard selection).
   * Shows the pointer indicator (鉂? when true.
   */
  isFocused: boolean;

  /**
   * Whether this item is selected (chosen/checked).
   * Shows the checkmark indicator (鉁? when true.
   * @default false
   */
  isSelected?: boolean;
 ...
 * - Pointer indicator (鉂? for focused items
 * - Checkmark indicator (鉁? for selected items
 * - Scroll indicators (鈫撯啈) for truncated lists
