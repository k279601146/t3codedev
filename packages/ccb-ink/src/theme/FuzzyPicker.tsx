...
import { clamp } from "../core/layout/geometry.ts";
import { Box, Text, useTerminalFocus } from "../index.ts";
...
type PickerAction<T> = {
  /** Hint label shown in the byline, e.g. "mention" 鈫?"Tab to mention". */
  action: string;
  handler: (item: T) => void;
};
...
type Props<T> = {
  ...
  /** Keep to one line 鈥?preview handles overflow. */
   ...
   * always match screen direction 鈥?鈫?walks visually up regardless.
   */
  direction?: 'down' | 'up';
  ...
  /**
   * Tab key. If provided, Tab no longer aliases Enter 鈥?it gets its own
   ...
   * Useful for async preview loading 鈥?keeps I/O out of renderPreview.
   */
  ...
  /**
   * Status line below the list, e.g. "500+ matches" or "42 matches鈥?.
   * Caller decides when to show it 鈥?pass undefined to hide.
   */
  matchLabel?: string;
  selectAction?: string;
...
export function FuzzyPicker<T>({
  ...
  // no-op 鈥?return/downArrow are handled by handleKeyDown below. onCancel
  ...
  // Structure must not depend on preview truthiness 鈥?when focused goes
  // undefined (e.g. delete clears matches), switching row鈫抐ragment would
  ...
  const listGroup =
    renderPreview && previewPosition === 'right' ? (
      ...
      // between list/matchLabel/preview 鈥?that read as extra space above the
  ...
  return (
    <Pane color="permission">
      <Box flexDirection="column" gap={1} tabIndex={0} autoFocus onKeyDown={handleKeyDown}>
        ...
        <Text dimColor>
          <Byline>
            <KeyboardShortcutHint shortcut="鈫?鈫? action={compact ? 'nav' : 'navigate'} />
