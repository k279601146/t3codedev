import {  feature  } from "../../../ccb-engine/src/featureFlags.ts";
import React, { createContext, useContext, useEffect, useMemo, useState } from 'react';
import useStdin from "../hooks/use-stdin.ts";
import { getSystemThemeName, type SystemTheme } from "./systemTheme.ts";
import type { ThemeName, ThemeSetting } from "./theme-types.ts";
...
export function ThemeProvider({ children, initialState, onThemeSave = defaultSaveTheme }: Props) {
  ...
  useEffect(() => {
    if (feature('AUTO_THEME')) {
      ...
      let cancelled = false;
      void import('../../utils/systemThemeWatcher.ts').then(({ watchSystemTheme }) => {
        if (cancelled) return;
