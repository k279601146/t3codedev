 ...
 * cursor 鈥?so parking it at the text input's caret makes CJK input
 ...
 * phase 鈥?after resetAfterCommit calls scheduleRender. scheduleRender
...
export function useDeclaredCursor({
  ...
  //      a memo'd Footer) doesn't re-render this commit 鈥?an inactive
  //      instance re-rendering here must not clobber it.
  //   2. Sibling handoff (menu focus moving between list items) 鈥?when
  ...
  // Clear on unmount (conditionally 鈥?another instance may own by then).
  // Separate effect with empty deps so cleanup only fires once 鈥?not on
