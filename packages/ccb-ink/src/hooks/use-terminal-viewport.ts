 ...
 * re-renders on their own 鈥?callers that re-render for other reasons (e.g.
...
export function useTerminalViewport(): [
  ...
  // without React being aware. Only updates the ref 鈥?no setState
  ...
  useLayoutEffect(() => {
    ...
    // positions without scroll offset 鈥?scrollTop is applied at render time.
    ...
    while (parent) {
      ...
      // Non-scroll nodes have undefined scrollTop 鈫?falsy fast-path.
    ...
    // scrollback by log-update (content change 鈫?full reset 鈫?flicker).
