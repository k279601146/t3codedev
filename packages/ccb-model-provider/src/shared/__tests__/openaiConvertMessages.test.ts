...
import { anthropicMessagesToOpenAI } from "../openaiConvertMessages.ts"
import type { UserMessage, AssistantMessage } from "../../types/message.ts"
...
describe('DeepSeek thinking mode (enableThinking)', () => {
  ...
  test('always preserves reasoning_content from all turns', () => {
    const result = anthropicMessagesToOpenAI(
      [
        // Turn 1: user 鈫?assistant (with thinking)
        makeUserMsg('question 1'),
        makeAssistantMsg([
          { type: 'thinking' as const, thinking: 'Turn 1 reasoning...' },
          { type: 'text', text: 'Turn 1 answer' },
        ]),
        // Turn 2: new user message 鈫?reasoning should still be preserved
  ...
  test('preserves reasoning_content in multi-iteration tool call within same turn', () => {
    ...
    // user 鈫?assistant(thinking+tool_call) 鈫?tool_result 鈫?assistant(thinking+tool_call) 鈫?tool_result 鈫?assistant(thinking+text)
    const result = anthropicMessagesToOpenAI(
      [
        ...
        makeAssistantMsg([
          ...
          { type: 'text', text: 'Tomorrow will be cloudy, 7-13掳C.' },
        ]),
      ],
      [] as any,
      { enableThinking: true },
  ...
  // 鈹€鈹€ fix: reorder tool and user messages for OpenAI API compatibility (#168) 鈹€鈹€

  test('tool messages come BEFORE user text when mixed in same turn', () => {
    // OpenAI requires: assistant(tool_calls) 鈫?tool 鈫?user
