...
import { adaptOpenAIStreamToAnthropic } from "../openaiStreamAdapter.ts"
...
describe('prompt caching support', () => {
  ...
  test('captures output_tokens and input_tokens from trailing chunk sent after finish_reason', async () => {
    ...
    // queryModelOpenAI's spread can override the zeros from message_start 鈥?which is
    ...
    const events = await collectEvents([
      ...
      // finish_reason chunk 鈥?usage not yet available
      makeChunk({
        choices: [{ index: 0, delta: {}, finish_reason: 'stop' }],
      }),
  ...
  // 鈹€鈹€ cache_read_input_tokens in message_delta (the core bug fix) 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
  ...
  test('message_delta carries cache_read_input_tokens from trailing usage chunk', async () => {
    ...
    // queryModelOpenAI's spread 鈥?even though cachedTokens was captured internally.
    const events = await collectEvents([
      makeChunk({
        choices: [
          { index: 0, delta: { content: 'answer' }, finish_reason: null },
    ...
    // message_start is emitted before trailing chunk 鈥?cache fields are 0
    const msgStart = events.find(e => e.type === 'message_start') as any
    expect(msgStart.message.usage.cache_read_input_tokens).toBe(0)
