...
import type { SystemPrompt } from "../types/systemPrompt.ts"
 ...
 *
 * Key conversions:
 * - system prompt 鈫?role: "system" message prepended
 * - tool_use blocks 鈫?tool_calls[] on assistant message
 * - tool_result blocks 鈫?role: "tool" messages
 * - thinking blocks 鈫?preserved as reasoning_content (DeepSeek requires passing it back)
 * - cache_control 鈫?stripped
 */
...
/**
 * 灏?Anthropic image 鍧楄浆鎹负 OpenAI image_url 鏍煎紡銆?
 *
 * Anthropic 鏍煎紡: { type: "image", source: { type: "base64", media_type: "image/png", data: "..." } }
 * OpenAI 鏍煎紡: { type: "image_url", image_url: { url: "data:image/png;base64,..." } }
 */
