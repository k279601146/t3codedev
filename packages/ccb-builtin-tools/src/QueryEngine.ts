...
const messageSelector = ():
  ...
  | null => {
  try {
    return require('./components/MessageSelector.ts')
  } catch {
    return null
  }
}
...
export class QueryEngine {
  ...
  async *submitMessage(
    ...
    // The env var is an explicit opt-in signal ï¿?the caller has wired up
    ...
    let processUserInputContext: ProcessUserInputContext = {
      ...
      // slash-command processing) keeps the no-op ï¿?nothing else calls
    ...
    // an assistant/user/compact_boundary message ï¿?which doesn't happen until
    ...
    // ï¿?the single largest controllable critical-path cost after module eval.
    ...
    if (!shouldQuery) {
      ...
      for (const msg of messagesFromUserInput) {
        ...
        // Local command output ï¿?yield as a synthetic assistant message so
    ...
    // shift()s during the turn ï¿?the index slides. If this entry is rotated
    ...
    for await (const message of query({
      ...
      if (
        ...
        // between turns), tailUuid points to a never-written message ï¿?
        // applyPreservedSegmentRelinks fails its tailâ†’head walk ï¿?returns
        // without pruning ï¿?resume loads full pre-compact history.
        if (
          persistSession &&
          message.type === 'system' &&
        ...
        if (persistSession) {
          ...
          // one's message.usage/stop_reason on message_delta ï¿?relying on
      ...
      switch (message.type) {
        ...
        case 'progress': {
          ...
          // dedup walk freezes startingParentUuid at the wrong message ï¿?
        ...
        case 'system': {
          ...
          // stale markers. The yielded boundary is a signal, not data to push ï¿?
    ...
    // instead of the assistant ï¿?which makes textResult extraction below
    ...
    // Capture for the error_during_execution diagnostic ï¿?isResultSuccessful
    ...
    if (!isResultSuccessful(result, lastStopReason)) {
      yield {
        ...
        uuid: randomUUID(),
        // Diagnostic prefix: these are what isResultSuccessful() checks ï¿?if
