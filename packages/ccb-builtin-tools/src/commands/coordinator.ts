/**
 * /coordinator ï¿?Toggle coordinator (multi-worker orchestration) mode.
 *
    ...
    Promise.resolve({
      async call(
        ...
        if (mod.isCoordinatorMode()) {
          ...
          onDone('Coordinator mode disabled ï¿?back to normal mode', {
            display: 'system',
            metaMessages: [
          ...
          onDone(
            'Coordinator mode enabled ï¿?use Agent(subagent_type: "worker") to dispatch tasks',
            {
              display: 'system',
              metaMessages: [
