/**
 * /proactive ï¿?Toggle proactive (autonomous tick-driven) mode.
 *
    ...
    Promise.resolve({
      async call(
        ...
        if (mod.isProactiveActive()) {
          ...
          onDone(
            'Proactive mode enabled ï¿?model will work autonomously between ticks',
            {
              display: 'system',
              metaMessages: [
                '<system-reminder>\nProactive mode is now enabled. You will receive periodic <tick> prompts. Do useful work on each tick, or call Sleep if there is nothing to do. Do not output "still waiting" ï¿?either act or sleep.\n</system-reminder>',
              ],
            },
          )
        }
