...
import { isPluginGloballyInstalled } from "@t3tools/ccb-engine/src/utils/plugins/installedPluginsManager.ts";
import {
  createPluginId,
...
import { loadKnownMarketplacesConfig } from "@t3tools/ccb-engine/src/utils/plugins/marketplaceManager.ts";
...
export function DiscoverPlugins({
  ...
  const installSelectedPlugins = async () => {
    ...
    if (failureCount === 0) {
      const message =
        `âœ?Installed ${successCount} ${plural(successCount, 'plugin')}. ` + `Run /reload-plugins to activate.`;
      setResult(message);
    } else if (successCount === 0) {
      setError(`Failed to install: ${formatFailureDetails(newFailedPlugins, true)}`);
    } else {
      const message =
        `âœ?Installed ${successCount} of ${successCount + failureCount} plugins. ` +
    ...
    return (
      <PluginOptionsFlow
        ...
        onDone={(outcome, detail) => {
          switch (outcome) {
            case 'configured':
              finish(`âœ?Installed and configured ${plugin.name}. Run /reload-plugins to apply.`);
              break;
            case 'skipped':
              finish(`âœ?Installed ${plugin.name}. Run /reload-plugins to apply.`);
              break;
            case 'error':
          ...
          >
            <Box>
              ...
              <Text>
                ...
                {plugin.entry.name}
                <Text dimColor> Â· {plugin.marketplaceName}</Text>
