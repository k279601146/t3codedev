...
import { loadMcpServerUserConfig, saveMcpServerUserConfig } from "@t3tools/ccb-engine/src/utils/plugins/mcpbHandler.ts";
import { getUnconfiguredChannels, type UnconfiguredChannel } from "@t3tools/ccb-engine/src/utils/plugins/mcpPluginIntegration.ts";
 ...
 * if the plugin somehow didn't make it into the fresh load â€?callers treat
 ...
 * collapse to this shape â€?the only difference is which save function runs.
 */
type ConfigStep = {
  key: string;
  title: string;
...
type Props = {
  plugin: LoadedPlugin;
  /** `name@marketplace` â€?the savePluginOptions / saveMcpServerUserConfig key. */
  pluginId: string;
  /**
...
export function PluginOptionsFlow({ plugin, pluginId, onDone }: Props): React.ReactNode {
  ...
  // Nothing to configure â†?tell the caller and render nothing. Effect,
  ...
  // key forces a remount when advancing to the next step â€?React would
