...
import { shouldSkipPluginAutoupdate } from "@t3tools/ccb-engine/src/utils/config.ts";
...
import { updatePluginsForMarketplaces } from "@t3tools/ccb-engine/src/utils/plugins/pluginAutoupdate.ts";
import { loadAllPlugins } from "@t3tools/ccb-engine/src/utils/plugins/pluginLoader.ts";
import { isMarketplaceAutoUpdate } from "@t3tools/ccb-engine/src/utils/plugins/schemas.ts";
...
export function ManageMarketplaces({
  ...
  if (loading) {
    return <Text>Loading marketplacesâ€?/Text>;
  }
      ...
      <Box flexDirection="column">
        ...
        {isUpdating && (
          <Box marginTop={1} flexDirection="column">
            <Text color="claude">Updating marketplaceâ€?/Text>
  ...
  return (
    <Box flexDirection="column">
              ...
              <Box flexDirection="column" flexGrow={1}>
                <Box flexDirection="row" gap={1}>
                  <Text bold strikethrough={state.pendingRemove} dimColor={state.pendingRemove}>
                    {state.name === 'claude-plugins-official' && <Text color="claude">âœ?</Text>}
                    {state.name}
                    {state.name === 'claude-plugins-official' && <Text color="claude"> âœ?/Text>}
                  </Text>
                ...
                <Text dimColor>
                  ...
                  {state.installedPlugins && state.installedPlugins.length > 0 && (
                    <> â€?{state.installedPlugins.length} installed</>
                  )}
                  {state.lastUpdated && <> â€?Updated {new Date(state.lastUpdated).toLocaleDateString()}</>}
                </Text>
              </Box>
            </Box>
          );
      ...
      {isProcessing && (
        <Box marginTop={1}>
          <Text color="claude">Processing changesâ€?/Text>
        </Box>
      )}
