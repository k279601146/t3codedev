...
import type { PluginOptionSchema, PluginOptionValues } from "@t3tools/ccb-engine/src/utils/plugins/pluginOptionsStorage.ts";
...
export function buildFinalValues(
  ...
  for (const fieldKey of fields) {
    ...
    if (schema?.type === 'number') {
      // Number('') returns 0, not NaN â€?omit blank number inputs so
...
export function PluginOptionsDialog({
  ...
  // Prepopulate from initialValues but skip sensitive fields â€?we don't
    ...
    <Dialog title={title} subtitle={subtitle} onCancel={onCancel} isCancelActive={false}>
      ...
      <Box flexDirection="column">
        ...
        {currentFieldIndex < fields.length - 1 && (
          <Text dimColor={true}>Tab: Next field Â· Enter: Save and continue</Text>
        )}
