...
import type { LoadedPlugin } from "@t3tools/ccb-engine/src/types/plugin.ts";
...
import { formatInstallCount, getInstallCounts } from "@t3tools/ccb-engine/src/utils/plugins/installCounts.ts";
import { isPluginGloballyInstalled, isPluginInstalled } from "@t3tools/ccb-engine/src/utils/plugins/installedPluginsManager.ts";
import {
  createPluginId,
...
} from "@t3tools/ccb-engine/src/utils/plugins/marketplaceHelpers.ts";
import { getMarketplace, loadKnownMarketplacesConfig } from "@t3tools/ccb-engine/src/utils/plugins/marketplaceManager.ts";
import { OFFICIAL_MARKETPLACE_NAME } from "@t3tools/ccb-engine/src/utils/plugins/officialMarketplace.ts";
import { installPluginFromMarketplace } from "@t3tools/ccb-engine/src/utils/plugins/pluginInstallationHelpers.ts";
...
import { truncateToWidth } from "@t3tools/ccb-engine/src/utils/truncate.ts";
...
export function BrowseMarketplace({
      ...
      try {
        ...
        if (targetPlugin) {
            ...
            if (marketplace) {
              ...
              if (plugin) {
                ...
                foundPlugin = {
                  ...
                  // block 鈥?user may want to promote to user scope (gh-29997).
          ...
          if (foundPlugin && foundMarketplace) {
            // Block only on global (user/managed) install 鈥?project/local scope
            ...
            // (installPluginOp 鈫?addInstalledPlugin) already supports multiple
      ...
      try {
        ...
        for (const entry of marketplace.plugins) {
          ...
          installablePlugins.push({
            ...
            // Project/local installs don't block 鈥?user can add user scope
  ...
  const installSelectedPlugins = async () => {
    ...
    if (failureCount === 0) {
      // All succeeded
      const message =
        `鉁?Installed ${successCount} ${plural(successCount, 'plugin')}. ` + `Run /reload-plugins to activate.`;
      ...
      const message =
        `鉁?Installed ${successCount} of ${successCount + failureCount} plugins. ` +
    ...
    return (
      <PluginOptionsFlow
        ...
        onDone={(outcome, detail) => {
          switch (outcome) {
            case 'configured':
              finish(`鉁?Installed and configured ${plugin.name}. Run /reload-plugins to apply.`);
              break;
            case 'skipped':
              finish(`鉁?Installed ${plugin.name}. Run /reload-plugins to apply.`);
              break;
            case 'error':
          ...
          <Box key={marketplace.name} flexDirection="column" marginBottom={index < marketplaces.length - 1 ? 1 : 0}>
            ...
            <Box marginLeft={2}>
              <Text dimColor>
                ...
                {marketplace.installedCount > 0 && ` 路 ${marketplace.installedCount} already installed`}
                {marketplace.source && ` 路 ${marketplace.source}`}
              </Text>
            </Box>
          </Box>
        ))}
      ...
      <Box flexDirection="column">
        ...
        <Box flexDirection="column" marginBottom={1}>
          ...
          {selectedPlugin.entry.hooks && (
            <Text dimColor>路 Hooks: {Object.keys(selectedPlugin.entry.hooks).join(', ')}</Text>
          )}
            ...
            !selectedPlugin.entry.mcpServers && (
              <>
                ...
                (selectedPlugin.entry.source.source === 'github' ||
                  ...
                  <Text dimColor>路 Component summary not available for remote plugin</Text>
                ) : (
                  ...
                  <Text dimColor>路 Components will be discovered at installation</Text>
                )}
              </>
            )}
        </Box>
          ...
          {menuOptions.map((option, index) => (
            <Box key={option.action}>
              ...
              <Text bold={detailsMenuIndex === index}>
                {isInstalling && option.action === 'install' ? 'Installing鈥? : option.label}
              </Text>
            </Box>
          ))}
        </Box>
          ...
          <Box key={plugin.pluginId} flexDirection="column" marginBottom={isLast && !error ? 0 : 1}>
            ...
            {plugin.entry.description && (
              <Box marginLeft={4}>
                ...
                {plugin.entry.version && <Text dimColor> 路 v{plugin.entry.version}</Text>}
              </Box>
            )}
          </Box>
        );
