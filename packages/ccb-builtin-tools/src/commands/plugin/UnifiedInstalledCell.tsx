...
export function UnifiedInstalledCell({ item, isSelected }: Props): React.ReactNode {
  ...
  if (item.type === 'plugin') {
    ...
    return (
      <Box>
        ...
        <Text dimColor> 路 {item.marketplace}</Text>
        <Text dimColor={!isSelected}> 路 {statusIcon} </Text>
  ...
  if (item.type === 'failed-plugin') {
    ...
    const statusText = `failed to load 路 ${item.errorCount} ${plural(item.errorCount, 'error')}`;
  ...
  if (item.indented) {
    return (
      <Box>
        <Text color={isSelected ? 'suggestion' : undefined}>{isSelected ? `${figures.pointer} ` : '  '}</Text>
        <Text dimColor={!isSelected}>鈹?</Text>
        <Text color={isSelected ? 'suggestion' : undefined}>{item.name}</Text>
        <Text dimColor={!isSelected}>
          {' '}
