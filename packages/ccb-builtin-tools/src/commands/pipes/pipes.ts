...
import {
  ...
  isMainMachine,
  mergeWithLanPeers,
} from "@t3tools/ccb-engine/src/utils/pipeRegistry.ts"
...
export const call: LocalCommandCall = async (_args, context) => {
  ...
  if (args.startsWith('select ') || args.startsWith('sel ')) {
    ...
    return {
      type: 'text',
      value: `Selected ${pipeName} é”?messages will be broadcast to this pipe.`,
    }
  }
  ...
  lines.push('')
  lines.push(
    `Selected: ${selected.length > 0 ? selected.join(', ') : '(none é”?messages run locally only)'}`,
  )
  lines.push('')
  lines.push('Commands:')
  lines.push('  /pipes select <name>    é”?select pipe for broadcast')
  lines.push('  /pipes deselect <name>  é”?deselect pipe')
  lines.push('  /pipes all              é”?select all connected')
  lines.push('  /pipes none             é”?deselect all')
  lines.push('  /send <name> <msg>      é”?send to specific pipe')
  lines.push('  /claim-main             é”?claim this machine as main')
