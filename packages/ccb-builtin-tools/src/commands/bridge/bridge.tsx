...
import { shouldShowRemoteCallout } from "@t3tools/ccb-engine/src/components/RemoteCallout.ts";
...
import type { LocalJSXCommandContext, LocalJSXCommandOnDone } from "@t3tools/ccb-engine/src/types/command.ts";
...
/**
 * /remote-control command â€?manages the bidirectional bridge connection.
 *
...
function BridgeToggle({ onDone, name }: Props): React.ReactNode {
  ...
  useEffect(() => {
    ...
    // disconnect confirmation. Outbound-only (CCR mirror) doesn't count â€?
    ...
    void (async () => {
      ...
      // cache is stale â€?so Max users don't get a false "not enabled" error)
      ...
      // Enable the bridge â€?useReplBridge in REPL.tsx handles the rest:
  ...
  return (
    <Dialog title="Remote Control" onCancel={handleContinue} hideInputGuide>
      <Box flexDirection="column" gap={1}>
        ...
        <Text dimColor>Enter to select Â· Esc to continue</Text>
      </Box>
    </Dialog>
  );
}
...
async function checkBridgePrerequisites(): Promise<string | null> {
  // Check organization policy â€?remote control may be disabled
  ...
  // initReplBridge onto the v1 path â€?so the prerequisite check must match.
  let useV2 = isEnvLessBridgeEnabled();
  if (feature('KAIROS') && useV2) {
    const { isAssistantMode } = await import('../../assistant/index.ts');
    if (isAssistantMode()) {
      useV2 = false;
    }
  }
