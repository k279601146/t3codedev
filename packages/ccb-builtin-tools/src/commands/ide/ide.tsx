...
import { Select } from "@t3tools/ccb-engine/src/components/CustomSelect/index.ts";
...
} from "@t3tools/ccb-engine/src/components/IdeAutoConnectDialog.ts";
import { Box, Text } from '@anthropic/ink';
import { clearServerCache } from "@t3tools/ccb-engine/src/services/mcp/client.ts";
import type { ScopedMcpServerConfig } from "@t3tools/ccb-engine/src/services/mcp/types.ts";
            ...
            <Box marginTop={1} flexDirection="column">
              {unavailableIDEs.map((ide, index) => (
                <Box key={index} paddingLeft={3}>
                  <Text dimColor>
                    â€?{ide.name}: {formatWorkspaceFolders(ide.workspaceFolders)}
                  </Text>
                </Box>
              ))}
            </Box>
...
function IDECommandFlow({
  ...
  if (connectingIDE) {
    return <Text dimColor>Connecting to {connectingIDE.name}â€?/Text>;
  }
...
export function formatWorkspaceFolders(folders: string[], maxLength: number = 100): string {
  ...
  // Account for ", â€? if there are more folders
  const ellipsisOverhead = hasMore ? 3 : 0; // ", â€?
  ...
  const formattedFolders = foldersToShow.map(folder => {
    ...
    return 'â€? + folder.slice(-(maxLengthPerPath - 1));
  });
