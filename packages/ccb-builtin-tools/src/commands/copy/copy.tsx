...
import type { OptionWithDescription } from "@t3tools/ccb-engine/src/components/CustomSelect/select.ts";
...
import { extractTextContent, stripPromptXMLTags } from "@t3tools/ccb-engine/src/utils/messages.ts";
import { countCharInString } from "@t3tools/ccb-engine/src/utils/stringUtils.ts";
...
async function copyOrWriteToFile(text: string, filename: string): Promise<string> {
  ...
  // Also write to a temp file â€?clipboard paths are best-effort (OSC 52 needs
