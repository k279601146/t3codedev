...
import { getGhAuthStatus } from "@t3tools/ccb-engine/src/utils/github/ghAuthStatus.ts";
import {
...
function Web({ onDone }: { onDone: LocalJSXCommandOnDone }) {
  ...
  const handleConfirm = async (token: RedactedGithubToken) => {
    ...
    // Token import succeeded. Environment creation is best-effort â€?if it
  ...
  if (step.name === 'checking') {
    return <LoadingState message="Checking login statusâ€? />;
  }

  if (step.name === 'uploading') {
    return <LoadingState message="Connecting GitHub to Claudeâ€? />;
  }
