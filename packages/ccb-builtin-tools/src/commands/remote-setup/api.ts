...
import { fetchEnvironments } from "@t3tools/ccb-engine/src/utils/teleport/environments.ts"
 ...
 * non-fatal â€?the token import already succeeded, and the web state
...
export async function createDefaultEnvironment(): Promise<boolean> {
  ...
  // auth dep). The public path uses build_flexible_auth â€?same path
