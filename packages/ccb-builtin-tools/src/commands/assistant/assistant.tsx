...
import { findGitRoot } from "@t3tools/ccb-engine/src/utils/git.ts";
import { buildCliLaunch, spawnCli } from "@t3tools/ccb-engine/src/utils/cliLaunch.ts";
import { getKairosActive, setKairosActive } from "@t3tools/ccb-engine/src/bootstrap/state.ts";
...
export function NewInstallWizard({ defaultDir, onInstalled, onCancel, onError }: WizardProps): React.ReactNode {
  ...
  function startDaemon(): void {
    ...
    try {
      ...
      // and create a CCR session â€?main.tsx will tell the user to reconnect.
      setTimeout(() => {
        onInstalled(dir);
      }, 1500);
    } catch (err) {
  ...
  return (
    <Dialog title="Assistant Setup" onCancel={onCancel} hideInputGuide>
      <Box flexDirection="column" gap={1}>
        ...
        <Text dimColor>Enter to select Â· Esc to cancel</Text>
      </Box>
    </Dialog>
  );
}
