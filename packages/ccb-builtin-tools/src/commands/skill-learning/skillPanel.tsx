...
import { isSkillLearningEnabled } from "@t3tools/ccb-engine/src/services/skillLearning/featureCheck.ts";
...
Skill Learning 是一个闭环学习系统，通过观察用户的操作模式自动提取直�?instinct)�?
并在达到阈值后生成可复用的 skill 文件、agent �?command�?

## 工作流程
1. **Observe** �?记录每轮对话中的工具调用、用户纠正、错误解决模�?
2. **Analyze** �?使用启发式或 LLM 后端分析观察数据，提�?instinct candidate
3. **Evolve** �?将高置信�?instinct 聚类，生�?skill/agent/command 候�?
4. **Lifecycle** �?对生成的 skill 进行去重、版本比较、归档或替换

## 子命�?
- /skill-learning status       �?查看当前项目的观察和直觉数量
- /skill-learning ingest       �?�?transcript 导入观察数据
- /skill-learning evolve       �?生成 skill 候�?(--generate 写入磁盘)
- /skill-learning export       �?导出 instinct �?JSON
- /skill-learning import       �?导入 instinct JSON
- /skill-learning prune        �?清理过期�?pending instinct
- /skill-learning promote      �?�?instinct/gap 提升为全局范围
- /skill-learning projects     �?列出所有已知的项目范围

## 启用方式
- SKILL_LEARNING_ENABLED=1 �?FEATURE_SKILL_LEARNING=1
- 状�? ${isSkillLearningEnabled() ? '已启�? : '未启�?}
`;

async function getStatusText(): Promise<string> {
  const { readObservations, loadInstincts, resolveProjectContext } = await import(
    '../../services/skillLearning/index.ts'
  );
...
async function startSkillLearning(): Promise<string> {
  ...
  try {
    const { initSkillLearning } = await import('../../services/skillLearning/runtimeObserver.ts');
    initSkillLearning();
...
export async function call(onDone: LocalJSXCommandOnDone, _context: unknown, args?: string): Promise<React.ReactNode> {
  ...
  if (trimmed) {
    const { call: textCall } = await import('./skill-learning.ts');
