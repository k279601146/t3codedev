...
import {
  ...
  saveInstinct,
  upsertInstinct,
} from "@t3tools/ccb-engine/src/services/skillLearning/index.ts"
  ...
  switch (sub) {
    ...
    case 'projects': {
      ...
      for (const record of projects) {
        ...
        lines.push(
          `- ${record.projectName} (${record.projectId}) â€?instincts: ${instincts.length}, observations: ${observations.length}, lastSeen: ${record.lastSeenAt}`,
        )
      }
      return { type: 'text', value: lines.join('\n') }
    }
