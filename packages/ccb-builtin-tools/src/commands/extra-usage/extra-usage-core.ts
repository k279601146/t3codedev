import {
  ...
  createAdminRequest,
  getMyAdminRequests,
} from "@t3tools/ccb-engine/src/services/api/adminRequests.ts"
import { invalidateOverageCreditGrantCache } from "@t3tools/ccb-engine/src/services/api/overageCreditGrant.ts"
import { type ExtraUsage, fetchUtilization } from "@t3tools/ccb-engine/src/services/api/usage.ts"
import { getSubscriptionType } from "@t3tools/ccb-engine/src/utils/auth.ts"
import { hasClaudeAiBillingAccess } from "@t3tools/ccb-engine/src/utils/billing.ts"
...
export async function runExtraUsage(): Promise<ExtraUsageResult> {
  ...
  if (!hasBillingAccess && isTeamOrEnterprise) {
    ...
    try {
      ...
      logError(error as Error)
      // If eligibility check fails, continue â€?the create endpoint will enforce if necessary
    }
