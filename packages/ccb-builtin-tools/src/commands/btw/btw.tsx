...
import { Markdown } from "@t3tools/ccb-engine/src/components/Markdown.ts";
import { SpinnerGlyph } from "@t3tools/ccb-engine/src/components/Spinner/SpinnerGlyph.ts";
import { DOWN_ARROW, UP_ARROW } from "@t3tools/ccb-engine/src/constants/figures.ts";
...
import { type CacheSafeParams, getLastCacheSafeParams } from "@t3tools/ccb-engine/src/utils/forkedAgent.ts";
import { getMessagesAfterCompactBoundary } from "@t3tools/ccb-engine/src/utils/messages.ts";
import type { ProcessUserInputContext } from "@t3tools/ccb-engine/src/utils/processUserInput/processUserInput.ts";
import { runSideQuestion } from "@t3tools/ccb-engine/src/utils/sideQuestion.ts";
  ...
  return (
    <Box flexDirection="column" paddingLeft={2} marginTop={1} tabIndex={0} autoFocus onKeyDown={handleKeyDown}>
      ...
      {(response || error) && (
        <Box marginTop={1}>
          <Text dimColor>
            {UP_ARROW}/{DOWN_ARROW} to scroll Â· Space, Enter, or Escape to dismiss
          </Text>
        </Box>
      )}
    </Box>
 ...
 *
 * The preferred source is getLastCacheSafeParams â€?the exact
