...
import { ExportDialog } from "@t3tools/ccb-engine/src/components/ExportDialog.ts";
...
import { renderMessagesToPlainText } from "@t3tools/ccb-engine/src/utils/exportRenderer.tsx";
import { writeFileSync_DEPRECATED } from "@t3tools/ccb-engine/src/utils/slowOperations.ts";
...
export function extractFirstPrompt(messages: Message[]): string {
  ...
  result = result.split('\n')[0] || '';
  if (result.length > 50) {
    result = result.substring(0, 49) + 'â€?;
  }
