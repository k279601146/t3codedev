...
import { isSkillSearchEnabled } from "@t3tools/ccb-engine/src/services/skillSearch/featureCheck.ts";
...
const ABOUT_TEXT = `# Skill Search (自动技能匹�?

Skill Search 控制对话中的自动技能匹配功能�?

启用后，Claude Code 会在每轮对话中自动搜索并加载与当前任务最相关�?skill 文件�?
无需手动指定。搜索基�?TF-IDF 向量余弦相似度，支持英文词干化和 CJK bi-gram 分词�?

## 工作原理
1. 对话开始时，自动索�?.claude/skills/ �?~/.claude/skills/ 下的 Markdown 文件
2. 每轮对话根据上下文自动匹配最相关�?skill
3. 匹配到的 skill 内容会作为上下文注入，指�?Claude Code 的行�?

## 控制方式
- /skill-search start  �?启用自动匹配
- /skill-search stop   �?禁用自动匹配
- /skill-search status �?查看当前状�?

当前状�? ${isSkillSearchEnabled() ? '已启�? : '未启�?}
`;
...
async function startSkillSearch(): Promise<string> {
  ...
  try {
    const { clearSkillIndexCache } = await import('../../services/skillSearch/localSearch.ts');
