...
import type {
  ...
  SerializedMessage,
  TranscriptMessage,
} from "@t3tools/ccb-engine/src/types/logs.ts"
import { parseJSONL } from "@t3tools/ccb-engine/src/utils/json.ts"
import {
  getProjectDir,
  getTranscriptPath,
...
import { escapeRegExp } from "@t3tools/ccb-engine/src/utils/stringUtils.ts"
 ...
 * Collapses whitespace â€?multiline first messages (pasted stacks, code)
...
async function createFork(customTitle?: string): Promise<{
  ...
  // with an empty replacements Map â†?previously-replaced results are classified
