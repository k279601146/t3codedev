...
import { LogSelector } from "@t3tools/ccb-engine/src/components/LogSelector.ts";
import { MessageResponse } from "@t3tools/ccb-engine/src/components/MessageResponse.ts";
...
import type { LogOption } from "@t3tools/ccb-engine/src/types/logs.ts";
import { agenticSessionSearch } from "@t3tools/ccb-engine/src/utils/agenticSessionSearch.ts";
import { checkCrossProjectResume } from "@t3tools/ccb-engine/src/utils/crossProjectResume.ts";
import { getWorktreePaths } from "@t3tools/ccb-engine/src/utils/getWorktreePaths.ts";
...
import { validateUuid } from "@t3tools/ccb-engine/src/utils/uuid.ts";
...
function ResumeCommand({
  ...
  if (loading) {
    return (
      <Box>
        <Spinner />
        <Text> Loading conversationsâ€?/Text>
      </Box>
    );
  }
  ...
  if (resuming) {
    return (
      <Box>
        <Spinner />
        <Text> Resuming conversationâ€?/Text>
      </Box>
    );
  }
...
export const call: LocalJSXCommandCall = async (onDone, context, args) => {
  ...
  if (maybeSessionId) {
    ...
    // Enriched logs didn't find it â€?try direct file lookup. This handles
