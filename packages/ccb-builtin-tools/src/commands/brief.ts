...
import { isBriefEntitled } from '@claude-code-best/builtin-tools/tools/BriefTool/BriefTool.ts'
import { BRIEF_TOOL_NAME } from '@claude-code-best/builtin-tools/tools/BriefTool/prompt.ts'
import type {
  Command,
...
// No TTL ï¿?this gate controls slash-command *visibility*, not a kill switch.
  ...
  load: () =>
    Promise.resolve({
      async call(
        ...
        // Entitlement check only gates the on-transition ï¿?off is always
        ...
        // each toggle (tool list changes), but a stale tool list is worse ï¿?
        ...
        // Inline <system-reminder> wrap ï¿?importing wrapInSystemReminder from
        ...
        const metaMessages = getKairosActive()
          ...
          : [
              `<system-reminder>\n${
                newState
                  ? `Brief mode is now enabled. Use the ${BRIEF_TOOL_NAME} tool for all user-facing output ï¿?plain text outside it is hidden from the user's view.`
                  : `Brief mode is now disabled. The ${BRIEF_TOOL_NAME} tool is no longer available ï¿?reply with plain text.`
              }\n</system-reminder>`,
            ]
