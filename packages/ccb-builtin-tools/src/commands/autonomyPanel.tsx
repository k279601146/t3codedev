...
import { getAutonomyCommandText, getAutonomyDeepSectionText, getAutonomyStatusText } from "../cli/handlers/autonomy.ts";
import { listAutonomyFlows, type AutonomyFlowRecord } from "../utils/autonomyFlows.ts";
      ...
      <Box flexDirection="column">
        {actions.map((action, index) => (
          <Box key={`${action.label}-${index}`} flexDirection="row">
            <Text>{`${index === selectedIndex ? 'â€? : ' '} ${action.label}`.padEnd(ACTION_LABEL_COLUMN_WIDTH)}</Text>
            <Text dimColor>{action.description}</Text>
          </Box>
        ))}
        <Box marginTop={1}>
          <Text dimColor>â†?â†?select Â· Enter run Â· Esc close</Text>
        </Box>
      </Box>
    </Dialog>
  );
