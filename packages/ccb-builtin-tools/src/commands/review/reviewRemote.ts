 ...
 * session via task-notification. Mirrors the /ultraplan â†?CCR flow.
 *
...
import { fetchUltrareviewQuota } from "@t3tools/ccb-engine/src/services/api/ultrareviewQuota.ts"
import { fetchUtilization } from "@t3tools/ccb-engine/src/services/api/usage.ts"
...
} from "@t3tools/ccb-engine/src/tasks/RemoteAgentTask/RemoteAgentTask.tsx"
import { isEnterpriseSubscriber, isTeamSubscriber } from "@t3tools/ccb-engine/src/utils/auth.ts"
import { detectCurrentRepositoryWithHost } from "@t3tools/ccb-engine/src/utils/detectRepository.ts"
import { execFileNoThrow } from "@t3tools/ccb-engine/src/utils/execFileNoThrow.ts"
import { getDefaultBranch, gitExe } from "@t3tools/ccb-engine/src/utils/git.ts"
import { teleportToRemote } from "@t3tools/ccb-engine/src/utils/teleport.tsx"
...
export async function checkOverageGate(): Promise<OverageGate> {
  // Team and Enterprise plans include ultrareview â€?no free-review quota
  ...
  // No quota info (non-subscriber or endpoint down) â€?let it through,
  ...
  // Utilization fetch failed (transient network error, timeout, etc.) â€?
  // let it through, same rationale as the quota fallback above.
  if (!utilization) {
    return { kind: 'proceed', billingNote: '' }
  }

  // Free reviews exhausted â€?check Extra Usage setup.
...
export async function launchRemoteReview(
  ...
  // UUID{...,0x02}) encodes with version prefix '01' â€?NOT Python's
  ...
  // the only tuning surface â€?without them, run_hunt.sh's bash defaults
  ...
  // match autoDream.ts â€?GB cache can return stale wrong-type values.
  ...
  // hang we're fixing comes back â€?fall to the safe default instead.
  const commonEnvVars = {
    BUGHUNTER_DRY_RUN: '1',
  ...
  if (isPrNumber) {
    ...
    // deletes refs/remotes/origin/* â€?the base branch name won't resolve
  ...
  // Concise â€?the tool-output block is visible to the user, so the model
  ...
  return [
    {
      type: 'text',
      text: `Ultrareview launched for ${target} (~10â€?0 min, runs in the cloud). Track: ${sessionUrl}${resolvedBillingNote} Findings arrive via task-notification. Briefly acknowledge the launch to the user without repeating the target or URL â€?both are already visible in the tool output above.`,
    },
  ]
}
