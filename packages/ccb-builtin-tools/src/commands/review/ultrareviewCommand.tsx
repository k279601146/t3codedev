...
import type { LocalJSXCommandCall, LocalJSXCommandOnDone } from "@t3tools/ccb-engine/src/types/command.ts";
...
async function launchAndDone(
  ...
  // User hit Escape during the ~5s launch â€?the dialog already showed
  ...
  if (result) {
    ...
    // repo â€?both are CCR/repo connectivity issues.
    ...
    return (
      <UltrareviewOverageDialog
        onProceed={async signal => {
          ...
          // Only persist the confirmation flag after a non-aborted launch â€?
