...
import type { AgentDefinitionsResult } from '@claude-code-best/builtin-tools/tools/AgentTool/loadAgentsDir.ts'
import type { AllowedPrompt } from '@claude-code-best/builtin-tools/tools/ExitPlanModeTool/ExitPlanModeV2Tool.ts'
...
export type AppState = DeepImmutable<{
  ...
  // through PromptInput â†?PromptInputFooter.
  ...
  // AppState.tasks is always empty in viewer mode â€?the tasks live in a
  // different process.
  ...
  // Name â†?AgentId registry populated by Agent tool when `name` is provided.
  ...
  // Timestamp of last /buddy pet â€?CompanionSprite renders hearts while recent
  ...
  // Sticky tmux panel visibility â€?mirrors globalConfig.tungstenPanelVisible for reactivity.
  tungstenPanelVisible?: boolean
  // Transient auto-hide at turn end â€?separate from tungstenPanelVisible so the
  ...
  // structurally â€?wrapper.tsx assigns via structural compatibility. Only
  ...
  computerUseMcpState?: {
    ...
    // display unplugged â†?Swift fell back to main) and on
    // `switch_display("auto")`.
    ...
    // changed since â€?keeps the resolver from yanking on every screenshot.
  ...
  // Channel permission callbacks â€?permission prompts over Telegram/iMessage/etc.
