...
export const SDKControlSeedReadStateRequestSchema = lazySchema(() =>
  z
    ...
    .describe(
      'Seeds the readFileState cache with a path+mtime entry. Use when a prior Read was removed from context (e.g. by snip) so Edit validation would fail despite the client having observed the Read. The mtime lets the CLI detect if the file changed since the seeded Read â€?same staleness check as the normal path.',
    ),
)
  ...
  z
    .object({
      ...
      applied: z
        .object({
          model: z.string(),
          // String levels only â€?numeric effort is ant-only and the
          // Zodâ†’proto generator can't emit enumâˆªnumber unions.
