...
const TICK_MS = 500;
const BUBBLE_SHOW = 20; // ticks â†?~10s at 500ms
...
// Narrow terminals: 0 â€?REPL.tsx stacks the one-liner on its own row
...
export function CompanionSprite(): React.ReactNode {
  ...
  // has petStartTick=tick and petAge=0 â€?otherwise frame 0 is skipped.
  ...
  if (columns < MIN_COLS_FOR_FULL_SPRITE) {
    const quip =
      reaction && reaction.length > NARROW_QUIP_CAP ? reaction.slice(0, NARROW_QUIP_CAP - 1) + 'â€? : reaction;
  ...
  // Name row doubles as hint row â€?unfocused shows dim name + â†?discovery,
