...
import type { BackgroundTaskState } from "./types.ts"
...
export function getPillLabel(tasks: BackgroundTaskState[]): string {
  ...
  if (allSameType) {
    switch (tasks[0]!.type) {
      ...
      case 'remote_agent': {
        const first = tasks[0]!
        // Per design mockup: â—?open diamond while running/needs-input,
        // â—?filled once ExitPlanMode is awaiting approval.
...
/**
 * True when the pill should show the dimmed " Â· â†?to view" call-to-action.
