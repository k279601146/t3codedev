...
import { tailFile } from "@t3tools/ccb-engine/src/utils/fsOperations.ts";
...
import type { ShellCommand } from "@t3tools/ccb-engine/src/utils/ShellCommand.ts";
import { evictTaskOutput, getTaskOutputPath } from "@t3tools/ccb-engine/src/utils/task/diskOutput.ts";
import { registerTask, updateTaskState } from "@t3tools/ccb-engine/src/utils/task/framework.ts";
import { escapeXml } from "@t3tools/ccb-engine/src/utils/xml.ts";
...
// input. Used to gate the stall notification â€?we stay silent on commands that
        ...
        void tailFile(outputPath, STALL_TAIL_BYTES).then(
          ({ content }) => {
            ...
            if (!looksLikePrompt(content)) {
              // Not a prompt â€?keep watching. Reset so the next check is
            ...
            // No <status> tag â€?print.ts treats <status> as a terminal
...
function enqueueShellNotification(
  ...
  if (feature('MONITOR_TOOL') && kind === 'monitor') {
    // Monitor is streaming-only (post-#22764) â€?the script exiting means
...
export async function spawnShellTask(
  ...
  // TaskOutput owns the data â€?use its taskId so disk writes are consistent
  ...
  // Data flows through TaskOutput automatically â€?no stream listeners needed.
...
function backgroundTask(taskId: string, getAppState: () => AppState, setAppState: SetAppState): boolean {
  ...
  // Transition to backgrounded â€?TaskOutput continues receiving data automatically
 ...
 * Unlike spawn(), this does NOT re-register the task â€?it flips isBackgrounded
 ...
 * Used when backgrounding raced with completion â€?the tool result already
