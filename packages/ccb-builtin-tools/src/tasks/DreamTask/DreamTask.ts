...
// Shift+Down dialog. The dream agent itself is unchanged â€?this is pure UI
// surfacing via the existing task registry.

import { rollbackConsolidationLock } from "@t3tools/ccb-engine/src/services/autoDream/consolidationLock.ts"
...
// No phase detection â€?the dream prompt has a 4-stage structure
...
export type DreamTaskState = TaskStateBase & {
   ...
   * INCOMPLETE reflection of what the dream agent actually changed â€?it misses
...
export function completeDreamTask(
  ...
  // notified: true immediately â€?dream has no model-facing notification path
