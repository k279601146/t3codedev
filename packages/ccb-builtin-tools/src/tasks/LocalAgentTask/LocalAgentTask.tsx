...
import { getSdkAgentProgressSummariesEnabled } from "@t3tools/ccb-engine/src/bootstrap/state.ts";
import {
  OUTPUT_FILE_TAG,
  STATUS_TAG,
  SUMMARY_TAG,
  ...
  WORKTREE_PATH_TAG,
  WORKTREE_TAG,
} from "@t3tools/ccb-engine/src/constants/xml.ts";
import { abortSpeculation } from "@t3tools/ccb-engine/src/services/PromptSuggestion/speculation.ts";
...
import type { Tools } from "@t3tools/ccb-engine/src/Tool.ts";
import { findToolByName } from "@t3tools/ccb-engine/src/Tool.ts";
import type { AgentToolResult } from '@claude-code-best/builtin-tools/tools/AgentTool/agentToolUtils.ts';
...
import { asAgentId } from "@t3tools/ccb-engine/src/types/ids.ts";
import type { Message } from "@t3tools/ccb-engine/src/types/message.ts";
import { createAbortController, createChildAbortController } from "@t3tools/ccb-engine/src/utils/abortController.ts";
import { registerCleanup } from "@t3tools/ccb-engine/src/utils/cleanupRegistry.ts";
import { getToolSearchOrReadInfo } from "@t3tools/ccb-engine/src/utils/collapseReadSearch.ts";
import { enqueuePendingNotification } from "@t3tools/ccb-engine/src/utils/messageQueueManager.ts";
import { getAgentTranscriptPath } from "@t3tools/ccb-engine/src/utils/sessionStorage.ts";
import { evictTaskOutput, getTaskOutputPath, initTaskOutputAsSymlink } from "@t3tools/ccb-engine/src/utils/task/diskOutput.ts";
import { PANEL_GRACE_MS, registerTask, updateTaskState } from "@t3tools/ccb-engine/src/utils/task/framework.ts";
import { emitTaskProgress } from "@t3tools/ccb-engine/src/utils/task/sdkProgress.ts";
...
export type LocalAgentTaskState = TaskStateBase & {
  ...
  // (which is "what am I LOOKING at") â€?retain is "what am I HOLDING."
  retain: boolean;
 ...
 * This is the ONE predicate that all pill/panel filters must agree on â€?if
...
export function enqueueAgentNotification({
  ...
  // Abort any active speculation â€?background task state changed, so speculated
