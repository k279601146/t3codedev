...
export function registerMainSessionTask(
  ...
  // sub-agents). Do NOT use getTranscriptPath() â€?that's the main session's
...
export function startBackgroundSession({
  ...
  // concurrent async chains â€?this wrapper doesn't affect the foreground.
    ...
    try {
      ...
      for await (const event of query({
        ...
        if (abortSignal.aborted) {
          // Aborted mid-stream â€?completeMainSessionTask won't be reached.
        ...
        // Per-message write (matches runAgent.ts pattern) â€?gives live
