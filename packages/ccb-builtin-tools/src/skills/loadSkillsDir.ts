...
export function createSkillCommand({
  ...
  return {
    ...
    async getPromptForCommand(args, toolUseContext) {
      ...
      // Security: MCP skills are remote and untrusted ï¿?never execute inline
      // shell commands (!`â€¦` / ```! ï¿?```) from their markdown body.
...
export const getSkillDirCommands = memoize(
  async (cwd: string): Promise<Command[]> => {
    ...
    // register separately. skillsLocked still applies ï¿?--bare is not a
    // policy bypass.
    if (isBareMode()) {
      ...
      // No dedup needed ï¿?explicit dirs, user controls uniqueness.
    ...
    // (all independent ï¿?different directories, no shared state)
    const [
      managedSkills,
      userSkills,
      projectSkillsNested,
      ...
      // here when skills are locked ï¿?these ARE skills, regardless of the
...
export function onDynamicSkillsLoaded(callback: () => void): () => void {
  ...
  // Same callSafe pattern as growthbook.ts ï¿?createSignal.emit() has no
...
export async function discoverSkillDirsForPaths(
  ...
  for (const filePath of filePaths) {
    ...
    while (currentDir.startsWith(resolvedCwd + pathSep)) {
      ...
      // Skip if we've already checked this path (hit or miss) ï¿?avoids
      ...
      if (!dynamicSkillDirs.has(skillDir)) {
        ...
        try {
          ...
          // is gitignored ï¿?blocks e.g. node_modules/pkg/.claude/skills from
          // loading silently. `git check-ignore` handles nested .gitignore,
          // .git/info/exclude, and global gitignore. Fails open outside a
          // git repo (exit 128 ï¿?false); the invocation-time trust dialog
          ...
          // Directory doesn't exist ï¿?already recorded above, continue
        }
      }
