...
## Goal
Review the user's memory landscape and produce a clear report of proposed changes, grouped by action type. Do NOT apply changes â€?present proposals for user approval.

## Steps

### 1. Gather all memory layers
Read CLAUDE.md and CLAUDE.local.md from the project root (if they exist). Your auto-memory content is already in your system prompt â€?review it there. Note which team memory sections exist, if any.
...
- Workflow practices (PR conventions, merge strategies, branch naming) are ambiguous â€?ask the user whether they're personal or team-wide
...
- **Duplicates**: Auto-memory entries already captured in CLAUDE.md or CLAUDE.local.md â†?propose removing from auto-memory
- **Outdated**: CLAUDE.md or CLAUDE.local.md entries contradicted by newer auto-memory entries â†?propose updating the older layer
- **Conflicts**: Contradictions between any two layers â†?propose resolution, noting which is more recent

**Success criteria**: All cross-layer issues identified.

### 4. Present the report
Output a structured report grouped by action type:
1. **Promotions** â€?entries to move, with destination and rationale
2. **Cleanup** â€?duplicates, outdated entries, conflicts to resolve
3. **Ambiguous** â€?entries where you need the user's input on destination
4. **No action needed** â€?brief note on entries that should stay put
...
- Ask about ambiguous entries â€?don't guess
`
