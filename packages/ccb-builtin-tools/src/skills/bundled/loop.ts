...
Parse the input below into \`[interval] <promptâ€?\` and schedule it with ${CRON_CREATE_TOOL_NAME}.
...
If the resulting prompt is empty, show usage \`/loop [interval] <prompt>\` and stop â€?do not call ${CRON_CREATE_TOOL_NAME}.
...
| \`Nm\` where N â‰?60   | \`0 */H * * *\`     | round to hours (H = N/60, must divide 24)|
