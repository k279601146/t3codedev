import { getSessionMemoryContent } from "@t3tools/ccb-engine/src/services/SessionMemory/sessionMemoryUtils.ts"
...
- Ask where the skill should be saved. Suggest a default based on context (repo-specific workflows â†?repo, cross-repo personal workflows â†?user). Options:
  - **This repo** (\`.claude/skills/<name>/SKILL.md\`) â€?for workflows specific to this project
  - **Personal** (\`~/.claude/skills/<name>/SKILL.md\`) â€?follows you across all repos
...
Before writing the file, output the complete SKILL.md content as a yaml code block in your response so the user can review it with proper syntax highlighting. Then ask for confirmation using AskUserQuestion with a simple question like "Does this SKILL.md look good to save?" â€?do NOT use the body field, keep the question concise.
