...
import {
  ...
  NON_REBINDABLE,
  TERMINAL_RESERVED,
} from "../../keybindings/reservedShortcuts.ts"
import type { KeybindingsSchemaType } from "../../keybindings/schema.ts"
import {
  KEYBINDING_ACTIONS,
  KEYBINDING_CONTEXT_DESCRIPTIONS,
  KEYBINDING_CONTEXTS,
} from "../../keybindings/schema.ts"
...
function generateReservedShortcuts(): string {
  ...
  for (const s of TERMINAL_RESERVED) {
    lines.push(
      `- \`${s.key}\` â€?${s.reason} (${s.severity === 'error' ? 'will not work' : 'may conflict'})`,
    )
  }
...
const SECTION_INTRO = [
  ...
  '',
  '**Always read `~/.claude/keybindings.json` first** (it may not exist yet). Merge changes with existing bindings â€?never replace the entire file.',
  '',
...
const SECTION_KEYSTROKE_SYNTAX = [
  ...
  '- `ctrl` (alias: `control`)',
  '- `alt` (aliases: `opt`, `option`) â€?note: `alt` and `meta` are identical in terminals',
  '- `shift`',
...
const SECTION_INTERACTION = [
  ...
  '',
  '- User bindings are **additive** â€?they are appended after the default bindings',
...
const SECTION_DOCTOR = [
  ...
  '  â”?[Error] Unknown context "chat"',
  '    â†?Valid contexts: Global, Chat, Autocomplete, ...',
  '  â”?[Warning] "ctrl+c" may not work: Terminal interrupt (SIGINT)',
  '```',
  '',
