/**
 * CCR upstreamproxy â€?container-side wiring.
 *
 ...
 *   4. Starts a local CONNECTâ†’WebSocket relay (see relay.ts)
 ...
 *
 * Design doc: api-go/ccr/docs/plans/CCR_AUTH_DESIGN.md Â§ "Week-1 pilot scope".
 */
...
import { startUpstreamProxyRelay } from "./relay.ts"
...
const NO_PROXY_LIST = [
  ...
  //   *.anthropic.com  â€?Bun, curl, Go (glob match)
  //   .anthropic.com   â€?Python urllib/httpx (suffix match, strips leading dot)
  //   anthropic.com    â€?apex domain fallback
  'anthropic.com',
  '.anthropic.com',
  '*.anthropic.com',
  'github.com',
 ...
 * feature is off or the token file is absent â€?returns {enabled: false}.
 *
...
async function downloadCaBundle(
  ...
  try {
    ...
    const resp = await fetch(`${baseUrl}/v1/code/upstreamproxy/ca-cert`, {
      // Bun has no default fetch timeout â€?a hung endpoint would block CLI
