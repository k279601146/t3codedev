 ...
 *   1. CLAUDE_CODE_DISABLE_AUTO_MEMORY env var (1/true ï¿?OFF, 0/false ï¿?ON)
 *   2. CLAUDE_CODE_SIMPLE (--bare) ï¿?OFF
 *   3. CCR without persistent storage ï¿?OFF (no CLAUDE_CODE_REMOTE_MEMORY_DIR)
 ...
 * this gate ï¿?when the main agent writes memories, the background agent
 * skips that range (hasMemoryWritesSince in extractMemories.ts); when it
 * doesn't, the background agent catches anything missed.
 *
 * Callers must also gate on feature('EXTRACT_MEMORIES') ï¿?that check cannot
 ...
 * - relative (!isAbsolute): "../foo" ï¿?would be interpreted relative to CWD
 * - root/near-root (length < 3): "/" ï¿?"" after strip; "/a" too short
 * - Windows drive-root (C: regex): "C:\" ï¿?"C:" after strip
 * - UNC paths (\\server\share): network paths ï¿?opaque trust boundary
...
function validateMemoryPath(
  ...
  // expanded ï¿?they would make isAutoMemPath() match all of $HOME or its
 ...
 * intentionally excluded ï¿?a malicious repo could otherwise set
 ...
 * the auto-memory mechanics ï¿?e.g. to decide whether to inject the
 ...
 *
 * Memoized: render-path callers (collapseReadSearchGroups ï¿?isAutoManagedMemoryFile)
 * fire per tool-use message per Messages re-render; each miss costs
 * getSettingsForSource Ã— 4 ï¿?parseSettingsFile (realpathSync + readFileSync).
 ...
 * write permission in that case ï¿?the filesystem.ts write carve-out is gated
 ...
 * excluded ï¿?see getAutoMemPathSetting), and hasAutoMemPathOverride() remains
 * false for it.
 */
