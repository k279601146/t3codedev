...
export function truncateEntrypointContent(raw: string): EntrypointTruncation {
  ...
  // Check original byte count ï¿?long lines are the failure mode the byte cap
  ...
  const reason =
    wasByteTruncated && !wasLineTruncated
      ? `${formatFileSize(byteCount)} (limit: ${formatFileSize(MAX_ENTRYPOINT_BYTES)}) ï¿?index entries are too long`
...
export const DIR_EXISTS_GUIDANCE =
  'This directory already exists ï¿?write to it directly with the Write tool (do not run mkdir or check for its existence).'
export const DIRS_EXIST_GUIDANCE =
  'Both directories already exist ï¿?write to them directly with the Write tool (do not run mkdir or check for their existence).'

/**
 * Ensure a memory directory exists. Idempotent ï¿?called from loadMemoryPrompt
...
export async function ensureMemoryDirExists(memoryDir: string): Promise<void> {
  ...
  try {
    ...
    // a real problem (EACCES/EPERM/EROFS) ï¿?log so --debug shows why. Prompt
 ...
 * Fire-and-forget ï¿?doesn't block prompt building.
 */
function logMemoryDirCounts(
  ...
  void fs.readdir(memoryDir).then(
    ...
    () => {
      // Directory unreadable ï¿?log without counts
 ...
 * reference) ï¿?content that is derivable from the current project state (code
...
export function buildMemoryLines(
  ...
  const howToSave = skipIndex
    ...
    : [
        ...
        ...MEMORY_FRONTMATTER_EXAMPLE,
        '',
        `**Step 2** ï¿?add a pointer to that file in \`${ENTRYPOINT_NAME}\`. \`${ENTRYPOINT_NAME}\` is an index, not a memory ï¿?each entry should be one line, under ~150 characters: \`- [Title](file.md) ï¿?one-line hook\`. It has no frontmatter. Never write memory content directly into \`${ENTRYPOINT_NAME}\`.`,
        '',
        `- \`${ENTRYPOINT_NAME}\` is always loaded into your conversation context ï¿?lines after ${MAX_ENTRYPOINT_LINES} will be truncated, so keep the index concise`,
        '- Keep the name, description, and type fields in memory files up-to-date with the content',
        '- Organize memory semantically by topic, not chronologically',
        '- Update or remove memories that turn out to be wrong or outdated',
        '- Do not write duplicate memories. First check if there is an existing memory you can update before writing a new one.',
 ...
 * as the distilled index ï¿?this prompt only changes where NEW memories go.
 */
function buildAssistantDailyLogPrompt(skipIndex = false): string {
  ...
  // than the user-context message ï¿?the latter is intentionally left stale to
  ...
  const lines: string[] = [
    ...
    '',
    'Write each entry as a short timestamped bullet. Create the file (and parent directories) on first write if it does not exist. Do not rewrite or reorganize the log ï¿?it is append-only. A separate nightly process distills these logs into `MEMORY.md` and topic files.',
    '',
    '## What to log',
    ...
    ...(skipIndex
      ...
      : [
          `## ${ENTRYPOINT_NAME}`,
          `\`${ENTRYPOINT_NAME}\` is the distilled index (maintained nightly from your logs) and is loaded into your context automatically. Read it for orientation, but do not edit it directly ï¿?record new information in today's log instead.`,
          '',
        ]),
...
export function buildSearchingPastContextSection(autoMemDir: string): string[] {
  ...
  // In REPL mode, both Grep and Bash are hidden from direct use ï¿?the model
  ...
  return [
    ...
    memSearch,
    '```',
    '2. Session transcript logs (last resort ï¿?large files, slow):',
    '```',
    transcriptSearch,
    '```',
...
export async function loadMemoryPrompt(): Promise<string | null> {
  ...
  // Gate on the GB flag directly, not isTeamMemoryEnabled() ï¿?that function
