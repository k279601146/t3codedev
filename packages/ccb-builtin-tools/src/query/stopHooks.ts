...
> {
  ...
  // Only save params for main session queries ï¿?subagents must not overwrite.
  ...
  // the turn returns ï¿?otherwise `claude list` shows stale state for the gap.
  ...
  if (
    ...
    // Full turn history ï¿?assistantMessages resets each queryLoop iteration,
  ...
  const poorMode = feature('POOR')
    ? (await import('../commands/poor/poorMode.ts')).isPoorModeActive()
    : false
  if (!isBareMode()) {
    ...
    if (
      ...
      void extractMemoriesModule!.executeExtractMemories(
        ...
        toolUseContext.appendSystemMessage as
          | ((msg: import('../types/message.ts').SystemMessage) => void)
          | undefined,
      )
    }
  ...
  // Main thread only ï¿?the CU lock is a process-wide module-level variable,
  // so a subagent's stopHooks releasing it leaves the main thread's cleanup
  // seeing isLockHeldLocally()===false ï¿?no exit notification, and unhides
  ...
  try {
    ...
    if (isTeammate()) {
      ...
      // Each hook executor generates its own toolUseID ï¿?capture from progress
