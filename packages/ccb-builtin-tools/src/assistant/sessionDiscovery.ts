 ...
 * ID format is `session_*` (compat prefix) â€?viewer endpoints use /v1/sessions/*.
 */
 ...
 *
 * Throws on failure â€?main.tsx L4720-4725 catch displays the error.
...
export async function discoverAssistantSessions(): Promise<AssistantSession[]> {
  ...
  // Filter to active/working sessions only â€?completed/archived are not attachable
  return allSessions
    .filter(
      s =>
