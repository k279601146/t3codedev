...
export function useAwaySummary(
  ...
  useEffect(() => {
    ...
    function onFocusChange(): void {
      ...
      if (state === 'blurred' || state === 'unknown') {
        ...
        // potentially away ï¿?start idle timer. The isLoading effect
  ...
  // Timer fired mid-turn ï¿?fire when turn ends (if still away)
  useEffect(() => {
    if (isLoading) return
  ...
  // User starts a turn ï¿?they're present, cancel idle timer.
  // Turn ends ï¿?restart idle timer.
  useEffect(() => {
    ...
    if (isLoading) {
      // User is actively using ï¿?cancel idle timer
      if (timerRef.current !== null) {
        clearTimeout(timerRef.current)
        timerRef.current = null
      }
      ...
      // Turn ended ï¿?restart idle timer
      if (timerRef.current !== null) {
        clearTimeout(timerRef.current)
      }
