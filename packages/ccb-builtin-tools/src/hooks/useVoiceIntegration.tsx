...
const voiceNs: { useVoice: typeof import('./useVoice.ts').useVoice } = feature('VOICE_MODE')
  ? require('./useVoice.ts')
  : {
...
// `input` arg â€?KeyboardEvent.key holds normalized names (e.g. 'space',
...
function matchesKeyboardEvent(e: KeyboardEvent, target: ParsedKeystroke): boolean {
  ...
  // KeyboardEvent.meta folds alt|option (terminal limitation â€?esc-prefix);
...
// lookup returns null â€?that means the user null-unbound or reassigned
...
type StripOpts = {
  ...
  anchor?: boolean;
  // Minimum trailing count to leave behind â€?prevents stripping the
...
export function useVoiceIntegration({
  ...
  // submitted or edited â€?both write paths bail to avoid clobbering. This is
  ...
  // stopImmediatePropagation â€?listener order is not guaranteed) and
  ...
  const stripTrailing = useCallback(
    (maxStrip: number, { char = ' ', anchor = false, floor = 0 }: StripOpts = {}) => {
      ...
      // Always overwrite on anchor â€?if a prior activation failed to start
  ...
  // effect (voiceState useEffect below) â€?which only fires on voiceState transitions â€?can't
  ...
  useEffect(() => {
    ...
    // cleared on voiceStateâ†’idle, so it's still set during the 'processing'
    // window between CloseStream and WS close â€?this catches refined
  ...
  const handleVoiceTranscript = useCallback(
    (text: string) => {
      if (!feature('VOICE_MODE')) return;
      const prefix = voicePrefixRef.current;
      // No voice anchor â€?voice was reset (or never started). Nothing to do.
      if (prefix === null) return;
      const suffix = voiceSuffixRef.current;
      // Submit race: finishRecording() â†?user presses Enter (input cleared)
      // â†?WebSocket close â†?this callback fires with stale prefix/suffix.
      // If the input isn't what this hook last set (via the interim effect
      // or anchor), the user submitted or edited â€?don't re-fill. Comparing
 ...
 * the first press â€?a modifier combo is unambiguous intent (can't be
 ...
 * untypable â€?normal typing (>120ms between keystrokes) flows through;
 * only rapid auto-repeat from a held key triggers activation.
 *
 * Known broken: modifier+space (NUL â†?parsed as ctrl+backtick), chords
...
export function useVoiceKeybindingHandler({
  ...
  // action, the voice binding is discarded and null is returned â€?the
  ...
  // when there's no provider at all. Context filter is required â€?space
  ...
  // and the char flows into the text input â€?we need flow-through + strip.
  ...
  // default (space) this is precise â€?pre-existing trailing spaces are
  ...
  // letter (e.g. "hav" + hold "v" â†?"ha"). We don't track that
  // boundary â€?it's best-effort and the warning says so.
  const charsInInputRef = useRef(0);
  // Trailing-char count remaining after the activation strip â€?these
  ...
  // ends when key-repeat stops (state â†?'processing'); keeping the ref
  ...
  const handleKeyDown = (e: KeyboardEvent): void => {
    ...
    // PromptInput is not a valid transcript target â€?let the hold key
    ...
    //     without registering an overlay â€?e.g. /install-github-app,
    ...
    // null means the user overrode the default (null-unbind/reassign) â€?
    ...
    if (bareChar !== null) {
      ...
      // When bound to space, also accept U+3000 (full-width space) â€?
    ...
    if (isHoldActiveRef.current && currentVoiceState !== 'idle') {
      // Already recording â€?swallow continued keypresses and forward
    ...
    // unconditionally â€?user may be typing during focus-recording.
    ...
    // â”€â”€ Activation â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    // Handled first so the warmup branch below does NOT also run
    // on this event â€?two strip calls in the same tick would both
    // read the stale inputValueRef and the second would under-strip.
    // Modifier combos activate on the first press â€?they can't be
    ...
    if (bareChar === null || rapidCountRef.current >= HOLD_THRESHOLD) {
      ...
      // and swallow keypresses. Store is synchronous â€?the check is
    ...
    // â”€â”€ Warmup (bare-char only; modifier combos activated above) â”€â”€
    ...
    // guaranteed â€?text input may have already added the char). The
  ...
  useInput(
    (_input, _key, event) => {
      ...
      // emitter actually checks â€?forward it so the text input's useInput
