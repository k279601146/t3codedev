/**
 * usePipePermissionForward ï¿?Forward slave permission requests to master UI.
 *
 * Subscribes to slave pipe messages via subscribePipeEntries, and:
 * 1. permission_request ï¿?enqueue into toolUseConfirmQueue for master approval
 * 2. permission_cancel ï¿?remove from queue
 * 3. stream/error/done ï¿?display as system messages
 */
...
export function usePipePermissionForward({
  ...
  useEffect(() => {
    ...
    const { subscribePipeEntries, getSlaveClient } =
      require('./useMasterMonitor.ts') as typeof import('./useMasterMonitor.ts')
    const { getPipeIpc } =
      require('../utils/pipeTransport.ts') as typeof import('../utils/pipeTransport.ts')
    const { createAssistantMessage, createSystemMessage } =
      require('../utils/messages.ts') as typeof import('../utils/messages.ts')
      ...
      (pipeName: string, entry: { type: string; content: string }) => {
        ...
        if (entry.type === 'permission_request') {
          try {
            ...
            // Malformed permission request ï¿?ignore
          }
          return
        }
