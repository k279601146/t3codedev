...
import { getIsRemoteMode, getIsScrollDraining } from "@t3tools/ccb-engine/src/bootstrap/state.ts";
import { useNotifications } from "../../context/notifications.tsx";
import { Text } from '@anthropic/ink';
import { getInitializationStatus, getLspServerManager } from "@t3tools/ccb-engine/src/services/lsp/manager.ts";
import { useSetAppState } from "@t3tools/ccb-engine/src/state/AppState.tsx";
...
export function useLspInitializationNotification(): void {
  ...
  // Lazy initializer â€?eager form re-evaluates isEnvTruthy on every REPL
      ...
      addNotification({
        ...
        jsx: (
          <>
            ...
            <Text dimColor> Â· /plugin for details</Text>
          </>
        ),
        priority: 'medium',
        timeoutMs: 8000,
  ...
  const poll = React.useCallback(() => {
    ...
    // Skip during scroll drain â€?iterating all LSP servers + setAppState
