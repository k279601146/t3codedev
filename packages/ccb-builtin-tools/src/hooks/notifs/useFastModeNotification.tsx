    ...
    return onOrgFastModeChanged(orgEnabled => {
      if (orgEnabled) {
        addNotification({
          ...
          color: 'fastMode',
          priority: 'immediate',
          text: 'Fast mode is now available 路 /fast to turn on',
        });
      } else if (isFastMode) {
        // Org disabled fast mode 鈥?permanently turn off fast mode
  ...
  useEffect(() => {
    ...
    const unsubExpired = onCooldownExpired(() => {
      addNotification({
        ...
        color: 'fastMode',
        text: `Fast limit reset 路 now using fast mode`,
        priority: 'immediate',
      });
    });
    return () => {
...
function getCooldownMessage(reason: CooldownReason, resetIn: string): string {
  switch (reason) {
    case 'overloaded':
      return `Fast mode overloaded and is temporarily unavailable 路 resets in ${resetIn}`;
    case 'rate_limit':
      return `Fast limit reached and temporarily disabled 路 resets in ${resetIn}`;
  }
}
