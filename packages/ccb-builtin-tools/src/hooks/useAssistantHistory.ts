...
type Props = {
  /** Gated on viewerOnly â€?non-viewer sessions have no remote history to page. */
...
const SENTINEL_LOADING = 'loading older messagesâ€?
const SENTINEL_LOADING_FAILED =
  'failed to load older messages â€?scroll up to retry'
...
export function useAssistantHistory({
  ...
  // Stable sentinel UUID â€?reused across swaps so virtual-scroll treats it
  ...
  const prepend = useCallback(
    (page: HistoryPage, isInitial: boolean) => {
      ...
      setMessages(prev => {
        // Drop existing sentinel (index 0, known stable UUID â€?O(1)).
        const base =
  ...
  // Initial fetch on mount â€?best-effort.
  useEffect(() => {
  ...
  const loadOlder = useCallback(async () => {
    ...
    // Swap sentinel to "loadingâ€? â€?O(1) slice since sentinel is at index 0.
    setMessages(prev => {
      const base =
        prev[0]?.uuid === sentinelUuidRef.current ? prev.slice(1) : prev
    ...
    try {
      ...
      if (!page) {
        // Fetch failed â€?revert sentinel back to "start" placeholder so the user
  ...
  // Scroll-anchor compensation â€?after React commits the prepended items,
  ...
  //
  // The ScrollBox content wrapper has flexGrow:1 flexShrink:0 â€?it's clamped
