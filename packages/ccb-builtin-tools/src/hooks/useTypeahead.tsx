...
import {
  ...
  getBestCommandMatch,
  isCommandInput,
} from "../utils/suggestions/commandSuggestions.ts";
import {
  getDirectoryCompletions,
  getPathCompletions,
  isPathLikeToken,
} from "../utils/suggestions/directoryCompletion.ts";
...
export function extractCompletionToken(
  ...
  // Non-@ token or cursor outside @ token â€?use $ anchor on (short) tail
...
export function useTypeahead({
  ...
  // PromptInput hides suggestion ghost text in teammate view â€?mirror that
  ...
  // delay first render â€?it just races the user's first @ keystroke.
  //
  ...
  // background build outlives the test â€?its setImmediate chain leaks into
  ...
  // instead of stuttering on each repeated key. The search itself is ~8â€?5ms
  ...
  const updateSuggestions = useCallback(
    async (value: string, inputCursorOffset?: number): Promise<void> => {
      ...
      if (atMatch) {
        ...
        // Imperative read â€?reading at call-time fixes staleness for
        ...
        for (const [name, agentId] of state.agentNameRegistry) {
          ...
          members.push({
            id: `dm-${name}`,
            displayText: `@${name}`,
            description: status ? `send message Â· ${status}` : 'send message',
          });
        }
      ...
      // Only consider text up to the cursor â€?when the cursor is mid-input (e.g.,
  ...
  useEffect(() => {
    ...
    // This fixes: type @readme.md, clear, retype @readme.md â†?no suggestions.
