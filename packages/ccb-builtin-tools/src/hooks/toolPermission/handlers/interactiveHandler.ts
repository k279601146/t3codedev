...
import { executeAsyncClassifierCheck } from '@claude-code-best/builtin-tools/tools/BashTool/bashPermissions.ts'
...
} from "../../../utils/pipePermissionRelay.ts"
...
function handleInteractivePermission(
  ...
  // remove the abort listener ï¿?not just the timer callback.
  ...
  // entry. No "tell remote to dismiss" equivalent ï¿?the text sits in your
  ...
  // wins via claim().
  //
  // All tools are forwarded ï¿?CCR's generic allow/deny modal handles any
  ...
  // Channel permission relay ï¿?races alongside the bridge block above. Send a
  ...
  //
  // Unlike the bridge block, this still guards on `requiresUserInteraction` ï¿?
  ...
  // ï¿?the local dialog is always there as the floor.
  if (
    ...
    if (channelClients.length > 0) {
      // Outbound is structured too (Kenneth's symmetry ask) ï¿?server owns
      ...
      // hack is gone ï¿?no more guessing which arg name each plugin takes.
      ...
      // hook/classifier wins previously only deleted the map entry ï¿?the
      ...
      const mapUnsub = channelCallbacks.onResponse(
        ...
        response => {
          ...
          clearClassifierIndicator()
          ctx.removeFromQueue()
          // Bridge is the other remote ï¿?tell it we're done.
          if (bridgeCallbacks && bridgeRequestId) {
            bridgeCallbacks.cancelRequest(bridgeRequestId)
          }
  ...
  if (
    ...
    // UI indicator for "classifier running" ï¿?set here (not in
        ...
        onAllow: decisionReason => {
          ...
          checkmarkAbortHandler = () => {
            if (checkmarkTransitionTimer) {
              ...
              // cascades via siblingAbortController) ï¿?must drop the
              // cosmetic ï¿?dialog or it blocks the next queued item.
              ctx.removeFromQueue()
            }
          }
