/**
 * usePipeMuteSync â€?Sync master's UI selection state to slave relay mute flags.
 *
...
import { getPipeIpc } from "../utils/pipeTransport.ts"
import {
  setMasterMutedPipes,
...
export function usePipeMuteSync({
  ...
  useEffect(() => {
    ...
    // Step 2: For newly muted slaves â€?abort pending permissions, then send relay_mute
      ...
      if (!prevMuted.has(name)) {
        ...
        setToolUseConfirmQueue((queue: Record<string, unknown>[]) => {
          ...
          for (const item of toAbort) {
            try {
              ...
              // onAbort may throw if client disconnected â€?safe to ignore
            }
          }
          return queue.filter(
            (item: Record<string, unknown>) => item.pipeName !== name,
        ...
        if (client?.connected) {
          try {
            ...
            // send may fail if socket is closing â€?non-fatal
          }
        }
      }
    }

    // Step 3: For newly unmuted slaves â€?send relay_unmute
