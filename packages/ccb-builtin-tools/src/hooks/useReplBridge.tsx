...
} from "../bridge/bridgePermissionCallbacks.ts";
import { handleRemoteInterrupt } from "../bridge/remoteInterruptHandling.ts";
import { isTranscriptResetResultReady, shouldDeferBridgeResult } from "../bridge/bridgeResultScheduling.ts";
...
import { buildTaskStateMessage, getTaskStateSnapshotKey } from "../utils/taskStateMessage.ts";
import {
...
import { getTaskListId, getTasksDir, listTasks, onTasksUpdated } from "../utils/tasks.ts";
...
export function useReplBridge(
  ...
  useEffect(() => {
    // feature() check must use positive pattern for dead code elimination â€?
    ...
    if (feature('BRIDGE_MODE')) {
      ...
      void (async () => {
        try {
          ...
          const { initReplBridge } = await import('../bridge/initReplBridge.ts');
          const { shouldShowAppUpgradeMessage } = await import('../bridge/envLessBridgeConfig.ts');
          ...
          async function handleInboundMessage(msg: SDKMessage): Promise<void> {
            try {
              ...
              const { resolveAndPrepend } = await import('../bridge/inboundAttachments.ts');
              ...
              enqueue({
                ...
                uuid,
                // skipSlashCommands stays true as defense-in-depth â€?
                    ...
                    try {
                      ...
                      handleRef.current?.writeSdkMessages([
                        buildSystemInitMessage({
                          ...
                          // Remote clients can only invoke bridge-safe commands â€?
          ...
          const rawHandle = await initReplBridge({
            ...
            onSetPermissionMode(mode) {
              // Policy guards MUST fire before transitionPermissionMode â€?
