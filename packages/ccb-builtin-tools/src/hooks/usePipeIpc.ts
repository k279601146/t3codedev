/**
 * usePipeIpc ï¿?Pipe IPC lifecycle hook.
 *
...
import * as pt from "../utils/pipeTransport.ts"
import * as pr from "../utils/pipeRegistry.ts"
import * as mm from "./useMasterMonitor.ts"
import { getSessionId as _getSessionId } from "../bootstrap/state.ts"
import * as lb from "../utils/lanBeacon.ts"
import * as pp from "../utils/pipePermissionRelay.ts"
    ...
    try {
      ...
      for (const target of attachTargets) {
        ...
        try {
          ...
          // Connection failed ï¿?skip this cycle
        }
      }
      ...
      // Heartbeat cycle error ï¿?non-fatal
    }
  })()
}
...
function runSubHeartbeat(
  ...
  void (async () => {
    try {
      ...
      // Heartbeat check error ï¿?non-fatal
    }
  })()
}
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        // PipeServer creation failed ï¿?non-fatal
      }
    })()
