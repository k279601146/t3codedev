...
// structured arrays â€?a shared signature would ping-pong and never match.
 ...
 * middle files (same length, same endpoints â†?stale entry stuck in nucleo).
 *
 * Samples every Nth path (plus length). On a 346k-path list this hashes ~700
 * paths instead of 14MB â€?enough to catch git operations (checkout, rebase,
 ...
 * Returns null for worktrees (.git is a file â†?ENOTDIR), fresh repos with no
 * index yet (ENOENT), and non-git dirs â€?caller falls back to time throttle.
 */
...
async function mergeUntrackedIntoNormalizedCache(
  ...
  if (sig === loadedMergedSignature) {
    logForDebugging(
      `[FileIndex] skipped index rebuild â€?merged paths unchanged`,
    )
    return
  }
...
export async function getPathsForSuggestions(): Promise<FileIndex> {
  ...
  try {
    ...
    // during a typing session â€?git ls-files returns the same output.
    ...
    if (sig !== loadedTrackedSignature) {
      ...
      // build yields every ~4ms so the UI stays responsive â€?user can keep
      ...
      logForDebugging(
        `[FileIndex] skipped index rebuild â€?tracked paths unchanged`,
      )
    }
  } catch (error) {
    logError(error)
...
export function startBackgroundCacheRefresh(): void {
  ...
  // Throttle only when a cache exists â€?cold start must always populate.
  // Refresh immediately when .git/index mtime changed (tracked files).
  // Otherwise refresh at most once per 5s â€?this floor picks up new UNTRACKED
  ...
  // Ensure the FileIndex singleton exists â€?it's progressively queryable
...
export async function generateFileSuggestions(
  ...
  try {
    // Kick a background refresh. The index is progressively queryable â€?
    ...
    // when the build finishes to upgrade partial â†?full.
