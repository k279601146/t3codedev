...
export function useDirectConnect({
    ...
    const manager = new DirectConnectSessionManager(config, {
      ...
      onDisconnected: () => {
        ...
        if (!isConnectedRef.current) {
          // Never connected â€?connection failure (e.g. auth rejected)
          process.stderr.write(
            `\nFailed to connect to server at ${config.wsUrl}\n`,
          )
        } else {
          // Was connected then lost â€?server process exited or network dropped
          process.stderr.write('\nServer disconnected.\n')
        }
  ...
  // Same stability concern as useRemoteSession â€?memoize so consumers
