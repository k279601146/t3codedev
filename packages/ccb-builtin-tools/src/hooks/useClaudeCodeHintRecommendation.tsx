...
import {
  ...
  resolvePluginHint,
} from "../utils/plugins/hintRecommendation.ts";
...
export function useClaudeCodeHintRecommendation(): UseClaudeCodeHintRecommendationResult {
  ...
  React.useEffect(() => {
    ...
    tryResolve(async () => {
      ...
      // Drop the slot â€?but only if it still holds the hint we just
  ...
  const handleResponse = React.useCallback(
    (response: 'yes' | 'no' | 'disable') => {
      ...
      // Record show-once here, not at resolution-time â€?the dialog may have
