...
export function GlobalKeybindingHandlers({
  ...
  const handleToggleTodos = useCallback(() => {
    ...
    setAppState(prev => {
      const { getAllInProcessTeammateTasks } =
        ...
        require('../tasks/InProcessTeammateTask/InProcessTeammateTask.tsx') as typeof import('../tasks/InProcessTeammateTask/InProcessTeammateTask.tsx');
      const hasTeammates = count(getAllInProcessTeammateTasks(prev.tasks), t => t.status === 'running') > 0;

      if (hasTeammates) {
        // Both exist: none â†?tasks â†?teammates â†?none
  ...
  // Toggle transcript mode (ctrl+o). Two-way prompt â†?transcript.
  ...
  const handleToggleTranscript = useCallback(() => {
    if (feature('KAIROS') || feature('KAIROS_BRIEF')) {
      ...
      // view. Users will reach for ctrl+o â€?clear the stuck state first.
      // Only needed in the prompt screen â€?transcript mode already ignores
  ...
  // Toggle brief-only view (ctrl+shift+b). Pure display filter toggle â€?
  ...
  useKeybinding('transcript:exit', handleExitTranscript, {
    ...
    // visible, n/N active, bar closed) is NOT â€?Esc exits transcript
