/**
 * useMasterMonitor â€?master-side slave registry helpers plus an optional hook
 *
 ...
 *
 * The master CLI itself remains fully functional â€?this hook only collects
 ...
 * useMasterMonitor's inline handler â€?keeps the two gates in sync.
 */
    ...
    if (shouldDropMutedMessage(name, msg.type)) {
      ...
      if (msg.type === 'permission_request') {
        try {
          ...
          // Malformed payload â€?safe to ignore
        }
      }
      return
    }
