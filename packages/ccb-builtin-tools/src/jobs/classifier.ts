...
export async function classifyAndWriteState(
  ...
  try {
    ...
    // No state file or corrupt â€?not a valid job directory
    return
  }
 ...
 *
 * - Has tool_use blocks â†?still running (tools executing)
 * - stop_reason === 'end_turn' â†?completed (model finished)
 * - Otherwise â†?running
 */
