...
export class FileIndex {
  ...
  search(query: string, limit: number): SearchResult[] {
    ...
    for (let i = 0; i < readyCount; i++) {
      ...
      // first needle char appears early (e.g. 's' in "../../") while the real
      // match lives deeper (e.g. "settings/"). We score from multiple start
      // positions â€?the leftmost hit plus every word-boundary occurrence of
      // needle[0] â€?and keep the best. Typical paths have 2â€? boundary starts,
