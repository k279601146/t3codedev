...
export type SerializedMessage = Message & {
  cwd: string
  userType: string
  entrypoint?: string // CLAUDE_CODE_ENTRYPOINT â€?distinguishes cli/sdk-ts/sdk-py/etc.
  sessionId: string
  timestamp: string
  version: string
  gitBranch?: string
...
export type LogOption = {
  ...
  contextCollapseCommits?: ContextCollapseCommitEntry[] // Ordered â€?commit B may reference commit A's summary
  contextCollapseSnapshot?: ContextCollapseSnapshotEntry // Last-wins â€?staged queue + spawn state
 ...
 * Subset of WorktreeSession from utils/worktree.ts â€?excludes ephemeral
 ...
 * NOT persisted â€?they're already in the transcript as ordinary user/
...
export type ContextCollapseCommitEntry = {
  ...
  collapseId: string
  /** The summary placeholder's uuid â€?registerSummary() needs it. */
  summaryUuid: string
  /** Full <collapsed id="...">text</collapsed> string for the placeholder. */
  summaryContent: string
  /** Plain summary text for ctx_inspect. */
  summary: string
  /** Span boundaries â€?projectView finds these in the resumed Message[]. */
 ...
 * (append-only, replay-all), snapshots are last-wins â€?only the most
...
export type ContextCollapseSnapshotEntry = {
  ...
  /** Spawn trigger state â€?so the +interval clock picks up where it left off. */
  armed: boolean
