...
// 'auto' is always available ï¿?when TRANSCRIPT_CLASSIFIER is off, the
...
export type PermissionDecisionReason =
  ...
  | {
      ...
      // shell configs) ï¿?the classifier can see context and decide. False
...
export type YoloClassifierResult = {
  ...
  unavailable?: boolean
  /**
   * API returned "prompt is too long" ï¿?the classifier transcript exceeded
   * the context window. Deterministic (same transcript ï¿?same error), so
   ...
   * legacy 1-stage (tool_use) classifier ï¿?the single request goes here.
   */
