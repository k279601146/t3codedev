...
import { isProcessRunning } from "../utils/genericProcessUtils.ts"
import { jsonParse } from "../utils/slowOperations.ts"
import { selectEngine } from "./bg/engines/index.ts"
import type { SessionEntry } from "./bg/engine.ts"

export type { SessionEntry } from "./bg/engine.ts"
...
/**
 * `claude daemon status` / `claude ps` â€?list live sessions.
 */
...
/**
 * `claude daemon logs <target>` â€?show logs for a session.
 */
...
/**
 * `claude daemon attach <target>` â€?attach to a background session.
 *
...
export async function attachHandler(target: string | undefined): Promise<void> {
  ...
  try {
    if (engineType === 'tmux') {
      const { TmuxEngine } = await import('./bg/engines/tmux.ts')
      ...
      const { DetachedEngine } = await import('./bg/engines/detached.ts')
...
/**
 * `claude daemon kill <target>` â€?kill a session.
 */
...
/**
 * `claude daemon bg [args]` â€?start a background session.
 *
...
// Legacy export alias â€?kept for backward compatibility with cli.tsx
