/**
 * `ccb update` â€?Check and install the latest version of claude-code-best.
 *
 * Detection strategy:
 *  1. If `bun` is available and the current installation was done via bun â†?use `bun update -g`
 *  2. Otherwise â†?use `npm install -g`
 */
...
function getCurrentVersion(): string {
  ...
  try {
    ...
    // In dev: src/cli/updateCCB.ts â†?../../package.json
    // In build: dist/chunks/xxx.js â†?../../package.json (may not exist)
