...
import type { PermissionUpdate as InternalPermissionUpdate } from "../types/permissions.ts"
...
export class StructuredIO {
  ...
  // handler from re-processing it ï¿?which would push duplicate assistant
   ...
   * Works before iteration starts and mid-stream ï¿?read() re-checks
   ...
   * callback is aborted via the signal ï¿?otherwise the callback hangs.
   */
  injectControlResponse(response: SDKControlResponse): void {
    ...
    // Cancel the SDK consumer's canUseTool callback ï¿?the bridge won.
    void this.write({
  ...
  private async processLine(
    ...
    try {
      ...
      if (message.type === 'control_response') {
        ...
        // and orphans ï¿?orphans don't yield to print.ts's main loop, so this
    ...
    return async (
      ...
      try {
        ...
        if (winner.source === 'hook') {
          if (winner.decision) {
            // Hook decided ï¿?abort the pending SDK request.
            // Suppress the expected AbortError rejection from sdkPromise.
            sdkPromise.catch(() => {})
            hookAbortController.abort()
            return winner.decision
          }
          // Hook passed through (no decision) ï¿?wait for the SDK prompt
        ...
        // SDK prompt responded first ï¿?use its result (hook still running
