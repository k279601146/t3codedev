...
import { CircularBuffer } from "@t3tools/ccb-engine/src/utils/CircularBuffer.ts"
import { logForDebugging } from "@t3tools/ccb-engine/src/utils/debug.ts"
import { rcLog } from "../../bridge/rcDebugLog.ts"
...
const DEFAULT_KEEPALIVE_INTERVAL = 120_000 // 2 minutes â€?must be under Bun's 255s idleTimeout
 ...
 * the reconnection budget and retry â€?the server will reject with permanent
...
const PERMANENT_CLOSE_CODES = new Set([
  1002, // protocol error â€?server rejected handshake (e.g. session reaped)
...
export class WebSocketTransport implements Transport {
  ...
  // ws.send). Used to compute idle time at close â€?the signal for diagnosing
  ...
  public async connect(): Promise<void> {
    ...
    if (typeof Bun !== 'undefined') {
      ...
      // 'pong' is Bun-specific â€?not in DOM typings.
  ...
  private onBunError = () => {
    ...
    // close event fires after error â€?let it call handleConnectionError
  }
  ...
  private onNodeOpen = () => {
    // Capture ws before handleOpenEvent() invokes onConnectCallback â€?if the
  ...
  private handleOpenEvent(): void {
    ...
    // Reconnect success â€?capture attempt count + downtime before resetting.
  ...
  protected sendLine(line: string): boolean {
    ...
    try {
      ...
      // Don't null this.ws here â€?let doDisconnect() (via handleConnectionError)
   ...
   * until GC â€?these accumulate under network instability. Mirrors the
  ...
  private handleConnectionError(closeCode?: number): void {
    ...
    if (this.isBridge) {
      // Fire on every close â€?including intermediate ones during a reconnect
    ...
    // Permanent codes: don't retry â€?server has definitively ended the session.
    ...
    // (e.g. laptop lid closed). Reset the budget and retry from scratch â€?
  ...
  private replayBufferedMessages(lastId: string): void {
    ...
    if (lastId) {
      ...
      if (lastConfirmedIndex >= 0) {
        // Server confirmed messages up to lastConfirmedIndex â€?evict them
    ...
    // Do NOT clear the buffer after replay â€?messages remain buffered until
  ...
  private startPingInterval(): void {
    ...
    this.pingInterval = setInterval(() => {
      if (this.state === 'connected' && this.ws) {
        ...
        // missed ticks â€?it coalesces â€?so on wake this callback fires
        // once with a huge gap. The socket is almost certainly dead:
        // NAT mappings drop in 30sâ€?min, and the server has been
        // retransmitting into the void. Don't wait for a ping/pong
        // round-trip to confirm (ws.ping() on a dead socket returns
        // immediately with no error â€?bytes go into the kernel send
        // buffer). Assume dead and reconnect now. A spurious reconnect
        // after a short sleep is cheap â€?replayBufferedMessages() handles
        // it and the server dedups by UUID.
        if (gap > SLEEP_DETECTION_THRESHOLD_MS) {
          logForDebugging(
            `WebSocketTransport: ${Math.round(gap / 1000)}s tick gap detected â€?process was suspended, forcing reconnect`,
          )
          logForDiagnosticsNoPII(
            'info',
