 ...
 * - No backpressure needed â€?naturally bounded at 2 slots
 *
 * Coalescing rules:
 * - Top-level keys (worker_status, external_metadata) â€?last value wins
 * - Inside external_metadata / internal_metadata â€?RFC 7396 merge:
...
export class WorkerStateUploader {
   ...
   * patch. Fire-and-forget â€?callers don't need to await.
   */
 ...
 * one level deep â€?overlay keys are added/overwritten, null values
...
function coalescePatches(
  ...
  for (const [key, value] of Object.entries(overlay)) {
    if (
      ...
      // RFC 7396 merge â€?overlay keys win, nulls preserved for server
      merged[key] = {
