...
import { errorMessage, getErrnoCode } from "@t3tools/ccb-engine/src/utils/errors.ts"
import { createAxiosInstance } from "@t3tools/ccb-engine/src/utils/proxy.ts"
import {
...
} from "@t3tools/ccb-engine/src/utils/sessionIngressAuth.ts"
import type {
  RequiresActionDetails,
  SessionState,
} from "@t3tools/ccb-engine/src/utils/sessionState.ts"
import { sleep } from "@t3tools/ccb-engine/src/utils/sleep.ts"
import { getClaudeCodeUserAgent } from "@t3tools/ccb-engine/src/utils/userAgent.ts"
import {
  RetryableError,
 ...
 * snapshot per flush â€?each emitted event is self-contained so a client
 ...
 * expired JWT short-circuits this (exits immediately â€?deterministic,
 ...
 * clock skew). 10 Ã— 20s heartbeat â‰?200s to ride it out.
 */
 ...
 * of SDKPartialAssistantMessage â€?RawMessageStreamEvent's delta is a union and
 ...
 * lifetime is tied to the assistant message â€?cleared when the complete
...
export type StreamAccumulatorState = {
  /** API message ID (msg_...) â†?blocks[blockIndex] â†?chunk array. */
  byMessage: Map<string, string[][]>
  /**
   * {session_id}:{parent_tool_use_id} â†?active message ID.
 ...
 * accumulated text from the start of the block â€?a client connecting
...
export function accumulateStreamEvents(
  ...
  // chunks[] â†?snapshot already in `out` this flush. Keyed by the chunks
    ...
    switch (evt.type) {
      ...
      case 'content_block_delta': {
        ...
        if (!blocks) {
          ...
          // through raw â€?can't produce a full-so-far snapshot without the
          // prior chunks anyway.
          out.push(msg)
          break
        }
 ...
 * writeEvent when the SDKAssistantMessage arrives â€?the reliable end-of-stream
...
export class CCRClient {
  ...
  // stream_event delay buffer â€?accumulates content deltas for up to
  ...
  // text_delta event carries the complete text from the start of the block â€?
   ...
   * Default: process.exit(1) â€?correct for spawn-mode children where the
   ...
   * concurrent sessions with distinct JWTs MUST inject this â€?the env-var
  ...
  constructor(
    ...
    opts?: {
       ...
       * CLAUDE_CODE_SESSION_ACCESS_TOKEN (single-session callers â€?REPL,
    ...
    this.eventUploader = new SerialBatchEventUploader<ClientEvent>({
      ...
      // HybridTransport's bound â€?high enough to be memory-only.
    ...
    // moment new CCRClient() returns â€?remoteIO must be free to call
   ...
   *
   * In-process callers (replBridge) pass the epoch directly â€?they
  ...
  async initialize(epoch?: number): Promise<Record<string, unknown> | null> {
    ...
    // Concurrent with the init PUT â€?neither depends on the other.
    ...
    const result = await this.request(
      ...
      '/worker',
      {
        ...
        // worker crash â€?the in-session clears don't survive process restart.
        external_metadata: {
          pending_action: null,
          task_summary: null,
    ...
    if (!result.ok) {
      // 409 â†?onEpochMismatch may throw, but request() catches it and returns
    ...
    // has succeeded â€?logging inside getWorkerState() raced: if the GET
  ...
  private async request(
    ...
    try {
      ...
      if (response.status === 401 || response.status === 403) {
        // A 401 with an expired JWT is deterministic â€?no retry will
        ...
        if (exp !== null && exp * 1000 < Date.now()) {
          logForDebugging(
            `CCRClient: session_token expired (exp=${new Date(exp * 1000).toISOString()}) â€?no refresh was delivered, exiting`,
            { level: 'error' },
          )
          logForDiagnosticsNoPII('error', 'cli_worker_token_expired_no_refresh')
          this.onEpochMismatch()
        }
        // Token looks valid but server says 401 â€?possible server-side
        // blip (userauth down, KMS hiccup). Count toward threshold.
        this.consecutiveAuthFailures++
        if (this.consecutiveAuthFailures >= MAX_CONSECUTIVE_AUTH_FAILURES) {
          logForDebugging(
            `CCRClient: ${this.consecutiveAuthFailures} consecutive auth failures with a valid-looking token â€?server-side auth unrecoverable, exiting`,
            { level: 'error' },
          )
   ...
   * flush(). close() drops the buffer â€?call flush() first if you need
   * delivery.
   */
   ...
   * These events are NOT visible to frontend clients â€?they store worker-internal
   ...
   * when the caller needs delivery confirmation â€?close() abandons the
  ...
  /** Internal-event queue depth â€?shutdown-snapshot backpressure signal. */
