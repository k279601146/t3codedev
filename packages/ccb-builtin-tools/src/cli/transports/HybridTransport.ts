...
// Void-ed (nothing awaits it) so this is a last resort â€?replBridge teardown
 ...
 * Write flow:
 *
 *   write(stream_event) â”€â”?
 *                        â”?(100ms timer)
 *                        â”?
 *                        â–?
 *   write(other) â”€â”€â”€â”€â–?uploader.enqueue()  (SerialBatchEventUploader)
 *                        â–?   â”?
 *   writeBatch() â”€â”€â”€â”€â”€â”€â”€â”€â”?   â”?serial, batched, retries indefinitely,
 *                             â”?backpressure at maxQueueSize
 *                             â–?
 ...
 * enqueue() blocks â€?giving awaiting callers backpressure.
 *
 * Why serialize? Bridge mode fires writes via `void transport.write()`
 * (fire-and-forget). Without this, concurrent POSTs â†?concurrent Firestore
 * writes to the same document â†?collisions â†?retry storms â†?pages oncall.
 */
export class HybridTransport extends WebSocketTransport {
  ...
  constructor(
    ...
    this.uploader = new SerialBatchEventUploader<StdoutMessage>({
      // Large cap â€?session-ingress accepts arbitrary batch sizes. Events
      // naturally batch during in-flight POSTs; this just bounds the payload.
      maxBatchSize: 500,
      // Bridge callers use `void transport.write()` â€?backpressure doesn't
   ...
   * callers (`void transport.write()`) are unaffected â€?they don't await,
  ...
  override async write(message: StdoutMessage): Promise<void> {
    if (message.type === 'stream_event') {
      ...
      // Promise resolves immediately â€?callers don't await stream_events.
  ...
  /** Delay timer fired â€?enqueue accumulated stream_events. */
  ...
  override close(): void {
    ...
    this.streamEventBuffer = []
    // Grace period for queued writes â€?fallback. replBridge teardown now
  ...
  private async postOnce(events: StdoutMessage[]): Promise<void> {
    ...
    // 4xx (except 429) are permanent â€?drop, don't retry.
    if (
      response.status >= 400 &&
      response.status < 500 &&
      response.status !== 429
    ...
    // 429 / 5xx â€?retryable. Throw so uploader re-queues and backs off.
    logForDebugging(
