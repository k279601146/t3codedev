 ...
 *   until success or close() â€?unless maxConsecutiveFailures is set,
 ...
 * is set, it overrides exponential backoff for that attempt â€?clamped to
...
type SerialBatchEventUploaderConfig<T> = {
  ...
  maxQueueSize: number
  /** The actual HTTP call â€?caller controls payload format */
...
export class SerialBatchEventUploader<T> {
  ...
  /**
   * Pending queue depth. After close(), returns the count at close time â€?
   ...
   * available. Blocks (awaits) if the buffer is full â€?caller pauses
   ...
   * dropped in place â€?they can never be sent and leaving them at
  ...
  private retryDelay(failures: number, retryAfterMs?: number): number {
    ...
    if (retryAfterMs !== undefined) {
      ...
      // Retry-After. Clamp first, then spread â€?same shape as the
