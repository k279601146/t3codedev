...
import { getSessionIngressAuthHeaders } from "@t3tools/ccb-engine/src/utils/sessionIngressAuth.ts"
 ...
 * to worker subscribers â€?delivery_update, session_update, ephemeral_event,
...
export class SSETransport implements Transport {
  ...
  constructor(
     ...
     * for concurrent multi-session callers â€?the env-var path is a process
    ...
    // SSETransport always asks the server to replay from sequence 0 â€?
  ...
  private async readStream(body: ReadableStream<Uint8Array>): Promise<void> {
    ...
    let buffer = ''
    const MAX_BUFFER_BYTES = 1024 * 1024 // 1MB â€?SSE frames include event/data/id prefixes
    ...
    try {
      while (true) {
        ...
        if (buffer.length > MAX_BUFFER_BYTES) {
          logForDebugging(
            `SSETransport: Buffer exceeded ${MAX_BUFFER_BYTES} bytes â€?dropping connection`,
            { level: 'error' },
          )
        ...
        for (const frame of frames) {
          ...
          if (frame.event && frame.data) {
            ...
            // data: without event: â€?server is emitting the old envelope format
            // or a bug. Log so incidents show as a signal instead of silent drops.
            logForDebugging(
              'SSETransport: Frame has data: but no event: field â€?dropped',
              { level: 'warn' },
            )
    ...
    // Stream ended â€?reconnect unless we're closing
   ...
   *
   * Worker subscribers only receive client_event frames (see notifier.go) â€?
  ...
  // -----------------------------------------------------------------------
  // Write (HTTP POST) â€?same pattern as HybridTransport
  // -----------------------------------------------------------------------
