...
import { MCPServerDesktopImportDialog } from "@t3tools/ccb-engine/src/components/MCPServerDesktopImportDialog.ts";
...
import {
  ...
  readClientSecret,
  saveMcpClientSecret,
} from "@t3tools/ccb-engine/src/services/mcp/auth.ts";
import { connectToServer, getMcpServerConnectionBatchSize } from "@t3tools/ccb-engine/src/services/mcp/client.ts";
import {
  addMcpConfig,
  getAllMcpConfigs,
  getMcpConfigByName,
...
import type { ConfigScope, ScopedMcpServerConfig } from "@t3tools/ccb-engine/src/services/mcp/types.ts";
import { describeMcpConfigFilePath, ensureConfigScope, getScopeLabel } from "@t3tools/ccb-engine/src/services/mcp/utils.ts";
import { AppStateProvider } from "@t3tools/ccb-engine/src/state/AppState.tsx";
import { getCurrentProjectConfig, getGlobalConfig, saveCurrentProjectConfig } from "@t3tools/ccb-engine/src/utils/config.ts";
import { isFsInaccessible } from "@t3tools/ccb-engine/src/utils/errors.ts";
...
export async function mcpServeHandler({ debug, verbose }: { debug?: boolean; verbose?: boolean }): Promise<void> {
  ...
  try {
    ...
    const { startMCPServer } = await import('../../entrypoints/mcp.ts');
...
export async function mcpAddFromDesktopHandler(options: { scope?: string }): Promise<void> {
  try {
    ...
    const { readClaudeDesktopMcpServers } = await import('../../utils/claudeDesktop.ts');
