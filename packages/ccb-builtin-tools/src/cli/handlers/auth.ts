...
import { fetchAndStoreClaudeCodeFirstTokenDate } from "@t3tools/ccb-engine/src/services/api/firstTokenDate.ts"
import {
...
} from "@t3tools/ccb-engine/src/services/oauth/client.ts"
import { getOauthProfileFromOauthToken } from "@t3tools/ccb-engine/src/services/oauth/getOauthProfile.ts"
import { OAuthService } from "@t3tools/ccb-engine/src/services/oauth/index.ts"
import type { OAuthTokens } from "@t3tools/ccb-engine/src/services/oauth/types.ts"
import {
...
import { saveGlobalConfig } from "@t3tools/ccb-engine/src/utils/config.ts"
import { logForDebugging } from "@t3tools/ccb-engine/src/utils/debug.ts"
import { isRunningOnHomespace } from "@t3tools/ccb-engine/src/utils/envUtils.ts"
import { errorMessage } from "@t3tools/ccb-engine/src/utils/errors.ts"
import { logError } from "@t3tools/ccb-engine/src/utils/log.ts"
import { getAPIProvider } from "@t3tools/ccb-engine/src/utils/model/providers.ts"
import { getInitialSettings } from "@t3tools/ccb-engine/src/utils/settings/settings.ts"
import { jsonStringify } from "@t3tools/ccb-engine/src/utils/slowOperations.ts"
import {
  buildAccountProperties,
  buildAPIProviderProperties,
} from "@t3tools/ccb-engine/src/utils/status.tsx"
...
export async function installOAuthTokens(tokens: OAuthTokens): Promise<void> {
  ...
  if (shouldUseClaudeAIAuth(tokens.scopes)) {
    ...
    // API key creation is critical for Console users â€?let it throw.
...
export async function authLogin({
  ...
  // forceLoginMethod is a hard constraint (enterprise setting) â€?matches ConsoleOAuthFlow behavior.
  ...
  if (envRefreshToken) {
    ...
    try {
      ...
      // Mark onboarding complete â€?interactive paths handle this via
  ...
  try {
    ...
    const result = await oauthService.startOAuthFlow(
      async url => {
        process.stdout.write('Opening browser to sign inâ€¦\n')
