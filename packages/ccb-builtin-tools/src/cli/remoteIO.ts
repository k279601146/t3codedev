...
export class RemoteIO extends StructuredIO {
  ...
  constructor(
    ...
    // synchronously, so new CCRClient() MUST run before transport.connect() â€?
    ...
    if (isEnvTruthy(process.env.CLAUDE_CODE_USE_CCR_V2)) {
      ...
      // different files â€?assert the invariant so a future decoupling
    ...
    // need it â€?different network path.
