 ...
 * the growthbook.ts â†?config.ts â†?file.ts â†?sessionStorage.ts â†?commands.ts
 * transitive dependency chain.
 */

/**
 * Poll interval when actively seeking work (no transport / below maxSessions).
 * Governs user-visible "connectingâ€? latency on initial work pickup and
 ...
 * heartbeat â€?when both are enabled, the heartbeat loop breaks out to poll
 * at this interval. Set to 0 to disable at-capacity polling entirely.
 *
 * Server-side constraints that bound this value:
 * - BRIDGE_LAST_POLL_TTL = 4h (Redis key expiry â†?environment auto-archived)
 * - max_poll_stale_seconds = 24h (session-creation health gate, currently disabled)
 *
 * 10 minutes gives 24Ã— headroom on the Redis TTL while still picking up
 ...
 * failures, so poll is not the recovery path â€?it's strictly a liveness
...
export const DEFAULT_POLL_CONFIG: PollIntervalConfig = {
  ...
  // at this interval. Independent of poll_interval_ms_at_capacity â€?both may
  // run (heartbeat periodically yields to poll). 60s gives 5Ã— headroom under
  ...
  // clients â€?heartbeat suppressed poll). Old clients ignore this key; ops
