...
// Memoized ï¿?secureStorage.read() spawns a macOS `security` subprocess (~40ms).
...
//
// Only the storage read is memoized ï¿?the GrowthBook gate is checked live so
 ...
 * in-flight (enrollTrustedDevice is async ï¿?bridge API calls between login and
...
export function clearTrustedDeviceToken(): void {
  ...
  try {
    ...
    // Best-effort ï¿?don't block login if storage is inaccessible
  }
  readStoredToken.cache?.clear?.()
}
 ...
 * to keychain. Best-effort ï¿?logs and returns on failure so callers
...
export async function enrollTrustedDevice(): Promise<void> {
  try {
    ...
    // skip enrollment ï¿?the env var takes precedence in readStoredToken() so
    ...
    // Lazy require ï¿?utils/auth.ts transitively pulls ~1300 modules
    // (config ï¿?file ï¿?permissions ï¿?sessionStorage ï¿?commands). Daemon callers
    ...
    // Always re-enroll on /login ï¿?the existing token may belong to a
    ...
    try {
      response = await axios.post<{
        ...
        { display_name: `Claude Code on ${hostname()} Â· ${process.platform}` },
        {
          headers: {
