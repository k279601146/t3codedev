...
import { getWorktreePathsPortable } from "../utils/getWorktreePathsPortable.ts"
import { lazySchema } from "../utils/lazySchema.ts"
import {
  getProjectsDir,
  sanitizePath,
} from "../utils/sessionStoragePortable.ts"
 ...
 * so that a periodic re-write with the same content serves as a refresh â€?
...
/**
 * Write the pointer. Also used to refresh mtime during long sessions â€?
 * calling with the same IDs is a cheap no-content-change write that bumps
 * the staleness clock. Best-effort â€?a crash-recovery file must never
 ...
 * and handles errors â€?no existence check (CLAUDE.md TOCTOU rule). Returns
...
export async function readBridgePointer(
  ...
  try {
    ...
    // are needed â€?mtime IS the data we return, not a TOCTOU guard.
 ...
 * mutate to a worktree path â€?but `claude remote-control --continue` runs
 ...
 * that misses â€?the common case (pointer in launch dir) is one stat, zero
...
export async function readBridgePointerAcrossWorktrees(
  ...
  // rare ones that have one. Promise.all â†?latency â‰?slowest single stat.
...
/**
 * Delete the pointer. Idempotent â€?ENOENT is expected when the process
