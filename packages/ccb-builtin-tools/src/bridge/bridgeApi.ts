...
import { debugBody, extractErrorDetail } from "./debugUtils.ts"
...
type BridgeApiDeps = {
   ...
   * handleOAuth401Error from utils/auth.ts transitively pulls in config.ts â†?
   * file.ts â†?permissions/filesystem.ts â†?sessionStorage.ts â†?commands.ts
   * (~1300 modules). Daemon callers using env-var tokens omit this â€?their
   ...
   * Optional â€?when absent or returning undefined, the header is omitted
...
export function createBridgeApiClient(deps: BridgeApiDeps): BridgeApiClient {
  ...
  async function withOAuthRetry<T>(
    ...
    // Attempt token refresh â€?matches the pattern in withRetry.ts
    ...
    // Refresh failed â€?return 401 for handleErrorStatus to throw
    return response
  }
        ...
        (token: string) =>
          axios.post<{
            ...
            `${deps.baseUrl}/v1/environments/bridge`,
            {
              ...
              // the old one expired â€?callers must compare the response.
