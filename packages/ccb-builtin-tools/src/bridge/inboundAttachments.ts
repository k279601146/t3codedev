...
import { getBridgeAccessToken, getBridgeBaseUrl } from "./bridgeConfig.ts"
...
async function resolveOne(att: InboundAttachment): Promise<string | undefined> {
  ...
  try {
    ...
    // CLAUDE_CODE_CUSTOM_OAUTH_URL â€?keep it inside the try so a bad
  ...
  // (same filename, different files). 8 chars is enough â€?this isn't security.
  ...
  debug(`resolved ${att.file_uuid} â†?${outPath} (${data.length} bytes)`)
  return outPath
}
...
export async function resolveInboundAttachments(
  ...
  // Quoted form â€?extractAtMentionedFiles truncates unquoted @refs at the
 ...
 * Targets the LAST text block â€?processUserInputBase reads inputString
...
export function prependPathRefs(
  ...
  // No text block â€?append one at the end so it's last.
  return [...content, { type: 'text', text: prefix.trimEnd() }]
}

/**
 * Convenience: extract + resolve + prepend. No-op when the message has no
 * file_attachments field (fast path â€?no network, returns same reference).
 */
