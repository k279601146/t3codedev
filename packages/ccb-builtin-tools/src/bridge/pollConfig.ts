...
// clamp, Zod rejects the whole object on violation â€?a config with one bad
// field falls back to DEFAULT_POLL_CONFIG entirely rather than being
// partially trusted.
//
// The at_capacity intervals use a 0-or-â‰?00 refinement: 0 means "disabled"
// (heartbeat-only mode), â‰?00 is the fat-finger floor. Values 1â€?9 are
...
// restoring at_capacity) falls through every throttle site with no sleep â€?
// tight-looping /poll at HTTP-round-trip speed.
const zeroOrAtLeast100 = {
  message: 'must be 0 (disabled) or â‰?00ms',
}
const pollIntervalConfigSchema = lazySchema(() =>
  z
    .object({
      poll_interval_ms_not_at_capacity: z.number().int().min(100),
      // 0 = no at-capacity polling. Independent of heartbeat â€?both can be
