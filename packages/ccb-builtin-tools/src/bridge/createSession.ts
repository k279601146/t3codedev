...
export async function createBridgeSession({
  ...
  const { getDefaultBranch } = await import('../utils/git.ts')
  const { getMainLoopModel } = await import('../utils/model/model.ts')
  const { default: axios } = await import('axios')
  const { isSelfHostedBridge } = await import('./bridgeConfig.ts')
  ...
  // Self-hosted bridges don't require a claude.ai org UUID â€?the local server
  ...
  if (gitRepoUrl) {
    const { parseGitRemote } = await import('../utils/detectRepository.ts')
