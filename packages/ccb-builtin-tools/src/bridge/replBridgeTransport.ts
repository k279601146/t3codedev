 ...
 *
 * The v2 write path goes through CCRClient.writeEvent â†?SerialBatchEventUploader,
 * NOT through SSETransport.write() â€?SSETransport.write() targets the
...
export type ReplBridgeTransport = {
   ...
   *
   * v1 returns 0 â€?Session-Ingress WS doesn't use SSE sequence numbers;
   ...
   * v2 returns 0 â€?the v2 write path doesn't set maxConsecutiveFailures.
   */
  readonly droppedBatchCount: number
  /**
   * PUT /worker state (v2 only; v1 is a no-op). `requires_action` tells
   * the backend a permission prompt is pending â€?claude.ai shows the
   ...
   * immediately â€?HybridTransport POSTs are already awaited per-write).
   */
  flush(): Promise<void>
}
 ...
 * The JWT is refreshed when the poll loop re-dispatches work â€?the caller
 ...
 * is one async step. registerWorker failure propagates â€?replBridge will
...
export async function createV2ReplTransport(opts: {
   ...
   * already bumped epoch (the /bridge call IS the register â€?see server
  ...
  /** Â±fraction per-beat jitter. Defaults to 0 (no jitter) when omitted. */
  heartbeatJitterFraction?: number
  /**
   * When true, skip opening the SSE read stream â€?only the CCRClient write
   ...
   * multiple concurrent sessions â€?the env-var path stomps across sessions.
  ...
  // the process-wide env var (legacy single-session path â€?CCRClient's
  ...
  if (getAuthToken) {
    ...
    // getSessionIngressAuthHeaders() â†?this env var. Set it before either
    // touches the network.
  ...
  const ccr = new CCRClient(sse, new URL(sessionUrl), {
    ...
    // Default is process.exit(1) â€?correct for spawn-mode children. In-process,
    ...
    onEpochMismatch: () => {
      logForDebugging(
        '[bridge:repl] CCR v2: epoch superseded (409) â€?closing for poll-loop recovery',
      )
      // Close resources in a try block so the throw always executes.
      // If ccr.close() or sse.close() throw, we still need to unwind
      // the caller (request()) â€?otherwise handleEpochMismatch's `never`
      ...
      // Don't return â€?the calling request() code continues after the 409
  ...
  // CCRClient's constructor wired sse.setOnEvent â†?reportDelivery('received').
  ...
  // transport's only caller (replBridge/daemonBridge) has no such wiring â€?the
  ...
  // them on every daemon restart (observed: 21â†?4â†?5 phantom prompts as
  ...
  // SSE receipt and transcript-write is narrow (queue â†?SDK â†?child stdin â†?
  // model); a crash there loses one prompt vs. the observed N-prompt flood on
  // every restart. Overwrite the constructor's wiring to do both â€?setOnEvent
  ...
  // replBridge's calling order is newTransport â†?setOnConnect â†?setOnData â†?
  // setOnClose â†?connect(), and both calls need those callbacks wired first:
  ...
  // awaits its read loop and never resolves â€?don't gate on it.
  ...
  return {
    ...
    isConnectedStatus() {
      // Write-readiness, not read-readiness â€?replBridge checks this
    ...
    setOnClose(cb) {
      onCloseCb = cb
      // SSE reconnect-budget exhaustion fires onClose(undefined) â€?map to
    ...
    // v2 write path (CCRClient) doesn't set maxConsecutiveFailures â€?no drops.
    droppedBatchCount: 0,
    reportState(state) {
    ...
    connect() {
      // Outbound-only: skip the SSE read stream entirely â€?no inbound
      ...
      if (!opts.outboundOnly) {
        // Fire-and-forget â€?SSETransport.connect() awaits readStream()
