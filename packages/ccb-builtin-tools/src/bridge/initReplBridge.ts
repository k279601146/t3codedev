 ...
 * bootstrap state ï¿?gates, cwd, session ID, git context, OAuth, title
 * derivation ï¿?then delegates to the bootstrap-free core.
 *
 * Split out of replBridge.ts because the sessionStorage import
 * (getCurrentSessionTitle) transitively pulls in src/commands.ts ï¿?the
...
export type InitBridgeOptions = {
  ...
  // Optional ï¿?print.ts's SDK enableRemoteControl path has no REPL message
  ...
  // Mutated in place ï¿?newly flushed UUIDs are added after each flush.
  previouslyFlushedUUIDs?: Set<string>
  /** See BridgeCoreParams.perpetual. */
  perpetual?: boolean
  /**
   * When true, the bridge only forwards events outbound (no SSE inbound
   * stream). Used by CCR mirror mode ï¿?local sessions visible on claude.ai
...
export async function initReplBridge(
  ...
  // GrowthBook gate. Daemon/SDK paths skip this ï¿?shim defaults to active.
  ...
  // 1b. Minimum version check ï¿?deferred to after the v1/v2 branch below,
  ...
  // 2. Check OAuth ï¿?must be signed in with claude.ai. Runs before the
  ...
  // 3. Check organization policy ï¿?remote control may be disabled
  ...
  // uses that token directly via getBridgeAccessToken() ï¿?keychain state is
  ...
  if (!getBridgeTokenOverride()) {
    ...
    // dead token (matched by expiresAt), skip silently ï¿?no event, no refresh
    ...
    // The expiresAt key is content-addressed: /login ï¿?new token ï¿?new expiresAt
    // ï¿?this stops matching without any explicit clear.
    ...
    // 2b. Proactively refresh if expired. Mirrors bridgeMain.ts:2096 ï¿?the REPL
    ...
    // registrations hit the server with a >8h-expired token ï¿?401 ï¿?withOAuthRetry
    ...
    // explicit clearOAuthTokenCache() here ï¿?that would force a blocking
    // keychain spawn on the 91%+ fresh-token path.
    await checkAndRefreshOAuthTokenIfNeeded()

    // 2c. Skip if token is still expired post-refresh-attempt. Env-var / FD
    // tokens (auth.ts:894-917) have expiresAt=null ï¿?never trip this. But a
    // keychain token whose refresh token is dead (password change, org left,
    // token GC'd) has expiresAt<now AND refresh just failed ï¿?the client would
    // otherwise loop 401 forever: withOAuthRetry ï¿?handleOAuth401Error ï¿?
    // refresh fails again ï¿?retry with same stale token ï¿?401 again.
    ...
    //
    // Intentionally NOT using isOAuthTokenExpired here ï¿?that has a 5-minute
    ...
    // Check actual expiry instead: past-expiry AND refresh-failed ï¿?truly dead.
    ...
    if (tokens && tokens.expiresAt !== null && tokens.expiresAt <= Date.now()) {
      ...
      // is never reached again ï¿?writes are capped at 3 per dead token.
  ...
  // 4. Compute baseUrl ï¿?needed by both v1 (env-based) and v2 (env-less)
  // paths. Hoisted above the v2 gate so both can use it.
  const baseUrl = getBridgeBaseUrl()

  // 5. Derive session title. Precedence: explicit initialName ï¿?/rename
  // (session storage) ï¿?last meaningful user message ï¿?generated slug.
  // Cosmetic only (claude.ai session list); the model never sees it.
  // Two flags: `hasExplicitTitle` (initialName or /rename ï¿?never auto-
  // overwrite) vs. `hasTitle` (any title, including auto-derived ï¿?blocks
  ...
  if (initialName) {
    ...
    if (customTitle) {
      ...
      // continuedï¿?), non-human origins (task notifications, channel pushes),
      // and synthetic interrupts ([Request interrupted by user]) ï¿?none are
  ...
  // Shared by both v1 and v2 ï¿?fires on every title-worthy user message until
  ...
  // title is explicit (/remote-control <name> or /rename) ï¿?re-checks
  ...
  // out-of-order resolution (genSeq ï¿?count-1's Haiku resolving after count-3
  ...
  // Fetch orgUUID before the v1/v2 branch ï¿?both paths need it. v1 for
  ...
  // Self-hosted bridges skip this check ï¿?the local server doesn't require
  // org-based auth.
  ...
  // poll/ack/heartbeat) and connects directly via POST /bridge ï¿?worker_jwt.
  // See server PR #292605 (renamed in #293280). REPL-only ï¿?daemon/print stay
  // on env-based.
  //
  ...
  // env-coupled and not yet implemented here ï¿?fall back to env-based when set
  ...
  if (isEnvLessBridgeEnabled() && !perpetual) {
    ...
    return initEnvLessBridgeCore({
      ...
      // previouslyFlushedUUIDs is not passed ï¿?there's no cross-session
      // UUID collision risk, and the ref persists across enableâ†’disableï¿?
  ...
  // Gather git context ï¿?this is the bootstrap-read boundary.
  ...
  // so no adapter needed ï¿?just the narrower type on the way out.
  return initBridgeCore({
    ...
    archiveSession: sessionId =>
      archiveBridgeSession(sessionId, {
        ...
        // archiveBridgeSession has no try/catch ï¿?5xx/timeout/network throw
    ...
    // `title` directly ï¿?both paths are picked up here.
...
function deriveTitle(raw: string): string | undefined {
  // Strip <ide_opened_file>, <session-start-hook>, etc. ï¿?these appear in
  ...
  // Capture group instead of lookbehind ï¿?keeps YARR JIT happy.
  const firstSentence = /^(.*?[.!?])\s/.exec(clean)?.[1] ?? clean
  // Collapse newlines/tabs ï¿?titles are single-line in the claude.ai list.
