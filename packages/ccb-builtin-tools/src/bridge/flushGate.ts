 ...
 *
 * Lifecycle:
 *   start() â†?enqueue() returns true, items are queued
 *   end()   â†?returns queued items for draining, enqueue() returns false
 *   drop()  â†?discards queued items (permanent transport close)
 *   deactivate() â†?clears active flag without dropping items
 *                   (transport replacement â€?new transport will drain)
 */
export class FlushGate<T> {
   ...
   * Used when the transport is replaced (onWorkReceived) â€?the new
