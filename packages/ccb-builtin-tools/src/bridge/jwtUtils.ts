...
export function createTokenRefreshScheduler({
  ...
  // Generation counter per session â€?incremented by schedule() and cancel()
  ...
  function schedule(sessionId: string, token: string): void {
    ...
    if (!expiry) {
      ...
      // chain is not broken.
      logForDebugging(
        `[${label}:token] Could not decode JWT expiry for sessionId=${sessionId}, token prefix=${token.slice(0, 15)}â€? keeping existing timer`,
      )
      return
    }

    // Clear any existing refresh timer â€?we have a concrete expiry to replace it.
  ...
  function scheduleFromExpiresIn(
    ...
    const gen = nextGeneration(sessionId)
    // Clamp to 30s floor â€?if refreshBufferMs exceeds the server's expires_in
    // (e.g. very large buffer for frequent-refresh testing, or server shortens
    // expires_in unexpectedly), unclamped delayMs â‰?0 would tight-loop.
  ...
  async function doRefresh(sessionId: string, gen: number): Promise<void> {
    ...
    // the generation will have changed â€?bail out to avoid orphaned timers.
    ...
    logForDebugging(
      `[${label}:token] Refreshing token for sessionId=${sessionId}: new token prefix=${oauthToken.slice(0, 15)}â€¦`,
    )
