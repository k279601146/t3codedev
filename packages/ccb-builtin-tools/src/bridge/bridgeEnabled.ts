...
import { isSelfHostedBridge } from "./bridgeConfig.ts"
// Namespace import breaks the bridgeEnabled ï¿?auth ï¿?config ï¿?bridgeEnabled
// cycle ï¿?authModule.foo is a live binding, so by the time the helpers below
 ...
 * and Console API logins ï¿?none of which have the OAuth token CCR needs.
...
export function isBridgeEnabled(): boolean {
  ...
  // Positive ternary pattern ï¿?see docs/feature-gating.md.
 ...
 * config.oauthAccount ï¿?populated by /api/oauth/profile during login.
...
// program, before enableConfigs() runs. isClaudeAISubscriber() ï¿?getGlobalConfig()
// throws "Config accessed before allowed" there. Pre-config, no OAuth token can
// exist anyway ï¿?false is correct. Same swallow getFeatureValue_CACHED_MAY_BE_STALE
 ...
 *
 * This gates which implementation initReplBridge uses ï¿?NOT whether bridge
...
/**
 * Kill-switch for the `cse_*` ï¿?`session_*` client-side retag shim.
 *
 ...
 * Defaults to true ï¿?the shim stays active until explicitly disabled.
 */
 ...
 * in envLessBridgeConfig.ts instead ï¿?the two implementations have independent
 * version floors.
 *
 * Uses cached (non-blocking) GrowthBook config. If GrowthBook hasn't
 * loaded yet, the default '0.0.0' means the check passes ï¿?a safe fallback.
 */
export function checkBridgeMinVersion(): string | null {
  // Positive pattern ï¿?see docs/feature-gating.md.
  // Negative pattern (if (!feature(...)) return) does not eliminate
  // inline string literals from external builds.
  if (feature('BRIDGE_MODE')) {
 ...
 * default ï¿?the user can still opt out by setting remoteControlAtStartup=false
 * in config (explicit settings always win over this default).
 *
 * Defined here rather than in config.ts to avoid a direct
 * config.ts ï¿?growthbook.ts import cycle (growthbook.ts ï¿?user.ts ï¿?config.ts).
 */
...
/**
 * Opt-in CCR mirror mode ï¿?every local session spawns an outbound-only
