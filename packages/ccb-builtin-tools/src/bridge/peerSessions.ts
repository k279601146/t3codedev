...
import { getBridgeAccessToken } from "./bridgeConfig.ts"
import { getReplBridgeHandle } from "./replBridgeHandle.ts"
...
export async function listBridgePeers(): Promise<BridgePeerSession[]> {
  const { listAllLiveSessions } = await import('../utils/udsClient.ts')
...
export async function postInterClaudeMessage(
  ...
  try {
    ...
    // Validate against path traversal â€?same allowlist as bridgeApi.ts
