...
import { buildCliLaunch, spawnCli } from "../utils/cliLaunch.ts"
 ...
 *
 * Subcommands:
 *   (none)  â€?unified status (supervisor + sessions)
 *   start   â€?start the supervisor with default workers
 *   stop    â€?send SIGTERM to supervisor
 *   status  â€?unified status (supervisor + sessions)
 *   ps      â€?alias for status
 *   bg      â€?start a background session
 *   attach  â€?attach to a background session
 *   logs    â€?show session logs
 *   kill    â€?kill a session
 */
...
function printHelp(): void {
  console.log(`
Claude Code Daemon â€?background process management
...
function spawnWorker(
  ...
  child.on('exit', (code, sig) => {
    ...
    if (code === EXIT_CODE_PERMANENT) {
      console.error(
        `[daemon] worker '${worker.kind}' exited with permanent error â€?parking`,
      )
      worker.parked = true
      return
    }
    ...
    if (runDuration < 10_000) {
      ...
      if (worker.failureCount >= MAX_RAPID_FAILURES) {
        console.error(
          `[daemon] worker '${worker.kind}' failed ${worker.failureCount} times rapidly â€?parking`,
        )
        worker.parked = true
        return
      }
