 ...
 * â€?it runs the headless bridge loop that accepts remote sessions.
 */
...
/**
 * Remote Control worker â€?runs `runBridgeHeadless()` with config from
 * environment variables set by the daemon supervisor.
 *
 * Environment variables (set by daemonMain):
 *   DAEMON_WORKER_DIR          â€?working directory
 *   DAEMON_WORKER_NAME         â€?optional session name
 *   DAEMON_WORKER_SPAWN_MODE   â€?'same-dir' | 'worktree'
 *   DAEMON_WORKER_CAPACITY     â€?max concurrent sessions
 *   DAEMON_WORKER_PERMISSION   â€?permission mode
 *   DAEMON_WORKER_SANDBOX      â€?'1' for sandbox mode
 *   DAEMON_WORKER_TIMEOUT_MS   â€?session timeout in ms
 *   DAEMON_WORKER_CREATE_SESSION â€?'1' to pre-create session on start
 */
async function runRemoteControlWorker(): Promise<void> {
  ...
  const opts: HeadlessBridgeOpts = {
    ...
    onAuth401: async (_failedToken: string) => {
      // In daemon context, re-check auth â€?supervisor may have refreshed token.
