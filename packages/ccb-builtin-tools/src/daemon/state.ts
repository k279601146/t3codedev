 ...
 *
 * Returns:
 *  - { status: 'running', state } â€?PID is alive
 *  - { status: 'stopped' }       â€?no state file
 *  - { status: 'stale' }         â€?state file exists but PID is dead (auto-cleaned)
 */
export function queryDaemonStatus(name = 'remote-control'): {
  ...
  // Stale â€?process is dead but state file remains
