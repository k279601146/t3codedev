...
import { getIsInteractive } from "@t3tools/ccb-engine/src/bootstrap/state.ts"
import {
  CLAUDE_AI_INFERENCE_SCOPE,
  getOauthConfig,
  OAUTH_BETA_HEADER,
} from "@t3tools/ccb-engine/src/constants/oauth.ts"
import {
  checkAndRefreshOAuthTokenIfNeeded,
  getClaudeAIOAuthTokens,
} from "@t3tools/ccb-engine/src/utils/auth.ts"
import { clearMemoryFileCaches } from "@t3tools/ccb-engine/src/utils/claudemd.ts"
...
import { classifyAxiosError } from "@t3tools/ccb-engine/src/utils/errors.ts"
...
import { markInternalWrite } from "@t3tools/ccb-engine/src/utils/settings/internalWrites.ts"
import { getSettingsFilePathForSource } from "@t3tools/ccb-engine/src/utils/settings/settings.ts"
import { resetSettingsCache } from "@t3tools/ccb-engine/src/utils/settings/settingsCache.ts"
 ...
 * when this returns true ï¿?applyRemoteEntriesToLocal uses markInternalWrite
 ...
 * settingsSync ï¿?changeDetector cycle edge.
 */
 ...
 *
 * Only checks user:inference (not user:profile) ï¿?CCR's file-descriptor token
