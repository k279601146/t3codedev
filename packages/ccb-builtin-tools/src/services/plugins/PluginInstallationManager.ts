...
import { clearPluginCache } from "@t3tools/ccb-engine/src/utils/plugins/pluginLoader.ts"
import {
  diffMarketplaces,
  reconcileMarketplaces,
} from "@t3tools/ccb-engine/src/utils/plugins/reconciler.ts"
import { refreshActivePlugins } from "@t3tools/ccb-engine/src/utils/plugins/refresh.ts"
 ...
 * reconciled:
 * - New installs â†?auto-refresh plugins (fixes "plugin-not-found" errors
 *   from the initial cache-only load on fresh homespace/cleared cache)
 * - Updates only â†?set needsRefresh, show notification for /reload-plugins
 */
export async function performBackgroundPluginInstallations(
  ...
  try {
    ...
    // Initialize AppState with pending status. No per-plugin pending status â€?
    ...
    if (result.installed.length > 0) {
      // New marketplaces were installed â€?auto-refresh plugins. This fixes
      ...
      // Existing marketplaces updated â€?notify user to run /reload-plugins.
