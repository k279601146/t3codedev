...
import { getManagedPluginNames } from "@t3tools/ccb-engine/src/utils/plugins/managedPlugins.ts"
import { parsePluginIdentifier } from "@t3tools/ccb-engine/src/utils/plugins/pluginIdentifier.ts"
import type { PluginScope } from "@t3tools/ccb-engine/src/utils/plugins/schemas.ts"
import { writeToStdout } from "@t3tools/ccb-engine/src/utils/process.ts"
import {
  buildPluginTelemetryFields,
  classifyPluginCommandError,
} from "@t3tools/ccb-engine/src/utils/telemetry/pluginTelemetry.ts"
import {
...
export async function updatePluginCli(
  ...
  try {
    writeToStdout(
      `Checking for updates for plugin "${plugin}" at ${scope} scopeâ€¦\n`,
    )
