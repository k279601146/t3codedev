...
import { updateAgentSummary } from "@t3tools/ccb-engine/src/tasks/LocalAgentTask/LocalAgentTask.tsx"
...
import { getAgentTranscript } from "@t3tools/ccb-engine/src/utils/sessionStorage.ts"
import { buildSummaryContext } from "./summaryContext.ts"
import {
  buildSummaryPrompt,
...
export function startAgentSummarization(
  ...
  // Drop forkContextMessages from the closure â€?runSummary rebuilds it each
  ...
  async function runSummary(): Promise<void> {
    ...
    if (isPoorModeActiveImpl()) {
      logForDebuggingImpl('[AgentSummary] Skipping summary â€?poor mode active')
      scheduleNext()
      return
    }
    ...
    try {
      ...
      if (!transcript || transcript.messages.length < 3) {
        // Not enough context yet â€?finally block will schedule next attempt
        logForDebuggingImpl(
