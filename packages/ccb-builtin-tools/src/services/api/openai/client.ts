...
import { openaiAdapter } from "../../../services/providerUsage/adapters/openai.ts"
import { updateProviderBuckets } from "../../../services/providerUsage/store.ts"
...
function wrapFetchForUsage(base: typeof fetch): typeof fetch {
  const wrapped = async (
    ...
    try {
      ...
      // Ignore â€?usage tracking must not affect the request path.
    }
    return res
  }
