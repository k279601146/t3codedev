...
import { getOAuthHeaders, prepareApiRequest } from "@t3tools/ccb-engine/src/utils/teleport/api.ts"
 ...
 * Callers should render nothing (not block) when this returns null â€?
...
export async function refreshOverageCreditGrantCache(): Promise<void> {
  ...
  if (!info) return
  // Skip rewriting info if grant data is unchanged â€?avoids config write
  ...
  saveGlobalConfig(prev => {
    ...
    // read â€?saveConfigWithLock re-reads config from disk under the file lock,
