...
import { djb2Hash } from "@t3tools/ccb-engine/src/utils/hash.ts"
import { logError } from "@t3tools/ccb-engine/src/utils/log.ts"
import { getClaudeTempDir } from "@t3tools/ccb-engine/src/utils/permissions/filesystem.ts"
...
type PreviousState = {
   ...
   *  (globalâ†”org, 1hâ†?m) that stripCacheControl erases from systemHash. */
  ...
  model: string
  fastMode: boolean
  /** 'tool_based' | 'system_prompt' | 'none' â€?flips when MCP tools are
   *  discovered/removed. */
  globalCacheStrategy: string
  /** Sorted beta header list. Diffed to show which headers were added/removed. */
  betas: string[]
  /** AFK_MODE_BETA_HEADER presence â€?should NOT break cache anymore
   *  (sticky-on latched in claude.ts). Tracked to verify the fix. */
  autoModeActive: boolean
  /** Overage state flip â€?should NOT break cache anymore (eligibility is
   *  latched session-stable in should1hCacheTTL). Tracked to verify the fix. */
  isUsingOverage: boolean
  /** Cache-editing beta header presence â€?should NOT break cache anymore
   *  (sticky-on latched in claude.ts). Tracked to verify the fix. */
  cachedMCEnabled: boolean
  /** Resolved effort (env â†?options â†?model default). Goes into output_config
   *  or anthropic_internal.effort_override. */
  effortValue: string
  /** Hash of getExtraBodyParams() â€?catches CLAUDE_CODE_EXTRA_BODY and
   ...
   *  will legitimately drop â€?this is expected, not a break. */
 ...
 * value â€?they run 1-3 turns with a fresh agentId each time, so there's
...
/** Extended tracking snapshot â€?everything that could affect the server-side
 ...
 * Does NOT fire events â€?just stores pending changes for phase 2 to use.
 */
export function recordPromptState(snapshot: PromptStateSnapshot): void {
  try {
    ...
    // Hash the full system array INCLUDING cache_control â€?this catches
    // scope flips (globalâ†”org/none) and TTL flips (1hâ†?m) that the stripped
    ...
    // Only compute per-tool hashes when the aggregate changed â€?common case
...
export async function checkResponseForCacheBreak(
  ...
  try {
    ...
    // Skip the first call â€?no previous value to compare against
    if (prevCacheRead === null) return

    const changes = state.pendingChanges

    // Cache deletions via cached microcompact intentionally reduce the cached
    // prefix. The drop in cache read tokens is expected â€?reset the baseline
    // so we don't false-positive on the next call.
    if (state.cacheDeletionsPending) {
      state.cacheDeletionsPending = false
      logForDebugging(
        `[PROMPT CACHE] cache deletion applied, cache read: ${prevCacheRead} â†?${cacheReadTokens} (expected drop)`,
      )
      // Don't flag as a break â€?the remaining state is still valid
    ...
    if (changes) {
      if (changes.modelChanged) {
        parts.push(
          `model changed (${changes.previousModel} â†?${changes.newModel})`,
        )
      }
      ...
      if (changes.globalCacheStrategyChanged) {
        parts.push(
          `global cache strategy changed (${changes.prevGlobalCacheStrategy || 'none'} â†?${changes.newGlobalCacheStrategy || 'none'})`,
        )
      }
      if (
        ...
        // Only report as standalone cause if nothing else explains it â€?
      ...
      if (changes.effortChanged) {
        parts.push(
          `effort changed (${changes.prevEffortValue || 'default'} â†?${changes.newEffortValue || 'default'})`,
        )
      }
    ...
    // the summary log so ants can find it (DevBar UI removed â€?event data
    ...
    const summary = `[PROMPT CACHE BREAK] ${reason} [source=${querySource}, call #${state.callCount}, cache read: ${prevCacheRead} â†?${cacheReadTokens}, creation: ${cacheCreationTokens}${diffSuffix}]`
 ...
 * The next API response will have lower cache read tokens â€?that's
 ...
 * will naturally drop on the next call â€?that's not a break.
 */
