...
import { withOAuth401Retry } from "@t3tools/ccb-engine/src/utils/http.ts"
...
async function fetchBootstrapAPI(): Promise<BootstrapResponse | null> {
  ...
  // OAuth preferred (requires user:profile scope ï¿?service-key OAuth tokens
  ...
  // through on 401 (no refresh mechanism ï¿?no OAuth token to pass).
  try {
...
export async function fetchBootstrapData(): Promise<void> {
  try {
    ...
    // Only persist if data actually changed ï¿?avoids a config write on every startup.
