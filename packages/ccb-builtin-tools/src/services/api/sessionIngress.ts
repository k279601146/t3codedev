...
import type { Entry, TranscriptMessage } from "@t3tools/ccb-engine/src/types/logs.ts"
...
import { sequential } from "@t3tools/ccb-engine/src/utils/sequential.ts"
...
import { getOAuthHeaders } from "@t3tools/ccb-engine/src/utils/teleport/api.ts"
...
async function appendSessionLogImpl(
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
      ...
      if (response.status === 409) {
        ...
        if (serverLastUuid) {
          ...
          if (adoptedUuid) {
            ...
            // Can't determine server state â€?give up
    ...
    logForDebugging(
      `Remote persistence attempt ${attempt}/${MAX_RETRIES} failed, retrying in ${delayMs}msâ€¦`,
    )
    await sleep(delayMs)
  }
 ...
 * WorkerEvent.payload IS the Entry (TranscriptMessage struct) â€?the CLI
...
type TeleportEventsResponse = {
  ...
  // Unset when there are no more pages â€?this IS the end-of-stream
 ...
 * threadstore for pre-backfill session_* IDs. The cursor is opaque to us â€?
...
export async function getTeleportEvents(
  ...
  // Infinite-loop guard: 1000/page Ã— 100 pages = 100k events. Larger than
  // session-ingress's 50k one-shot. If we hit this, something's wrong
  // (server not advancing cursor) â€?bail rather than hang.
  const maxPages = 100
  ...
  while (pages < maxPages) {
    ...
    if (response.status === 404) {
      ...
      //       threadstore) â€?nothing to fetch.
      ...
      // null â†?same "Failed to fetch session logs" error as today.
      //
      // 404 mid-pagination (pages > 0) means session was deleted between
      // pages â€?return what we have.
      logForDebugging(
    ...
    // events (server skips them) or encryption failures â€?skip here too.
    ...
    pages++
    // == null covers both `null` and `undefined` â€?the proto omits the
  ...
  if (pages >= maxPages) {
    // Don't fail â€?return what we have. Better to teleport with a
