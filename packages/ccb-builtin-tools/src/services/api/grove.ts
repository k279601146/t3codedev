...
import { getOauthAccountInfo, isConsumerSubscriber } from "@t3tools/ccb-engine/src/utils/auth.ts"
...
import { writeToStderr } from "@t3tools/ccb-engine/src/utils/process.ts"
...
import {
  ...
  getUserAgent,
  withOAuth401Retry,
} from "@t3tools/ccb-engine/src/utils/http.ts"
...
export const getGroveSettings = memoize(
  async (): Promise<ApiResult<AccountSettings>> => {
    ...
    try {
      ...
      logError(err)
      // Don't cache failures ï¿?transient network issues would lock the user
...
export async function markGroveNoticeViewed(): Promise<void> {
  try {
    ...
    // This mutates grove_notice_viewed_at server-side ï¿?Grove.tsx:87 reads it
