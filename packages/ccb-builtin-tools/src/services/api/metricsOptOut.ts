...
import { hasProfileScope, isClaudeAISubscriber } from "@t3tools/ccb-engine/src/utils/auth.ts"
...
import { getAuthHeaders, withOAuth401Retry } from "@t3tools/ccb-engine/src/utils/http.ts"
import { logError } from "@t3tools/ccb-engine/src/utils/log.ts"
import { memoizeWithTTLAsync } from "@t3tools/ccb-engine/src/utils/memoize.ts"
import { isEssentialTrafficOnly } from "@t3tools/ccb-engine/src/utils/privacyLevel.ts"
...
// In-memory TTL â€?dedupes calls within a single process
const CACHE_TTL_MS = 60 * 60 * 1000

// Disk TTL â€?org settings rarely change. When disk cache is fresher than this,
 ...
 * Errors are not persisted â€?a transient failure should not overwrite a
...
async function refreshMetricsStatus(): Promise<MetricsStatus> {
  ...
  // Skip write when unchanged AND timestamp still fresh â€?avoids config churn
 ...
 *
 * Two-tier cache:
 * - Disk (24h TTL): survives process restarts. Fresh disk cache â†?zero network.
 * - In-memory (1h TTL): dedupes the background refresh within a process.
 *
 * The caller (bigqueryExporter) tolerates stale reads â€?a missed export or
 * an extra one during the 24h window is acceptable.
 */
export async function checkMetricsEnabled(): Promise<MetricsStatus> {
  // Service key OAuth sessions lack user:profile scope â†?would 403.
  ...
  // answers â€?only real API responses go to disk. Otherwise a service-key
  ...
  if (cached) {
    if (Date.now() - cached.timestamp > DISK_CACHE_TTL_MS) {
      ...
      // locked and fallback writes fail â€?catch here so fire-and-forget
