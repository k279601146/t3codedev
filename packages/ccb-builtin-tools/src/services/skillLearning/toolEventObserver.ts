...
export function pruneEmittedTurns(): void {
  // Prune over-sized Sets first. FIFO by insertion order â€?NOT by turn
...
// Cached import promise â€?resolved once so the hot path pays no repeated
...
function _getDeps() {
  if (!_depImportCache) {
    _depImportCache = Promise.all([
      import('./projectContext.ts'),
      import('./featureCheck.ts'),
      import('./runtimeObserver.ts'),
    ]).then(([pc, fc, ro]) => ({
...
export async function runToolCallWithSkillLearningHooks<T>(
  ...
  try {
    ...
    // Fire-and-forget: do NOT await â€?tool invoke must not be blocked.
