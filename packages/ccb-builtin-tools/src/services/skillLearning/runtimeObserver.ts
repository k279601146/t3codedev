...
import { registerPostSamplingHook } from "@t3tools/ccb-engine/src/utils/hooks/postSamplingHooks.ts"
import { getSkillLearningConfig } from "./config.ts"
import { isSkillLearningEnabled } from "./featureCheck.ts"
import {
  appendObservation,
...
import { resolveProjectContext } from "./projectContext.ts"
import './sessionObserver.ts'
import { createInstinct } from "./instinctParser.ts"
import {
  analyzeWithActiveBackend,
  resolveDefaultObserverBackend,
} from "./observerBackend.ts"
import {
...
import type { StoredSkillObservation } from "./observationStore.ts"
...
} from "./skillLifecycle.ts"
import {
  generateAgentCandidates,
  generateCommandCandidates,
  clusterInstincts,
} from "./evolution.ts"
import { generateOrMergeSkillDraft } from "./skillGenerator.ts"
import { shouldGenerateSkillFromInstincts } from "./learningPolicy.ts"
import { writeLearnedCommand } from "./commandGenerator.ts"
import { writeLearnedAgent } from "./agentGenerator.ts"
import { readObservations } from "./observationStore.ts"
import { checkPromotion } from "./promotion.ts"
...
let runtimeTurn = 0
// Timestamp watermark for consumed tool-hook observations â€?enables replay of
...
// Key: `${sessionId}:${messageId}` â€?prevents reprocessing the same message
...
export function initSkillLearning(): void {
  ...
  // registered first (heuristic) â€?which means the env switch would silently
  // be a no-op in production. Swallow registry errors so a typo in the env
  // variable can never crash startup.
  try {
    resolveDefaultObserverBackend()
  } catch {
    // No backend registered yet, or env points at unknown name â€?leave the
...
export async function runSkillLearningPostSampling(
  ...
  // thread â€?`startsWith` covers 'repl_main_thread:outputStyle:<name>'), sub-
  ...
  // Always reconstruct from the REPL message stream â€?it is the only source
  ...
  // consumption watermark â€?deterministic records with precise outcomes.
  ...
  // H5: throttle LLM calls â€?minimum observation count, per-session cap, and
  ...
  if (shouldCallLLM) {
    ...
    const { heuristicObserverBackend } = await import('./sessionObserver.ts')
...
function observationsFromMessages(
  ...
  return messages.flatMap((message): StoredSkillObservation[] => {
    // H6: watermark dedup â€?skip messages already processed in this session.
