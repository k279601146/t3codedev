...
import type { SearchResult } from "../skillSearch/localSearch.ts"
import { createInstinct, type StoredInstinct } from "./instinctParser.ts"
import {
  getProjectStorageDir,
  resolveProjectContext,
} from "./projectContext.ts"
import { generateSkillDraft, writeLearnedSkill } from "./skillGenerator.ts"
import type {
  InstinctDomain,
  SkillGapStatus,
...
function buildGapAction(prompt: string): string {
  if (
    /feature\s*\(|feature flag|flag_name|stub|no-op|noop|最小实�?i.test(prompt)
  ) {
...
function buildNameFragment(prompt: string): string {
  const mapped = prompt
    ...
    .replaceAll('最小实�?, ' minimal implementation ')
    .toLowerCase()
  const stop = new Set([
    'the',
    'and',
...
async function clearRuntimeSkillCaches(): Promise<void> {
  try {
    const { clearCommandsCache } = await import('../../commands.ts')
    clearCommandsCache()
  } catch {
