...
import type { InstinctCandidate } from "./instinctParser.ts"
 ...
 * - On ANY failure (abort, parse error, empty output) returns `[]` â€?
...

{
  ...
  "confidence": number,     // 0..1 â€?how strongly these observations support the pattern
...
async function analyseWithHaiku(
  ...
  try {
    ...
    // Haiku failure (timeout / rate limit / bad response) â€?increment failure
  ...
  if (parsed.length === 0) {
    // Empty / malformed LLM output â€?count as a failure so the circuit
...
function evidenceField(value: unknown): string[] {
  ...
  for (const entry of value) {
    ...
    if (!trimmed) continue
    entries.push(trimmed.length > 200 ? `${trimmed.slice(0, 200)}â€¦` : trimmed)
