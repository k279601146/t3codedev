...
import type { InstinctDomain, SkillObservationOutcome } from "./types.ts"
import {
...
import { llmObserverBackend } from "./llmObserverBackend.ts"
...
function parseCorrection(
  ...
  const noUsePattern =
    /(?:不要|别|不应(?:�??|不要�?\s*(?<avoid>[^�?�?;；]+)[�?\s]*(?:用|使用|改用|应该用|要用)\s*(?<prefer>[^�?�?;；]+)/i
