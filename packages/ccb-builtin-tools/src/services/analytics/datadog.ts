...
import { getCanonicalName } from "@t3tools/ccb-engine/src/utils/model/model.ts"
import { getAPIProvider } from "@t3tools/ccb-engine/src/utils/model/providers.ts"
import { MODEL_COSTS } from "@t3tools/ccb-engine/src/utils/modelCost.ts"
...
export const initializeDatadog = memoize(async (): Promise<boolean> => {
  ...
  // No custom endpoint configured ï¿?skip Datadog entirely
...
export async function trackDatadogEvent(
  ...
  try {
    ...
    // log search API ï¿?the `message` field (where eventName also lives)
