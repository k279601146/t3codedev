 ...
 * gitleaks (https://github.com/gitleaks/gitleaks, MIT license) â€?only
...
import { capitalize } from "@t3tools/ccb-engine/src/utils/stringUtils.ts"
...
// â”€â”€â”€ Curated rules â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
const SECRET_RULES: SecretRule[] = [
  ...
  },
  {
    ...
    // gitleaks: hf_(?i:[a-z]{34}) â†?JS: hf_[a-zA-Z]{34}
  ...
  },
  {
    ...
    // gitleaks: SG\.(?i)[a-z0-9=_\-\.]{66} â†?JS: case-insensitive via flag
  ...
  },
  {
    ...
    // â†?JS: case-insensitive hex+alnum prefix, literal "atlasv1", case-insensitive suffix
  ...
  },
  {
    ...
    // gitleaks: PMAK-(?i)[a-f0-9]{24}\-[a-f0-9]{34} â†?JS: use [a-fA-F0-9]
    source:
...
// Lazily compiled pattern cache â€?compile once on first scan.
 ...
 * e.g., "github-pat" â†?"GitHub PAT", "aws-access-token" â†?"AWS Access Token"
 */
 ...
 * actual matched text is intentionally NOT returned â€?we never log or
...
export function redactSecrets(content: string): string {
  ...
  for (const re of redactRules) {
    // Replace only the captured group, not the full match â€?patterns include
