...
import { getGithubRepo } from "@t3tools/ccb-engine/src/utils/git.ts"
import {
  getAPIProvider,
...
import { scanForSecrets } from "./secretScanner.ts"
import {
...
export async function pullTeamMemory(
  ...
  if (filesWritten > 0) {
    const { clearMemoryFileCaches } = await import('../../utils/claudemd.ts')
