 ...
 * this is an API-safe split point â€?the API contract requires every
 ...
 *
 * Extracted to its own file to break the compact.ts â†?compactMessages.ts
 * cycle (CC-1180) â€?the cycle shifted module-init order enough to surface
...
export function groupMessagesByApiRound(messages: Message[]): Message[][] {
  ...
  // (yield order, not concat order â€?see query.ts:613). The id check
  ...
  // after resume-from-partial-batch or max_tokens truncation) â€?and in that
