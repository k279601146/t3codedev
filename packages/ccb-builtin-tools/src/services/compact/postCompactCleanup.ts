...
import { clearSystemPromptSections } from "@t3tools/ccb-engine/src/constants/systemPromptSections.ts"
import { getUserContext } from "../../context.ts"
import { clearSpeculativeChecks } from '@claude-code-best/builtin-tools/tools/BashTool/bashPermissions.ts'
import { clearClassifierApprovals } from "@t3tools/ccb-engine/src/utils/classifierApprovals.ts"
...
import { clearSessionMessagesCache } from "@t3tools/ccb-engine/src/utils/sessionStorage.ts"
import { clearBetaTracingState } from "@t3tools/ccb-engine/src/utils/telemetry/betaSessionTracing.ts"
 ...
 * pass querySource ï¿?undefined is only safe for callers that are
...
export function runPostCompactCleanup(querySource?: QuerySource): void {
  ...
  if (isMainThreadCompact) {
    // getUserContext is a memoized outer layer wrapping getClaudeMds() ï¿?
    ...
    // auto-compact and reactive-compact did not ï¿?this centralizes the
  ...
  if (feature('COMMIT_ATTRIBUTION')) {
    ...
    // own state transitions without an extra microtask round-trip ï¿?the sweep
    ...
    void import('../../utils/attributionHooks.ts')
