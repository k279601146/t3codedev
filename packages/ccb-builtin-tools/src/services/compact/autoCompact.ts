...
import { getSdkBetas } from "@t3tools/ccb-engine/src/bootstrap/state.ts"
...
import { getContextWindowForModel } from "@t3tools/ccb-engine/src/utils/context.ts"
...
import type { CacheSafeParams } from "@t3tools/ccb-engine/src/utils/forkedAgent.ts"
...
export async function shouldAutoCompact(
  ...
  // marble_origami is the ctx-agent ï¿?if ITS context blows up and
  ...
  // trySessionMemoryCompaction in the query loop ï¿?the /compact call site
  ...
  // management system when it's on ï¿?the 90% commit / 95% blocking-spawn
...
export async function autoCompactIfNeeded(
  ...
  try {
    ...
    if (nextFailures >= MAX_CONSECUTIVE_AUTOCOMPACT_FAILURES) {
      logForDebugging(
        `autocompact: circuit breaker tripped after ${nextFailures} consecutive failures ï¿?skipping future attempts this session`,
        { level: 'warn' },
      )
    }
