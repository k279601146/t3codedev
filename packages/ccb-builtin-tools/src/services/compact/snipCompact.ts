 ...
 *
 * Called from:
 * - `query.ts` â€?strips snipped messages from the model-facing array
 *   before sending to the API.
 * - `QueryEngine.ts` `snipReplay` â€?trims `mutableMessages` so the
 ...
 * @param options   `force` â€?if true, always execute when a boundary is
...
export function snipCompactIfNeeded(
  ...
  // No removedUuids metadata â€?fallback: keep boundary + everything after
