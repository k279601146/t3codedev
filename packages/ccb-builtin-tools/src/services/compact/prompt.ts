...
import type { PartialCompactDirection } from "@t3tools/ccb-engine/src/types/message.ts"
...
// no text output ï¿?falls through to the streaming fallback (2.79% on 4.6 vs
...
- Tool calls will be REJECTED and will waste your only turn ï¿?you will fail the task.
...
const PARTIAL_COMPACT_PROMPT = `Your task is to create a detailed summary of the RECENT portion of the conversation ï¿?the messages that follow earlier retained context. The earlier messages are being kept intact and do NOT need to be summarized. Focus your summary on what was discussed, learned, and accomplished in the recent messages only.
...
const NO_TOOLS_TRAILER =
  '\n\nREMINDER: Do NOT call any tools. Respond with plain text only ï¿?' +
...
export function formatCompactSummary(summary: string): string {
  ...
  // Strip analysis section ï¿?it's a drafting scratchpad that improves summary
...
Continue the conversation from where it left off without asking the user any further questions. Resume directly ï¿?do not acknowledge the summary, do not recap what was happening, do not preface with "I'll continue" or similar. Pick up the last task as if the break never happened.`
...
You are running in autonomous/proactive mode. This is NOT a first wake-up ï¿?you were already working autonomously before compaction. Continue your work loop: pick up where you left off based on the summary above. Do not greet the user or ask what to work on.`
    }
