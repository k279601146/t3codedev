 ...
 *
 * Key conversions from Anthropic â†?OpenAI format:
 *   - tool_use blocks  â†?tool_calls[] at message level
 *   - tool_result blocks â†?separate { role: 'tool' } messages
 */

import type { AssistantMessage, UserMessage } from "@t3tools/ccb-engine/src/types/message.ts"
...
/** Convert internal or OpenAI-style messages â†?Langfuse input format */
export function convertMessagesToLangfuse(
  ...
  for (const msg of messages) {
    ...
    if (role === 'assistant') {
      // Extract tool_use â†?tool_calls at message level
      ...
      // User messages: extract tool_result â†?separate tool messages
...
/** Convert AssistantMessage[] (newMessages) â†?Langfuse output format (last assistant turn) */
