 ...
 * Bedrock) and we fall back to the rough estimate â€?an underestimate can
...
function roughTokenCountEstimationForBlock(
  ...
  if (block.type === 'image' || block.type === 'document') {
    ...
    // jsonStringify catch-all â€?a 1MB PDF is ~1.33M base64 chars â†?
  ...
  if (block.type === 'tool_use') {
    // input is the JSON the model generated â€?arbitrarily large (bash
  ...
  // server_tool_use, web_search_tool_result, mcp_tool_use, etc. â€?
