 ...
 *
 * Environment:
 *   CLAUDE_CODE_BALANCE_URL        â€?GET endpoint returning JSON (required)
 *   CLAUDE_CODE_BALANCE_KEY        â€?optional Bearer token (falls back to OPENAI_API_KEY / ANTHROPIC_API_KEY)
 *   CLAUDE_CODE_BALANCE_JSON_PATH  â€?dot path into the JSON for the remaining number (default: "balance")
 *                                    array indices allowed, e.g. "data.0.credit"
 *   CLAUDE_CODE_BALANCE_CURRENCY   â€?display currency label (default: "USD")
 *
...
export const genericBalanceProvider: BalanceProvider = {
  ...
  async fetchBalance(signal?: AbortSignal): Promise<ProviderBalance | null> {
    ...
    // Fallback chain: BALANCE_KEY â†?OPENAI_API_KEY â†?ANTHROPIC_API_KEY.
