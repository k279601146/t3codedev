import type { Attachment } from "@t3tools/ccb-engine/src/utils/attachments.ts"
...
import type { DiscoverySignal } from "./signals.ts"
import { isSkillSearchEnabled } from "./featureCheck.ts"
import {
  getSkillIndex,
  searchSkills,
  type SearchResult,
} from "./localSearch.ts"
import { normalizeQueryIntent } from "./intentNormalize.ts"
import { logForDebugging } from "@t3tools/ccb-engine/src/utils/debug.ts"
import { readFile } from 'node:fs/promises'
import { join } from 'node:path'
import { parseFrontmatter } from "@t3tools/ccb-engine/src/utils/frontmatterParser.ts"
 ...
 *
 * FIFO eviction by insertion order â€?once the cap is hit, the oldest
  ...
  for (let i = messages.length - 1; i >= 0; i--) {
    ...
    if (Array.isArray(content)) {
      ...
      for (const block of content) {
        ...
        // Skip tool_result and other non-text blocks â€?they carry no discovery
...
async function markAutoLoadedSkill(
  ...
  try {
    const { addInvokedSkill } = await import('../../bootstrap/state.ts')
...
async function maybeRecordSkillGap(
  ...
  try {
    const [{ isSkillLearningEnabled }, { recordSkillGap }] = await Promise.all([
      import('../skillLearning/featureCheck.ts'),
      import('../skillLearning/skillGapStore.ts'),
    ])
...
export async function getTurnZeroSkillDiscovery(
  ...
  try {
    ...
    // fallback to original). Turn-zero is the one blocking entry â€?acceptable
