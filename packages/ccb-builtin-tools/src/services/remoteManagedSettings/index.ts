...
import { getOauthConfig, OAUTH_BETA_HEADER } from "@t3tools/ccb-engine/src/constants/oauth.ts"
import {
...
import { classifyAxiosError, getErrnoCode } from "@t3tools/ccb-engine/src/utils/errors.ts"
import { settingsChangeDetector } from "@t3tools/ccb-engine/src/utils/settings/changeDetector.ts"
import {
  type SettingsJson,
  SettingsSchema,
...
} from './securityCheck.jsx.tsx'
import { isRemoteManagedSettingsEligible, resetSyncCache } from "./syncCache.ts"
import {
...
export async function loadRemoteManagedSettings(): Promise<void> {
  ...
  // the session cache internally â€?no need to call setSessionCache here.
  ...
  try {
    ...
    // listeners â€?env vars, telemetry, and permissions update on next read.
    if (settings !== null) {
