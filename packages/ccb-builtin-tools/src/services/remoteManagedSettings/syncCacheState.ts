 ...
 *
 * Split from syncCache.ts to break the settings.ts â†?syncCache.ts â†?auth.ts â†?
 ...
 * settings/settingsCache â€?also a leaf, only type-imports validation). settings.ts
 ...
 *
 * Eligibility is a tri-state here: undefined (not yet determined â€?return
 * null), false (ineligible â€?return null), true (proceed). managedEnv.ts
 * calls isRemoteManagedSettingsEligible() just before the policySettings
 * read â€?after userSettings/flagSettings env vars are applied, so the check
...
import { readFileSync } from "@t3tools/ccb-engine/src/utils/fileRead.ts"
import { stripBOM } from "@t3tools/ccb-engine/src/utils/jsonRead.ts"
import { resetSettingsCache } from "@t3tools/ccb-engine/src/utils/settings/settingsCache.ts"
import type { SettingsJson } from "@t3tools/ccb-engine/src/utils/settings/types.ts"
...
// sync IO â€?settings pipeline is sync. fileRead and jsonRead are leaves;
...
export function getRemoteManagedSettingsSyncFromCache(): SettingsJson | null {
  ...
  if (cachedSettings) {
    ...
    // loadSettingsFromDisk returns) â€?no-op. The async-fetch arm (index.ts
    // setSessionCache + notifyChange) already handles its own reset.
    //
    // gh-23085: isBridgeEnabled() at main.tsx Commander-definition time
    // (before preAction â†?init() â†?isRemoteManagedSettingsEligible()) reached
