 ...
 * This file keeps isRemoteManagedSettingsEligible â€?the one function that
 * needs auth.ts â€?plus resetSyncCache wrapped to clear the local eligibility
 * mirror alongside the leaf's state.
 */

import { CLAUDE_AI_INFERENCE_SCOPE } from "@t3tools/ccb-engine/src/constants/oauth.ts"
import {
 ...
 *   CLAUDE_CODE_OAUTH_TOKEN / FD, or keychain tokens missing metadata): Eligible â€?
...
export function isRemoteManagedSettingsEligible(): boolean {
  ...
  // exist yet. MDM/file-based managed settings still apply via settings.ts â€?
  ...
  // subscriptionType metadata â€?getClaudeAIOAuthTokens() constructs them with
