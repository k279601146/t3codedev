 ...
 * servers are. Per-server gating was overengineering â€?a plugin that
...
import { BUILTIN_MARKETPLACE_NAME } from "../../plugins/builtinPlugins.ts"
...
/**
 * Overall channels on/off. Always enabled â€?GrowthBook gate bypassed.
 */
...
/**
 * Pure allowlist check keyed off the connection's pluginSource â€?for UI
 ...
 * but standalone (no session/marketplace coupling â€?those are tautologies
 * when the entry is derived from pluginSource).
 *
 * Returns false for undefined pluginSource (non-plugin server â€?can never
 * match the {marketplace, plugin}-keyed ledger) and for @-less sources
 * (builtin/inline â€?same reason).
 */
