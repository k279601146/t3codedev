...
import { getPlatform } from "@t3tools/ccb-engine/src/utils/platform.ts"
import type { PluginError } from "@t3tools/ccb-engine/src/types/plugin.ts"
import { getPluginErrorMessage } from "@t3tools/ccb-engine/src/types/plugin.ts"
...
import {
  ...
  saveGlobalConfig,
} from "@t3tools/ccb-engine/src/utils/config.ts"
...
import { getFsImplementation } from "@t3tools/ccb-engine/src/utils/fsOperations.ts"
import { safeParseJSON } from "@t3tools/ccb-engine/src/utils/json.ts"
import { logError } from "@t3tools/ccb-engine/src/utils/log.ts"
import { getPluginMcpServers } from "@t3tools/ccb-engine/src/utils/plugins/mcpPluginIntegration.ts"
import { loadAllPluginsCacheOnly } from "@t3tools/ccb-engine/src/utils/plugins/pluginLoader.ts"
import { isSettingSourceEnabled } from "@t3tools/ccb-engine/src/utils/settings/constants.ts"
import { getManagedFilePath } from "@t3tools/ccb-engine/src/utils/settings/managedPath.ts"
import { isRestrictedToPluginOnly } from "@t3tools/ccb-engine/src/utils/settings/pluginOnlyPolicy.ts"
import {
  getInitialSettings,
...
import {
  ...
  isMcpServerUrlEntry,
  type SettingsJson,
} from "@t3tools/ccb-engine/src/utils/settings/types.ts"
import type { ValidationError } from "@t3tools/ccb-engine/src/utils/settings/validation.ts"
 ...
 * with manual servers in the merge ï¿?this content-based check catches the case
 ...
 * manual servers in the merge ï¿?this content-based check catches the case where
 ...
 *
 * Only enabled manual servers count as dedup targets ï¿?a disabled manual server
 ...
 * Denylists always merge from all sources ï¿?users can always deny servers
 ...
 *
 * SDK-type servers are exempt ï¿?they are SDK-managed transport placeholders,
 ...
 * ï¿?the policy check only reads, never requires any field to be present.
...
export function getMcpConfigByName(name: string): ScopedMcpServerConfig | null {
  ...
  // by name. User/project/local servers are blocked ï¿?same as getClaudeCodeMcpConfigs().
 ...
 * returned set ï¿?they're fetched separately and merged by callers).
...
export async function getClaudeCodeMcpConfigs(
  ...
  // Load other scopes ï¿?unless the managed policy locks MCP to plugin-only.
  ...
  // manual keys in the merge below ï¿?this content-based filter catches the case
  // where both would launch the same underlying process/connection.
  // Only servers that will actually connect are valid dedup targets ï¿?a
  ...
  // first-plugin-wins race against an enabled duplicate ï¿?same invariant as
  ...
  // so these don't go to the error log ï¿?they're informational, not errors.
...
export async function getAllMcpConfigs(): Promise<{
  ...
  // with loadAllPluginsCacheOnly() inside. Memoized ï¿?the awaited call below is a cache hit.
  ...
  // won't catch this ï¿?need content-based dedup by URL signature.
