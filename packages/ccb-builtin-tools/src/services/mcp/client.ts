...
import { getOriginalCwd, getSessionId } from "@t3tools/ccb-engine/src/bootstrap/state.ts"
import type { Command } from "../../commands.ts"
import { getOauthConfig } from "@t3tools/ccb-engine/src/constants/oauth.ts"
import { PRODUCT_URL } from "@t3tools/ccb-engine/src/constants/product.ts"
import type { AppState } from "@t3tools/ccb-engine/src/state/AppState.tsx"
import {
  type Tool,
...
import {
  type MCPProgress,
  MCPTool,
} from '@claude-code-best/builtin-tools/tools/MCPTool/MCPTool.ts'
import { createMcpAuthTool } from '@claude-code-best/builtin-tools/tools/McpAuthTool/McpAuthTool.ts'
import { ReadMcpResourceTool } from '@claude-code-best/builtin-tools/tools/ReadMcpResourceTool/ReadMcpResourceTool.ts'
import { createAbortController } from "@t3tools/ccb-engine/src/utils/abortController.ts"
import { count } from "@t3tools/ccb-engine/src/utils/array.ts"
import {
  checkAndRefreshOAuthTokenIfNeeded,
  getClaudeAIOAuthTokens,
  handleOAuth401Error,
} from "@t3tools/ccb-engine/src/utils/auth.ts"
import { registerCleanup } from "@t3tools/ccb-engine/src/utils/cleanupRegistry.ts"
import { detectCodeIndexingFromMcpServerName } from "@t3tools/ccb-engine/src/utils/codeIndexing.ts"
import { logForDebugging } from "@t3tools/ccb-engine/src/utils/debug.ts"
import { isEnvDefinedFalsy, isEnvTruthy } from "@t3tools/ccb-engine/src/utils/envUtils.ts"
import {
  errorMessage,
  TelemetrySafeError_I_VERIFIED_THIS_IS_NOT_CODE_OR_FILEPATHS,
} from "@t3tools/ccb-engine/src/utils/errors.ts"
import { getMCPUserAgent } from "@t3tools/ccb-engine/src/utils/http.ts"
import { maybeNotifyIDEConnected } from "@t3tools/ccb-engine/src/utils/ide.ts"
import { maybeResizeAndDownsampleImageBuffer } from "@t3tools/ccb-engine/src/utils/imageResizer.ts"
import { logMCPDebug, logMCPError } from "@t3tools/ccb-engine/src/utils/log.ts"
import {
  getBinaryBlobSavedMessage,
  getFormatDescription,
  getLargeOutputInstructions,
  persistBinaryContent,
} from "@t3tools/ccb-engine/src/utils/mcpOutputStorage.ts"
import {
  getContentSizeEstimate,
  type MCPToolResult,
  mcpContentNeedsTruncation,
  truncateMcpContentIfNeeded,
} from "@t3tools/ccb-engine/src/utils/mcpValidation.ts"
import { WebSocketTransport } from "@t3tools/ccb-engine/src/utils/mcpWebSocketTransport.ts"
import { memoizeWithLRU } from "@t3tools/ccb-engine/src/utils/memoize.ts"
import { getWebSocketTLSOptions } from "@t3tools/ccb-engine/src/utils/mtls.ts"
import {
  getProxyFetchOptions,
  getWebSocketProxyAgent,
  getWebSocketProxyUrl,
} from "@t3tools/ccb-engine/src/utils/proxy.ts"
import { getSessionIngressAuthToken } from "@t3tools/ccb-engine/src/utils/sessionIngressAuth.ts"
import { subprocessEnv } from "@t3tools/ccb-engine/src/utils/subprocessEnv.ts"
import {
  isPersistError,
  persistToolResult,
} from "@t3tools/ccb-engine/src/utils/toolResultStorage.ts"
import {
...
// Package imports ï¿?delegate to mcp-client package utilities where applicable
import {
...
} from '@claude-code-best/mcp-client/index.ts'
import { recursivelySanitizeUnicode } from '@claude-code-best/mcp-client/index.ts'
...
import type { AssistantMessage } from "@t3tools/ccb-engine/src/types/message.ts"
/* eslint-enable @typescript-eslint/no-require-imports */
import { classifyMcpToolForCollapse } from '@claude-code-best/builtin-tools/tools/MCPTool/classifyForCollapse.ts'
import { clearKeychainCache } from "@t3tools/ccb-engine/src/utils/secureStorage/macOsKeychainHelpers.ts"
import { sleep } from "@t3tools/ccb-engine/src/utils/sleep.ts"
import {
  ClaudeAuthProvider,
 ...
 * so SDK consumers can still receive it ï¿?per the MCP spec, `_meta` is on the
...
import { isClaudeInChromeMCPServer } from "@t3tools/ccb-engine/src/utils/claudeInChrome/common.ts"
...
const claudeInChromeToolRendering =
  (): typeof import('../../utils/claudeInChrome/toolRendering.tsx') =>
    require('../../utils/claudeInChrome/toolRendering.tsx')
// Lazy: wrapper.tsx ï¿?hostAdapter.ts ï¿?executor.ts pulls both native modules
...
const computerUseWrapper = feature('CHICAGO_MCP')
  ? (): typeof import('../../utils/computerUse/wrapper.tsx') =>
      require('../../utils/computerUse/wrapper.tsx')
  : undefined
...
/* eslint-enable @typescript-eslint/no-require-imports */
import { jsonParse, jsonStringify } from "@t3tools/ccb-engine/src/utils/slowOperations.ts"
...
export function createClaudeAiProxyFetch(innerFetch: FetchLike): FetchLike {
  return async (url, init) => {
    const doRequest = async () => {
      ...
      // finds same-as-keychain ï¿?returns false ï¿?skips retry. Same pattern as
    ...
    // that ï¿?otherwise we double round-trip time for every connector whose
    ...
    if (!tokenChanged) {
      // ELOCKED contention: another connector may have won the lockfile and refreshed ï¿?check if token changed underneath us
...
export function wrapFetchWithTimeout(baseFetch: FetchLike): FetchLike {
  return async (url: string | URL, init?: RequestInit) => {
    ...
    // and existing Headers instances ï¿?so whatever shape the SDK handed us, the
    ...
    // signal is GC'd, which in Bun is lazy ï¿?~2.4KB of native memory per request
    ...
    try {
      ...
      if (serverRef.type === 'sse') {
        ...
        const transportOptions: SSEClientTransportOptions = {
          ...
          // SDK's handler calls auth() ï¿?tokens().
        ...
        // authProvider will set Authorization ï¿?don't override with the
        ...
        // Run the Computer Use MCP server in-process ï¿?same rationale as
      ...
      if (
        ...
        instructions =
          rawInstructions.slice(0, MAX_MCP_DESCRIPTION_LENGTH) + 'ï¿?[truncated]'
        logMCPDebug(
          name,
      ...
      // client.close() ï¿?transport.close() ï¿?transport.onclose ï¿?SDK's _onclose():
      ...
      // Calling client.onclose?.() directly would only clear the cache ï¿?pending
      ...
      const isTerminalConnectionError = (msg: string): boolean => {
        return (
          ...
          // SDK SSE reconnection intermediate errors ï¿?may be wrapped around the
      ...
      client.onerror = (error: Error) => {
        ...
        if (
          ...
          // own SSE reconnect attempts (default maxRetries: 2) ï¿?but it never
          ...
          return {
            ...
            async prompt() {
              ...
              return desc.length > MAX_MCP_DESCRIPTION_LENGTH
                ? desc.slice(0, MAX_MCP_DESCRIPTION_LENGTH) + 'ï¿?[truncated]'
                : desc
            },
            isConcurrencySafe() {
            ...
            async call(
              ...
              for (let attempt = 0; ; attempt++) {
                try {
                  ...
                  // Session expired ï¿?the connection cache has been
...
export async function getMcpToolsCommandsAndResources(
  ...
  // Partition into disabled and active entries ï¿?disabled servers should
...
// mcpConfigs object ref leaked ï¿?main.tsx creates fresh config objects each call.
  ...
  for (let attempt = 0; ; attempt++) {
    try {
      ...
      for (const elicitation of elicitations) {
        ...
        // Run elicitation hooks ï¿?they can resolve URL elicitations programmatically
        ...
        if (hookResponse) {
          ...
          // Hook accepted ï¿?skip the UI and proceed to retry
          continue
        }
        ...
        // Run ElicitationResult hooks ï¿?they can modify or block the response
...
async function callMCPTool({
  ...
  try {
    ...
    if (e instanceof Error) {
      ...
      // Check for session expiry ï¿?two error shapes can surface here:
      // 1. Direct 404 + JSON-RPC -32001 from the server (StreamableHTTPError)
      // 2. -32000 "Connection closed" (McpError) ï¿?the SDK closes the transport
