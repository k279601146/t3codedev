...
import type { AnyObject, Tool, ToolUseContext } from "@t3tools/ccb-engine/src/Tool.ts"
...
import { checkRuleBasedPermissions } from "@t3tools/ccb-engine/src/utils/permissions/permissions.ts"
import { formatError } from "@t3tools/ccb-engine/src/utils/toolErrors.ts"
  ...
  try {
    ...
    for await (const result of executePostToolUseFailureHooks(
      ...
      try {
        ...
        // Skip hook_blocking_error in result.message â€?blockingError path
 ...
 * deny/ask rules â€?checkRuleBasedPermissions still applies (inc-4788 analog).
...
export async function resolveHookPermissionDecision(
  ...
  if (hookPermissionResult?.behavior === 'allow') {
    ...
    // Hook provided updatedInput for an interactive tool â€?the hook IS the
    ...
    // ask rule â€?dialog required despite hook approval
    logForDebugging(
  ...
  // No hook decision or 'ask' â€?normal permission flow, possibly with
