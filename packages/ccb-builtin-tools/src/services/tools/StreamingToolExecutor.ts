...
import { findToolByName, type Tools, type ToolUseContext } from "@t3tools/ccb-engine/src/Tool.ts"
import { BASH_TOOL_NAME } from '@claude-code-best/builtin-tools/tools/BashTool/toolName.ts'
import type { AssistantMessage, Message } from "@t3tools/ccb-engine/src/types/message.ts"
...
import { createToolBatchSpan, endToolBatchSpan } from "../langfuse/index.ts"
...
export class StreamingToolExecutor {
  ...
  addTool(block: ToolUseBlock, assistantMessage: AssistantMessage): void {
    // Create turn span on first tool â€?will be ended in getRemainingResults
  ...
  private async executeTool(tool: TrackedTool): Promise<void> {
    ...
    const collectResults = async () => {
      ...
      // (PermissionContext.ts cancelAndAbort) â€?that abort must bubble up to
      ...
      for await (const update of generator) {
        ...
        if (isErrorResult) {
          ...
          // dependency chains (e.g. mkdir fails â†?subsequent commands pointless).
          // Read/WebFetch/etc are independent â€?one failure shouldn't nuke the rest.
