...
import { isProcessRunning } from "@t3tools/ccb-engine/src/utils/genericProcessUtils.ts"
import { listCandidates } from "@t3tools/ccb-engine/src/utils/listSessionsImpl.ts"
...
/**
 * Acquire: write PID â†?mtime = now. Returns the pre-acquire mtime
 * (for rollback), or null if blocked / lost a race.
 *
 *   Success â†?do nothing. mtime stays at now.
 *   Failure â†?rollbackConsolidationLock(priorMtime) rewinds mtime.
 *   Crash   â†?mtime stuck, dead PID â†?next process reclaims.
 */
export async function tryAcquireConsolidationLock(): Promise<number | null> {
  ...
  if (mtimeMs !== undefined && Date.now() - mtimeMs < HOLDER_STALE_MS) {
    ...
    // Dead PID or unparseable body â€?reclaim.
  }
  ...
  // Two reclaimers both write â†?last wins the PID. Loser bails on re-read.
  let verify: string
  try {
...
/**
 * Rewind mtime to pre-acquire after a failed fork. Clears the PID body â€?
 * otherwise our still-running process would look like it's holding.
 * priorMtime 0 â†?unlink (restore no-file).
 */
export async function rollbackConsolidationLock(
  ...
  try {
    ...
    logForDebugging(
      `[autoDream] rollback failed: ${(e as Error).message} â€?next trigger delayed to minHours`,
    )
  }
}
 ...
 * Caller excludes the current session. Scans per-cwd transcripts â€?it's
...
/**
 * Stamp from manual /dream. Optimistic â€?fires at prompt-build time,
