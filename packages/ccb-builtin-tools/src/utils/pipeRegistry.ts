/**
 * Pipe Registry â€?central registry for multi-instance pipe coordination.
 *
...
import { isPipeAlive, getPipesDir } from "./pipeTransport.ts"
import type { TcpEndpoint } from "./pipeTransport.ts"
import type { LanAnnounce } from "./lanBeacon.ts"
...
// ---------------------------------------------------------------------------
// Machine ID â€?stable OS-level fingerprint
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// File lock â€?simple .lock file with timeout
// ---------------------------------------------------------------------------
...
async function acquireLock(): Promise<void> {
  ...
  while (Date.now() < deadline) {
    try {
      // O_CREAT | O_EXCL â€?fails if file exists
        ...
        try {
          ...
          if (lockPid && lockPid !== process.pid) {
            try {
              ...
              // Process dead â€?remove stale lock
              await unlink(lockPath).catch(() => {})
              continue
            }
          }
        } catch {
          // Can't read lock file â€?try to remove
          await unlink(lockPath).catch(() => {})
          continue
        }
  ...
  // Timeout â€?force remove and retry once
  ...
  try {
    ...
    if (registry.mainMachineId === machineId) {
      if (registry.main && (await isPipeAlive(registry.main.pipeName, 1000))) {
        // Main instance is alive â†?this is a same-machine sub
        const subIndex = registry.subs.length + 1
        return { role: 'sub', subIndex }
      }
      // Main instance is dead â†?recover main on same machine
...
export async function unregister(id: string): Promise<void> {
  ...
  try {
    ...
    if (registry.main?.id === id) {
      registry.main = null
      // Don't clear mainMachineId â€?same machine can recover
    }
 ...
 * Local entries take precedence â€?LAN peers are only added if not
