...
export type MemoryFileInfo = {
  ...
  // so callers can cache a `isPartialView` readFileState entry ï¿?presence in
...
export function stripHtmlComments(content: string): {
  ...
  // gfm:false is fine here ï¿?html-block detection is a CommonMark rule.
...
/**
 * Parses raw memory file content into a MemoryFileInfo. Pure function ï¿?no I/O.
 *
...
function parseMemoryFileContent(
  ...
  // Only rebuild via tokens when a comment actually needs stripping ï¿?
...
function handleMemoryFileReadError(error: unknown, filePath: string): void {
  ...
  // ENOENT = file doesn't exist, EISDIR = is a directory ï¿?both expected
...
/**
 * Used by processMemoryFile ï¿?getMemoryFiles so the event loop stays
...
// ignored ï¿?the caller may pass pre-strip tokens.
...
function resolveExcludePatterns(patterns: string[]): string[] {
  ...
  for (const normalized of expanded) {
    // Only resolve absolute patterns ï¿?glob-only patterns like "**/*.md" don't have
...
export const getMemoryFiles = memoize(
  async (forceIncludeExternal: boolean = false): Promise<MemoryFileInfo[]> => {
    ...
    // directories above the worktree but within the main repo ï¿?the worktree
    ...
    // AutoMem/TeamMem are intentionally excluded ï¿?they're a separate
    ...
    // for building context ï¿?firing the hook there would double-fire on startup.
    ...
    // when no hook is configured ï¿?otherwise a mid-session hook registration
