 ...
 * - Grapheme clusters (like ðŸ‘¨â€ðŸ‘©â€ðŸ‘§â€ðŸ‘? are treated as single units
...
export class Cursor {
  ...
  render(
    ...
    return allLines
      ...
      .map((text, i) => {
        ...
        // segmenter pass) â€?the intermediate stringIndex from that approach is
  ...
  deleteToLineStart(): { cursor: Cursor; killed: string } {
    ...
    // newline â€?symmetric with deleteToLineEnd's newline handling. This lets
  ...
  deleteTokenBefore(): Cursor | null {
    // Cursor at chip.start is the "selected" state â€?backspace deletes the
