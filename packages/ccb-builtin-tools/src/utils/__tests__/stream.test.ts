...
describe('Stream', () => {
  ...
  test('next() blocks until enqueue provides a value', async () => {
    ...
    // Not resolved yet â€?enqueue after a microtask
    stream.enqueue(99)
  ...
  test('done() with no pending reader â€?subsequent next returns done:true', async () => {
    const stream = new Stream<number>()
    stream[Symbol.asyncIterator]()
    stream.done()
  ...
  test('error() after done â€?hasError is set but next returns done:true (isDone checked first)', async () => {
    const stream = new Stream<number>()
    stream[Symbol.asyncIterator]()
    stream.done()
  ...
  test('enqueue after done â€?queue is checked before isDone, value is consumed', async () => {
    const stream = new Stream<number>()
    stream[Symbol.asyncIterator]()
    stream.done()
    stream.enqueue(1)
