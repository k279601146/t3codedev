...
describe('truncateStartToWidth', () => {
  ...
  test('truncates CJK from start preserving characters', () => {
    expect(truncateStartToWidth('你好世界', 6)).toBe('…世�?)
  })
})
...
describe('truncatePathMiddle', () => {
  test('returns original when path fits', () => {
    expect(truncatePathMiddle('../../index.ts', 50)).toBe('../../index.ts')
  })

  test('truncates middle of long path', () => {
    const path = '../../components/deeply/nested/folder/MyComponent.tsx'
  ...
  test('returns ellipsis for maxLength 0', () => {
    expect(truncatePathMiddle('../../index.ts', 0)).toBe('�?)
  })
  ...
  test('handles short maxLength < 5', () => {
    expect(truncatePathMiddle('../../components/foo.ts', 4)).toBe('src�?)
  })
