...
} from '../autonomyFlows.ts'
import { AUTONOMY_DIR } from '../autonomyAuthority.ts'
...
describe('listAutonomyFlows', () => {
  ...
  test('reads and normalizes flow records from disk', async () => {
    ...
    expect(flows[0]?.boundary).toEqual(['../../utils/**', 'docs/*.md'])
...
describe('startManagedAutonomyFlow', () => {
  ...
  test('normalizes and preserves boundary across completed flow restarts', async () => {
    ...
    expect(first!.flow.boundary).toEqual(['../../utils/**', 'docs/*.md'])
    ...
    expect(restarted!.flow.boundary).toEqual(['../../utils/**', 'docs/*.md'])
  })
...
describe('full lifecycle: start â†?queue â†?running â†?completed â†?succeeded', () => {
  test('advances through all steps to succeeded', async () => {
    ...
    // Complete step 0 â€?should auto-advance to step 1
    ...
    // Complete step 1 â€?no more steps, should succeed
...
describe('lifecycle: waitFor â†?resume', () => {
...
describe('requestManagedAutonomyFlowCancel', () => {
  ...
  test('deferred cancel when step is running, completes on next step completion', async () => {
    ...
    // Request cancel while running â€?should be deferred
    const cancelResult = await requestManagedAutonomyFlowCancel({
      flowId,
      rootDir: tempDir,
      nowMs: 4000,
    ...
    // Complete the step â€?cancel should now take effect
...
describe('markManagedAutonomyFlowStepFailed with cancelRequestedAt', () => {
  test('marks flow as cancelled (not failed) when cancel was requested', async () => {
    ...
    // Step fails â€?should result in cancelled (because cancel was requested)
