 ...
 * __esm wrapper evaluates the ENTIRE module when any symbol is accessed ï¿?
 * so a heavy transitive import here defeats the prefetch. The execa ï¿?
 * human-signals ï¿?cross-spawn chain alone is ~58ms of synchronous init.
 *
...
// DO NOT change this value ï¿?it's part of the keychain lookup key and would
...
// triggers repeat sync reads ï¿?observed as a 5.5s event-loop stall
...
// `let` bindings aren't writable across module boundaries ï¿?both this file
 ...
 * Only writes if the cache hasn't been touched yet ï¿?if sync read() or
...
export function primeKeychainCacheFromPrefetch(stdout: string | null): void {
  ...
  if (stdout) {
    try {
      ...
      // malformed prefetch result ï¿?let sync read() re-fetch
      return
    }
  }
