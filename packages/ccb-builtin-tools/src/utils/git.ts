 ...
 * unbounded growth ï¿?gitDiff calls this with dirname(file), so editing many
 ...
 * `.git` file ï¿?`gitdir:` ï¿?`commondir` chain to find the main repo's
 * working directory.
 *
...
const resolveCanonicalRoot = memoizeWithLRU(
  (gitRoot: string): string => {
    try {
      ...
      // Submodules have no commondir (readFileSync throws ENOENT) ï¿?fall through.
      ...
      //      ï¿?ensures the commondir file we read lives inside the resolved
      //        common dir, not inside the attacker's repo
      //   2. <worktreeGitDir>/gitdir points back to <gitRoot>/.git
      //      ï¿?ensures an attacker can't borrow a victim's existing worktree
      ...
      // /tmp ï¿?/private/tmp) aren't rejected. Realpath the directory then join
      // '.git' ï¿?realpathing the .git file itself would follow a symlinked .git
...
export function normalizeGitRemoteUrl(url: string): string | null {
  ...
  if (urlMatch && urlMatch[1] && urlMatch[2]) {
    ...
    if (isLocalHost(host) && path.startsWith('git/')) {
      ...
      // 3+ segments where first contains a dot ï¿?host/owner/repo (GHE format)
      if (segments.length >= 3 && segments[0]!.includes('.')) {
        return proxyPath.toLowerCase()
      }
      // 2 segments ï¿?owner/repo (legacy format, assume github.com)
...
export async function getGithubRepo(): Promise<string | null> {
  ...
  // Only return results for github.com ï¿?callers (e.g. issue submission)
...
export async function preserveGitStateForIssue(): Promise<PreservedGitState | null> {
  try {
    ...
    // All 5 commands below depend only on remoteBaseSha ï¿?run them in parallel.
    // ~5Ã—90ms serial ï¿?~90ms parallel on Bun native (used by /issue and /share).
    const [
      { stdout: patch },
      untrackedFiles,
...
export function isCurrentDirectoryBareGitRepo(): boolean {
  ...
  try {
    ...
    if (stats.isFile()) {
      // worktree/submodule ï¿?Git follows the gitdir reference
      return false
    }
    if (stats.isDirectory()) {
      ...
      try {
        ...
        if (fs.statSync(gitHeadPath).isFile()) {
          // normal repo ï¿?.git/HEAD valid, Git won't fall back to cwd
          return false
        }
        // .git/HEAD exists but is not a regular file ï¿?fall through
      } catch {
        // .git exists but no HEAD ï¿?fall through to bare-repo check
      }
    }
  } catch {
    // no .git ï¿?fall through to bare-repo indicator check
  }

  // No valid .git/HEAD found. Check if cwd has bare git repo indicators.
  // Be cautious ï¿?flag if ANY of these exist without a valid .git reference.
