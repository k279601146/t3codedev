...
export class ExitPlanModeScanner {
  ...
  /**
   * True when an ExitPlanMode tool_use exists with no tool_result yet â€?
...
// 'teleport' scrapes from the ULTRAPLAN_TELEPORT_SENTINEL in a deny tool_result â€?
...
export async function pollForApprovedExitPlanMode(
  ...
  while (Date.now() < deadline) {
    ...
    try {
      // Metadata fetch (session_status) is the needs_input signal â€?
    ...
    // in RemoteAgentTask). Only trust idle when no new events arrived â€?
