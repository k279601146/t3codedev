...
const SEND_USER_FILE_TOOL_NAME: string | null = feature('KAIROS')
  ? (
      require('@claude-code-best/builtin-tools/tools/SendUserFileTool/prompt.ts') as typeof import('@claude-code-best/builtin-tools/tools/SendUserFileTool/prompt.ts')
 ...
 * message ï¿?they are bookkeeping artifacts that should not mask a genuine
...
function detectTurnInterruption(
  ...
  // before API send (normalizeMessagesForAPI) ï¿?skipping them here lets
  ...
  if (lastMessage.type === 'user') {
    ...
    // Plain text user prompt ï¿?CC hadn't started responding
    return {
      kind: 'interrupted_prompt',
  ...
  if (lastMessage.type === 'attachment') {
    // Attachments are part of the user turn ï¿?the user provided context but
 ...
 * the turn's final act ï¿?there is no follow-up assistant text (#20467
...
export function restoreSkillStateFromMessages(messages: Message[]): void {
  for (const message of messages) {
    ...
    // A prior process already injected the skills-available reminder ï¿?it's
 ...
 * runs internally ï¿?loadTranscriptFile ï¿?find newest non-sidechain
 * leaf ï¿?buildConversationChain ï¿?removeExtraFields ï¿?just starting
 * from an arbitrary path instead of the sid-derived one.
 *
 * leafUuids is populated by loadTranscriptFile as "uuids that no
 * other message's parentUuid points at" ï¿?the chain tips.  There can
...
export async function loadMessagesFromJsonlPath(path: string): Promise<{
  ...
  return {
    ...
    // Leaf's sessionId ï¿?forked sessions copy chain[0] from the source
    ...
    if (source === undefined) {
      ...
      if (feature('BG_SESSIONS')) {
        try {
          ...
          // UDS unavailable ï¿?treat all sessions as continuable
        }
      }
      ...
      // Same chain walk as the sid branch below ï¿?only the starting
      // path differs.
