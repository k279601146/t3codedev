...
/**
 * Initialize Sentry SDK. Safe to call multiple times â€?subsequent calls are no-ops.
...
export function initSentry(): void {
  ...
  Sentry.init({
    ...
    ignoreErrors: [
      // Network errors from unreachable hosts â€?not actionable
      'ECONNREFUSED',
      'ECONNRESET',
      'ENOTFOUND',
      'ETIMEDOUT',
    ...
    beforeSendTransaction(event) {
      // Don't send performance transactions for now â€?errors only
      return null
    },
  })
...
export function captureException(
  ...
  try {
    ...
    // Sentry itself failed â€?don't let it crash the app
  }
}
