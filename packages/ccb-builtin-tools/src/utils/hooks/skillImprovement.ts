...
import { queryModelWithoutStreaming } from "@t3tools/ccb-engine/src/services/api/claude.ts"
import {
  createTrace,
  endTrace,
  isLangfuseEnabled,
} from "@t3tools/ccb-engine/src/services/langfuse/index.ts"
...
function createSkillImprovementHook() {
  ...
  const config: ApiQueryHookConfig<SkillUpdate[]> = {
    ...
    buildMessages(context) {
      ...
      // Only analyze messages since the last check ï¿?the skill definition
 ...
 * Fire-and-forget ï¿?does not block the main conversation.
 */
