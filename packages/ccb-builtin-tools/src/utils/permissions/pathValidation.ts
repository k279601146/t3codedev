...
export function isPathInSandboxWriteAllowlist(resolvedPath: string): boolean {
  ...
  // their resolution to avoid N Ã— config.length redundant syscalls per
 ...
 *   (nonexistent files, UNC paths, etc.) ï¿?parent-directory symlink
...
export function isPathAllowed(
  ...
  // and internal editable paths live under ~/.claude/ ï¿?matching the ordering in
  ...
  // have explicitly configured writable directories (e.g. /tmp/claude/) ï¿?
...
export function validatePath(
  ...
  // The shell expands these differently (~root ï¿?/var/root, ~+ ï¿?$PWD, ~- ï¿?$OLDPWD),
