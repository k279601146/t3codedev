...
import { isAgentMemoryPath } from '@claude-code-best/builtin-tools/tools/AgentTool/agentMemory.ts'
import {
  CLAUDE_FOLDER_PERMISSION_PATTERN,
  FILE_EDIT_TOOL_NAME,
  GLOBAL_CLAUDE_FOLDER_PERMISSION_PATTERN,
} from '@claude-code-best/builtin-tools/tools/FileEditTool/constants.ts'
...
import type { AnyObject, Tool, ToolPermissionContext } from "@t3tools/ccb-engine/src/Tool.ts"
 ...
 * shared /tmp ï¿?sticky bit prevents deletion, not creation ï¿?and either
...
// permission check. Keyed by path string ï¿?getPathsForPermissionCheck is
...
export function checkReadPermissionForTool(
  ...
  // Computed once here and threaded through checkWritePermissionForTool ï¿?
  // checkPathSafetyForAutoEdit ï¿?pathInAllowedWorkingPath to avoid redundant
 ...
 *   from the same `tool` and `input` in the same synchronous frame ï¿?`path` is
...
export function checkWritePermissionForTool<Input extends AnyObject>(
  ...
  if (!safetyCheck.safe) {
    ...
    // back to generateSuggestions ï¿?its setMode suggestion doesn't bypass
...
export function generateSuggestions(
  ...
  // downgrades auto ï¿?acceptEdits, which then prompts for MCP/Bash.
...
export function checkEditableInternalPath(
  ...
  // builds ï¿?spawn.test.ts asserts the string matches. Hijack guard: the env
  ...
  // sides handles the macOS /tmp ï¿?/private/tmp case where the config dir
  ...
  // so it gets NO special permission treatment here ï¿?writes go through normal
  // permission flow (step 5 ï¿?ask). SDK callers who want silent memory should
  ...
  // .claude/launch.json ï¿?desktop preview config (dev server command + port).
  ...
  // cascades: user clicks "Always allow" ï¿?setMode:acceptEdits suggestion
  // applied ï¿?silent downgrade from auto mode. Matches the project-level
...
export function checkReadableInternalPath(
  ...
  // SECURITY: See getBundledSkillsRoot() ï¿?the per-process nonce in the path
