...
// NormalizedMessages â€?1:1 with block count. String content = 1 block.
// AttachmentMessage etc. have no .message and normalize to â‰?.
 ...
 * fresh renderToAnsiString â€?yoga layout tree + Ink's screen buffer are sized
 * to the tallest CHUNK instead of the full session. Measured (Mar 2026,
 * 538-msg session): âˆ?5% plateau RSS vs a single full render. The sink owns
 * the output â€?write to stdout for `[` dump-to-scrollback, appendFile for `v`.
 *
 * Messages.renderRange slices AFTER normalizeâ†’groupâ†’collapse, so tool-call
 * grouping stays correct across chunk seams; buildMessageLookups runs on
 * the full normalized array so tool_useâ†”tool_result resolves regardless of
...
export async function streamRenderedMessages(
  ...
  // see from here â€?normalize splits each Message into one NormalizedMessage
