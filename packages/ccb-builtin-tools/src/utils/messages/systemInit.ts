...
import { getSdkBetas, getSessionId } from "@t3tools/ccb-engine/src/bootstrap/state.ts"
import { DEFAULT_OUTPUT_STYLE_NAME } from "@t3tools/ccb-engine/src/constants/outputStyles.ts"
import type {
  ApiKeySource,
  PermissionMode,
  SDKMessage,
...
// to the 'Agent' tool name. The wire name was renamed Task ï¿?Agent in #19647,
...
/**
 * Build the `system/init` SDKMessage ï¿?the first message on the SDK stream
 ...
 *   - QueryEngine (spawn-bridge / print-mode / SDK) ï¿?yielded as the first
 *     stream message per query turn
 *   - useReplBridge (REPL Remote Control) ï¿?sent via writeSdkMessages() on
...
export function buildSystemInitMessage(inputs: SystemInitInputs): SDKMessage {
  ...
  // Hidden from public SDK types ï¿?ant-only UDS messaging socket path
