...
import type { TaskState } from "@t3tools/ccb-engine/src/tasks/types.ts"
...
export function registerTask(task: TaskState, setAppState: SetAppState): void {
  ...
  // Replacement (resume) â€?not a new start. Skip to avoid double-emit.
...
export function evictTerminalTask(
  ...
  setAppState(prev => {
    ...
    // Panel grace period â€?blocks eviction until deadline passes.
...
export async function generateTaskAttachments(state: AppState): Promise<{
  ...
  // Only the offset patch â€?NOT the full task. The task may transition to
  ...
  for (const taskState of Object.values(tasks)) {
    if (taskState.notified) {
      switch (taskState.status) {
        ...
        case 'failed':
        case 'killed':
          // Evict terminal tasks â€?they've been consumed and can be GC'd
          evictedTaskIds.push(taskState.id)
          continue
        case 'pending':
          // Keep in map â€?hasn't run yet, but parent already knows about it
          continue
        case 'running':
    ...
    // Completed tasks are NOT notified here â€?each task type handles its own
...
export function applyTaskOffsetsAndEvictions(
  ...
  setAppState(prev => {
    ...
    for (const id of offsetIds) {
      ...
      // Re-check status on fresh state â€?task may have completed during the
