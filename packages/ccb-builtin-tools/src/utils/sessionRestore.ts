...
export function restoreSessionStateFromLog(
  ...
  // first ï¿?without that, an in-session /resume into a session with no
 ...
 *
 * process.chdir is the TOCTOU-safe existence check ï¿?it throws ENOENT if
...
export function restoreWorktreeForResume(
  ...
  // EnterWorktreeTool's behavior ï¿?skills/history stay anchored to the
  // original project.
 ...
 * *different* worktree fails entirely ï¿?the getCurrentWorktreeSession()
...
export function exitRestoredWorktree(): void {
  ...
  try {
    ...
    // Original dir is gone (rare). Stay put ï¿?restoreWorktreeForResume
...
export async function processResumedConversation(
  ...
  if (!opts.forkSession) {
    ...
    // ï¿?they're classified as FROZEN ï¿?full content sent (cache miss, permanent
  ...
  // original session's worktree ï¿?a "Remove" on the fork's exit dialog
  // would delete a worktree the original session still references ï¿?so
  ...
  if (!opts.forkSession) {
    ...
    // path doesn't leak), but that blocks reAppendSessionMetadata ï¿?which
    // bails on null ï¿?from running in the exit cleanup handler. For fork,
  ...
  // ï¿?see the restoreSessionStateFromLog callsite above for why.
  if (feature('CONTEXT_COLLAPSE')) {
    /* eslint-disable @typescript-eslint/no-require-imports */
    ;(
      require('../services/contextCollapse/persist.ts') as typeof import('../services/contextCollapse/persist.ts')
