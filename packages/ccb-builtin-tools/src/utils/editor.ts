...
// ('start /wait notepad') does not ï¿?notepad treats +42 as a filename.
 ...
 * Note: this is classification only ï¿?spawn the user's actual binary, not
 ...
 * directory component. code-insiders ï¿?still matches 'code', /usr/bin/code ï¿?
 * 'code' ï¿?matches.
 */
 ...
 *
 * For GUI editors (code, subl, etc.): spawns detached ï¿?the editor opens
...
export function openFileInExternalEditor(
  ...
  if (guiFamily) {
    ...
    if (process.platform === 'win32') {
      // shell: true on win32 so code.cmd / cursor.cmd / windsurf.cmd resolve ï¿?
      ...
      // POSIX: argv array with no shell ï¿?injection-safe. shell: true would
    ...
    // user-config error, not an internal bug ï¿?don't pollute error telemetry.
    child.on('error', e =>
  ...
  // Terminal editor ï¿?needs alt-screen handoff since it takes over the
  // terminal. Blocks until the editor exits.
  const inkInstance = instances.get(process.stdout)
  if (!inkInstance) return false
  // Only prepend +N for editors known to support it ï¿?notepad treats +42 as a
