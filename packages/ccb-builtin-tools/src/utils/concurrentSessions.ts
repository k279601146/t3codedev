 ...
 * write the file for it ï¿?cleanup-on-exit wiring then works for free.
 ...
 *
 * Registers all top-level sessions ï¿?interactive CLI, SDK (vscode, desktop,
 * typescript, python, -p), bg/daemon spawns ï¿?so `claude ps` sees everything
 ...
 * status-change effect ï¿?a dropped write just means ps falls back to
...
export async function countConcurrentSessions(): Promise<number> {
  ...
  for (const file of files) {
    ...
    // parse as PID 2026 and get swept as stale ï¿?silent user data loss.
    ...
    if (isProcessRunning(pid)) {
      ...
      // Stale file from a crashed session ï¿?sweep it. Skip on WSL: if
