...
import { SHELL_KEYWORDS } from "./bashParser.ts"
import type { Node } from "./parser.ts"
import { PARSE_ABORTED, parseCommandRaw } from "./parser.ts"
 ...
 * `VAR="prefix$(cmd)"` â†?`"prefix__CMDSUB_OUTPUT__"` â€?the substring check
 * catches these where exact-match Set.has() would miss.
 *
 * Also catches user-typed literals that collide with placeholder strings:
 * `VAR=__TRACKED_VAR__ && rm $VAR` â€?treated as non-literal (conservative).
 */
 ...
 * single string â€?but at runtime bash may produce MULTIPLE args, or paths
 * matched by a glob. A value containing these metacharacters cannot be
 * trusted as a bare arg: `VAR="-rf /" && rm $VAR` â†?bash runs `rm -rf /`
 * (two args) but our argv would have `['rm', '-rf /']` (one arg). Similarly
 * `VAR="/etc/*" && cat $VAR` â†?bash expands to all /etc files.
 *
 * Inside double-quotes ("$VAR"), neither splitting nor globbing applies â€?
 * the value IS a single literal argument.
 */
const BARE_VAR_UNSAFE_RE = /[ \t\n*?[]/

// stdbuf flag forms â€?hoisted from the wrapper-stripping while-loop
 ...
 * via $VAR is safe â€?the expansion is deterministic and doesn't introduce
...
const SAFE_ENV_VARS = new Set([
  ...
  // Special bash variables â€?always set, values are shell-controlled:
 ...
 * ONLY inside strings (same rationale as SAFE_ENV_VARS â€?as bare args
 * their value IS the argument and might be a path/flag from $1 etc.).
 *
 * SECURITY: '@' and '*' are NOT in this set. Inside "...", they expand to
 * the positional params â€?which are EMPTY in a fresh BashTool shell (how we
 ...
 * tooComplex for `$*` / `$@`. `echo "args: $*"` becomes too-complex â€?
 ...
 *
 * This set is not exhaustive â€?it documents KNOWN dangerous types. The real
...
/**
 * Redirect operator tokens â†?canonical operator. tree-sitter produces these
 ...
 * Reject both â€?the escaped-brace case is rare and trivially rewritten
 * with single quotes.
 */
 ...
 * reimplement bash's unescaping rules, we reject these â€?they're rare in
 * practice and trivial to rewrite with quotes.
 *
 * Also matches `\` before newline (line continuation) when adjacent to a
 * non-whitespace char. `tr\<NL>aceroute` â€?bash joins to `traceroute`, but
 * tree-sitter splits into two words (differential). When `\<NL>` is preceded
 * by whitespace (e.g. `foo && \<NL>bar`), there's no word to join â€?both
 ...
 * Only matches word-initial `=` followed by a command-name char â€?`VAR=val`
...
function maskBracesInQuotedContexts(cmd: string): string {
  // Fast path: no `{` â†?nothing to mask. Skips the char-by-char scan for
  ...
  while (i < cmd.length) {
    ...
    if (inSingle) {
      ...
      // newline â€?but those don't affect quote state so we let them pass).
 ...
 * loaded â€?caller should fall back to conservative behavior.
 */
export async function parseForSecurity(
  ...
  // Don't use .trim() â€?it strips Unicode whitespace (\u00a0 etc.) which the
 ...
 * still run on `cmd` â€?they catch tree-sitter/bash differentials that a
...
export function parseForSecurityFromAst(
  ...
  if (root === PARSE_ABORTED) {
    ...
    // panic). Adversarially triggerable â€?`(( a[0][0]... ))` with ~2800
    // subscripts hits PARSE_TIMEOUT_MICROS under the 10K length limit.
    // Previously indistinguishable from module-not-loaded â†?routed to
    // legacy (parse-unavailable), which lacks EVAL_LIKE_BUILTINS â€?`trap`,
    // `enable`, `hash` leaked with Bash(*). Fail closed: too-complex â†?ask.
    return {
      kind: 'too-complex',
      reason:
        'Parser aborted (timeout or resource limit) â€?possible adversarial input',
...
function walkProgram(root: Node): ParseForSecurityResult {
  // ERROR-node check folded into collectCommands â€?any unhandled node type
  ...
  // `NOW=$(date) && jq --arg now "$NOW" ...` â€?$NOW is known to be the
...
function collectCommands(
  ...
  if (node.type === 'command') {
    // Pass `commands` as the innerCommands accumulator â€?any $() extracted
  ...
  if (STRUCTURAL_TYPES.has(node.type)) {
    ...
    //   `||` RHS runs conditionally â†?vars set there MAY not be set
    //   `|`/`|&` stages run in subshells â†?vars set there are NEVER visible after
    //   `&` LHS runs in a background subshell â†?same as above
    // Flag-omission attack: `true || FLAG=--dry-run && cmd $FLAG` â€?bash skips
    // the `||` RHS (FLAG unset â†?$FLAG empty), runs `cmd` WITHOUT --dry-run.
    // With linear scope, our argv has ['cmd','--dry-run'] â†?looks SAFE â†?bypass.
    //
    // Fix: snapshot incoming scope at entry. After these separators, reset to
    // the snapshot â€?vars set in clauses between separators don't leak. `scope`
    ...
    // caller's varScope is only mutated for the `&&`/`;` prefix â€?this is
    // conservative (vars set in `A && B | C && D` leak A+B into caller, not
    // C+D) but safe.
    //
    // Efficiency: snapshot is only needed if we hit `||`/`|`/`|&`/`&`. For
    // the dominant case (`ls`, `git status` â€?no such separators), skip the
    // Map alloc via a cheap pre-scan. For `pipeline`, node.type already tells
    // us stages are subshells â€?copy once at entry, no snapshot needed (each
    ...
    // For `pipeline`, ALL stages run in subshells â€?start with a copy so
  ...
  if (node.type === 'negated_command') {
    // `! cmd` inverts exit code only â€?doesn't execute code or affect
    ...
    for (const child of node.children) {
      ...
      switch (child.type) {
        ...
        case 'concatenation': {
          ...
          // (`declare -i 42`). Mirrors walkCommand's argv handling â€?before
          ...
          // then `$X` dereferences to $Y's VALUE â€?varScope stores 'Y'
          ...
          // guards in $((â€?)). -a/-A (array): subscript arithmetic on
          ...
          // evaluates â€?no -a/-i flag needed. `declare 'x[$(id)]=val'`
          ...
          if (
            ...
            return {
              kind: 'too-complex',
              reason: `declare positional '${arg}' contains array subscript â€?bash evaluates $(cmd) in subscripts`,
              nodeType: 'declaration_command',
            }
          }
          argv.push(arg)
        ...
        case 'variable_name':
          // `export FOO` â€?bare name, no assignment.
          argv.push(child.text)
          break
        default:
  ...
  if (node.type === 'variable_assignment') {
    ...
    // Sets a shell variable â€?no code execution, no filesystem I/O.
    // The value is validated via walkVariableAssignment â†?walkArgument,
    // so `VAR=$(evil)` still recursively extracts/rejects based on the
    // inner command. Does NOT push to commands â€?a bare assignment needs
  ...
  if (node.type === 'for_statement') {
    // `for VAR in WORD...; do BODY; done` â€?iterate BODY once per word.
    ...
    //  - Absolute paths: `for i in /etc/passwd; do rm $i; done` â€?body argv
    //    would have placeholder, path validation never sees /etc/passwd.
    //  - Globs: `for i in /etc/*; do rm $i; done` â€?`/etc/*` is a static word
    //    at parse time but bash expands it at runtime.
    //  - Flags: `for i in -rf /; do rm $i; done` â€?flag smuggling.
    //
    // VAR_PLACEHOLDER means bare `$i` in body â†?too-complex. Only
    // string-embedding (`echo "item: $i"`) stays simple. This reverts some
    // of the too-complexâ†’simple rescues in the original PR â€?each one was a
    ...
    for (const child of node.children) {
      ...
      if (child.type === 'variable_name') {
        ...
        // `for i in $(seq 1 3)` â€?inner cmd IS extracted and rule-checked.
        const err = collectCommandSubstitution(child, commands, varScope)
        if (err) return err
      } else {
        // Iteration values â€?validated via walkArgument. Value discarded:
        // body argv gets VAR_PLACEHOLDER regardless of the iteration words,
        // and bare `$i` in body â†?too-complex (see SECURITY comment above).
    ...
    // via varScope.set below â€?walkVariableAssignment's PS4/IFS checks never
    ...
    // SECURITY: Body uses a scope COPY â€?vars assigned inside the loop
    // body don't leak to commands after `done`. The loop var itself is
    // set in the REAL scope (bash semantics: $i still set after loop)
    // and copied into the body scope. ALWAYS VAR_PLACEHOLDER â€?see above.
  ...
  if (node.type === 'if_statement' || node.type === 'while_statement') {
    ...
    //
    // SECURITY: Branch bodies use scope COPIES â€?vars assigned inside a
    ...
    // check, so assignments there are unconditional â€?e.g., `while read V`
    ...
    for (const child of node.children) {
      ...
      if (child.type === 'elif_clause' || child.type === 'else_clause') {
        ...
        // Scope COPY â€?elif/else branch assignments don't leak past fi.
      ...
      // read var value is UNKNOWN (stdin input) â†?use VAR_PLACEHOLDER
          ...
          if (c?.argv[0] === 'read') {
            for (const a of c.argv.slice(1)) {
              ...
              if (!a.startsWith('-') && /^[A-Za-z_][A-Za-z0-9_]*$/.test(a)) {
                ...
                // IS still pushed to commands[] â€?we can't tell it was
  ...
  if (node.type === 'subshell') {
    // `(cmd1; cmd2)` â€?run commands in a subshell. Inner commands ARE
  ...
  if (node.type === 'unset_command') {
    ...
    // variables/functions from the current shell â€?no code execution, no
...
function walkTestExpr(
  ...
  switch (node.type) {
    ...
    case 'extglob_pattern':
      // RHS of =~ or ==/!= in [[ ]]. Pattern text only â€?no code execution.
      ...
      return null
    default: {
      // Operand â€?word, string, number, etc. Validate via walkArgument.
...
function walkFileRedirect(
  ...
  for (const child of node.children) {
    ...
    if (child.type === 'file_descriptor') {
      ...
      // `NN#<expansion>` arithmetic-base grammar quirk â€?same issue as
      ...
      // Unescape backslash sequences â€?same as walkArgument. Bash quote
      // removal turns `\X` â†?`X`. Without this, `cat < /proc/self/\environ`
      ...
      // `echo > "foo"bar` â€?tree-sitter produces a concatenation of string +
...
/**
 * Heredoc redirect. Only quoted-delimiter heredocs (<<'EOF') are safe â€?
 ...
 *
 * SECURITY: tree-sitter-bash has a grammar gap â€?backticks (`...`) inside
 ...
 * by checking body children for expansion nodes â€?we'd miss backtick
...
function walkHeredocRedirect(node: Node): ParseForSecurityResult | null {
  ...
  for (const child of node.children) {
    ...
    if (child.type === 'heredoc_start') startText = child.text
      ...
      // expected structural tokens â€?safe to skip. file_descriptor
      // covers fd-prefixed heredocs (`cat 3<<'EOF'`) â€?walkFileRedirect
...
/**
 * Here-string redirect (`<<< content`). The content becomes stdin â€?not
 * argv, not a path. Safe when content is a literal word, raw_string, or
 * string with no expansions. Reject when content contains $()/${}/$VAR â€?
 ...
 * unless the var is tracked/safe. The result string is discarded â€?we only
 ...
 * currently rejected conservatively â€?walkString's solo-placeholder guard
...
function walkHerestringRedirect(
  ...
  for (const child of node.children) {
    ...
    // (which we discard â€?content is stdin, irrelevant to permissions) or
...
function walkCommand(
  ...
  for (const child of node.children) {
    ...
    switch (child.type) {
      case 'variable_assignment': {
        ...
        // bash â€?VAR is only visible to `cmd` as an env var, NOT to
        // subsequent commands. Do NOT add to global varScope â€?that would
      ...
      // is intentionally NOT handled here â€?the $() output IS the argument,
      ...
      case 'simple_expansion': {
        ...
        // value (e.g. VAR=/etc â†?'/etc'). Values with IFS/glob chars or
      ...
      case 'herestring_redirect': {
        // `cmd <<< "content"` â€?content is stdin, not argv. Validate it's
  ...
  // .text is the raw source span. Downstream (bashToolCheckPermission â†?
  // splitCommand_DEPRECATED) re-tokenizes it via shell-quote. Normally .text
  // is used unchanged â€?but if we resolved a $VAR into argv, .text diverges
  // (has raw `$VAR`) and downstream RULE MATCHING would miss deny rules.
  //
  // SECURITY: `SUB=push && git $SUB --force` with `Bash(git push:*)` deny:
  //   argv = ['git', 'push', '--force']  â†?correct, path validation sees 'push'
  //   .text = 'git $SUB --force'         â†?deny rule 'git push:*' doesn't match
  //
  ...
  // position â€?command_name, word, string interior, concatenation part.
  ...
  // FP on `echo '$VAR'`. But single-quoted $ is LITERAL in bash â€?argv has
  ...
  //
  // NOTE: This does NOT include redirects/envVars in the rebuilt .text â€?
  ...
  // them) but preserved in node.text. `timeout 5 \<LF>curl evil.com` â†?argv
  // is correct, but raw .text â†?stripSafeWrappers matches `timeout 5 ` (the
  // space before \), leaving `\<LF>curl evil.com` â€?Bash(curl:*) deny doesn't
  // prefix-match. Rebuilt .text joins argv with ' ' â†?no newlines â†?
 ...
 * (inner) â€?permission rules must match BOTH for the whole command to allow.
 */
...
function walkArgument(
  ...
  switch (node.type) {
    case 'word': {
      ...
      // removal turns `\X` â†?`X` for any character X. tree-sitter preserves
      // the raw text. Required for checkSemantics: `\eval` must match
      // EVAL_LIKE_BUILTINS, `\zmodload` must match ZSH_DANGEROUS_BUILTINS.
      // Also makes argv accurate: `find -exec {} \;` â†?argv has `;` not
      // `\;`. (Deny-rule matching on .text already worked via downstream
      // splitCommand_DEPRECATED unescaping â€?see walkCommand comment.) `\<whitespace>`
    ...
    case 'number':
      ...
      // command_substitution â€?bash runs the substitution. .text on a node
    ...
    // is intentionally NOT handled â€?the output is/becomes-part-of a positional
...
function walkString(
  ...
  // the placeholder â€?which downstream path validation resolves as a
  ...
  // (`"prefix: $(cmd)"`) are safe â€?runtime value can't equal a bare path.
  ...
  for (const child of node.children) {
    ...
    // tree-sitter whitespace-only-string quirk (space/tab, not newline) â€?let
    ...
    switch (child.type) {
      ...
      case 'string_content':
        ...
        // escapes $ ` " \ â€?other sequences like \n stay literal. So
        // `"fix \"bug\""` â†?`fix "bug"`, but `"a\nb"` â†?`a\nb` (backslash
      ...
      case 'command_substitution': {
        ...
        // placeholder argv value â€?the actual content doesn't matter for
        ...
        if (heredocBody !== null) {
          ...
          // dropped it â†?`rm "$(cat <<'EOF'\n/etc/passwd\nEOF)"` produced
          // argv ['rm',''] while bash runs `rm /etc/passwd`. validatePath('')
          // resolves to cwd â†?allowed. Every path-constrained command
          // bypassed via this. Now: append the body (trailing LF trimmed â€?
          // bash $() strips trailing newlines).
          //
          // Tradeoff: bodies with internal newlines are multi-line text
          // (markdown, scripts) which cannot be valid paths â€?safe to drop
        ...
        // `echo "SHA: $(git rev-parse HEAD)"` â†?extracts BOTH
        // `echo "SHA: $(...)"` AND `git rev-parse HEAD` â€?both must match
      ...
      case 'simple_expansion': {
        ...
        // value from a tracked static var (e.g. VAR=/tmp â†?v='/tmp').
      ...
      case 'arithmetic_expansion': {
        ...
        if (err) return err
        result += child.text
        // Validated to be literal-numeric â€?static content.
        sawLiteralContent = true
        break
      }
      default:
  ...
  // placeholder â€?which bypasses downstream path validation (validatePath
  ...
  // SECURITY: tree-sitter-bash quirk â€?a double-quoted string containing
  ...
  // (both flags false â€?neither literal nor placeholder added) but the
  // source span is longer than bare `""`. Genuine `""` has text.length==2.
  // `"$V"` with V="" doesn't hit this â€?the simple_expansion child sets
 ...
 * numeric expressions â€?no variables, no substitutions. Returns null if
 ...
 *
 * When safe, the caller puts the full `$((â€?)` span into argv as a literal
 ...
 *     $(
 *     redirected_statement
 *       command â†?command_name â†?word "cat"    (exactly one child)
 *       heredoc_redirect
 *         <<
...
function extractSafeCatHeredoc(subNode: Node): string | 'DANGEROUS' | null {
  ...
  for (const child of stmt.children) {
    ...
    if (child.type === 'command') {
      // Must be bare `cat` â€?no args, no env vars
  ...
  // not inspected there) â€?effectively bypassing this check. Return a
...
function walkVariableAssignment(
  ...
  for (const child of node.children) {
    ...
    if (child.type === 'variable_name') {
      ...
      // `PATH+=":/new"` â€?tree-sitter emits `+=` as a distinct operator
      // node. Without this case it falls through to walkArgument below
      // â†?tooComplex on unknown type `+=`.
      isAppend = child.type === '+='
    } else if (child.type === 'command_substitution') {
      // $() as the variable's value. The output becomes a STRING stored in
      // the variable â€?it's NOT a positional argument (no path/flag concern).
      // `VAR=$(date)` runs `date`, stores output. `VAR=$(rm -rf /)` runs
      // `rm` â€?the inner command IS checked against permission rules, so
      ...
      // `VAR=$OTHER` â€?assignment RHS does NOT word-split or glob-expand
      ...
      // contain spaces/globs â€?if B is later used as a bare arg, THAT use
      // will correctly reject via BARE_VAR_UNSAFE_RE.
      const v = resolveSimpleExpansion(child, varScope, true)
      if (typeof v !== 'string') return v
      // If v is VAR_PLACEHOLDER (OTHER holds unknown), store it â€?combined
  ...
  // as variable_assignment. Bash only recognizes [A-Za-z_][A-Za-z0-9_]* â€?
  // anything else is run as a COMMAND. `1VAR=value` â†?bash tries to execute
  ...
  // unquoted $VAR expansions. `IFS=: && VAR=a:b && rm $VAR` â†?bash splits
  // on `:` â†?`rm a b`. Our BARE_VAR_UNSAFE_RE only checks default IFS
  // chars (space/tab/NL) â€?we can't model custom IFS. Reject.
  if (name === 'IFS') {
    return {
      kind: 'too-complex',
      reason: 'IFS assignment changes word-splitting â€?cannot model statically',
      nodeType: 'variable_assignment',
    }
  }
  ...
  // argv is only [["set","-x"],[":"]] â€?the payload is invisible to
  ...
  //     (octal for `$`) becomes `$(id)` at trace time â€?any literal-char
  ...
  //
  // Policy: (1) reject += outright â€?no scope-tracking dependency; user can
  // combine into one PS4=... (2) reject placeholders â€?runtime unknowable.
  ...
  // encoding vectors and future ones â€?anything off the allowlist fails.
  ...
  if (name === 'PS4') {
    if (isAppend) {
      return {
        kind: 'too-complex',
        reason:
          'PS4 += cannot be statically verified â€?combine into a single PS4= assignment',
        nodeType: 'variable_assignment',
      }
    }
    if (containsAnyPlaceholder(value)) {
      return {
        kind: 'too-complex',
        reason: 'PS4 value derived from cmdsub/variable â€?runtime unknowable',
        nodeType: 'variable_assignment',
      }
    }
    if (
      ...
      return {
        ...
        reason:
          ...
          'PS4 value outside safe charset â€?only ${VAR} refs and [A-Za-z0-9 _+:.=/[]-] allowed',
        nodeType: 'variable_assignment',
      }
    }
  }
  // SECURITY: Tilde expansion in assignment RHS. `VAR=~/x` (unquoted) â†?
  // bash expands `~` at ASSIGNMENT time â†?VAR='/home/user/x'. We see the
  // literal `~/x`. Later `cd $VAR` â†?our argv `['cd','~/x']`, bash runs
  // `cd /home/user/x`. Tilde expansion also happens after `=` and `:` in
  // assignment values (e.g. PATH=~/bin:~/sbin). We can't model it â€?reject
  // any value containing `~` that isn't already quoted-literal (where bash
  // doesn't expand). Conservative: any `~` in value â†?reject.
  if (value.includes('~')) {
    return {
      kind: 'too-complex',
      reason: 'Tilde in assignment value â€?bash may expand at assignment time',
      nodeType: 'variable_assignment',
    }
  }
 ...
 *   tracked vars are only allowed inside strings â€?as bare args their runtime
...
function resolveSimpleExpansion(
  ...
  // Non-literal values (containing any placeholder â€?loop vars, $() output,
  ...
  // critical fix. `VAR=/etc && rm $VAR` â†?argv ['rm', '/etc'] â†?validatePath
  // correctly rejects. Previously returned a placeholder â†?validatePath saw
  // '__LOOP_STATIC__', resolved as cwd-relative â†?PASSED â†?bypass.
  const trackedValue = varScope.get(varName)
  if (trackedValue !== undefined) {
    if (containsAnyPlaceholder(trackedValue)) {
      // Non-literal: bare â†?reject, inside string â†?VAR_PLACEHOLDER
      // (walkString's solo-placeholder gate rejects `"$VAR"` alone).
      if (!insideString) return tooComplex(node)
      return VAR_PLACEHOLDER
    }
    // Pure literal (e.g. '/tmp', 'foo') â€?return it directly. Downstream
    // path validation / checkSemantics operate on the REAL value.
    //
    // SECURITY: For BARE args (not inside a string), bash word-splits on
    // $IFS and glob-expands the result. `VAR="-rf /" && rm $VAR` â†?bash
    // runs `rm -rf /` (two args); `VAR="/etc/*" && cat $VAR` â†?expands to
    ...
    // ZERO fields â€?the expansion disappears. `V="" && $V eval x` â†?bash
    // runs `eval x` (our argv would be ["","eval","x"] with name="" â€?
    // every EVAL_LIKE/ZSH/keyword check misses). `V="" && ls $V /etc` â†?
    // bash runs `ls /etc`, our argv has a phantom "" shifting positions.
    // Inside "...": `"$V"` â†?bash produces one empty-string arg â†?our ""
 ...
 * placeholder, the result is non-literal â€?store VAR_PLACEHOLDER so later
...
/**
 * Zsh module builtins. These are not binaries on PATH â€?they're zsh
...
const EVAL_LIKE_BUILTINS = new Set([
  ...
  'noglob',
  'nocorrect',
  // `trap 'cmd' SIGNAL` â€?cmd runs as shell code on signal/exit. EXIT fires
  // at end of every BashTool invocation, so this is guaranteed execution.
  'trap',
  // `enable -f /path/lib.so name` â€?dlopen arbitrary .so as a builtin.
  // Native code execution.
  'enable',
  // `mapfile -C callback -c N` / `readarray -C callback` â€?callback runs as
  // shell code every N input lines.
  'mapfile',
  'readarray',
  // `hash -p /path cmd` â€?poisons bash's command-lookup cache. Subsequent
  // `cmd` in the same command resolves to /path instead of PATH lookup.
  'hash',
  // `bind -x '"key":cmd'` / `complete -C cmd` â€?interactive-only callbacks
  ...
  'complete',
  'compgen',
  // `alias name='cmd'` â€?aliases not expanded in non-interactive bash by
  ...
  'alias',
  // `let EXPR` arithmetically evaluates EXPR â€?identical to $(( EXPR )).
 ...
 * evaluate `arr[EXPR]` subscripts â€?including $(cmd) in the subscript â€?
 * even when the argv element arrived from a single-quoted raw_string.
 * `test -v 'a[$(id)]'` â†?tree-sitter sees an opaque leaf, bash runs id.
 * Maps: builtin name â†?set of flags whose next argument is a NAME.
 */
const SUBSCRIPT_EVAL_FLAGS: Record<string, Set<string>> = {
  ...
  // is `arr[EXPR]`, bash arithmetically evaluates the subscript â€?running
 ...
 * flag), these are binary â€?the subscript can appear on EITHER side, so
 ...
 * both forms, so they get this check too â€?mild over-blocking, safe side.
 */
const TEST_ARITH_CMP_OPS = new Set(['-eq', '-ne', '-lt', '-le', '-gt', '-ge'])

/**
 * Builtins where EVERY non-flag positional argument is a NAME that bash
 * re-parses and arithmetically evaluates subscripts on â€?no flag required.
 ...
 * raw_string children before reaching here â€?this is defense-in-depth).
 ...
 * prompt string. `-a` is intentionally absent â€?its operand IS a NAME.
 */
const READ_DATA_FLAGS = new Set(['-p', '-d', '-n', '-N', '-t', '-u', '-i'])

// SHELL_KEYWORDS imported from bashParser.ts â€?shell reserved words can never
...
// Use `.*` not `[^/]*` â€?Linux resolves `..` in procfs, so
  ...
  for (const cmd of commands) {
    ...
    for (;;) {
      if (a[0] === 'time' || a[0] === 'nohup') {
        ...
        // flags AND fail closed on any unrecognized flag â€?an unknown flag
        ...
        while (i < a.length) {
          ...
          if (
            ...
            i += 2 // -k DURATION / -s SIGNAL â€?separate value
          } else if (/^-[ks][A-Za-z0-9_.+-]+$/.test(arg)) {
            i++ // fused: -k5, -sTERM
          } else if (arg.startsWith('-')) {
            // Unknown flag OR -k/-s with non-allowlisted value â€?can't locate
            ...
            break // non-flag â€?should be the duration
          }
        }
        if (a[i] && /^\d+(?:\.\d+)?[smhd]?$/.test(a[i]!)) {
          ...
          // accepts `.5`, `+5`, `5e-1`, `inf`, `infinity`, hex floats â€?none
          ...
          // name='timeout' and eval was never checked. Now fail CLOSED â€?
          ...
          break // no more args â€?`timeout` alone, inert
        }
      } else if (a[0] === 'nice') {
        ...
        if (a[1] === '-n' && a[2] && /^-?\d+$/.test(a[2])) {
          ...
          // Fail closed â€?mirrors the timeout-duration fail-closed above.
          return {
            ok: false,
            reason: `nice argument '${a[1]}' contains expansion â€?cannot statically determine wrapped command`,
          }
        } else {
        ...
        // SECURITY: -S splits a string into argv (mini-shell) â€?must reject.
        // -C/-P change cwd/PATH â€?wrapped cmd runs elsewhere, reject.
        // Any OTHER flag â†?reject (fail-closed, not fail-open to name='env').
        let i = 1
        while (i < a.length) {
          const arg = a[i]!
          if (arg.includes('=') && !arg.startsWith('-')) {
            ...
            // or unknown flag. Can't model â€?reject the whole command.
            return {
              ok: false,
        ...
        if (i < a.length) {
          ...
          break // `env` alone (no wrapped cmd) â€?inert, name='env'
        }
      } else if (a[0] === 'stdbuf') {
        ...
        // â†?['0','eval',...] â†?name='0' hid eval. Now iterate all known flag
        ...
        while (i < a.length) {
          ...
          if (STDBUF_SHORT_SEP_RE.test(arg) && a[i + 1]) {
            ...
            // accepts space-separated â€?we can't enumerate safely, reject.
            return {
              ok: false,
        ...
        if (i > 1 && i < a.length) {
          ...
          break // `stdbuf` with no flags or no wrapped cmd â€?inert
        }
      } else {
        break
      }
    ...
    // SECURITY: Empty command name. Quoted empty (`"" cmd`) is harmless â€?
    ...
    if (name === '') {
      return {
        ok: false,
        reason: 'Empty command name â€?argv[0] may not reflect what bash runs',
      }
    }
    ...
    // But if a bug upstream ever lets one through, catch it here â€?a
    // placeholder-as-command-name means runtime-determined command â†?unsafe.
    ...
    // `printf '[%s]' x` stays safe â€?`[` in format string, not after `-v`.
    ...
    if (dangerFlags !== undefined) {
      for (let i = 1; i < a.length; i++) {
        ...
        if (dangerFlags.has(arg) && a[i + 1]?.includes('[')) {
          return {
            ok: false,
            reason: `'${name} ${arg}' operand contains array subscript â€?bash evaluates $(cmd) in subscripts`,
          }
        }
          ...
          for (const flag of dangerFlags) {
            if (flag.length === 2 && arg.includes(flag[1]!)) {
              if (a[i + 1]?.includes('[')) {
                return {
                  ok: false,
                  reason: `'${name} ${flag}' (combined in '${arg}') operand contains array subscript â€?bash evaluates $(cmd) in subscripts`,
                }
              }
            }
          }
        ...
        for (const flag of dangerFlags) {
          if (
            ...
            return {
              ok: false,
              reason: `'${name} ${flag}' (fused) operand contains array subscript â€?bash evaluates $(cmd) in subscripts`,
            }
          }
        }
      }
    ...
    // the operand adjacent to each arith-cmp operator on BOTH sides â€?
    // SUBSCRIPT_EVAL_FLAGS's "flag then next-arg" pattern can't express
    // "either side of a binary op". String comparisons (==/!=/=~) do NOT
    // trigger arithmetic eval â€?`[[ 'a[x]' == y ]]` is a literal string cmp.
    if (name === '[[') {
      ...
      for (let i = 2; i < a.length; i++) {
        ...
        if (a[i - 1]?.includes('[') || a[i + 1]?.includes('[')) {
          return {
            ok: false,
            reason: `'[[ ... ${a[i]} ... ]]' operand contains array subscript â€?bash arithmetically evaluates $(cmd) in subscripts`,
          }
        }
      }
    }

    // SECURITY: `read`/`unset` treat EVERY bare positional as a NAME â€?
    ...
    if (BARE_SUBSCRIPT_NAME_BUILTINS.has(name)) {
      ...
      for (let i = 1; i < a.length; i++) {
        ...
        if (arg[0] === '-') {
          if (name === 'read') {
            if (READ_DATA_FLAGS.has(arg)) {
              ...
              // (`-p[foo]` â†?prompt=`[foo]`), or next-arg if last
              // (`-rp '[foo]'` â†?prompt=`[foo]`). So skipNext iff a
        ...
        if (arg.includes('[')) {
          return {
            ok: false,
            reason: `'${name}' positional NAME '${arg}' contains array subscript â€?bash evaluates $(cmd) in subscripts`,
          }
        }
      }
    }
    ...
    // + `command "do :"` + `command "done"` â€?tree-sitter fails to recognize
    ...
    if (SHELL_KEYWORDS.has(name)) {
      return {
        ok: false,
        reason: `Shell keyword '${name}' as command name â€?tree-sitter mis-parse`,
      }
    }
    ...
    if (EVAL_LIKE_BUILTINS.has(name)) {
      ...
      // only print paths â€?they never execute argv[1]. Bare `command foo`
      ...
      if (name === 'command' && (a[1] === '-v' || a[1] === '-V')) {
        ...
        // `fc -l`, `fc -ln` list history â€?safe. `fc -e ed` invokes an
        // editor then executes. `fc -s [pat=rep]` RE-EXECUTES the last
        // matching command (optionally with substitution) â€?as dangerous
        ...
        // `compgen -c/-f/-v` only list completions â€?safe. `compgen -C cmd`
    ...
    // Check argv and redirect targets â€?`cat /proc/self/environ` and
