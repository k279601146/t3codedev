...
export function rearrangePipeCommand(command: string): string {
  ...
  // valid bash that executes `calc.exe` â€?turning a syntax error into an
 ...
 *   â†?eval's stdin is /dev/null, eval evaluates 'cmd', pipes inside work correctly
 *
 * The previous approach `quote([cmd, '<', '/dev/null'])` produced: eval 'cmd' \< /dev/null
 *   â†?eval concatenates args to 'cmd < /dev/null', redirect applies to LAST pipe command
 */
