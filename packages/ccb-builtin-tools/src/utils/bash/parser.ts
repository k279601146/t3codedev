...
export async function parseCommand(
  ...
  // branch lets Bun DCE the NAPI import AND keeps telemetry honest ï¿?we
...
/**
 * Raw parse ï¿?skips findCommandNode/extractEnvVars which the security
...
export async function parseCommandRaw(
  ...
  if (feature('TREE_SITTER_BASH') || feature('TREE_SITTER_BASH_SHADOW')) {
    ...
    try {
      ...
      // Previously collapsed into `return null` ï¿?parse-unavailable ï¿?legacy
      // path, which lacks EVAL_LIKE_BUILTINS ï¿?`trap`, `enable`, `hash` leaked.
      if (result === null) {
