...
/**
 * CliLaunchSpec â€?normalized descriptor for spawning a child CLI process.
 *
...
// ---------------------------------------------------------------------------
// Frozen bootstrap snapshot â€?computed once at module load time.
//
...
function sanitizeExecArgv(raw: readonly string[]): string[] {
  ...
  for (let i = 0; i < raw.length; i++) {
    ...
    if (arg.startsWith('--') && !arg.includes('=') && i + 1 < raw.length) {
      // Unknown two-part flag â€?skip conservatively in bundled mode only
...
export function buildCliLaunch(
  ...
  // In bundled mode the execPath IS the CLI binary â€?no script path needed.
 ...
 * `detached` as-is â€?the child needs `detached` to outlive the parent.
 */
