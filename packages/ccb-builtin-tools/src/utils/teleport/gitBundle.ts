 ...
 *
 * Flow:
 *   1. git stash create â†?update-ref refs/seed/stash (makes it reachable)
...
import { type FilesApiConfig, uploadFile } from "@t3tools/ccb-engine/src/services/api/filesApi.ts"
...
// Bundle --all â†?HEAD â†?squashed-root. HEAD drops side branches/tags but
// keeps full current-branch history. Squashed-root is a single parentless
// commit of HEAD's tree (or the stash tree if WIP exists) â€?no history,
...
async function _bundleWithFallback(
  ...
  // when WIP exists (bakes uncommitted changes in â€?can't bundle the stash
...
// seed_bundle_file_id. --all â†?HEAD â†?squashed-root fallback chain.
// Tracked WIP via stash create â†?refs/seed/stash (or baked into the
...
export async function createAndUploadGitBundle(
  ...
  // elsewhere still bundle â€?`--all` packs those refs regardless of HEAD.
  ...
  // stash create writes a dangling commit â€?doesn't touch refs/stash or
  ...
  try {
    ...
    // Always delete â€?also sweeps a stale ref from a crashed prior run.
