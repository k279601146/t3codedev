 ...
 * also use that key ï¿?and fail if it's stale/wrong-org.
 */
...
export function isAnthropicAuthEnabled(): boolean {
  ...
  // flip this ï¿?they'd cause a header mismatch with the proxy and a bogus
...
export function getAuthTokenSource() {
  ...
  if (oauthTokenFromFd) {
    ...
    // doesn't exist. Call sites fall through correctly ï¿?the new source is
    // !== 'none' (cli/handlers/auth.ts ï¿?oauth_token) and not in the
    // isEnvVarToken set (auth.ts:1844 ï¿?generic re-login message).
...
export function getAnthropicApiKeyWithSource(
  ...
  // Check for apiKeyHelper ï¿?use sync cache, never block
  ...
  if (apiKeyHelperCommand) {
    ...
    // source='apiKeyHelper' rather than falling through to keychain ï¿?
 ...
 * In bare mode, only the --settings flag source is consulted ï¿?apiKeyHelper
...
// Epoch bumps on clearApiKeyHelperCache() ï¿?orphaned executions check their
...
export async function getApiKeyFromApiKeyHelper(
  ...
  if (_apiKeyHelperCache) {
    ...
    // Stale ï¿?return stale value now, refresh in the background.
  ...
  // Cold cache ï¿?deduplicate concurrent calls
...
async function _runAndCache(
  ...
  try {
    ...
    // the ' ' sentinel ï¿?keep serving the stale value and bump timestamp
    ...
    // Cold cache or prior error ï¿?cache ' ' so callers don't fall back to OAuth
    _apiKeyHelperCache = { value: ' ', timestamp: Date.now() }
    return ' '
  } finally {
...
async function _executeApiKeyHelper(
  ...
  if (result.failed) {
    // reject:false ï¿?execa resolves on exitï¿?/timeout, stderr is on result
...
/**
 * Sync cache reader ï¿?returns the last fetched apiKeyHelper value without executing.
...
export function prefetchApiKeyFromApiKeyHelperIfSafe(
  ...
  // Skip if trust not yet accepted ï¿?the inner _executeApiKeyHelper check
...
export function refreshGcpAuth(gcpAuthRefresh: string): Promise<boolean> {
  ...
  // despite the name ï¿?print.ts emits its updates as generic SDK 'auth_status' messages.
  const authStatusManager = AwsAuthStatusManager.getInstance()
  authStatusManager.startAuthentication()
  ...
  (): { key: string; source: ApiKeySource } | null => {
    ...
    if (process.platform === 'darwin') {
      ...
      if (prefetch) {
        ...
        // Prefetch completed with no key ï¿?fall through to config, not keychain.
      } else {
...
export function saveOAuthTokensIfNeeded(tokens: OAuthTokens): {
  ...
  try {
    ...
    storageData.claudeAiOauth = {
      ...
      // stored subscription with null ï¿?fall back to the existing value.
      subscriptionType:
...
// re-reads ï¿?infinite /login regress (CC-1096, GH#24317).
async function invalidateOAuthCacheIfDiskChanged(): Promise<void> {
  try {
    ...
    // ENOENT ï¿?macOS keychain path (file deleted on migration). Clear only
...
// token simultaneously (common at startup ï¿?#20930), only one should clear
// caches and re-read the keychain. Without this, each call's clearOAuthTokenCache()
// nukes readInFlight in macOsKeychainStorage and triggers a fresh spawn ï¿?
...
async function handleOAuth401ErrorImpl(
  ...
  // Clear caches and re-read from keychain (async ï¿?sync read blocks ~100ms/call)
  clearOAuthTokenCache()
...
async function checkAndRefreshOAuthTokenIfNeededImpl(
  ...
  try {
    ...
    const refreshedTokens = await refreshOAuthToken(lockedTokens.refreshToken, {
      ...
      // CLAUDE_AI_OAUTH_SCOPES applies ï¿?this allows scope expansion
...
/**
 * Result of org validation ï¿?either success or a descriptive error.
 */
...
export async function validateForceLoginOrg(): Promise<OrgValidationResult> {
  ...
  if (!profile) {
    // Fail closed ï¿?we can't verify the org
    return {
      valid: false,
      message:
