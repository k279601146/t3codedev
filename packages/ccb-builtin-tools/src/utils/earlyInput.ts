...
import { lastGrapheme } from "./intl.ts"
...
export function startCapturingEarlyInput(): void {
  ...
  try {
    ...
    // consumeEarlyInput() â†?stopCapturingEarlyInput() long before this.
...
function processChunk(str: string): void {
  ...
  while (i < str.length) {
    ...
    if (code === 27) {
      ...
      // SS2 (N), SS3 (O) â€?2-byte sequences, just skip both
      // Other simple escape sequences: ESC <byte 0x40-0x7E> â€?just skip the one byte
      if (i < str.length) i++
      continue
    }
...
export function stopCapturingEarlyInput(): void {
  ...
  // unref is safe even when Ink does take over â€?the two ref/unref calls
  // balance out.
  try {
    process.stdin.unref()
  } catch {
  ...
  // Don't reset setRawMode here â€?Ink's App.handleSetRawMode(true)
