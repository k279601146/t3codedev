 ...
 * entry â€?sync statSync there triggers the slow-operation indicator on network/
 * slow disks).
 */
 ...
 * `     Nâ†’`). The padded-arrow format costs 9 bytes/line overhead; at
 * 1.35B Read calls Ã— 132 lines avg this is 2.18% of fleet uncached input
...
export function isCompactLinePrefixEnabled(): boolean {
  // 3P default: killswitch off = compact format enabled. Client-side only â€?
...
export function addLineNumbers({
  ...
  return lines
    .map((line, index) => {
      ...
      if (numStr.length >= 6) {
        return `${numStr}â†?{line}`
      }
      return `${numStr.padStart(6, ' ')}â†?{line}`
    })
    .join('\n')
}

/**
 * Inverse of addLineNumbers â€?strips the `Nâ†’` or `N\t` prefix from a single
...
export function writeFileSyncAndFlush_DEPRECATED(
  ...
  try {
    ...
    // ENOENT (doesn't exist) or EINVAL (not a symlink) â€?keep targetPath = filePath
  }
