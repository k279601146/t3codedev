 ...
 * cmd.exe ï¿?which can cause brief console window flashes in detached
...
export const windowsPathToPosixPath = memoizeWithLRU(
  (windowsPath: string): string => {
    ...
    // Already POSIX or relative ï¿?just flip slashes
    return windowsPath.replace(/\\/g, '/')
  },
  (p: string) => p,
  500,
...
export const posixPathToWindowsPath = memoizeWithLRU(
  (posixPath: string): string => {
    ...
    // Already Windows or relative ï¿?just flip slashes
    return posixPath.replace(/\//g, '\\')
  },
  (p: string) => p,
  500,
