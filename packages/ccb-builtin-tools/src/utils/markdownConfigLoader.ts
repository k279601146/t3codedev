 ...
 * vendored dep with its own `.git`), that nested root isn't the right boundary ï¿?
 ...
 *     (submodule/vendored clone ï¿?not a worktree, which resolves back to main)
...
function resolveStopBoundary(cwd: string): string | null {
  ...
  if (
    ...
    // Nested repo inside the project ï¿?skip past it, stop at the project's root.
...
export function getProjectDirsUpToHome(
  ...
  while (true) {
    ...
    // handling instead of existsSync ï¿?re-throws unexpected errors rather
...
export const loadMarkdownFilesForSubdir = memoize(
  async function (
    ...
    // worktree already has identical .claude/<subdir> content ï¿?loading the main
