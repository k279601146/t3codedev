...
// Re-entrancy guard: prevents getConfig ï¿?logEvent ï¿?getGlobalConfig ï¿?getConfig
...
export type GlobalConfig = {
  ...
  // /buddy companion soul ï¿?bones regenerated from userId on read. See src/buddy/.
  ...
  passesLastSeenRemaining?: number // Last seen remaining_passes count ï¿?reset upsell when it increases

  // Overage credit grant upsell tracking (keyed by org UUID ï¿?multi-org users).
  ...
  hasVisitedExtraUsage?: boolean // Whether the user has visited /extra-usage ï¿?hides credit upsells

  // Display language preference
  preferredLanguage?: 'auto' | 'en' | 'zh' // auto = follow system locale, en = English, zh = ä¸­æ–‡
  ...
  voiceLangHintLastLanguage?: string // Resolved STT language code when the hint was last shown ï¿?reset count when it changes
  ...
  // Push-notification toggles (set via /config). Default off ï¿?explicit opt-in required.
  ...
  // `expiresAt` is the dedup key ï¿?content-addressed, self-clears when /login
  ...
  // Opus 4.0/4.1 ï¿?current Opus migration (shows one-time notif)
  legacyOpusMigrationTimestamp?: number

  // Sonnet 4.5 ï¿?4.6 migration (pro/max/team premium)
  ...
  copyOnSelect?: boolean // Auto-copy to clipboard on mouse-up (undefined ï¿?true; lets cmd+c "work" via no-op)
  ...
  claudeCodeHints?: {
    ...
    // 100 entries to bound config growth ï¿?past that, hints stop entirely.
    plugin?: string[]
 ...
 * shared constant ï¿?the nested containers (arrays, records) are all empty, so
...
export function checkHasTrustDialogAccepted(): boolean {
  ...
  // so once true we can latch it. false is not cached ï¿?it gets re-checked
 ...
 * the memoized project path ï¿?use when the target dir differs from cwd (e.g.
...
// changed ï¿?a stalled stat never blocks the main thread.
function startGlobalConfigFreshnessWatcher(): void {
  ...
  watchFile(
    ...
    curr => {
      // Our own writes fire this too ï¿?the write-through's Date.now()
      ...
      // (initial callback or deletion) ï¿?the <= handles that too.
...
export function getGlobalConfig(): GlobalConfig {
  ...
  // Fast path: pure memory read. After startup, this always hits ï¿?our own
  ...
  // self-corrects (old mtime + new content ï¿?watcher re-reads next tick).
  configCacheMisses++
  try {
 ...
 *   1. User's explicit config value (always wins ï¿?honors opt-out)
...
function getConfig<A>(
  ...
  try {
    ...
    if (error instanceof ConfigParseError) {
      ...
      // Guard: logEvent ï¿?shouldSampleEvent ï¿?getGlobalConfig ï¿?getConfig
...
export function getAutoUpdaterDisabledReason(): AutoUpdaterDisabledReason | null {
  ...
  // æœ¬é¡¹ç›®é»˜è®¤å…³é—­è‡ªåŠ¨æ›´æ–°ï¼›é€šè¿‡ ENABLE_AUTOUPDATER=1 æ˜¾å¼å¼€ï¿?
