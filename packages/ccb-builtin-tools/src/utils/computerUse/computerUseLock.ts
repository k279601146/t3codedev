 ...
 *
 * Note: there is a small window for PID reuse â€?if the owning process
 ...
 * `list_granted_applications` â€?the package's `defersLockAcquire` contract:
 ...
 * `request_access`. Does NOT create â€?that's `tryAcquireComputerUseLock`'s job.
 */
 ...
 *
 * `{kind: 'acquired', fresh: true}` â€?first tool call of a CU turn. Callers fire
 * enter notifications on this. `{kind: 'acquired', fresh: false}` â€?re-entrant,
 * same session already holds it. `{kind: 'blocked', by}` â€?another live session
 * holds it.
 *
 * Uses O_EXCL (open 'wx') for atomic test-and-set â€?the OS guarantees at
...
export async function tryAcquireComputerUseLock(): Promise<AcquireResult> {
  ...
  // Corrupt/unparseable â€?treat as stale (can't extract a blocking ID).
  if (!existing) {
    await unlink(getLockPath()).catch(() => {})
    if (await tryCreateExclusive(lock)) {
      registerLockCleanup()
  ...
  // Another live session holds it â€?blocked.
  if (isProcessRunning(existing.pid)) {
    return { kind: 'blocked', by: existing.sessionId }
  }

  // Stale lock â€?recover. Unlink then retry the exclusive create.
 ...
 * `true` if we actually unlinked the file (i.e., we held it) â€?callers fire
