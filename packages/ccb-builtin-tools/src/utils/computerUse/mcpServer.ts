...
import { initializeAnalyticsSink } from "@t3tools/ccb-engine/src/services/analytics/sink.ts"
import { StdioServerTransport } from '@modelcontextprotocol/sdk/server/stdio.js'
import { ListToolsRequestSchema } from '@modelcontextprotocol/sdk/types.js'
import { homedir } from 'os'

import { shutdownDatadog } from "@t3tools/ccb-engine/src/services/analytics/datadog.ts"
import { shutdown1PEventLogging } from "@t3tools/ccb-engine/src/services/analytics/firstPartyEventLogger.ts"
...
/**
 * Enumerate installed apps, timed. Fails soft â€?if Spotlight is slow or
...
async function tryGetInstalledAppNames(): Promise<string[] | undefined> {
  ...
  if (!installed) {
    // The enumeration continues in the background â€?swallow late rejections.
    void enumP.catch(() => {})
    logForDebugging(
 ...
 *
 * Async so the 1s app-enumeration timeout doesn't block startup â€?called from
 ...
 * `runClaudeInChromeMcpServer` â€?stdio transport, exit on stdin close,
