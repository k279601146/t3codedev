...
// timeout â€?unhide should be ~instant; if it takes 5s something is wrong
...
export async function cleanupComputerUseAfterTurn(
  ...
  // CU session ends. Idempotent â€?no-ops if registration failed at acquire.
  // Swallow throws so a NAPI unregister error never prevents lock release â€?
