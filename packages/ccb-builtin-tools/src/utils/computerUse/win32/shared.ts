 ...
 * Single source of truth â€?no more duplication across files.
 */

/** Validate HWND is a pure numeric string â€?prevents PowerShell/Python injection. */
export function validateHwnd(hwnd: string): string {
  if (!/^\d+$/.test(hwnd)) {
    throw new Error(`Invalid HWND: "${hwnd}" â€?must be numeric`)
  }
  return hwnd
}
...
/** Virtual key code mapping â€?canonical, complete. */
