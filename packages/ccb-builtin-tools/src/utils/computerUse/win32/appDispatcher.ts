 ...
 * - .xlsx/.xls/.csv â†?Excel COM (headless, no window)
 * - .docx/.doc      â†?Word COM (headless, no window)
 * - .txt/.log/.md   â†?notepad + SendMessage + HWND bind (offscreen)
 * - Others          â†?generic exe + HWND bind (offscreen)
 */
  ...
  switch (type) {
    case 'excel': {
      ...
      const { createExcel, openExcel } =
        require('./comExcel.ts') as typeof import('./comExcel.ts')
      const isExisting = nameOrPath.match(/\.(xlsx|xls|csv|xlsm|xlsb)$/i)
      if (isExisting) {
        // Open existing file â€?just verify it's readable
        try {
      ...
      // "excel" or "excel.exe" without a file â€?create new
    ...
    case 'word': {
      ...
      const { createWord, openWord } =
        require('./comWord.ts') as typeof import('./comWord.ts')
    ...
    default:
      // text/browser/generic â€?HWND bind handled by caller (platforms/win32.ts open())
      return { type }
  }
}
