 ...
 * No SendInput / keybd_event / SendKeys â€?those are global and conflict with the user.
 *
 * Text input strategy:
 * 1. Short text (â‰?CLIPBOARD_THRESHOLD chars): SendMessageW(WM_CHAR) per codepoint
...
import { validateHwnd, runPs, VK_MAP, MODIFIER_KEYS } from "./shared.ts"

/** Character count above which we switch to clipboard paste */
const CLIPBOARD_THRESHOLD = 32

/** Cache findEditChild results â€?window structure doesn't change while bound */
...
public class WinMsg {
    ...
    // CRITICAL: CharSet.Unicode â†?resolves to SendMessageW
 ...
 *
 * Strategy:
 * 1. EnumChildWindows â€?search for known edit control class names
 * 2. UI Automation fallback â€?find the first Edit/Document element and get its native HWND
 *
 ...
 * NO global APIs (SendInput/keybd_event/SendKeys) â€?only window-targeted messages.
 */
 ...
 * Always uses the WM_CHAR path â€?reliable across all window types including
 ...
 * Uses PostMessage (async) instead of SendMessage â€?required for Windows Terminal
 ...
 * All via SendMessageW â€?no global APIs.
 */
export function sendKeys(hwnd: string, combo: string[]): boolean {
  ...
  for (const key of combo) {
    ...
    if (vk !== undefined) {
      ...
      // Single character â€?use its uppercase VK code
  ...
  // Uses PostMessage (async) + correct lParam (scan code) â€?required for
...
// â”€â”€ Console Input Buffer (WriteConsoleInput) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
 ...
 * Directly injects into the Console Input Buffer â€?works for raw-mode stdin.
 */
 ...
 * Via SendMessageW â€?window-targeted, no cursor movement.
 */
 ...
 * Via SendMessageW(WM_LBUTTONDOWN) â€?window-targeted, no cursor movement.
 */
 ...
 * Via SendMessageW(WM_LBUTTONUP) â€?window-targeted, no cursor movement.
 */
 ...
 * Used during drag operations. Via SendMessageW â€?window-targeted.
 */
 ...
 *
 * Works on Excel, browsers, modern UI â€?unlike WM_VSCROLL/WM_HSCROLL
