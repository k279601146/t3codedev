/**
 * Visual indicator for bound windows â€?DWM native border color.
 *
 * Uses DwmSetWindowAttribute(DWMWA_BORDER_COLOR) to set a green border
 * on the bound window. The border:
 * - Is the window's OWN border, not an overlay â€?zero offset, zero shadow issues
...
import { validateHwnd, ps } from "./shared.ts"
...
export function markBound(hwnd: string): boolean {
  ...
  // Green: R=0, G=200, B=0 â†?0x0000C800
  const hr = ps(
    `Add-Type @'
using System;
...
/**
 * Kill all borders â€?just reset all bound windows.
 * With DWM approach, no processes to kill.
 */
export function cleanupAllBorders(): void {
  // DWM border color is a window attribute â€?it resets automatically
