...
import { listWindows } from "../win32/windowEnum.ts"
import { detectAppType, openWithController } from "../win32/appDispatcher.ts"
import {
  markBound,
  unmarkBound,
  cleanupAllBorders,
} from "../win32/windowBorder.ts"
import {
  showVirtualCursor,
  hideVirtualCursor,
  moveVirtualCursor,
} from "../win32/virtualCursor.ts"
import { showIndicator, hideIndicator } from "../win32/inputIndicator.ts"
import {
  ps,
  psAsync,
  validateHwnd,
...
let _bridge: typeof import('../win32/bridgeClient.ts') | undefined
function getBridge() {
  if (!_bridge) {
    try {
      _bridge =
        require('../win32/bridgeClient.ts') as typeof import('../win32/bridgeClient.ts')
    } catch {}
  }
  return _bridge
}
...
// validateHwnd, ps, psAsync, VK_MAP, MODIFIER_KEYS imported from "../win32/shared.ts"
...
// VK_MAP and MODIFIER_KEYS imported from "../win32/shared.ts"

// ---------------------------------------------------------------------------
// Session-level HWND binding â€?all operations target this handle
// ---------------------------------------------------------------------------

let boundHwnd: string | null = null
let boundPid: number | null = null
let boundAppType: import('../win32/appDispatcher.ts').AppType | null = null
...
/** Bind to a window HWND â€?all subsequent input/screenshot operations target this handle */
export function bindWindow(hwnd: string, pid?: number): void {
  ...
  //    Save current foreground â†?activate target â†?restore original foreground.
...
/** Bind to a COM-controlled file (Excel/Word â€?no window needed) */
export function bindFile(
  filePath: string,
  appType: import('../win32/appDispatcher.ts').AppType,
): void {
  boundHwnd = null
  boundPid = null
...
/** Unbind â€?revert to global mode, remove overlays */
...
let _wm: typeof import('../win32/windowMessage.ts') | undefined
function getWm() {
  // eslint-disable-next-line @typescript-eslint/no-require-imports
  return (_wm ??=
    require('../win32/windowMessage.ts') as typeof import('../win32/windowMessage.ts'))
}

// ---------------------------------------------------------------------------
// Input â€?ALL text/key input goes through SendMessage when HWND is bound.
// Global SendInput/keybd_event is DISABLED to avoid interfering with user.
// ---------------------------------------------------------------------------

// ---------------------------------------------------------------------------
// Input â€?When HWND is bound, ALL operations go through SendMessage.
...
const input: InputPlatform = {
  ...
  async typeText(text) {
    ...
    if (boundAppType === 'word' && boundFilePath) {
      // eslint-disable-next-line @typescript-eslint/no-require-imports
      const { appendText } =
        require('../win32/comWord.ts') as typeof import('../win32/comWord.ts')
...
const screenshot: ScreenshotPlatform = {
  ...
  async captureWindow(hwnd) {
    // Python Bridge (ctypes PrintWindow + GDI â†?Pillow JPEG, ~300ms)
...
// ---------------------------------------------------------------------------
// Apps â€?EnumWindows + registry + AppxPackage
// ---------------------------------------------------------------------------
...
`)
  ...
  async open(name) {
    ...
    // Excel/Word â†?COM automation (no window, no HWND)
    ...
    // Text/Browser/Generic â†?exe launch + HWND bind (offscreen)
    ...
    if (name.includes('_') && name.includes('.')) {
      // Microsoft.WindowsNotepad_xxx â†?Notepad
      // Microsoft.WindowsCalculator_xxx â†?Calculator
      // Microsoft.WindowsTerminal_xxx â†?Terminal
      const parts = name.split('_')[0]?.split('.') ?? []
      const appPart = parts[parts.length - 1] ?? name
      // Strip "Windows" prefix: WindowsNotepad â†?Notepad
...
# 3. Search registry for install location by display name â†?find .exe
if (-not $proc) {
...
for ($i = 0; $i -lt 50; $i++) {
    ...
    if ($hwnd -ne 0) { break }
    # PID didn't match (process redirect) â€?accept new window matching title hint
    if ($i -gt 10) {
...
# Move offscreen instead of minimizing â€?keeps window restored so
...
`)
    if (!result) {
      throw new Error(
        `open(): failed to launch '${name}' â€?no output from launcher script`,
      )
    }
    if (result.startsWith('LAUNCH_FAILED')) {
      throw new Error(
        `open(): failed to launch '${name}' â€?process did not start (${result})`,
      )
    }
    ...
    if (hwnd && hwnd !== '0') {
      // Bind to the launched window â€?all subsequent operations target this HWND
      bindWindow(hwnd, pid)
    }
  },
...
// ---------------------------------------------------------------------------
// Window Management â€?Win32 API calls targeted at bound HWND.
...
import type { WindowManagementPlatform, WindowAction } from "./types.ts"
...
const windowManagement: WindowManagementPlatform = {
  manageWindow(action: WindowAction, opts?): boolean {
    ...
    switch (action) {
      case 'minimize': {
        // ShowWindow(SW_MINIMIZE) â€?targeted at HWND, not global
        const r = ps(
      ...
      case 'close': {
        // SendMessage(WM_CLOSE) â€?graceful close targeted at HWND
...
`)
      ...
      case 'move_offscreen': {
        // Move to -32000,-32000 â€?keeps window in restored state for SendMessage/PrintWindow
        ps(
