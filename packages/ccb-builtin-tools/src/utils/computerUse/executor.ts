...
import { drainRunLoop } from "./drainRunLoop.ts"
import { notifyExpectedEscape } from "./escHotkey.ts"
...
export function createCliExecutor(opts: {
  ...
  if (process.platform !== 'darwin') {
    ...
    const { createCrossPlatformExecutor } =
      require('./executorCrossPlatform.ts') as typeof import('./executorCrossPlatform.ts')
