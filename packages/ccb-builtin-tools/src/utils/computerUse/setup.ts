...
import { buildMcpToolName } from "@t3tools/ccb-engine/src/services/mcp/mcpStringUtils.ts"
 ...
 * `allowedTools` so they bypass the normal permission prompt â€?the package's
...
export function setupComputerUseMCP(): {
  ...
  // command/args are never spawned â€?client.ts intercepts by name and
