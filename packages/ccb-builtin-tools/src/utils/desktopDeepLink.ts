...
import { gte as semverGte } from "./semver.ts"
...
export async function getDesktopInstallStatus(): Promise<DesktopInstallStatus> {
  ...
  try {
    ...
    // Best effort â€?proceed with handoff if version detection fails
    return { status: 'ready', version: 'unknown' }
  }

  if (!version) {
    // Can't determine version â€?assume it's ready (dev mode or unknown install)
    return { status: 'ready', version: 'unknown' }
  }
