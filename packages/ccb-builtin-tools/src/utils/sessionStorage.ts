...
// Sync fs primitives for readFileTailSync é”?separate from fs/promises
...
import { REPL_TOOL_NAME } from '@claude-code-best/builtin-tools/tools/REPLTool/constants.ts'
import {
  type AgentId,
  asAgentId,
  asSessionId,
 ...
 * marker. Kept in sync with sessionStoragePortable.ts é”?generic pattern avoids
 * an ever-growing allowlist that falls behind as new notification types ship.
 */
// 50MB é”?prevents OOM in the tombstone slow path which reads + rewrites the
...
export function getTranscriptPathForSession(sessionId: string): string {
  ...
  // é”?different directories, so the hook sees MISSING (gh-30217). CC-34
  ...
  //
  // For OTHER session IDs we can only guess via originalCwd é”?we don't
  // track a sessionIdéˆ«æŠªrojectDir map. Callers wanting a specific other
...
// 50 MB é”?session JSONL can grow to multiple GB (inc-3930). Callers that
// read the raw transcript must bail out above this threshold to avoid OOM.
export const MAX_TRANSCRIPT_READ_BYTES = 50 * 1024 * 1024

// In-memory map of agentId é”?subdirectory for grouping related subagent
...
export function getAgentTranscriptPath(agentId: AgentId): string {
  // Same sessionProjectDir consistency as getTranscriptPathForSession é”?
...
export type AgentMetadata = {
   ...
   * of a placeholder. Optional é”?older metadata files lack this field. */
  description?: string
}

/**
 * Persist the agentType used to launch a subagent. Read by resume to
 * route correctly when subagent_type is omitted é”?without this, resuming
...
export type RemoteAgentMetadata = {
  ...
  /** CCR session ID é”?used to fetch live status from the Sessions API on resume. */
  sessionId: string
  title: string
  command: string
  spawnedAt: number
...
function getRemoteAgentsDir(): string {
  // Same sessionProjectDir fallback as getAgentTranscriptPath é”?the project
 ...
 * from CCR on restore é”?only identity is persisted locally.
 */
...
> {
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      // Skip unreadable or corrupt files é”?a partial write from a crashed
...
// (PostToolUse path, 5è„?turn) + various save* functions. Input is a cwd
// string; homedir/env/regex are all session-invariant so the result is
// stable for a given input. Worktree switches just change the key é”?no
// cache clear needed.
  ...
  if (!project) {
    ...
    if (!cleanupRegistered) {
      registerCleanup(async () => {
        ...
        // fields é”?if enough messages are appended after a /rename, the
        ...
        try {
          ...
          // Best-effort é”?don't let metadata re-append crash the cleanup
        }
      })
...
class Project {
  ...
  // on the first user/assistant message é”?prevents metadata-only session files.
  ...
  private async appendToFile(filePath: string, data: string): Promise<void> {
    try {
      ...
      // Directory may not exist é”?some NFS-like filesystems return
   ...
   * our stale cache absorbs it and the re-append below persists it é”?not
   ...
   * external-writer concern é”?their caches are authoritative.
   */
  reAppendSessionMetadata(skipTitleRefresh = false): void {
    ...
    // failure é”?extract returns null é”?cache is the only source of truth.
    const tail = readFileTailSync(this.sessionFile)

    // Absorb any fresher SDK-written title/tag into our cache. If the SDK
    // wrote while we had the session open, our cache is stale é”?the tail
  ...
  private async materializeSessionFile(): Promise<void> {
    // Guard here too é”?reAppendSessionMetadata writes via appendEntryToFile
    ...
    return this.trackWrite(async () => {
      ...
      for (const message of messages) {
        ...
        const transcriptMessage: TranscriptMessage = {
          ...
          // loadFullLog's sessionId-keyed contentReplacements lookup misses é”?
          // replacement records lost é”?FROZEN misclassification.
      ...
      // Cache this turn's user prompt for reAppendSessionMetadata é”?
  ...
  async appendEntry(entry: Entry, sessionId: UUID = getSessionId() as UUID) {
    ...
    if (entry.type === 'summary') {
      ...
      // Always append. Last-wins on restore é”?later entries supersede.
      void this.enqueueWrite(sessionFile, entry)
    } else {
      ...
      if (entry.type === 'queue-operation') {
        ...
        // Skip dedup for agent sidechain LOCAL writes é”?they go to a separate
        ...
        // context).
        //
        // The sidechain bypass applies ONLY to the local file write é”?remote
        ...
        // exhausts retries é”?gracefulShutdownSync(1). See inc-4718.
        const isNewUuid = !messageSet.has(entry.uuid)
        if (isAgentSidechain || isNewUuid) {
          // Enqueue write é”?appendToFile handles ENOENT by creating directories
          void this.enqueueWrite(targetFile, entry)

          if (!isAgentSidechain) {
            // messageSet is main-file-authoritative. Sidechain entries go to a
            // separate agent file é”?adding their UUIDs here causes recordTranscript
            ...
            // UUID the main thread hasn't written yet é”?409 when main writes it.
...
// UUIDs instead of the post-compact summary é”?orphaning the compact boundary.
//
...
//    trajectory): recorded messages are always a prefix é”?tracked é”?correct
//    parent chain for new messages.
//  - Compaction (useLogMessages): new CB/summary appear FIRST, then recorded
//    messagesToKeep é”?not a prefix é”?not tracked é”?CB gets parentUuid=null
...
export async function recordTranscript(
  ...
  // already in messageSet). Progress is skipped é”?it's written to the JSONL
 ...
 * mode/tag/agent).
 *
 * Setting sessionFile here é”?instead of waiting for materializeSessionFile
 * on the first user message é”?lets the exit cleanup handler's
 ...
 * no-op é”?unless --name was used, in which case it would clobber the fresh
 ...
 * restore é”?the loader keeps only the most recent snapshot entry.
 */
 ...
 * The server handles compaction filtering é”?it returns events starting
 ...
 * parentUuids (recordTranscript dedup-skipped them é”?can't rewrite).
 * The internal chain (keep[i+1]éˆ«æŠžeep[i]) is intact; only endpoints need
 * patching: headéˆ«æŠ‹nchor, and anchor's other childrenéˆ«æŠ°ail. Anchor is the
 * last summary for suffix-preserving, boundary itself for prefix-preserving.
 *
 * Only the LAST seg-boundary is relinked é”?earlier segs were summarized
...
function applyPreservedSegmentRelinks(
  ...
  // manual /compact after reactive compact é”?seg is stale).
  ...
  // No seg anywhere é”?no-op. findUnresolvedToolUse etc. read the full map.
  if (!lastSeg) return

  // Seg stale (no-seg boundary came after): skip relink, still prune at
  // absolute é”?otherwise the stale preserved chain becomes a phantom leaf.
  const segIsLive = lastSegBoundaryIdx === absoluteLastBoundaryIdx

  // Validate tailéˆ«æŠ™ead BEFORE mutating so malformed metadata is a true
  ...
  if (segIsLive) {
    ...
    if (!reachedHead) {
      // tailéˆ«æŠ™ead walk broke é”?a UUID in the preserved segment isn't in the
  ...
  if (segIsLive) {
    ...
    // Tail-splice: anchor's other children é”?tail. No-op if already pointing
    ...
    // (~190K) é”?stripStaleUsage only patched in-memory copies that were
    // dedup-skipped. Without this, resume é”?immediate autocompact spiral.
  ...
  // isn't preserved. preservedUuids empty when !segIsLive é”?full prune.
  const toDelete: UUID[] = []
 ...
 * and resume immediately PTLs (adamr-20260320-165831: 397K displayed é”?1.65M
 * actual).
 *
 ...
 * exact removal on load. Older boundaries without removedUuids are skipped é”?
...
function applySnipRemovals(messages: Map<UUID, TranscriptMessage>): void {
  // Structural check é”?snipMetadata only exists on the boundary subtype.
  ...
  // behavior é”?same as if compact truncated there, which it did).
 ...
 * é”?N parallel tool_uses é”?N messages, distinct uuid, same message.id. Each
 ...
 *   1. Sibling assistant orphaned: walk goes prevéˆ«æŠ‹sstAéˆ«æ‰µR_Aéˆ«æŠ§ext, drops asstB
...
function recoverOrphanedParallelToolResults(
  ...
  // already in chain order, so later iterations overwrite é”?last wins.
  ...
  // TRs indexed by parentUuid é”?insertMessageChain:~894 already wrote that
 ...
 * writeéˆ«æŠ£oad round-trip drift é”?the class of bugs where snip/compact/
 * parallel-TR operations mutate in-memory but the parentUuid walk on disk
 * reconstructs a different set (adamr-20260320-165831: 397K displayed é”?
 * 1.65M actual on resume).
 *
 * delta > 0: resume loaded MORE than in-session (the usual failure mode)
 * delta < 0: resume loaded FEWER (chain truncation é”?#22453 class)
 * delta = 0: round-trip consistent
 *
 * Called from loadConversationForResume é”?fires once per resume, not on
...
function buildFileHistorySnapshotChain(
  ...
  // messageId é”?last index in snapshots[] for O(1) update lookup
 ...
 *   at EOF é”?avoiding the clobber-on-resume bug where a stale AI title
 ...
 * not re-appended by reAppendSessionMetadata é”?it's a rolling snapshot of
...
export function saveWorktreeState(
  ...
  // may pass via full WorktreeSession objects é”?TypeScript structural typing
  ...
  // For --worktree startup, sessionFile is null é”?materializeSessionFile
...
export async function loadFullLog(log: LogOption): Promise<LogOption> {
  ...
  try {
    ...
    // Leaf's sessionId é”?forked sessions copy chain[0] from the source, but
 ...
 * Uses raw Buffer chunks and byte-level marker matching é”?no readline, no per-line
 * string conversion for the ~99% of lines that are message content.
 *
 * Fast path: if a chunk contains zero markers (the common case é”?metadata entries
...
async function scanPreBoundaryMetadata(
  ...
  for await (const chunk of stream) {
    ...
    if (hasAnyMarker) {
      ...
      // No markers in this chunk é”?just preserve the incomplete trailing line
    ...
    // are <1 KB, so if carry exceeds this we're mid-message-content é”?drop it.
 ...
 *      entries serialize a nested Message in data BEFORE uuid é”?both can
 ...
 * none are at depth 1 (shouldn't happen for well-formed JSONL é”?depth-1 is
 * where the top-level object's fields live).
 *
 * Only called when é”? suffix matches exist (agent_progress with a nested
 * Message, or mcpMeta with a coincidentally-suffixed object). Cost is
 * O(max(candidates) - lineStart) é”?one forward byte pass, stopping at the
 * first depth-1 hit.
 */
...
function walkChainBeforeParse(buf: Buffer): Buffer {
  ...
  while (pos < len) {
    ...
    if (
      ...
      //     serialized BEFORE top-level uuid é”?that inner Message has its
      //     own uuid,timestamp adjacent, so its bytes also satisfy the
      //     suffix check.
      //   - mcpMeta/toolUseResult come AFTER top-level uuid and hold
      //     server-controlled Record<string,unknown> é”?a server returning
      ...
      // variants put timestamp BEFORE uuid é”?`"uuid":"<36>"}` at EOL)
...
export async function loadTranscriptFile(
  ...
  >()
  // Array, not Map é”?commit order matters (nested collapses).
  const contextCollapseCommits: ContextCollapseCommitEntry[] = []
  // Last-wins é”?later entries supersede.
  ...
  try {
    ...
    // file size é”?a 151 MB session that is 84% stale attr-snaps allocates
    ...
    if (!isEnvTruthy(process.env.CLAUDE_CODE_DISABLE_PRECOMPACT_SKIP)) {
      ...
      if (size > SKIP_PRECOMPACT_THRESHOLD) {
        ...
        // boundary already truncated é”?in which case the preserved messages
    ...
    // attr-snaps already stripped at the fd level é”?the <5MB readFile path
    ...
    // the post-boundary buffer is harmless é”?later values overwrite earlier ones.
    ...
    // Bridge map for legacy progress entries: progress_uuid é”?progress_parent_uuid.
    ...
    // append-only (parents before children), we record each progresséˆ«æŠªarent link
    ...
    for (const entry of entries) {
      // Legacy progress check runs before the Entry-typed else-if chain é”?
...
export async function getLastSessionLog(
  ...
  // may call getLastSessionLog on the current session é”?overwriting a live cache
...
export async function loadMessageLogs(limit?: number): Promise<LogOption[]> {
  ...
  // fetchLogs returns lite (stat-only) logs é”?enrich them to get metadata.
  ...
  // enrichLogs returns fresh unshared objects é”?mutate in place to avoid
...
async function loadAllProjectsMessageLogsFull(
  ...
  // Deduplicate é”?same session+leaf can appear in multiple project dirs.
  ...
  // deduped values are fresh from getLogsWithoutIndex é”?safe to mutate
...
export async function loadAllProjectsMessageLogsProgressive(
  ...
  // Deduplicate é”?same session can appear in multiple project dirs
  const sorted = deduplicateLogsBySessionId(rawLogs)

  const { logs, nextIndex } = await enrichLogs(sorted, 0, initialEnrichCount)

  // enrichLogs returns fresh unshared objects é”?safe to mutate in place
  logs.forEach((log, i) => {
    log.value = i
  })
...
async function getStatOnlyLogsForWorktrees(
  ...
  // Deduplicate by sessionId é”?the same session can appear in multiple
...
// Globs the session's subagents dir directly é”?unlike AppState.tasks, this survives task eviction.
export async function loadAllSubagentTranscriptsFromDisk(): Promise<{
  ...
  // Filename format is the inverse of getAgentTranscriptPath() é”?keep in sync.
 ...
 * transformed é”?recordTranscript receives incremental slices where the REPL
 ...
 * Returns a map of sessionId é”?{path, mtime, ctime, size}.
 ...
 * means ~6.4 MB of I/O é”?fast on any modern filesystem while giving users
...
export async function loadAllLogsFromSessionFile(
  ...
  // Build parentUuid é”?children index once (O(n)), so trailing-message lookup is O(1) per leaf
...
async function readLiteMetadata(
  ...
  // Prefer the last-prompt tail entry é”?captured by extractFirstPrompt at
  ...
  // PR link fields é”?prNumber is a number not a string, so try both
...
function extractFirstPromptFromChunk(chunk: string): string {
    ...
    try {
      ...
      for (const text of texts) {
        ...
        if (commandNameTag) {
          ...
          // Custom command with meaningful args é”?use clean display
          return commandArgs
  ...
  // Session started with a slash command but had no subsequent real message é”?
  ...
  // Proactive sessions have only tick messages é”?give them a synthetic prompt
 ...
 * No file reads é”?instant. Call `enrichLogs` to enrich
...
export async function getSessionFilesLite(
  ...
  // logs are freshly pushed above é”?safe to mutate in place
 ...
 * (no firstPrompt, no customTitle é”?e.g., metadata-only session files).
 */
