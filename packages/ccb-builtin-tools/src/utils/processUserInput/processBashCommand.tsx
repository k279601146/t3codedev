...
import { BashModeProgress } from "@t3tools/ccb-engine/src/components/BashModeProgress.ts";
import type { SetToolJSXFn } from "@t3tools/ccb-engine/src/Tool.ts";
import { BashTool } from '@claude-code-best/builtin-tools/tools/BashTool/BashTool.tsx';
import type { AttachmentMessage, SystemMessage, UserMessage } from "@t3tools/ccb-engine/src/types/message.ts";
import type { ShellProgress } from "@t3tools/ccb-engine/src/types/tools.ts";
...
export async function processBashCommand(
  ...
  // Shell routing (docs/design/ps-shell-selection.md Â§5.2): consult
  ...
  try {
    ...
    // Progress UI â€?shared across both shell backends (both emit ShellProgress)
    ...
    // in shouldUseSandbox.ts). PS sandbox is Linux/macOS/WSL2 only â€?on Windows
    ...
    type PSMod = typeof import('@claude-code-best/builtin-tools/tools/PowerShellTool/PowerShellTool.tsx');
    let PowerShellTool: PSMod['PowerShellTool'] | null = null;
    if (usePowerShell) {
      /* eslint-disable @typescript-eslint/no-require-imports */
      PowerShellTool = (require('@claude-code-best/builtin-tools/tools/PowerShellTool/PowerShellTool.tsx') as PSMod)
        .PowerShellTool;
