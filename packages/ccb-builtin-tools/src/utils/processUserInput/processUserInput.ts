...
import { getContentText } from "@t3tools/ccb-engine/src/utils/messages.ts"
import {
  findCommand,
...
} from "@t3tools/ccb-engine/src/types/textInputTypes.ts"
import {
...
export async function processUserInput({
  ...
  // Skip for isMeta (system-generated prompts like scheduled tasks) ï¿?those
...
function applyTruncation(content: string): string {
  if (content.length > MAX_HOOK_OUTPUT_LENGTH) {
    return `${content.substring(0, MAX_HOOK_OUTPUT_LENGTH)}ï¿?[output truncated - exceeded ${MAX_HOOK_OUTPUT_LENGTH} characters]`
  }
  return content
}
...
async function processUserInputBase(
  ...
  // blocks actually reach the API ï¿?otherwise the resize work above is
  ...
  // immediate-command fast paths). Resolve the command here ï¿?if it passes
  ...
  if (bridgeOrigin && inputString !== null && inputString.startsWith('/')) {
    ...
    // Unknown /foo or unparseable ï¿?fall through to plain text, same as
    // pre-#19134. A mobile user typing "/shrug" shouldn't see "Unknown skill".
  }

  // Ultraplan keyword ï¿?route through /ultraplan. Detect on the
  ...
  // so routing to /ultraplan there yields "Unknown skill" ï¿?and there's no
  // rainbow animation in print mode anyway.
  // Runs before attachment extraction so this path matches the slash-command
  // path below (no await between setUserInputOnProcessing and setAppState ï¿?
  ...
  // Slash commands
  // Skip for remote bridge messages ï¿?input from CCR clients is plain text
  if (
    inputString !== null &&
    !effectiveSkipSlash &&
    inputString.startsWith('/')
