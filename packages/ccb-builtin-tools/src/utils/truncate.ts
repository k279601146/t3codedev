// Width-aware truncation/wrapping â€?needs ink/stringWidth (not leaf-safe).
 ...
 * "../components/â€?MyComponent.tsx" when maxLength is 30.
 *
...
export function truncatePathMiddle(path: string, maxLength: number): string {
  ...
  // Need at least room for "â€? + something meaningful
  if (maxLength < 5) {
  ...
  // Result format: directory + "â€? + filename
  ...
  return truncatedDir + 'â€? + filename
}
 ...
 * Appends 'â€? when truncation occurs.
 */
 ...
 * Prepends 'â€? when truncation occurs.
...
export function truncateStartToWidth(text: string, maxWidth: number): string {
  ...
  for (let i = segments.length - 1; i >= 0; i--) {
    ...
    if (width + segWidth > maxWidth - 1) break // -1 for 'â€?
    width += segWidth
    startIdx = i
  }
  return (
 ...
 * Useful when the caller adds its own separator (e.g. middle-truncation with 'â€? between parts).
 * Width-aware and grapheme-safe.
 */
