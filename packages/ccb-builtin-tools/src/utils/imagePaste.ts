...
import { getImageProcessor } from '@claude-code-best/builtin-tools/tools/FileReadTool/imageProcessor.ts'
...
// ï¿?module-scope helpers are NOT tree-shaken (see docs/feature-gating.md).
...
function getClipboardCommands() {
  ...
  > = {
    darwin: {
      ...
      saveImage: `osascript -e 'set png_data to (the clipboard as Â«class PNGfÂ»)' -e 'set fp to open for access POSIX file "${screenshotPath}" with write permission' -e 'write png_data to fp' -e 'close access fp'`,
...
export async function hasImageInClipboard(): Promise<boolean> {
  ...
  const result = await execFileNoThrowWithCwd('osascript', [
    '-e',
    'the clipboard as Â«class PNGfÂ»',
  ])
...
export async function getImageFromClipboard(): Promise<ImageWithDimensions | null> {
  ...
  // dimension cap. ~5ms cold, sub-ms warm ï¿?vs. ~1.5s for the osascript
  ...
  if (
    ...
    try {
      ...
      // 2000Ã—2000 PNG can still exceed the 3.75MB raw / 5MB base64 API
      // limit ï¿?for that edge case, run through the same size-cap that
  ...
  try {
    ...
    // BMP is not supported by the API ï¿?convert to PNG via Sharp.
 ...
 * MIME_BY_EXT in BriefTool/upload.ts ï¿?attachments.ts uses this to set isImage
 ...
 * /preview variant ï¿?broken thumbnail.
 */
