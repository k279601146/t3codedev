...
import { type OptionWithDescription, Select } from "@t3tools/ccb-engine/src/components/CustomSelect/index.ts";
...
export function It2SetupPrompt({ onDone, tmuxAvailable }: Props): React.ReactNode {
  ...
  function renderInstalling(): React.ReactNode {
    return (
      <Box flexDirection="column" gap={1}>
        <Box>
          <Spinner />
          <Text> Installing it2 using {packageManager}â€?/Text>
        </Box>
  ...
  function renderApiInstructions(): React.ReactNode {
    ...
    return (
      <Box flexDirection="column" gap={1}>
        <Text color="success">âœ?it2 installed successfully</Text>
        ...
        <Box marginTop={1}>
          <Text dimColor>Press Enter when ready to verifyâ€?/Text>
        </Box>
      </Box>
    );
  }
  ...
  function renderVerifying(): React.ReactNode {
    return (
      <Box>
        <Spinner />
        <Text> Verifying it2 can communicate with iTerm2â€?/Text>
      </Box>
    );
  }
  ...
  function renderSuccess(): React.ReactNode {
    return (
      <Box flexDirection="column">
        <Text color="success">âœ?iTerm2 split pane support is ready</Text>
    ...
    return (
      <Box flexDirection="column" gap={1}>
        ...
        <Box flexDirection="column" paddingLeft={2}>
          <Text>Â· Python API is enabled in iTerm2 preferences</Text>
          <Text>Â· You may need to restart iTerm2 after enabling</Text>
        </Box>
        <Box marginTop={1}>
          <Select
            options={options}
