...
import { AGENT_COLORS } from '@claude-code-best/builtin-tools/tools/AgentTool/agentColorManager.ts'
import { detectAndGetBackend } from "./backends/registry.ts"
import type { PaneBackend } from "./backends/types.ts"
 ...
 * detectAndGetBackend() caches internally â€?no need for a second cache here.
 */
...
export async function isInsideTmux(): Promise<boolean> {
  const { isInsideTmux: checkTmux } = await import('./backends/detection.ts')
  return checkTmux()
}
