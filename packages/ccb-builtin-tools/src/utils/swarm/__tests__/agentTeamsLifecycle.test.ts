...
describe('Agent Teams lifecycle', () => {
  test('runs TeamCreate -> spawn -> TaskUpdate -> SendMessage -> TeamDelete', async () => {
    const { TeamCreateTool } = await import(
      '@claude-code-best/builtin-tools/tools/TeamCreateTool/TeamCreateTool.ts'
    )
    const { spawnTeammate } = await import(
      '@claude-code-best/builtin-tools/tools/shared/spawnMultiAgent.ts'
    )
    const { TaskCreateTool } = await import(
      '@claude-code-best/builtin-tools/tools/TaskCreateTool/TaskCreateTool.ts'
    )
    const { TaskUpdateTool } = await import(
      '@claude-code-best/builtin-tools/tools/TaskUpdateTool/TaskUpdateTool.ts'
    )
    const { SendMessageTool } = await import(
      '@claude-code-best/builtin-tools/tools/SendMessageTool/SendMessageTool.ts'
    )
    const { TeamDeleteTool } = await import(
      '@claude-code-best/builtin-tools/tools/TeamDeleteTool/TeamDeleteTool.ts'
    )
