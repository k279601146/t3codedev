...
import { getSessionCreatedTeams } from "@t3tools/ccb-engine/src/bootstrap/state.ts"
...
import { getTasksDir, notifyTasksUpdated } from "../tasks.ts"
import { getAgentName, getTeamName, isTeammate } from "../teammate.ts"
import { type BackendType, isPaneBackend } from "./backends/types.ts"
...
/**
 * Reads a team file by name (sync â€?for sync contexts like React render paths)
...
/**
 * Reads a team file by name (async â€?for tool handlers and other async contexts)
 */
...
/**
 * Writes a team file (sync â€?for sync contexts)
 */
// sync IO: called from sync context
...
/**
 * Writes a team file (async â€?for tool handlers)
 */
 ...
 * TeamDelete â€?already cleaned, don't try again on shutdown).
 */
...
export async function cleanupSessionTeams(): Promise<void> {
  ...
  // Kill panes first â€?on SIGINT the teammate processes are still running;
  // deleting directories alone would orphan them in open tmux/iTerm2 panes.
  // (TeamDeleteTool's path doesn't need this â€?by then teammates have
 ...
 * dep graph â€?this only runs at shutdown, so the import cost is irrelevant.
 */
async function killOrphanedTeammatePanes(teamName: string): Promise<void> {
  ...
  await Promise.allSettled(
    paneMembers.map(async m => {
      ...
      logForDebugging(
        `cleanupSessionTeams: killPane ${m.name} (${m.backendType} ${m.tmuxPaneId}) â†?${ok}`,
      )
    }),
  )
}
