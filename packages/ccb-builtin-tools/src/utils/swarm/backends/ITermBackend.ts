...
import { IT2_COMMAND, isInITerm2, isIt2CliAvailable } from "./detection.ts"
import { registerITermBackend } from "./registry.ts"
import type { CreatePaneResult, PaneBackend, PaneId } from "./types.ts"
...
export class ITermBackend implements PaneBackend {
  ...
  async createTeammatePaneInSwarmView(
    ...
    try {
      ...
      // when empty â†?firstPaneUsed resets â†?next iteration has no target â†?throws.
      ...
      while (true) {
        ...
        if (splitResult.code !== 0) {
          ...
          // pruning â€?'session list' distinguishes dead-target from systemic
          // failure (Python API off, it2 removed, transient socket error).
          // Pruning on systemic failure would drain all live IDs â†?state corrupted.
          if (targetedTeammateId) {
            ...
            if (
              ...
              // Confirmed dead â€?prune and retry with next-to-last (or leader).
              logForDebugging(
            ...
            // Target is alive or we can't tell â€?don't corrupt state, surface the error.
          }
          throw new Error(
  ...
  async killPane(
    ...
    // Clean up module state regardless of close result â€?even if the pane is
