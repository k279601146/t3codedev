...
import { createInProcessBackend } from "./InProcessBackend.ts"
import { getPreferTmuxOverIterm2 } from "./it2Setup.ts"
import { createPaneBackendExecutor } from "./PaneBackendExecutor.ts"
import { getTeammateModeFromSnapshot } from "./teammateModeSnapshot.ts"
import type {
 ...
 * and never throws â€?it's the lightweight option when you only need class
...
export async function ensureBackendsRegistered(): Promise<void> {
  ...
  await import('./TmuxBackend.ts')
  await import('./ITermBackend.ts')
  await import('./WindowsTerminalBackend.ts')
...
export async function detectAndGetBackend(): Promise<BackendDetectionResult> {
  ...
  // Priority 3: Native Windows Terminal panes/tabs â€?only when actually
