...
import { TMUX_COMMAND } from "../constants.ts"
 ...
 * Do NOT run `wt.exe --version` â€?wt.exe is a UWP app bridge that opens
 * the Windows Terminal GUI to render version info, producing a phantom
 * "Windows ç»ˆç«¯ 1.24.x" window every time availability is checked.
 */
 ...
 * the Python API is disabled in iTerm2 preferences â€?which would cause
