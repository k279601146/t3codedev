...
import { isInWindowsTerminal } from "./detection.ts"
import { registerWindowsTerminalBackend } from "./registry.ts"
...
function wrapPowerShellCommand(command: string, pidFile: string): string {
  ...
  // PowerShell requires try/catch/finally to be a single compound statement ‚Ä?
  // semicolons between the blocks cause "Try ËØ≠Âè•Áº∫Â∞ëËá™Â∑±Áö?Catch Êà?Finally Âù?.
...
export class WindowsTerminalBackend implements PaneBackend {
  ...
  async isAvailable(): Promise<boolean> {
    ...
    // Do NOT run `wt.exe --version` ‚Ä?wt.exe is a UWP app bridge that opens
    // the Windows Terminal app to render version info, producing a phantom
    // "Windows ÁªàÁ´Ø 1.24.x" window every time availability is checked.
