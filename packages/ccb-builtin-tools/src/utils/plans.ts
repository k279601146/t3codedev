...
export async function copyPlanForResume(
  ...
  // Attempt to read the plan file directly ï¿?recovery triggers on ENOENT.
  ...
  try {
    ...
    if (!isENOENT(e)) {
      // Don't throw ï¿?called fire-and-forget (void copyPlanForResume(...)) with no .catch()
      logError(e)
      return false
    }
 ...
 *
 * 1. ExitPlanMode tool_use input ï¿?normalizeToolInput injects the plan content
 *    into the tool_use input, which persists in the transcript.
 *
 * 2. planContent field on user messages ï¿?set during the "clear context and
 *    implement" flow when ExitPlanMode is approved.
 *
 * 3. plan_file_reference attachment ï¿?created by auto-compact to preserve the
