/**
 * UDS Client â€?connect to peer Claude Code sessions via Unix Domain Sockets.
 *
...
import type { SessionKind } from "./concurrentSessions.ts"
import { MAX_UDS_FRAME_BYTES, type UdsMessage } from "./udsMessaging.ts"
import { attachUdsResponseReader, getChunkBytes } from "./udsResponseReader.ts"
...
export async function listAllLiveSessions(): Promise<PeerSession[]> {
  ...
  for (const file of files) {
    ...
    if (!isProcessRunning(pid)) {
      // Stale â€?skip (concurrentSessions handles cleanup)
      continue
    }
...
async function findAuthTokenForSocketPath(
  ...
  const { readUdsCapabilityToken } = await import('./udsMessaging.ts')
...
export async function sendToUdsSocket(
  ...
  const { parseUdsTarget } = await import('./udsMessaging.ts')
  ...
  const { getUdsMessagingSocketPath } = await import('./udsMessaging.ts')
