...
async function maybePersistLargeToolResult(
  ...
  // Narrow after the emptiness guard â€?content is non-nullish past this point.
  if (!content) {
 ...
 *     the model. Re-application is a Map lookup â€?no file I/O, guaranteed
 *     byte-identical, cannot fail.
 *
 * Lifecycle: one instance per conversation thread, carried on ToolUseContext.
 * Main thread: REPL provisions once, never resets â€?stale entries after
 ...
 * enforceToolResultBudget makes the same choices â†?same wire prefix â†?
 ...
 *   - Flag off â†?undefined (query.ts skips enforcement entirely)
 *   - No initialMessages (cold start) â†?fresh
 *   - initialMessages present â†?reconstruct (freeze all candidate IDs so the
 ...
 *
 * `replacement` is the exact string the model saw â€?stored rather than
...
/**
 * Walk messages and build tool_use_id â†?tool_name from assistant tool_use
 ...
 * boundaries â€?normalizeMessagesForAPI filters out progress entirely
 ...
 * into one over-budget wire message â€?defeating the feature.
 *
...
function collectCandidatesByMessage(
  ...
  // Track all assistant message.ids seen so far â€?same-ID fragments are
  ...
  //   â€?Consecutive: streamingToolExecution yields one AssistantMessage per
  //     content_block_stop (same id); a fast tool drains between blocks;
  //     abort/hook-stop leaves [asst(X), user(trA), asst(X), user(trB)].
  //   â€?Interleaved: coordinator/teammate streams mix different responses
  ...
  // message â€?so the budget must see them as one group too.
  ...
  for (const message of messages) {
    ...
    // normalizeMessagesForAPI â€?they don't create wire boundaries.
  }
  flush()
 ...
 *  - mustReapply: previously replaced â†?re-apply the cached replacement for
 *    prefix stability
 *  - frozen: previously seen and left unreplaced â†?off-limits (replacing
 *    now would change a prefix that was already cached)
 *  - fresh: never seen â†?eligible for new replacement decisions
 */
 ...
 * If frozen results alone exceed budget we accept the overage â€?microcompact
 ...
 * Messages are evaluated independently â€?a 150K result in one message and
 ...
 *
 * @param state â€?MUTATED: seenIds and replacements are updated in place
...
export async function enforceToolResultBudget(
  ...
  for (const candidates of candidatesByMessage) {
    ...
    if (fresh.length === 0) {
      // mustReapply/frozen are already in seenIds from their first pass â€?
      // re-adding is a no-op but keeps the invariant explicit.
      candidates.forEach(c => state.seenIds.add(c.toolUseId))
      continue
    }

    // Tools with maxResultSizeChars: Infinity (Read) â€?never persist.
    ...
    // the wire message is still large, that's the contract â€?Read's own
    ...
    // replacements.set â€?keeps the pair atomic under observation so no
    // concurrent reader (once subagents share state) ever sees XâˆˆseenIds
    // but Xâˆ‰replacements, which would misclassify X as frozen and send
    // full content while the main thread sends the preview â†?cache miss.
  ...
  for (const [candidate, replacement] of freshReplacements) {
    ...
    // is seen-but-unreplaced â€?the original content was sent to the
 ...
 *
 * Gates on `state` (undefined means feature disabled â†?no-op return),
 ...
 * â€?it passes a callback only for querySources that read records back on
 ...
 *     Records for IDs not in messages (e.g. after compact) are skipped â€?
 ...
 *     (never persisted â€?not newlyReplaced). On resume the sidechain has
 ...
 *
 * Kept out of AgentTool.tsx â€?that file is at the feature() DCE complexity
