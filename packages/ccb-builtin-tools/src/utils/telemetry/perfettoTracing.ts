...
// Metadata events (ph: 'M' ï¿?process/thread names, parent links) are kept
// separate so they survive eviction ï¿?Perfetto UI needs them to label
...
// does not truncate ï¿?it writes the full snapshot). At ~300B/event this is
 ...
 * amortized O(1) ï¿?we don't pay splice cost per-push. A synthetic marker
...
export function endLLMRequestPerfettoSpan(
  ...
  metadata: {
    ...
    /** Timestamps (Date.now()) of each attempt start ï¿?used to emit retry sub-spans */
 ...
 * transient I/O problem does not crash the session ï¿?the next periodic tick
 ...
 * Idempotent ï¿?sets `traceWritten` on success so subsequent calls are no-ops.
 */
