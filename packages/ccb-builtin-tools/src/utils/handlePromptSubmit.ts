...
} from "./autonomyQueueLifecycle.ts"
...
type BaseExecutionParams = {
   ...
   * (executeQueuedInput) â€?dequeued items were already queued past this check.
   */
...
export async function handlePromptSubmit(
  ...
  // Skip for remote bridge messages â€?"exit" typed on iOS shouldn't kill the local session
  if (
    !skipSlashCommands &&
  ...
  // Skip for remote bridge messages â€?slash commands from CCR clients are plain text
...
async function executeUserInput(params: ExecuteUserInputParams): Promise<void> {
  ...
  // Always create a fresh abort controller â€?queryGuard guarantees no concurrent
  ...
  // which transitions runningâ†’idle; cancelReservation() below is a no-op in
  // that case (only acts on dispatching state).
  try {
    // Reserve the guard BEFORE processUserInput â€?processBashCommand awaits
    ...
    // command agrees on the same non-undefined workload â€?a human in the
    ...
    // context â€?isolated from the parent's continuation. A process-global
    ...
    try {
      await runWithWorkload(turnWorkload, async () => {
        for (let i = 0; i < commands.length; i++) {
          ...
          // processUserInput â†?processUserInputBase â†?processTextPrompt â†?createUserMessage.
          // Derive origin from mode for task-notifications â€?mirrors the origin
        ...
        if (newMessages.length) {
          ...
          // Release the guard BEFORE clearing toolJSX to prevent spinner flash â€?
      ...
      }) // end runWithWorkload â€?ALS context naturally scoped, no finally needed
    } catch (error) {
      turnError = error
    }
    ...
    // via end(), or running â€?cancelReservation only acts on dispatching).
    ...
    // messages or threw â€?otherwise it would stay visible until the next
