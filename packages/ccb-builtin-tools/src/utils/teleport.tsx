  ...
  try {
    ...
    if (options.useDefaultEnvironment && !cloudEnv) {
      ...
      if (!cloudEnv) {
        logError(
          new Error(
            `No anthropic_cloud environment available after retry (got: ${(retried ?? environments).map(e => `${e.name} (${e.kind})`).join(', ')}). Silent byoc fallthrough would launch into a dead env â€?fail fast instead.`,
          ),
        );
        return null;
      }
