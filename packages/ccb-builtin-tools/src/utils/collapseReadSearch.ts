...
// ~5 lines è„?~60 cols. Generous static cap é”?the renderer lets Ink wrap.
...
export function getToolSearchOrReadInfo(
  ...
  // REPL is absorbed silently é”?its inner tool calls are emitted as virtual
  ...
  // as their own category é”?"Ran N bash commands" instead of breaking the group.
  return {
    isCollapsible:
      isCollapsible ||
...
function scanBashResultForGitOps(
  ...
  // git push writes the ref update to stderr é”?scan both streams.
...
type GroupAccumulator = {
  ...
  bashCount?: number
  // Bash tool_use_id é”?command string, so tool results can be scanned for
...
function createCollapsedGroup(
  ...
  // Adding bash operation count on top would double-count é”?e.g. Read(README.md)
  ...
  // memoryReadFilePaths é”?readFilePaths (both populated from Read tool calls),
  // so this count is safe to subtract from totalReadCount at readCount below.
  // Absorbed relevant_memories attachments are NOT in readFilePaths é”?added
  ...
  for (const msg of messages) {
    if (isCollapsibleToolUse(msg, tools)) {
      ...
      if (toolInfo.isMemoryWrite) {
        // Memory file write/edit é”?check if it's team memory
        ...
        // Snip/ToolSearch absorbed silently é”?no count, no summary text.
        ...
        // MCP search/read é”?counted separately so the summary says
        ...
        // Non-search/read Bash command é”?counted separately so the summary
        ...
        if (input?.command) {
          ...
          // for the human é”?same trigger as the comment-as-label tool-use render).
          currentGroup.latestDisplayHint =
            extractBashCommentLabel(input.command) ??
            commandAsHint(input.command)
          // Remember tool_use_id é”?command so the result (arriving next) can
        ...
        // Directory-listing bash commands (ls, tree, du) é”?counted separately
        ...
        if (
          ...
          // Regular (non-memory) search é”?collect pattern for display
        ...
        for (const filePath of filePaths) {
          ...
          if (feature('TEAMMEM') && teamMemOps?.isTeamMemFile(filePath)) {
            ...
            // Non-memory file read é”?update display hint
      ...
      // é”?block. Do NOT add paths to readFilePaths/memoryReadFilePaths é”?
      ...
      // é”?Loaded lines cluster tightly instead of being split by the badge's marginTop.
      if (
        currentGroup.messages.length > 0 &&
