...
import { closeSentry } from "./sentry.ts"
...
function cleanupTerminalModes(): void {
  ...
  try {
    ...
    // AGAIN inside forceExit() ï¿?process.exit(). Two problems with letting
    // that happen:
    //   1. If we write 1049l here and unmount writes it again later, the
    //      second one triggers another DECRC ï¿?the cursor jumps back over
    ...
    if (inst?.isAltScreenActive) {
      try {
        ...
        // Reconciler/render threw ï¿?fall back to manual alt-screen exit
    ...
    // Disable extended key reporting ï¿?always send both since terminals
    ...
    // Respect CLAUDE_CODE_DISABLE_TERMINAL_TITLE ï¿?if the user opted out of
...
function forceExit(exitCode: number): never {
  ...
  // drain the instance's stdin ï¿?when process.stdin is piped,
  ...
  try {
    ...
    // Terminal may be gone (SIGHUP). Ignore ï¿?we are about to exit.
  }
  try {
...
export const setupGracefulShutdown = memoize(() => {
  ...
  // kernel sigaction for that signal even when other JS listeners remain ï¿?
  ...
  // signal in its list (SIGTERM, SIGINT, SIGHUP, ï¿?, tripping the Bun bug and
  ...
  // Node.js ï¿?the pin also ensures signal-exit's process.exit hook stays
  ...
  process.on('SIGINT', () => {
    ...
    // avoid racing with it. Only check print mode ï¿?other non-interactive
  ...
  if (process.platform !== 'win32') {
    ...
    if (process.stdin.isTTY) {
      orphanCheckInterval = setInterval(() => {
        // Skip during scroll drain ï¿?even a cheap check consumes an event
        // loop tick that scroll frames need. 30s interval ï¿?missing one is fine.
...
export async function gracefulShutdown(
  ...
  // flush ï¿?which can take several seconds.
  cleanupTerminalModes()
  printResumeHint()

  // Flush session data first ï¿?this is the most critical cleanup. If the
  ...
  // Flush analytics ï¿?capped at 500ms. Previously unbounded: the 1P exporter
