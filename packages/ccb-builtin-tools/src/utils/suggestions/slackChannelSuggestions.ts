import { z } from 'zod'
import type { SuggestionItem } from "@t3tools/ccb-engine/src/components/PromptInput/PromptInputFooterSuggestions.ts"
...
// Plain Map (not LRUCache) â€?findReusableCacheEntry needs to iterate all
// entries for prefix matching, which LRUCache doesn't expose cleanly.
const cache = new Map<string, string[]>()
// Flat set of every channel name ever returned by MCP â€?used to gate
...
function unwrapResults(text: string): string {
  ...
  try {
    ...
    // jsonParse threw â€?fall through
  }
  return text
}
...
// matches for searchToken. Lets typing "c"â†?cl"â†?cla" reuse the "c" cache
