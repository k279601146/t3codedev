...
import { getInlinePlugins, getSessionId } from "@t3tools/ccb-engine/src/bootstrap/state.ts"
  ...
  try {
    ...
    return {
      ...
      async getPromptForCommand(args, context) {
        ...
        // resolve to a descriptive placeholder instead ï¿?skill content goes to
...
export const getPluginCommands = memoize(async (): Promise<Command[]> => {
  ...
  // works ï¿?getInlinePlugins() is set by main.tsx from --plugin-dir.
...
export const getPluginSkills = memoize(async (): Promise<Command[]> => {
  // --bare: same gate as getPluginCommands above ï¿?honor explicit
