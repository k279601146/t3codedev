 ...
 * Before this, fetch operations only had logForDebugging â€?no way to measure
...
import { OFFICIAL_MARKETPLACE_NAME } from "./officialMarketplace.ts"
...
// git, self-hosted, internal) is bucketed as 'other' â€?we don't want
...
const KNOWN_PUBLIC_HOSTS = new Set([
  ...
  'dev.azure.com',
  'ssh.dev.azure.com',
  'storage.googleapis.com', // GCS â€?where Dickson's migration points
])

/**
 * Extract hostname from a URL or git spec and bucket to the allowlist.
 * Handles `https://host/...`, `git@host:path`, `ssh://host/...`.
 * Returns a known public host, 'other' (parseable but not allowlisted â€?
...
/**
 * True if the URL/spec points at anthropics/claude-plugins-official â€?the
...
export function logPluginFetch(
  ...
  // String values are bounded enums / hostname-only â€?no code, no paths,
 ...
 * cardinality bounded â€?raw error messages would explode dashboard grouping.
 *
 ...
 * "timeout" â€?ordering the other way would misclassify git DNS as timeout.
 */
export function classifyFetchError(error: unknown): string {
  ...
  // Schema validation throws "Invalid response format" (install_counts) â€?
