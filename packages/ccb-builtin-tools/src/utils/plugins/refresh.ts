 ...
 * - Layer 2: materialization (~/.claude/plugins/) â€?reconcileMarketplaces()
 * - Layer 3: active components (AppState) â€?this file
 *
 * Called from:
 ...
 *
 * NOT called from:
 * - useManagePlugins needsRefresh effect â€?interactive mode shows a notification;
 *   user explicitly runs /reload-plugins (PR 5c)
 * - /plugin menu â€?sets needsRefresh, user runs /reload-plugins (PR 5b)
 */

import { getOriginalCwd } from "@t3tools/ccb-engine/src/bootstrap/state.ts"
import type { Command } from "../../commands.ts"
import { reinitializeLspServerManager } from "@t3tools/ccb-engine/src/services/lsp/manager.ts"
...
import { getAgentDefinitionsWithOverrides } from '@claude-code-best/builtin-tools/tools/AgentTool/loadAgentsDir.ts'
...
export async function refreshActivePlugins(
  ...
  // an explicit "disk changed, re-read it" signal â€?recompute them too.
  ...
  // loadAllPluginsCacheOnly (separate memoize) â€?racing them means they
  ...
  // cache slots NOT filled by loadAllPlugins() â€?they're written later by
