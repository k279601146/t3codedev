...
export async function parseMarketplaceInput(
  ...
  if (trimmed.startsWith('http://') || trimmed.startsWith('https://')) {
    ...
    // marketplace.json â€?the HTML response parses as "expected object,
  ...
  if (
    ...
    // of throwing â€?matches the old existsSync behavior which never threw.
    let stats
    try {
  ...
  // Accept both # and @ as ref separators â€?the display formatter uses @, so users
