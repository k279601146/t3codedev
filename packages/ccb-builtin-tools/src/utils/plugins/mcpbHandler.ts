...
import type { McpServerConfig } from "@t3tools/ccb-engine/src/services/mcp/types.ts"
 ...
 * `pluginSecrets` is a flat map â€?per-server secrets share it with top-level
 ...
 * secureStorage wins on collision â€?schema determines destination so
 * collision shouldn't happen, but if a user hand-edits settings.json we
 * trust the more secure source.
 *
 * Returns null only if NEITHER source has anything â€?callers skip
 ...
 *   - `sensitive: true` â†?secureStorage (keychain on macOS, .credentials.json 0600 elsewhere)
 *   - everything else   â†?settings.json pluginConfigs[pluginId].mcpServers[serverName]
 *
 * Without this split, per-channel `sensitive: true` was a false sense of
 * security â€?the dialog masked the input but the save went to plaintext
 ...
 *   or channels[].userConfig) â€?drives the sensitive/non-sensitive split
 */
export function saveMcpServerUserConfig(
  ...
  try {
    ...
    //  - sensitiveâ†’secureStorage â‡?remove stale plaintext from settings.json
    //  - nonSensitiveâ†’settings.json â‡?remove stale entry from secureStorage
    ...
    // untouched in BOTH stores â€?defense-in-depth against future callers.
    ...
    // Sensitive â†?secureStorage FIRST. If this fails (keychain locked,
    // .credentials.json perms), throw before touching settings.json â€?the
    // old plaintext stays as a fallback instead of losing BOTH copies.
    //
    // Also scrub non-sensitive keys from secureStorage â€?schema flipped
    // sensitiveâ†’false and they're being written to settings.json now. Without
    ...
    if (Object.keys(sensitive).length > 0 || needSecureScrub) {
      ...
      // secureStorage keyvault is a flat object â€?direct replace, no merge
    ...
    // Non-sensitive â†?settings.json. Write whenever there are new non-sensitive
    // values OR existing plaintext sensitive values to scrub â€?so reconfiguring
    ...
    // which PRESERVES destination keys absent from source â€?so simply omitting
    // sensitive keys doesn't scrub them, the disk copy merges back in. Instead:
    // set each sensitive key to explicit `undefined` â€?mergeWith (with the
    ...
    if (
      ...
      // needs explicit undefined to delete â€?cast is deliberate internal
...
async function downloadMcpb(
  ...
  try {
    ...
    // Fire telemetry before writeFile â€?the event measures the network
    // fetch, not disk I/O. A writeFile EACCES would otherwise match
    // classifyFetchError's /permission denied/ â†?misreport as auth.
 ...
 *
 * @param modes - nameâ†’mode map from `parseZipModes`. MCPB bundles can ship
...
async function extractMcpbContents(
  ...
  for (const [filePath, fileData] of entries) {
    ...
    // are filtered above â€?writeFile would create `bin/` as an empty regular
    ...
    if (mode && mode & 0o111) {
      // Swallow EPERM/ENOTSUP (NFS root_squash, some FUSE mounts) â€?losing +x
...
export async function loadMcpbFile(
  ...
  // fflate doesn't surface external_attr â€?parse the central directory so
