/**
 * Marketplace reconciler ï¿?makes known_marketplaces.json consistent with
...
export type MarketplaceDiff = {
  ...
  missing: string[]
  /** Present in both, but settings source ï¿?JSON source (settings wins) */
  sourceChanged: Array<{
    name: string
...
export function diffMarketplaces(
  ...
  for (const [name, intent] of Object.entries(declared)) {
    ...
    if (!state) {
      ...
      // Fallback: presence suffices. Don't compare sources ï¿?the declared source
      ...
      // would report sourceChanged ï¿?re-clone ï¿?stomp the materialized content.
      upToDate.push(name)
...
export async function reconcileMarketplaces(
  ...
  for (const item of work) {
    ...
    // canonicalize and produces a dead path ï¿?the materialized entry may still
  ...
  for (let i = 0; i < toProcess.length; i++) {
    ...
    try {
      // addMarketplaceSource is source-idempotent ï¿?same source returns
      // alreadyMaterialized:true without cloning. For 'update' (source
      // changed), the new source won't match existing ï¿?proceeds with clone
 ...
 * so `./foo` means "relative to this repo" ï¿?but known_marketplaces.json is
