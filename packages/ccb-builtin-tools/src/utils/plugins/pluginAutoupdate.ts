...
import { updatePluginOp } from "@t3tools/ccb-engine/src/services/plugins/pluginOperations.ts"
 ...
 * cwd â€?see isInstallationRelevantToCurrentProject), then calls updatePluginOp
 * per installation. Already-up-to-date plugins are silently skipped.
 *
 * Called by:
 * - updatePlugins() below â€?background autoupdate path (autoUpdate-enabled
 *   marketplaces only; third-party marketplaces default autoUpdate: false)
 * - ManageMarketplaces.tsx applyChanges() â€?user-initiated /plugin marketplace
