 ...
 *
 * Plugins declare user-configurable options in `manifest.userConfig` ï¿?a record
 ...
 *   - `sensitive: true`  ï¿?secureStorage (keychain on macOS, .credentials.json elsewhere)
 *   - everything else    ï¿?settings.json `pluginConfigs[pluginId].options`
 *
 ...
 * and `secureStorage.pluginSecrets`. Today this is `plugin.source` ï¿?always
 ...
 * produces the same key by convention ï¿?see PluginOptionsFlow, ManagePlugins.
 *
...
export const loadPluginOptions = memoize(
  (pluginId: string): PluginOptionValues => {
    ...
    // session-lifetime) + keychain's own 30s TTL cache ï¿?so one blocking spawn
    ...
    // secureStorage wins on collision ï¿?schema determines destination so
...
export function savePluginOptions(
  ...
  // Scrub sets ï¿?see saveMcpServerUserConfig (mcpbHandler.ts) for the
  ...
  // secureStorage FIRST ï¿?if keychain fails, throw before touching
  ...
  // settings.json AFTER secureStorage ï¿?scrub sensitive keys via explicit
...
/**
 * Delete all stored option values for a plugin ï¿?both the non-sensitive
 ...
 * alongside `markPluginVersionOrphaned`). Don't call on every uninstall ï¿?
...
export function deletePluginOptions(pluginId: string): void {
  // Settings side ï¿?also wipes the legacy mcpServers sub-key (same story:
  // orphaned on uninstall, never cleaned up before this PR).
  //
  // Use `undefined` (not `delete`) because `updateSettingsForSource` merges
  // via `mergeWith` ï¿?absent keys are ignored, only `undefined` triggers
  ...
  // mergeWith-deletion contract is internal plumbing ï¿?it shouldn't shape
  ...
  if (settings.pluginConfigs?.[pluginId]) {
    // Partial<Record<K,V>> = Record<K, V | undefined> ï¿?gives us the widening
  ...
  // Secure storage side ï¿?delete both the top-level pluginSecrets[pluginId]
...
/**
 * Find option keys whose saved values don't satisfy the schema ï¿?i.e., what to
 * prompt for. Returns the schema slice for those keys, or empty if everything
 * validates. Empty manifest.userConfig ï¿?empty result.
 *
...
export function getUnconfiguredOptions(
  ...
  // strings keyed by title/key ï¿?simpler to just re-check each field here than
  // parse error strings.
 ...
 *
 * ${CLAUDE_PLUGIN_ROOT} ï¿?version-scoped install dir (recreated on update)
 * ${CLAUDE_PLUGIN_DATA} ï¿?persistent state dir (survives updates)
 *
 ...
 *
 * Throws on missing keys ï¿?callers pass this only after `validateUserConfig`
 ...
 *
 * Use `substituteUserConfigInContent` for skill/agent prose ï¿?it handles
 ...
 *     the actual value ï¿?skill/agent content goes to the model prompt, and
 *     we don't put secrets in the model's context.
 *   - Unknown keys stay literal (no throw) ï¿?matches how `${VAR}` env refs
