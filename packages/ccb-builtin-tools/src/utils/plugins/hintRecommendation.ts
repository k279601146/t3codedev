 ...
 *
 * State persists in GlobalConfig.claudeCodeHints â€?a show-once record per
...
/**
 * Hard cap on `claudeCodeHints.plugin[]` â€?bounds config growth. Each shown
 ...
 *
 * Synchronous on purpose â€?shell tools shouldn't await a marketplace lookup
...
export function maybeRecordPluginHint(hint: ClaudeCodeHint): void {
  ...
  // Bound repeat lookups on the same slug â€?a CLI that emits on every
 ...
 * the plugin isn't in the marketplace cache â€?the hint is discarded.
 */
 ...
 * the user's yes/no response â€?show-once semantics.
 */
