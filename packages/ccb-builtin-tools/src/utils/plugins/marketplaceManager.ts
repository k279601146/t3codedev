 ...
 * ~/.claude/
 *   â””â”€â”€ plugins/
 *       â”œâ”€â”€ known_marketplaces.json    # Configuration of all known marketplaces
 *       â””â”€â”€ marketplaces/              # Cache directory for marketplace data
 *           â”œâ”€â”€ my-marketplace.json    # Cached marketplace from URL source
 *           â””â”€â”€ github-marketplace/    # Cloned repository for GitHub source
 *               â””â”€â”€ .claude-plugin/
 ...
 * settings-schema field â€?it's only ever set in code (never parsed from JSON).
 */
export type DeclaredMarketplace = {
   ...
   * entry as upToDate regardless of source shape â€?never reports sourceChanged.
   *
 ...
 * This is what SHOULD exist â€?used by the reconciler to find gaps.
 *
...
export function getDeclaredMarketplaces(): Record<string, DeclaredMarketplace> {
  ...
  // Only the official marketplace can be implicitly declared â€?it's the one
...
export function getMarketplaceDeclaringSource(
  ...
  // Check highest-precedence editable sources first â€?the one that wins
 ...
 * config should degrade gracefully rather than crash. DO NOT use on loadâ†’mutateâ†’save
 * paths â€?returning {} there would cause the save to overwrite the corrupted file
...
export async function loadKnownMarketplacesConfigSafe(): Promise<KnownMarketplacesConfig> {
  try {
    ...
    // Inner function already logged via logForDebugging. Don't logError here â€?
 ...
 * getPluginByIdCacheOnly, etc.) without any loader changes â€?they just follow
 * the installLocation wherever it points.
 *
 * Seed entries always win for marketplaces declared in the seed â€?the seed is
...
export async function registerSeedMarketplaces(): Promise<boolean> {
  ...
  // alone â€?two seeds with the same name will have different installLocations.
  ...
  for (const seedDir of seedDirs) {
    ...
    for (const [name, seedEntry] of Object.entries(seedConfig)) {
      ...
      if (!resolvedLocation) {
        // Seed content missing (incomplete build) â€?leave primary alone, but
      ...
      // Skip if primary already matches â€?idempotent no-op, no write.
      if (isEqual(primary[name], desired)) continue

      // Seed wins â€?admin-managed. Overwrite any existing primary entry.
 ...
 * directory. Seed-managed entries are admin-controlled â€?users can't
 ...
 * --recurse-submodules, but gitPull() didn't â€?the parent repo's submodule
 ...
 *
 * Skipped for sparse clones â€?gitClone's sparse path intentionally omits
 ...
 * even when no submodules exist. .gitmodules is a tracked file â€?pull
 * materializes it iff the repo has submodules â€?so gate on its presence to
...
function enhanceGitPullErrorMessages(result: {
  ...
  // when the process is killed by SIGTERM â€?the timeout info is only in error)
  ...
  // 'Could not read from remote' catch â€?that string appears in both cases).
  // OpenSSH emits "Host key verification failed" for BOTH host-not-in-known_hosts
  // and host-key-has-changed â€?the latter also includes the "REMOTE HOST
 ...
 *
 * Does NOT disable credential helpers â€?this allows the user's existing auth setup
 ...
 * automatically; for explicit git@host:â€?URLs, users see an actionable error.
 */
export async function gitClone(
  ...
  if (useSparse) {
    ...
    // for sparse clones â€?sparse monorepos rarely need them, and recursing
  ...
  // Detect timeout kills â€?when execFileNoThrowWithCwd kills the process via SIGTERM,
  ...
  if (result.stderr) {
    // Host key verification failure â€?check FIRST, before the generic
  ...
  // Fallback for empty stderr â€?gh-28373: user saw "Failed to clone
 ...
 * - Fullâ†’Sparse or SparseAâ†’SparseB: run `sparse-checkout set --cone` (idempotent)
 * - Sparseâ†’Full: return non-zero so caller falls back to rm+reclone. Avoids
 ...
 * - Fullâ†’Full (common case): single local `git config --get` check, no-op.
 *
 * Failures here (ENOENT, not a repo) are harmless â€?gitPull will also fail and
 ...
 *   â”?  â””â”€â”€ marketplace.json    # Default location for marketplace manifest
...
async function cacheMarketplaceFromGit(
  ...
  safeCallProgress(
    onProgress,
    `Refreshing marketplace cache (timeout: ${timeoutSec}s)â€¦`,
  )

  // Reconcile sparse-checkout config before pulling. If this requires a re-clone
  // (Sparseâ†’Full transition) or fails (missing dir, not a repo), skip straight
  ...
  try {
    ...
    // rm succeeded â€?a stale or partially-cloned directory existed; log for diagnostics
    logForDebugging(
    ...
    // ENOENT â€?cachePath didn't exist, this is a fresh install, nothing to clean up
  }

  // Clone the repository (one attempt â€?no internal retry loop)
 ...
 *   https://user:token@github.com/repo â†?https://***:***@github.com/repo
 *   https://:token@github.com/repo     â†?https://:***@github.com/repo
 *   https://token@github.com/repo      â†?https://***@github.com/repo
 *
 ...
 *   â”œâ”€â”€ official-marketplace.json     # From URL source
 *   â”œâ”€â”€ github-marketplace/          # From GitHub/Git source
 *   â”?  â””â”€â”€ .claude-plugin/
 *   â”?      â””â”€â”€ marketplace.json
 *   â””â”€â”€ local-marketplace.json       # From file source
 *
...
async function loadAndCacheMarketplace(
  ...
  try {
    switch (source.source) {
      ...
      case 'settings': {
        // Inline manifest from settings.json â€?no fetch. Synthesize the
        ...
        // diffMarketplaces detects settings edits via isEqual â€?no special
        ...
        // below widens and validates â€?that's the real check.
        await writeFile(
          marketplacePath,
          jsonStringify(
            {
    ...
    // cacheDir, nor rm cacheDir itself (e.g. name "." â†?join normalizes to cacheDir).
...
export async function addMarketplaceSource(
  ...
  if (oldEntry) {
    ...
    logForDebugging(
      `Marketplace '${marketplace.name}' exists with different source â€?overwriting`,
    )
    ...
    // to cachePath BEFORE we get here â€?rm-ing the same dir deletes the fresh
    // write. Settings sources always land on the same dir (name â†?path);
    ...
    // dir. If it's outside the cache dir, skip cleanup â€?the stale dir (if
    ...
    if (!isLocalMarketplaceSource(oldEntry.source)) {
      ...
      if (resolvedOld === resolvedNew) {
        // Same dir â€?loadAndCacheMarketplace already overwrote in place.
        // Nothing to clean.
      } else if (
        ...
        logForDebugging(
          `Skipping cleanup of old installLocation (${oldEntry.installLocation}) â€?` +
...
export async function removeMarketplaceSource(name: string): Promise<void> {
  ...
  // Seed-registered marketplaces are admin-baked into the container â€?removing
  ...
  // Also wipe their stored options/secrets â€?after marketplace removal
...
export const getMarketplace = memoize(
  async (name: string): Promise<PluginMarketplace> => {
    ...
    // These are meaningless outside the project that wrote them â€?resolving
    ...
    if (
      ...
      throw new Error(
        `Marketplace "${name}" has a relative source path (${entry.source.path}) ` +
          `in known_marketplaces.json â€?this is stale state from an older ` +
...
export async function refreshAllMarketplaces(): Promise<void> {
  ...
  for (const [name, entry] of Object.entries(config)) {
    // Seed-managed marketplaces are controlled by the seed image â€?refreshing
    ...
    // settings-sourced marketplaces have no upstream â€?see refreshMarketplace.
    if (entry.source.source === 'settings') {
      continue
    }
    // inc-5046: same GCS intercept as refreshMarketplace() â€?bulk update
...
export async function refreshMarketplace(
  ...
  // re-materializes via addMarketplaceSource â€?refresh is not the vehicle.
  if (entry.source.source === 'settings') {
    logForDebugging(
      `Skipping refresh for settings-sourced marketplace '${name}' â€?no upstream`,
    )
    return
  }
  ...
  try {
    ...
    // would be pointless â€?registerSeedMarketplaces() overwrites installLocation
    ...
    // marketplaces cache dir. A corrupted value (gh-32793, gh-32661 â€?e.g.
      ...
      if (resolvedLoc !== cacheDir && !resolvedLoc.startsWith(cacheDir + sep)) {
        throw new Error(
          `Marketplace '${name}' has a corrupted installLocation ` +
            `(${installLocation}) â€?expected a path inside ${cacheDir}. ` +
    ...
    // no data migration is needed â€?existing known_marketplaces.json entries
    ...
    if (name === OFFICIAL_MARKETPLACE_NAME) {
      ...
      // GCS failed â€?fall through to git ONLY if the kill-switch allows.
      ...
      if (
        ...
        // Throw, don't return â€?every other failure path in this function
    ...
    if (source.source === 'github' || source.source === 'git') {
      ...
      if (source.source === 'github') {
        ...
        // needs a URL â€?pick the right protocol up-front and fall back.
        const sshUrl = `git@github.com:${source.repo}.git`
        const httpsUrl = `https://github.com/${source.repo}.git`
...
export async function setMarketplaceAutoUpdate(
  ...
  // Also update intent in settings if declared there â€?write to the SAME
