...
import { expandEnvVarsInString } from "@t3tools/ccb-engine/src/services/mcp/envExpansion.ts"
import {
    ...
    if ('status' in result && result.status === 'needs-config') {
      ...
      logForDebugging(
        `MCPB ${mcpbPath} requires user configuration. ` +
          `User can configure via: /plugin â†?Manage plugins â†?${plugin.name} â†?Configure`,
      )
 ...
 * fields are not yet saved. Pure function â€?no React, no prompting.
...
export function getUnconfiguredChannels(
  ...
  // plugin.repository is already in "plugin@marketplace" format â€?same key
 ...
 * or channels without a userConfig schema â€?resolvePluginMcpEnvironment will
...
export async function extractMcpServersFromPlugins(
  ...
  const scopedResults = await Promise.all(
    plugins.map(async plugin => {
      ...
      // inside resolvePluginMcpEnvironment â€?catch per-server so one bad
 ...
 *
 * Returns undefined when neither source has anything â€?resolvePluginMcpEnvironment
...
function buildMcpUserConfig(
  ...
  // undefined â€?the `!topLevel` check below is dead, we return {} for
  // unconfigured plugins, and resolvePluginMcpEnvironment runs
  // substituteUserConfigVariables against an empty map â†?throws on any
...
export async function getPluginMcpServers(
  ...
  // and this function runs inside Promise.all at config.ts:911 â€?one
