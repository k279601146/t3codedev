...
export async function checkAndInstallOfficialMarketplace(): Promise<OfficialMarketplaceCheckResult> {
  ...
  try {
    ...
    // inc-5046: try GCS mirror first â€?doesn't need git, doesn't hit GitHub.
    ...
    // with source:'github' (still true â€?GCS is a mirror) and skip git
    // entirely.
    ...
    // ONLY if the kill-switch allows â€?same gate as refreshMarketplace().
    if (
      ...
      logForDebugging(
        'Official marketplace GCS failed; git fallback disabled by flag â€?skipping install',
      )
      // Same retry-with-backoff metadata as git_unavailable below â€?transient
    ...
    // then return silently without recording any attempt state â€?next startup
