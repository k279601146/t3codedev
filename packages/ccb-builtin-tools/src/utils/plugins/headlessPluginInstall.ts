...
import { detectAndUninstallDelistedPlugins } from "./pluginBlocklist.ts"
import { clearPluginCache } from "./pluginLoader.ts"
import { reconcileMarketplaces } from "./reconciler.ts"
import {
...
import { syncMarketplacesToZipCache } from "./zipCacheAdapters.ts"
...
export async function installPluginsForHeadless(): Promise<boolean> {
  ...
  // would see seed entries as missing â†?clone â†?defeats seed's purpose.
  //
  ...
  // the official marketplace through the same reconciler path as any other â€?
  ...
  // refreshPluginState() â†?clearCommandsCache/clearAgentDefinitionsCache
  ...
  try {
    if (declaredCount === 0) {
      ...
      // Plugin caching is the loader's job â€?after caches clear, the caller's
      // refreshPluginState() â†?loadAllPlugins() will cache any missing plugins
