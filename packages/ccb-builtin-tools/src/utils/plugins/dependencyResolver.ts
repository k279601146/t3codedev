/**
 * Plugin dependency resolution â€?pure functions, no I/O.
 *
 ...
 *
 * Two entry points:
 *  - `resolveDependencyClosure` â€?install-time DFS walk, cycle detection
 *  - `verifyAndDemote` â€?load-time fixed-point check, demotes plugins with
...
import type { PluginId } from "./schemas.ts"
 ...
 * `allowCrossMarketplaceDependenciesOn` allowlist â€?only the root's list
...
export async function resolveDependencyClosure(
  ...
  async function walk(
    ...
    // `cacheAndRegisterPlugin` would never fire â€?user sees
 ...
 * settings/types.ts:455-463 â€?a plugin at `"foo@bar": ["^1.0.0"]` IS enabled).
...
export function formatReverseDependentsSuffix(
  ...
  return ` â€?warning: required by ${rdeps.join(', ')}`
}
