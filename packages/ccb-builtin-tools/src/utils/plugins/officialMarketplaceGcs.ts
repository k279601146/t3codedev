...
import { waitForScrollIdle } from "@t3tools/ccb-engine/src/bootstrap/state.ts"
import type { AnalyticsMetadata_I_VERIFIED_THIS_IS_NOT_CODE_OR_FILEPATHS } from "@t3tools/ccb-engine/src/services/analytics/index.ts"
...
// binary ships from â€?nativeInstaller/download.ts:24 uses the raw GCS URL).
...
// Zip arc paths are seed-dir-relative (marketplaces/claude-plugins-official/â€?
 ...
 * Idempotent â€?checks a `.gcs-sha` sentinel before downloading the ~3.5MB zip.
 *
 * @param installLocation where to extract (must be inside marketplacesCacheDir)
 * @param marketplacesCacheDir the plugins marketplace cache root â€?passed in
...
export async function fetchOfficialMarketplaceFromGcs(
  ...
  // during the atomic swap. A corrupted known_marketplaces.json (gh-32793 â€?
  ...
  // This is a fire-and-forget startup call â€?delaying by a few hundred ms
  ...
  try {
    // 1. Latest pointer â€?~40 bytes, backend sets Cache-Control: no-cache,
    ...
    if (!sha) {
      // Empty /latest body â€?backend misconfigured. Bail (null), don't
      // lock into a permanently-broken empty-sentinel state.
      throw new Error('latest pointer returned empty body')
    }

    // 2. Sentinel check â€?`.gcs-sha` at the install root holds the last
    ...
    const currentSha = await readFile(sentinelPath, 'utf8').then(
      s => s.trim(),
      () => null, // ENOENT â€?first fetch, proceed to download
    )
    if (currentSha === sha) {
      outcome = 'noop'
      return sha
    ...
    for (const [arcPath, data] of Object.entries(files)) {
      ...
      if (mode && mode & 0o111) {
        // Only chmod when an exec bit is set â€?skip plain files to save syscalls.
    ...
    // doesn't exist â€?acceptable for a background refresh (caller retries next
    ...
    // (.daisy/inc-5046/index.md) â€?adds source:'marketplace_gcs'. All string
    // values below are static enums or a git SHA â€?not code/filepaths/PII.
 ...
 *
 * Telemetry from v2.1.83+ showed 50% of failures landing in 'other' â€?and
...
export function classifyGcsError(e: unknown): string {
  ...
  // .code (ERR_NETWORK, ERR_BAD_OPTION, EPROTO) â€?don't bucket those as fs.
  if (code && /^E[A-Z]+$/.test(code) && !code.startsWith('ERR_')) {
    return KNOWN_FS_CODES.has(code) ? `fs_${code}` : 'fs_other'
  }
  // fflate sets numeric .code (0-14) on inflate/unzip errors â€?catches
