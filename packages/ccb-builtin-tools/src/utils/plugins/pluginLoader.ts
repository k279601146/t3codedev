...
import { getInlinePlugins } from "@t3tools/ccb-engine/src/bootstrap/state.ts"
import {
...
import type {
  ...
  PluginLoadResult,
  PluginManifest,
} from "@t3tools/ccb-engine/src/types/plugin.ts"
...
import type { HookMatcher } from "../../schemas/hooks.ts"
 ...
 * Multiple versions within a single seed ï¿?ambiguous ï¿?try next seed.
...
export async function probeSeedCacheAnyVersion(
  ...
  for (const seedDir of getPluginSeedDirs()) {
    // The parent of the version dir ï¿?computed the same way as
...
export async function copyPluginToVersionedCache(
  ...
  // Seed cache hit ï¿?return seed path in place (read-only, no copy).
  ...
  if (entry && typeof entry.source === 'string' && marketplaceDir) {
    ...
    try {
      ...
      // Only remap ENOENT from the top-level sourceDir itself ï¿?nested ENOENTs
...
export async function gitClone(
  ...
  // complete ï¿?same telemetry-scope discipline as mcpb and marketplace_url.
  logPluginFetch(
    'plugin_clone',
    gitUrl,
    'success',
 ...
 * monorepos this is dramatically cheaper than a full clone ï¿?the tree objects
 ...
 *
 * The clone is ephemeral ï¿?it goes into a sibling temp directory and is
...
export async function installFromGitSubdir(
  ...
  // Clone into a sibling temp dir (same filesystem ï¿?rename works, no EXDEV).
  ...
  try {
    ...
    if (sha) {
      ...
      // lazy-fetched ï¿?the slow, network-bound step). It doesn't move HEAD;
 ...
 * ï¿?  â”œâ”€â”€ build.md         # /build command
 * ï¿?  â””â”€â”€ test.md          # /test command
 * â”œâ”€â”€ agents/              # Optional: Custom AI agents
 * ï¿?  â”œâ”€â”€ reviewer.md      # Code review agent
 * ï¿?  â””â”€â”€ optimizer.md     # Performance optimization agent
...
export async function createPluginFromPath(
  ...
  if (manifest.commands) {
    // Check if it's an object mapping (record of command name ï¿?metadata)
 ...
 *
 * Resolves enabledPlugins ï¿?marketplace entries, runs enterprise policy
 ...
 * and loadAllPluginsCacheOnly is which loader runs ï¿?discovery and policy
 * are identical.
 */
async function loadPluginsFromMarketplaces({
  ...
  const marketplacePluginEntries = Object.entries(enabledPlugins).filter(
    ([key, value]) => {
      ...
      // Skip built-in plugins ï¿?handled separately by getBuiltinPlugins()
  ...
  // loading ï¿?this is a read-only path, so returning {} degrades gracefully.
  ...
  // loads the plugin unchecked ï¿?a silent fail-open. This guard restores
  // fail-closed: unknown source + active policy ï¿?block.
  //
  // Allowlist: any value (including []) is active ï¿?empty allowlist = deny all.
  // Blocklist: empty [] is a semantic no-op ï¿?only non-empty counts as active.
  ...
  // hot path ï¿?with N plugins across M marketplaces, the old per-plugin
  ...
  const results = await Promise.allSettled(
    marketplacePluginEntries.map(async ([pluginId, enabledValue]) => {
      ...
      // with NO schema validation ï¿?if one entry is malformed enough to fail
      // our validation but readable enough for the raw cast, it would load
      // unchecked. Unverifiable source + active policy ï¿?block, always.
      if (!marketplaceConfig && hasEnterprisePolicy) {
        // We can't know whether the unverifiable source would actually be in
        // the blocklist or not in the allowlist ï¿?so pick the error variant
...
async function loadPluginFromMarketplaceEntryCacheOnly(
  ...
  if (typeof entry.source === 'string') {
    // Local relative path ï¿?read from the marketplace source dir directly.
    ...
    // finishLoadingPluginFromPath reads pluginPath ï¿?its error handling
    // surfaces ENOENT as a load failure, no need to pre-check here.
  } else {
    // External source (npm/github/url/git-subdir) ï¿?use recorded installPath.
  ...
  // Zip cache extraction ï¿?must still happen in cacheOnly mode (invariant 4)
  ...
  // Delegate to the shared tail ï¿?identical to the full loader from here
...
async function loadPluginFromMarketplaceEntry(
  ...
  if (typeof entry.source === 'string') {
    ...
    try {
      ...
      //    3. Marketplace entry version, 4. source.sha (pinned commits ï¿?the
      //    exact value the post-clone call at cached.gitCommitSha would see),
      //    5. 'unknown' ï¿?ref-tracked, falls through to clone by design.
      const version = await calculatePluginVersion(
        pluginId,
        entry.source,
        undefined,
      ...
      // Check for cached version ï¿?ZIP file (zip cache mode) or directory
      const zipPath = getVersionedZipCachePath(pluginId, version)
      if (isPluginZipCacheEnabled() && (await pathExists(zipPath))) {
        ...
        // frozen at image build time ï¿?no freshness concern, 'whatever's there'
        ...
        // disabled for this session ï¿?errorsOut.push below surfaces it.
        const seedPath =
        ...
        if (seedPath) {
          ...
          // ï¿?manifest.version (step 1) outranks gitCommitSha (step 3) ï¿?
          ...
          // (ref-tracked, no hints) ï¿?the clone is the ONLY way to learn.
          const actualVersion =
            version !== 'unknown'
              ? version
 ...
 * the rest of the load ï¿?manifest probe, createPluginFromPath, marketplace
 * entry supplementation ï¿?is identical. Extracted so the cache-only path
...
async function finishLoadingPluginFromPath(
  ...
  if (!hasManifest) {
    ...
    if (entry.skills) {
      ...
      // (once in a debug log template, once in the if) ï¿?now called once.
      const checks = await Promise.all(
 ...
 * same name ï¿?the user explicitly pointed at a directory for this session.
 *
...
export function mergePluginSources(sources: {
  ...
  // OR force-disabled ï¿?both are admin intent that --plugin-dir must not
  ...
  // plugin where entry.name ï¿?manifest.name, this guard will silently miss ï¿?
 ...
 * 1. Session-only plugins (from --plugin-dir CLI flag) ï¿?override
 ...
 * pointed at a directory for this session ï¿?that intent beats whatever
...
export const loadAllPlugins = memoize(async (): Promise<PluginLoadResult> => {
  ...
  // getAgentDefinitionsWithOverrides() ï¿?which now call
  // loadAllPluginsCacheOnly ï¿?see just-cloned plugins instead of reading
 ...
 *
 * CLAUDE_CODE_SYNC_PLUGIN_INSTALL=1 delegates to the full loader ï¿?that
 ...
 *
 * Separate memoize cache from loadAllPlugins ï¿?a cache-only result must
 ...
 *
 * The only difference between the two is which marketplace loader runs ï¿?
...
async function assemblePluginLoadResult(
  ...
  // Verify dependencies. Runs AFTER the parallel load ï¿?deps are presence
...
export function cachePluginSettings(plugins: LoadedPlugin[]): void {
  ...
  // anyway ï¿?resetting here would waste ~17ms on startup re-reading and
