...
import { getUseCoworkPlugins } from "@t3tools/ccb-engine/src/bootstrap/state.ts"
...
import { expandTilde } from "../permissions/pathValidation.ts"
 ...
 * read-only fallback layer under the primary plugins directory â€?marketplaces
 ...
 * (':' on Unix, ';' on Windows), in PATH-like precedence order â€?the first
...
/** Pure path â€?no mkdir. For display (e.g. uninstall dialog). */
 ...
 * this survives plugin updates â€?only removed on last-scope uninstall.
 *
 * Creates the directory on call (mkdir). The *lazy* behavior is at the
 * substitutePluginVariables call site â€?the DATA pattern uses function-form
 ...
 * String.replace) â€?making this async would cascade through 6 call sites
 ...
 * Recursive walk â€?not hot-path (only on uninstall).
 */
  ...
  const walk = async (p: string) => {
    for (const entry of await readdir(p, { withFileTypes: true })) {
      ...
      if (entry.isDirectory()) {
        ...
        // Without this, one broken link bubbles to the outer catch â†?
        // returns null â†?dialog skipped â†?data silently deleted.
        try {
          bytes += (await stat(full)).size
        } catch {
          // Broken symlink / raced delete â€?skip this entry, keep walking
        }
      }
    }
  }
 ...
 * not throw â€?the uninstall itself already succeeded; we don't want a
