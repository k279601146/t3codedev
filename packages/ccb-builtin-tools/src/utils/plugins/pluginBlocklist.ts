 ...
 *
 * The security.json fetch was removed (see #25447) â€?~29.5M/week GitHub hits
 * for UI reason/text only. If re-introduced, serve from downloads.claude.ai.
 */

import { uninstallPluginOp } from "@t3tools/ccb-engine/src/services/plugins/pluginOperations.ts"
...
export async function detectAndUninstallDelistedPlugins(): Promise<string[]> {
  ...
  // Read-only iteration â€?Safe variant so a corrupted config doesn't throw
  ...
  for (const marketplaceName of Object.keys(knownMarketplaces)) {
    try {
      ...
      for (const pluginId of delisted) {
        ...
        // Skip managed-only plugins â€?enterprise admin should handle those
      ...
      // Marketplace may not be available yet â€?log and continue
      logForDebugging(
