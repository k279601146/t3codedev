...
export const loadPluginHooks = memoize(async (): Promise<void> => {
  ...
  // atomic ï¿?old hooks stay valid until this point, new hooks take over.
  clearRegisteredPluginHooks()
...
export function clearPluginHookCache(): void {
  // Only invalidate the memoize ï¿?do NOT wipe STATE.registeredHooks here.
 ...
 * while newly-enabled plugins wait for /reload-plugins ï¿?consistent with
...
export async function pruneRemovedPluginHooks(): Promise<void> {
  // Early return when nothing to prune ï¿?avoids seeding the loadAllPluginsCacheOnly
 ...
 *
 * Hashes FOUR fields ï¿?not just enabledPlugins ï¿?because the memoized
 ...
 */
// Exported for testing ï¿?the listener at setupPluginHookHotReload uses this
