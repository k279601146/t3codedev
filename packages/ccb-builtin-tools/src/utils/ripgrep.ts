...
// In dev mode: __filename = <root>/src/utils/ripgrep.ts ï¿?__dirname = <root>/src/utils/
// In built mode (bun): __filename = <root>/dist/chunk-xxx.js ï¿?need <root>/dist/
// In built mode (vite): __filename = <root>/dist/chunks/chunk-xxx.js ï¿?need <root>/dist/
// Both built modes: the dist root is at <root>/dist/ where dist/vendor/ripgrep/ lives.
const __dirname = (() => {
  const dir = path.dirname(__filename)
  // Test mode: from src/utils/ ï¿?project root
  if (process.env.NODE_ENV === 'test') return path.resolve(dir, '../../../')
  // Check if we're inside a dist directory at any depth
  // (dist/ or dist/chunks/) ï¿?vendor lives at <dist-root>/vendor/ripgrep/
  const parts = dir.split(path.sep)
  const distIdx = parts.lastIndexOf('dist')
  if (distIdx !== -1) {
    return parts.slice(0, distIdx + 1).join(path.sep)
  }
  // Dev mode: from src/utils/ ï¿?src/utils/
  return dir
})()
 ...
 * lines as soon as each chunk arrives ï¿?first results paint while rg is still
 ...
 * signal ï¿?spawn's signal option kills rg. No EAGAIN retry, no internal
...
export async function ripGrepStream(
  ...
  return new Promise<void>((resolve, reject) => {
    ...
    child.on('close', code => {
      if (settled) return
      // Abort races close ï¿?don't flush a torn tail from a killed process.
      // Promise still settles: spawn's signal option fires 'error' with
      // AbortError ï¿?reject below.
...
export async function ripGrep(
  ...
  return new Promise((resolve, reject) => {
    const handleResult = (
      ...
      // explicitly aborted (not an error, just a cancellation ï¿?interactive
...
export const countFilesRoundedRg = memoize(
  ...
  // abortSignal is intentionally excluded ï¿?it doesn't affect the count.
