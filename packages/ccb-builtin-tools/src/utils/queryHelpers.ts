...
import type { Input as FileReadInput } from '@claude-code-best/builtin-tools/tools/FileReadTool/FileReadTool.ts'
import {
  FILE_READ_TOOL_NAME,
  FILE_UNCHANGED_STUB,
...
/** Transcript JSON may deserialize Write tool `content` as a nested object ï¿?LRU needs strings. */
...
export function isResultSuccessful(
  ...
  // no assistant content ï¿?last(messages) is still this turn's prompt.
  ...
  // outputTokens=4, textContentLength=0 ï¿?it saw the subagent result
      ...
      for (const content of message.message!.content) {
        if (content.type === 'tool_result' && content.tool_use_id) {
          ...
          if (
            ...
            // Dedup stubs contain no file content ï¿?the earlier real Read
          ...
          // Handle Edit tool results ï¿?post-edit content isn't in the
          ...
          if (editFilePath && content.is_error !== true) {
            try {
              ...
              // File deleted or inaccessible since the Edit ï¿?skip
            }
          }
        }
      }
 ...
 * env var assignments (e.g. `FOO=bar vercel` ï¿?`vercel`) and prefixes
