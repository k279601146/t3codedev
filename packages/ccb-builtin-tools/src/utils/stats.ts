...
async function processSessionFiles(
  ...
  for (let i = 0; i < sessionFiles.length; i += BATCH_SIZE) {
    ...
    for (const { sessionFile, entries, error, skipped } of results) {
      ...
      // Skip sessions with malformed timestamps ï¿?some transcripts on disk
...
// Transcript message types ï¿?must match isTranscriptMessage() in sessionStorage.ts.
 ...
 * ï¿?naive string search is unsafe here because `file-history-snapshot` entries
 ...
 * head (caller falls through to the full read ï¿?safe default).
 */
export async function readSessionStartDate(
  ...
  try {
    ...
    try {
      ...
      // Only trust complete lines ï¿?the 4KB boundary may bisect a JSON entry.
