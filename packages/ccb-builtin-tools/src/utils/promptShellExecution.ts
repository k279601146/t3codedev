...
// NOTE: call() is invoked directly here, bypassing validateInput â€?any
...
// Lazy: this file is on the startup import chain (main â†?commands â†?
// loadSkillsDir â†?here). A static import would load PowerShellTool.ts
 ...
 *   This is *never* read from settings.defaultShell â€?it comes from .md
 *   frontmatter (author's choice) or is undefined for built-in commands.
 *   See docs/design/ps-shell-selection.md Â§5.3.
 */
export async function executeShellCommandsInPrompt(
  ...
  // hit BashTool. PowerShell only when the runtime gate allows â€?a skill
  ...
  // skill content (265Âµs vs 2Âµs @ 17KB). 93% of skills have no !` at all,
    ...
    [...blockMatches, ...inlineMatches].map(async match => {
      ...
      if (command) {
        try {
          ...
          // Function replacer â€?String.replace interprets $$, $&, $`, $' in
