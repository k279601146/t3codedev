/**
 * UDS Messaging Layer â€?Unix Domain Socket IPC for Claude Code instances.
 *
...
import { attachNdjsonFramer } from "./ndjsonFramer.ts"
import { attachUdsResponseReader } from "./udsResponseReader.ts"
...
// ---------------------------------------------------------------------------
// Public API â€?socket path helpers
// ---------------------------------------------------------------------------
...
export async function startUdsMessaging(
  ...
  // Ensure parent directory exists (skip on Windows â€?pipe paths aren't files)
  if (process.platform !== 'win32') {
    await ensureSocketParent(path)
  }

  // Clean up stale socket file (skip on Windows â€?pipe paths aren't files)
  if (process.platform !== 'win32') {
    try {
      await unlink(path)
    } catch {
...
export async function stopUdsMessaging(): Promise<void> {
  ...
  // Remove socket file (skip on Windows â€?pipe paths aren't files)
  if (socketPath) {
...
/**
 * Send a UDS message to a specific socket path (outbound â€?used when this
