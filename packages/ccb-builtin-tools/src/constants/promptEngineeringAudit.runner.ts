 ...
 * 然后检查关键段落是否存在。大部分被测函数�?module-private�?
 * 只能通过最终输出间接验证�?
 */

import { describe, test, expect, mock, beforeEach } from 'bun:test'

// --- MACRO 全局注入 (编译�?define 在测试中不可�? ---
...
mock.module('../utils/undercover.js', () => ({
  isUndercover: () => false,
}))
mock.module('../utils/model/antModels.js', () => ({
  getAntModelOverrideConfig: () => null,
}))
mock.module('../utils/mcpInstructionsDelta.js', () => ({
...
describe('Opus 4.7 Prompt Engineering Audit', () => {
  // ------------------------------------------------------------------
  // #1 决策树结�?(Decision Tree)
  // TXT 来源: {request_evaluation_checklist} �?Step 0�?�?�?
  // ------------------------------------------------------------------
  ...
  // ------------------------------------------------------------------
  // #6 渐进式回退�?(Progressive Fallback Chain)
  ...
  // ------------------------------------------------------------------
  // #5 成本不对称分�?(Asymmetric Cost Analysis)
  ...
  // ------------------------------------------------------------------
  // #8 查询构造教�?(Query Construction Teaching)
  ...
  // ------------------------------------------------------------------
  // #22 先搜再说不知�?(Search Before Saying Unknown)
  ...
  // ------------------------------------------------------------------
  // #20 风险感知时说得更�?(Say Less When Risky)
  ...
  // ------------------------------------------------------------------
  // #23 不解释为什么搜�?(Don't Justify Search)
  ...
  // ------------------------------------------------------------------
  // #16 每回复最多一个问�?(One Question Per Response)
