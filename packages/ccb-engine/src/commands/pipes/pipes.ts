...
import {
  ...
  getPipeDisplayRole,
  isPipeControlled,
} from "../../utils/pipeTransport.ts"
import {
  cleanupStaleEntries,
  readRegistry,
  isMainMachine,
  mergeWithLanPeers,
} from "../../utils/pipeRegistry.ts"
...
export const call: LocalCommandCall = async (_args, context) => {
  ...
  if (args.startsWith('select ') || args.startsWith('sel ')) {
    ...
    return {
      type: 'text',
      value: `Selected ${pipeName} 锟?messages will be broadcast to this pipe.`,
    }
  }
  ...
  if (feature('LAN_PIPES')) {
    const { getLanBeacon } =
      require('../../utils/lanBeacon.ts') as typeof import('../../utils/lanBeacon.ts')
  ...
  lines.push('')
  lines.push(
    `Selected: ${selected.length > 0 ? selected.join(', ') : '(none 锟?messages run locally only)'}`,
  )
  lines.push('')
  lines.push('Commands:')
  lines.push('  /pipes select <name>    锟?select pipe for broadcast')
  lines.push('  /pipes deselect <name>  锟?deselect pipe')
  lines.push('  /pipes all              锟?select all connected')
  lines.push('  /pipes none             锟?deselect all')
  lines.push('  /send <name> <msg>      锟?send to specific pipe')
  lines.push('  /claim-main             锟?claim this machine as main')
