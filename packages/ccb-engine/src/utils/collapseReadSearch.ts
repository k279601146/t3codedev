...
import { extractBashCommentLabel } from '@claude-code-best/builtin-tools/tools/BashTool/commentLabel.ts'
...
import { getReplPrimitiveTools } from '@claude-code-best/builtin-tools/tools/REPLTool/primitiveTools.ts'
import {
  type BranchAction,
  type CommitKind,
  detectGitOperation,
  type PrAction,
} from '@claude-code-best/builtin-tools/tools/shared/gitOperationTracking.ts'
...
const SNIP_TOOL_NAME = feature('HISTORY_SNIP')
  ? (
      require('@claude-code-best/builtin-tools/tools/SnipTool/prompt.ts') as typeof import('@claude-code-best/builtin-tools/tools/SnipTool/prompt.ts')
    ).SNIP_TOOL_NAME
  : null
...
// ~5 lines 鑴?~60 cols. Generous static cap 閿?the renderer lets Ink wrap.
...
export function getToolSearchOrReadInfo(
  ...
  // REPL is absorbed silently 閿?its inner tool calls are emitted as virtual
  ...
  // as their own category 閿?"Ran N bash commands" instead of breaking the group.
  return {
    isCollapsible:
      isCollapsible ||
...
function scanBashResultForGitOps(
  ...
  // git push writes the ref update to stderr 閿?scan both streams.
...
type GroupAccumulator = {
  ...
  bashCount?: number
  // Bash tool_use_id 閿?command string, so tool results can be scanned for
...
function createCollapsedGroup(
  ...
  // Adding bash operation count on top would double-count 閿?e.g. Read(README.md)
  ...
  // memoryReadFilePaths 閿?readFilePaths (both populated from Read tool calls),
  // so this count is safe to subtract from totalReadCount at readCount below.
  // Absorbed relevant_memories attachments are NOT in readFilePaths 閿?added
  ...
  for (const msg of messages) {
    if (isCollapsibleToolUse(msg, tools)) {
      ...
      if (toolInfo.isMemoryWrite) {
        // Memory file write/edit 閿?check if it's team memory
        ...
        // Snip/ToolSearch absorbed silently 閿?no count, no summary text.
        ...
        // MCP search/read 閿?counted separately so the summary says
        ...
        // Non-search/read Bash command 閿?counted separately so the summary
        ...
        if (input?.command) {
          ...
          // for the human 閿?same trigger as the comment-as-label tool-use render).
          currentGroup.latestDisplayHint =
            extractBashCommentLabel(input.command) ??
            commandAsHint(input.command)
          // Remember tool_use_id 閿?command so the result (arriving next) can
        ...
        // Directory-listing bash commands (ls, tree, du) 閿?counted separately
        ...
        if (
          ...
          // Regular (non-memory) search 閿?collect pattern for display
        ...
        for (const filePath of filePaths) {
          ...
          if (feature('TEAMMEM') && teamMemOps?.isTeamMemFile(filePath)) {
            ...
            // Non-memory file read 閿?update display hint
      ...
      // 閿?block. Do NOT add paths to readFilePaths/memoryReadFilePaths 閿?
      ...
      // 閿?Loaded lines cluster tightly instead of being split by the badge's marginTop.
      if (
        currentGroup.messages.length > 0 &&
