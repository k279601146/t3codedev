...
// Sync fs primitives for readFileTailSync 锟?separate from fs/promises
 ...
 * marker. Kept in sync with sessionStoragePortable.ts 锟?generic pattern avoids
 * an ever-growing allowlist that falls behind as new notification types ship.
 */
// 50MB 锟?prevents OOM in the tombstone slow path which reads + rewrites the
...
export function getTranscriptPathForSession(sessionId: string): string {
  ...
  // 锟?different directories, so the hook sees MISSING (gh-30217). CC-34
  ...
  //
  // For OTHER session IDs we can only guess via originalCwd 锟?we don't
  // track a sessionId鈫抪rojectDir map. Callers wanting a specific other
...
// 50 MB 锟?session JSONL can grow to multiple GB (inc-3930). Callers that
// read the raw transcript must bail out above this threshold to avoid OOM.
export const MAX_TRANSCRIPT_READ_BYTES = 50 * 1024 * 1024

// In-memory map of agentId 锟?subdirectory for grouping related subagent
...
export function getAgentTranscriptPath(agentId: AgentId): string {
  // Same sessionProjectDir consistency as getTranscriptPathForSession 锟?
...
export type AgentMetadata = {
   ...
   * of a placeholder. Optional 锟?older metadata files lack this field. */
  description?: string
}

/**
 * Persist the agentType used to launch a subagent. Read by resume to
 * route correctly when subagent_type is omitted 锟?without this, resuming
...
export type RemoteAgentMetadata = {
  ...
  /** CCR session ID 锟?used to fetch live status from the Sessions API on resume. */
  sessionId: string
  title: string
  command: string
  spawnedAt: number
...
function getRemoteAgentsDir(): string {
  // Same sessionProjectDir fallback as getAgentTranscriptPath 锟?the project
 ...
 * from CCR on restore 锟?only identity is persisted locally.
 */
...
> {
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      // Skip unreadable or corrupt files 锟?a partial write from a crashed
...
// stable for a given input. Worktree switches just change the key 锟?no
// cache clear needed.
  ...
  if (!project) {
    ...
    if (!cleanupRegistered) {
      registerCleanup(async () => {
        ...
        // fields 锟?if enough messages are appended after a /rename, the
        ...
        try {
          ...
          // Best-effort 锟?don't let metadata re-append crash the cleanup
        }
      })
...
class Project {
  ...
  // on the first user/assistant message 锟?prevents metadata-only session files.
  ...
  private async appendToFile(filePath: string, data: string): Promise<void> {
    try {
      ...
      // Directory may not exist 锟?some NFS-like filesystems return
   ...
   * our stale cache absorbs it and the re-append below persists it 锟?not
   ...
   * external-writer concern 锟?their caches are authoritative.
   */
  reAppendSessionMetadata(skipTitleRefresh = false): void {
    ...
    // failure 锟?extract returns null 锟?cache is the only source of truth.
    const tail = readFileTailSync(this.sessionFile)

    // Absorb any fresher SDK-written title/tag into our cache. If the SDK
    // wrote while we had the session open, our cache is stale 锟?the tail
  ...
  private async materializeSessionFile(): Promise<void> {
    // Guard here too 锟?reAppendSessionMetadata writes via appendEntryToFile
    ...
    return this.trackWrite(async () => {
      ...
      for (const message of messages) {
        ...
        const transcriptMessage: TranscriptMessage = {
          ...
          // loadFullLog's sessionId-keyed contentReplacements lookup misses 锟?
          // replacement records lost 锟?FROZEN misclassification.
      ...
      // Cache this turn's user prompt for reAppendSessionMetadata 锟?
  ...
  async appendEntry(entry: Entry, sessionId: UUID = getSessionId() as UUID) {
    ...
    if (entry.type === 'summary') {
      ...
      // Always append. Last-wins on restore 锟?later entries supersede.
      void this.enqueueWrite(sessionFile, entry)
    } else {
      ...
      if (entry.type === 'queue-operation') {
        ...
        // Skip dedup for agent sidechain LOCAL writes 锟?they go to a separate
        ...
        // context).
        //
        // The sidechain bypass applies ONLY to the local file write 锟?remote
        ...
        // exhausts retries 锟?gracefulShutdownSync(1). See inc-4718.
        const isNewUuid = !messageSet.has(entry.uuid)
        if (isAgentSidechain || isNewUuid) {
          // Enqueue write 锟?appendToFile handles ENOENT by creating directories
          void this.enqueueWrite(targetFile, entry)

          if (!isAgentSidechain) {
            // messageSet is main-file-authoritative. Sidechain entries go to a
            // separate agent file 锟?adding their UUIDs here causes recordTranscript
            ...
            // UUID the main thread hasn't written yet 锟?409 when main writes it.
...
// UUIDs instead of the post-compact summary 锟?orphaning the compact boundary.
//
...
//    trajectory): recorded messages are always a prefix 锟?tracked 锟?correct
//    parent chain for new messages.
//  - Compaction (useLogMessages): new CB/summary appear FIRST, then recorded
//    messagesToKeep 锟?not a prefix 锟?not tracked 锟?CB gets parentUuid=null
...
export async function recordTranscript(
  ...
  // already in messageSet). Progress is skipped 锟?it's written to the JSONL
 ...
 * mode/tag/agent).
 *
 * Setting sessionFile here 锟?instead of waiting for materializeSessionFile
 * on the first user message 锟?lets the exit cleanup handler's
 ...
 * no-op 锟?unless --name was used, in which case it would clobber the fresh
 ...
 * restore 锟?the loader keeps only the most recent snapshot entry.
 */
 ...
 * The server handles compaction filtering 锟?it returns events starting
 ...
 * parentUuids (recordTranscript dedup-skipped them 锟?can't rewrite).
 * The internal chain (keep[i+1]鈫択eep[i]) is intact; only endpoints need
 * patching: head鈫抋nchor, and anchor's other children鈫抰ail. Anchor is the
 * last summary for suffix-preserving, boundary itself for prefix-preserving.
 *
 * Only the LAST seg-boundary is relinked 锟?earlier segs were summarized
...
function applyPreservedSegmentRelinks(
  ...
  // manual /compact after reactive compact 锟?seg is stale).
  ...
  // No seg anywhere 锟?no-op. findUnresolvedToolUse etc. read the full map.
  if (!lastSeg) return

  // Seg stale (no-seg boundary came after): skip relink, still prune at
  // absolute 锟?otherwise the stale preserved chain becomes a phantom leaf.
  const segIsLive = lastSegBoundaryIdx === absoluteLastBoundaryIdx

  // Validate tail鈫抙ead BEFORE mutating so malformed metadata is a true
  ...
  if (segIsLive) {
    ...
    if (!reachedHead) {
      // tail鈫抙ead walk broke 锟?a UUID in the preserved segment isn't in the
  ...
  if (segIsLive) {
    ...
    // Tail-splice: anchor's other children 锟?tail. No-op if already pointing
    ...
    // (~190K) 锟?stripStaleUsage only patched in-memory copies that were
    // dedup-skipped. Without this, resume 锟?immediate autocompact spiral.
  ...
  // isn't preserved. preservedUuids empty when !segIsLive 锟?full prune.
  const toDelete: UUID[] = []
 ...
 * and resume immediately PTLs (adamr-20260320-165831: 397K displayed 锟?1.65M
 * actual).
 *
 ...
 * exact removal on load. Older boundaries without removedUuids are skipped 锟?
...
function applySnipRemovals(messages: Map<UUID, TranscriptMessage>): void {
  // Structural check 锟?snipMetadata only exists on the boundary subtype.
  ...
  // behavior 锟?same as if compact truncated there, which it did).
 ...
 * 锟?N parallel tool_uses 锟?N messages, distinct uuid, same message.id. Each
 ...
 *   1. Sibling assistant orphaned: walk goes prev鈫抋sstA鈫扵R_A鈫抧ext, drops asstB
...
function recoverOrphanedParallelToolResults(
  ...
  // already in chain order, so later iterations overwrite 锟?last wins.
  ...
  // TRs indexed by parentUuid 锟?insertMessageChain:~894 already wrote that
 ...
 * write鈫抣oad round-trip drift 锟?the class of bugs where snip/compact/
 * parallel-TR operations mutate in-memory but the parentUuid walk on disk
 * reconstructs a different set (adamr-20260320-165831: 397K displayed 锟?
 * 1.65M actual on resume).
 *
 * delta > 0: resume loaded MORE than in-session (the usual failure mode)
 * delta < 0: resume loaded FEWER (chain truncation 锟?#22453 class)
 * delta = 0: round-trip consistent
 *
 * Called from loadConversationForResume 锟?fires once per resume, not on
...
function buildFileHistorySnapshotChain(
  ...
  // messageId 锟?last index in snapshots[] for O(1) update lookup
 ...
 *   at EOF 锟?avoiding the clobber-on-resume bug where a stale AI title
 ...
 * not re-appended by reAppendSessionMetadata 锟?it's a rolling snapshot of
...
export function saveWorktreeState(
  ...
  // may pass via full WorktreeSession objects 锟?TypeScript structural typing
  ...
  // For --worktree startup, sessionFile is null 锟?materializeSessionFile
...
export async function loadFullLog(log: LogOption): Promise<LogOption> {
  ...
  try {
    ...
    // Leaf's sessionId 锟?forked sessions copy chain[0] from the source, but
 ...
 * Uses raw Buffer chunks and byte-level marker matching 锟?no readline, no per-line
 * string conversion for the ~99% of lines that are message content.
 *
 * Fast path: if a chunk contains zero markers (the common case 锟?metadata entries
...
async function scanPreBoundaryMetadata(
  ...
  for await (const chunk of stream) {
    ...
    if (hasAnyMarker) {
      ...
      // No markers in this chunk 锟?just preserve the incomplete trailing line
    ...
    // are <1 KB, so if carry exceeds this we're mid-message-content 锟?drop it.
 ...
 *      entries serialize a nested Message in data BEFORE uuid 锟?both can
 ...
 * none are at depth 1 (shouldn't happen for well-formed JSONL 锟?depth-1 is
 * where the top-level object's fields live).
 *
 * Only called when 锟? suffix matches exist (agent_progress with a nested
 * Message, or mcpMeta with a coincidentally-suffixed object). Cost is
 * O(max(candidates) - lineStart) 锟?one forward byte pass, stopping at the
 * first depth-1 hit.
 */
...
function walkChainBeforeParse(buf: Buffer): Buffer {
  ...
  while (pos < len) {
    ...
    if (
      ...
      //     serialized BEFORE top-level uuid 锟?that inner Message has its
      //     own uuid,timestamp adjacent, so its bytes also satisfy the
      //     suffix check.
      //   - mcpMeta/toolUseResult come AFTER top-level uuid and hold
      //     server-controlled Record<string,unknown> 锟?a server returning
      ...
      // variants put timestamp BEFORE uuid 锟?`"uuid":"<36>"}` at EOL)
...
export async function loadTranscriptFile(
  ...
  >()
  // Array, not Map 锟?commit order matters (nested collapses).
  const contextCollapseCommits: ContextCollapseCommitEntry[] = []
  // Last-wins 锟?later entries supersede.
  ...
  try {
    ...
    // file size 锟?a 151 MB session that is 84% stale attr-snaps allocates
    ...
    if (!isEnvTruthy(process.env.CLAUDE_CODE_DISABLE_PRECOMPACT_SKIP)) {
      ...
      if (size > SKIP_PRECOMPACT_THRESHOLD) {
        ...
        // boundary already truncated 锟?in which case the preserved messages
    ...
    // attr-snaps already stripped at the fd level 锟?the <5MB readFile path
    ...
    // the post-boundary buffer is harmless 锟?later values overwrite earlier ones.
    ...
    // Bridge map for legacy progress entries: progress_uuid 锟?progress_parent_uuid.
    ...
    // append-only (parents before children), we record each progress鈫抪arent link
    ...
    for (const entry of entries) {
      // Legacy progress check runs before the Entry-typed else-if chain 锟?
...
export async function getLastSessionLog(
  ...
  // may call getLastSessionLog on the current session 锟?overwriting a live cache
...
export async function loadMessageLogs(limit?: number): Promise<LogOption[]> {
  ...
  // fetchLogs returns lite (stat-only) logs 锟?enrich them to get metadata.
  ...
  // enrichLogs returns fresh unshared objects 锟?mutate in place to avoid
...
async function loadAllProjectsMessageLogsFull(
  ...
  // Deduplicate 锟?same session+leaf can appear in multiple project dirs.
  ...
  // deduped values are fresh from getLogsWithoutIndex 锟?safe to mutate
...
export async function loadAllProjectsMessageLogsProgressive(
  ...
  // Deduplicate 锟?same session can appear in multiple project dirs
  const sorted = deduplicateLogsBySessionId(rawLogs)

  const { logs, nextIndex } = await enrichLogs(sorted, 0, initialEnrichCount)

  // enrichLogs returns fresh unshared objects 锟?safe to mutate in place
  logs.forEach((log, i) => {
    log.value = i
  })
...
async function getStatOnlyLogsForWorktrees(
  ...
  // Deduplicate by sessionId 锟?the same session can appear in multiple
...
// Globs the session's subagents dir directly 锟?unlike AppState.tasks, this survives task eviction.
export async function loadAllSubagentTranscriptsFromDisk(): Promise<{
  ...
  // Filename format is the inverse of getAgentTranscriptPath() 锟?keep in sync.
 ...
 * transformed 锟?recordTranscript receives incremental slices where the REPL
 ...
 * Returns a map of sessionId 锟?{path, mtime, ctime, size}.
 ...
 * means ~6.4 MB of I/O 锟?fast on any modern filesystem while giving users
...
export async function loadAllLogsFromSessionFile(
  ...
  // Build parentUuid 锟?children index once (O(n)), so trailing-message lookup is O(1) per leaf
...
async function readLiteMetadata(
  ...
  // Prefer the last-prompt tail entry 锟?captured by extractFirstPrompt at
  ...
  // PR link fields 锟?prNumber is a number not a string, so try both
...
function extractFirstPromptFromChunk(chunk: string): string {
    ...
    try {
      ...
      for (const text of texts) {
        ...
        if (commandNameTag) {
          ...
          // Custom command with meaningful args 锟?use clean display
          return commandArgs
  ...
  // Session started with a slash command but had no subsequent real message 锟?
  ...
  // Proactive sessions have only tick messages 锟?give them a synthetic prompt
 ...
 * No file reads 锟?instant. Call `enrichLogs` to enrich
...
export async function getSessionFilesLite(
  ...
  // logs are freshly pushed above 锟?safe to mutate in place
 ...
 * (no firstPrompt, no customTitle 锟?e.g., metadata-only session files).
 */
