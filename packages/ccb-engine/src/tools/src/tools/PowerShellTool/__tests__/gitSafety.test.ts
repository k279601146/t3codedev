...
mock.module('../../../../../utils/powershell/parser.js', () => ({
...
const { isGitInternalPathPS, isDotGitPathPS } = await import('../gitSafety.ts')
...
describe('isGitInternalPathPS', () => {
  ...
  test('normal file is not git-internal', () => {
    expect(isGitInternalPathPS('../../../../../main.ts')).toBe(false)
  })
...
describe('isDotGitPathPS', () => {
  ...
  test('normal file is not .git path', () => {
    expect(isDotGitPathPS('../../../../../main.ts')).toBe(false)
  })
